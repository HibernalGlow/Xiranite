...
describe("reader-http-client", () => {
  it("[neoview.react.control] sends token-authenticated open, navigation and close requests", async () => {
    ...
    const [openUrl, openInit] = fetchMock.mock.calls[2]!
    ...
    expect(String(fetchMock.mock.calls[3]?.[0])).toContain("/reader/s/reader-1/pages?cursor=64&limit=32")
    expect(String(fetchMock.mock.calls[4]?.[0])).toContain("/reader/s/reader-1/navigate")
    expect(JSON.parse(String(fetchMock.mock.calls[5]?.[1]?.body))).toEqual({ action: "goTo", pageIndex: 17 })
    expect(fetchMock.mock.calls[6]?.[1]).toMatchObject({ method: "DELETE", keepalive: true })
  })
