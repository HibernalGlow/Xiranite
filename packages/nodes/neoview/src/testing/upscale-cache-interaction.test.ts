...
describe("NeoView upscale artifact cache terminal interaction", () => {
  it("[neoview.super-resolution.cache-controls-tui] reuses the remote cache port and marks every cleanup destructive", async () => {
  ...
  it("[neoview.super-resolution.cache-controls-tui-cancel] aborts the remote request and still disposes its session", async () => {
...
function fakeController() {
  return {
    open: vi.fn(async () => ({ book: { displayName: "book.cbz", pageCount: 1 } })),
