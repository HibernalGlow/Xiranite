...
| Phase 4：复杂格式与 CLI/TUI | 进行中（图片阅读原型可用） | EPUB/7z/RAR/nested provider；共享 HeadlessController；`xneoview` inspect/pages/frame/extract；持久 TUI 工作台通过共享 `TerminalImagePreview` 流式显示单/双页 frame | 真实 EPUB 语料、SIXEL/Kitty 终端验收、缩略图 gallery、`--connect`、完整设置/feature 命令、平台能力诊断；PDF 不计入当前阶段验收 |
