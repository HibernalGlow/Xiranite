...
impl ProgressData {
    pub(crate) fn validate(&self) {
        ...
        if self.bytes_checked > self.bytes_to_check {
            error!(
                "Bytes checked: {}, bytes to check: {}, stage {:?}",
                self.bytes_checked, self.bytes_to_check, self.sstage
            );
        };
        ...
        let tool_type_checking_method: Option<ToolType> = match self.checking_method {
            ...
            | CheckingMethod::Size
            | CheckingMethod::Hash => Some(ToolType::Duplicate),
            CheckingMethod::None => None,
        };
        if let Some(tool_type) = tool_type_checking_method {
            assert_eq!(
                self.tool_type, tool_type,
                "Tool type: {:?}, checking method: {:?}",
                self.tool_type, self.checking_method
            );
        }
        let tool_type_current_stage: Option<ToolType> = match self.sstage {
            ...
            | CurrentStage::SimilarImagesComparingHashes => Some(ToolType::SimilarImages),
    ...
    pub(crate) fn get_max_stage(&self, checking_method: CheckingMethod) -> u8 {
        match *self {
            ...
            Self::SameMusic => match checking_method {
                ...
                _ => unreachable!(
                    "CheckingMethod {checking_method:?} in same music mode is not allowed"
                ),
            },
        }
    }
...
impl CurrentStage {
    pub fn is_special_non_tool_stage(&self) -> bool {
        matches!(
            self,
            Self::DeletingFiles | Self::RenamingFiles | Self::MovingFiles
        )
    }
