...
test("[neoview.super-resolution.cards-browser] preserves the legacy upscale Card controls and constrained geometry", async ({ page }, testInfo) => {
  ...
  await expect(modelCard.getByRole("alert")).toContainText("性能降级")
  await expect(modelCard.getByRole("alert")).toContainText("未检测到 Xiranite 托管的 Upscayl daemon")
  await modelCard.scrollIntoViewIfNeeded()
  await page.screenshot({ path: testInfo.outputPath("neoview-upscayl-daemon-warning-1920x1080.png"), fullPage: false })
  await expect(modelCard).not.toContainText("Downloadable Model")
  await expect(page.getByPlaceholder("添加包含 models 的目录")).toBeHidden()
  ...
  await page.getByRole("button", { name: "展开模型管理" }).click()
  await page.getByLabel("显示未下载模型").click()
  await expect(modelCard).toContainText("Downloadable Model")
  await page.getByLabel("显示未下载模型").click()
  await expect(modelCard).not.toContainText("Downloadable Model")
  ...
  await page.getByRole("button", { name: "收起模型管理" }).click()
  await expect(page.getByPlaceholder("添加包含 models 的目录")).toBeHidden()
  await modelCard.scrollIntoViewIfNeeded()
