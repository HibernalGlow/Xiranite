...
const cliNameOverrides: Record<string, string> = {
  gitalso: "also",
}

const header = "// Generated by scripts/generate-node-registries.ts. Do not edit by hand.\n\n"

const discoveredNodes = await discoverNodePackages()
const nodeFilter = await resolveNodeFilter(discoveredNodes)
const nodes = discoveredNodes.filter((node) => nodeFilter.has(node.id))

if (nodes.length === 0) {
  throw new Error("Node registry filter removed every node. Check XIRANITE_BUILD_ONLY_NODES/XIRANITE_BUILD_EXCLUDE_NODES.")
}
...
await writeIfChanged(
  join(repoRoot, "internal", "desktop", "external_node_launch_registry.generated.go"),
  generateExternalLaunchRegistry(nodes),
)

const skippedNodes = discoveredNodes.filter((node) => !nodeFilter.has(node.id)).map((node) => node.id)
console.log(`Generated node registries for ${nodes.length} node package(s).${skippedNodes.length ? ` Skipped: ${skippedNodes.join(", ")}.` : ""}`)

async function resolveNodeFilter(discovered: NodePackage[]): Promise<Set<string>> {
  const only = parseNodeIds(process.env.XIRANITE_BUILD_ONLY_NODES)
  const defaultExcluded = await getDisabledNodeIds({ cwd: repoRoot })
  ...
  // A frozen node-app snapshot may omit the repository config entirely. When
  // the config is present, keep its node IDs strict just like explicit filters.
  for (const id of [...only, ...defaultExcluded, ...requestedExcluded]) {
    if (!known.has(id)) throw new Error(`Unknown node id in build filter: ${id}`)
  }

  const included = only.length > 0 ? only : discovered.map((node) => node.id)
...
function runtimeSpec(node: NodePackage): string {
  ...
  if (override?.kind === "pure") {
    ...
    loadCore: createNodeModuleLoader(() => import(${stringLiteral(`${node.packageName}/core`)}), { nodeId: ${stringLiteral(node.id)}, entry: "core" }),
  ...
  return `{
    ...
    loadPlatform: createNodeModuleLoader(() => import(${stringLiteral(`${node.packageName}/platform`)}), { nodeId: ${stringLiteral(node.id)}, entry: "platform" }),
...
function generateCliRegistry(nodes: NodePackage[]): string {
  const entries = nodes
    ...
    packageName: ${stringLiteral(node.packageName)},
    bin: ${cliNameOverrides[node.id]
      ? stringLiteral(cliNameOverrides[node.id])
      : `nodeCliName(${stringLiteral(node.id)})`},
...
function generateExternalLaunchRegistry(nodes: NodePackage[]): string {
  const entries = nodes.filter((node) => node.def.externalLaunch).map((node) => {
    const declaration = node.def.externalLaunch!
    const intents = declaration.intents.map((intent) => `{
      ID: ${stringLiteral(intent.id)},
      TargetKinds: []string{${intent.targetKinds.map(stringLiteral).join(", ")}},
