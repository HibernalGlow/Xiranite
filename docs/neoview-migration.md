...
压缩包内视频沿用同一 `ReaderPageMaterializer` 和 `FilePageContent`，没有新增第二套 HTTP/文件读取实现。首次 GUI `HEAD/GET` 按需物化，后续请求在同一 Reader session 内复用一个 `ReaderSeekableMediaCache` lease 并返回 `Accept-Ranges: bytes`/`206 Content-Range`；普通目录视频继续直接读取原文件，压缩包图片继续端到端流式且不得伪报 decompressed Range。物化按单项 2 GiB、总量 4 GiB 默认预算准入，相同 page/contentVersion singleflight；单个请求取消只退出该 waiter，全部 waiter 离开才中止归档落盘。session close 释放无活动引用的临时目录，正在响应的文件 lease 在 body 结束或取消后释放，路径和密码均不进入 GUI DTO、asset URL、SQLite 或 TOML。
...
- GUI HTTP 与 CLI/TUI headless controller 已共用视频进度恢复/合并写回 service；GUI archive video 已通过按需、singleflight、有字节预算且随 session 释放的临时物化获得标准 HTTP Range seek，普通文件仍直接 Range、archive image 仍零落盘流式；播放器控件、音量/倍速、字幕和切页播放资源治理仍待迁移；
