...
function Harness() {
  const [config, setConfig] = useState<ReaderSuperResolutionConfigDto>({ provider: "opencomic-system", modelsDirectory: "D:/NeoView/models", modelSources: ["D:/Python/realsr", "D:/Python/realcugan", "D:/Python/realesrgan"], artifactCache: { retentionDays: 30, cleanupIntervalMinutes: 1_440 }, preferences: { autoUpscaleEnabled: true, preUpscaleEnabled: true, preloadPages: 3, backgroundConcurrency: 2, showPanelPreview: true, defaultModelId: "realesr-animevideov3", defaultScale: 2, defaultTileEnabled: true, defaultTileSize: 512, defaultNoise: 0, defaultGpuId: "0", progressiveEnabled: false, progressiveDwellTimeMs: 3000, progressiveMaxPages: 20, conditionalEnabled: true, conditions: [{ id: "default", name: "默认条件", enabled: true, priority: 0, match: { dimensionMode: "and", maxWidth: 1600 }, action: { skip: false, modelId: "realesr-animevideov3", scale: 2, tileEnabled: true, tileSize: 512, noise: 0, gpuId: "0", useCache: true } }] } })
  const update = async (patch: Parameters<NonNullable<ReaderHttpClient["updateSuperResolution"]>>[0]) => {
    const next = { ...config, ...(patch.superResolution.modelsDirectory === undefined ? {} : { modelsDirectory: patch.superResolution.modelsDirectory }), ...(patch.superResolution.modelSources === undefined ? {} : { modelSources: patch.superResolution.modelSources }), ...(patch.superResolution.artifactCache === undefined ? {} : { artifactCache: { ...config.artifactCache!, ...patch.superResolution.artifactCache } }), preferences: { ...config.preferences, ...(patch.superResolution.preferences ?? {}) } }
    setConfig(next)
  ...
  const client = {
    ...
    upscaleCapabilities: async () => ({ available: true as const, models: [
      ...
      available: true,
      executablePath: "D:/scoop/apps/Upscayl/current/upscayl-bin.exe",
  ...
  const props = { client, session, disabled: false, superResolution: config, onGoTo: () => undefined, onSuperResolutionChange: (preferences: typeof config.preferences) => rootChange({ preferences }), onSuperResolutionConfigChange: rootChange, pickDirectory: async () => "D:/Selected/models" }
