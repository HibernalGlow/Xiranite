...
> 最后更新：2026-07-15；最近已提交基线：`897886a`（原图阅读/预解码/虚拟缩略图）、`880ccff`（复杂 Shell/Card AST scaffold）、`1e4f70c`（懒加载四边 Shell 与首批 Panel/Card）、`5064b82`（只读 Shell 配置链）、`444bdbc`（React Compiler 友好的侧栏 resize/drag 与 TOML PATCH 原子持久化）、`532b304`（旧 panel layout 显隐/排序/位置兼容读取）。本节必须随每个 NeoView 实现提交同步更新；代码、feature matrix 和测试证据优先于文字。
...
| Panel/Card | 兼容布局与折叠持久化纵切 | 完整旧 panel ID metadata；Card lazy loader；左“页面导航”和右“书籍信息”；关闭/折叠后内容 DOM=0；旧 `panelsLayout.sidebarConfig.panels` 的 visible/order/position 与 `neoview_card_configs_v14` 的 visible/expanded/order/height 经 TOML 规范化，未知未来 panel/card 原样保留；新版 Card 状态写入 canonical `panels.card_state`，乐观折叠后单次 PATCH 原子持久化 | 其余全部卡片、管理 UI 与显隐/排序写回、Card 高度拖动、独立 window/tab |
...
- 前端 NeoView：10 个测试文件、28 项 component/hook/registry/shell/config/resize/card 测试通过；后端 NeoView 最近完整复跑为 35 个文件、Vitest 173 项 + Bun TUI 1 项通过；Backend CORS 17 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖 v14 Card 数据兼容、canonical 覆盖、Card PATCH 边界、TOML 原子写回、受控零 DOM、乐观折叠及失败恢复；
- Reader pipeline 最近复跑：backend startup `19.42 ms`、open `52.48 ms`、navigation p95 `4.28 ms`、response headers p95 `5.74 ms`、first byte p95 `5.86 ms`、full stream p50 `26.05 ms`、throughput p50 `109.00 MiB/s`、RSS delta `3.11 MiB`；全部通过预算，Card 配置写入没有进入图片数据热路径；
- 四边 Shell + 左右 lazy card 的真实 Chromium 路径已覆盖桌面与 `420×360`：关闭侧栏/底栏和折叠 Card 后内容 DOM 为 0，底栏卸载时预解码仍继续；真实“折叠→PATCH→TOML→展开”后的热翻页为 desktop `47.3/77.5 ms`、card `41.1/74.5 ms`（navigation/decode），仍明显低于 `150/200 ms` 硬门槛；原图 `<img>` 节点身份保持不变；
- 当前生产构建报告：NeoView entry `31,236 bytes`，ReaderSidebar `7,957 bytes`，PageNavigationCard `1,232 bytes`，BookInformationCard `1,015 bytes`；首屏 NeoView module `0`、前端 zip.js module `0`。本轮同时存在并行构建工具变更，因此不把体积下降单独归因于 Card 改动；可确认所有 NeoView chunk 继续低于硬门槛并保持独立延迟加载。
