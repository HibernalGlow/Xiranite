...
- 前端 NeoView：14 个测试文件、43 项 component/hook/registry/shell/config/resize/card/settings/presentation 测试通过；后端 NeoView 低 worker 完整复跑为 41 个文件、Vitest 204 项 + Bun TUI 1 项通过，Backend 26 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖旧 `fitHeight` 兼容、view default 严格 PATCH/TOML 写回、同进程后续 session 默认值刷新、单双页 session options、工具条状态及活动 `<img>` 身份；
