...
| ID | 状态 | 能力 | 验收标准 |
| --- | --- | --- | --- |
| R01 | `[-]` | 工具专属列 | 11 个工具分别显示完整字段，不再共用“路径/大小/详情”最小列集 |
| R02 | `[ ]` | 虚拟化 | 万级结果滚动流畅，DOM 行数受限 |
| R03 | `[ ]` | 列排序 | 多工具排序状态隔离，表头可切换方向 |
| R04 | `[ ]` | 列宽拖动 | 列宽可调且不会破坏缩略图行高 |
| R05 | `[x]` | 行复选 | 单行选择与全选 |
| R06 | `[ ]` | Shift 范围选择 | Shift+Click 选择连续可见范围 |
| R07 | `[ ]` | Ctrl 切换选择 | Ctrl+Click 不丢失现有选择 |
| R08 | `[ ]` | 框选 | 鼠标拖框与虚拟行相交选择 |
| R09 | `[-]` | 分组轨道/分隔 | 分组结构可见；参考项和普通项语义明确 |
| R10 | `[ ]` | 右键菜单 | 选中/取消整组、复制路径、复制文件、打开路径 |
| R11 | `[ ]` | 行操作 | 打开、定位、复制路径/名称、预览 |
| R12 | `[ ]` | 搜索命中联动 | 顶栏搜索实时过滤当前工具且保持分组合法 |
| R13 | `[ ]` | 每工具状态隔离 | 数据、排序、过滤、选择在切换工具后保留 |
...
| O08 | `[ ]` | 系统打开/定位 | 使用 Xiranite host API，不新增 Rust |
| O09 | `[ ]` | 复制路径/名称 | 使用 clipboard host API |
...
| Q11 | `[ ]` | 最终一次 release Node-API 构建、体积记录、Node/Bun smoke |
