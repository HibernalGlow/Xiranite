...
| 设置与数据迁移 | 部分完成 | LegacySettingsCodec、共享 ReaderSettingsMigrationService、CLI 与鉴权 HTTP legacy inspect/import；当前 `[nodes.neoview]` 的 Xiranite/NeoViewConfig v1 portable 导出/回读；Xiranite/NeoViewBackup v1 配置+原 thumbnails.db 在线一致目录 bundle；确认/备份/跨进程锁/原子写入 | GUI/TUI 确认界面、自动备份调度、bundle restore、Gist transport |
