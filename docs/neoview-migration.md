...
- 当前仅对 10 个可证明子集生成 React TSX scaffold。每个文件带源路径、SHA-256、feature ID 和 `scaffold` 状态，并通过 OXC TSX 语法检查及“无 Svelte import/rune”门禁；scaffold 仍不能直接进入生产目录，必须先替换 `$lib` 依赖并补 characterization/component test。

在生成 TSX 前还必须完成功能 ID 映射和 disposition 人工复核；随后 codemod 只把 `converted` 组件生成 React scaffold，`adapter-needed/manual/replaced/blocked` 分别进入 adapter、人工重构、宿主替代或 typed shell 流程。旧 `.svelte` 文件不会复制到生产目录，也不会形成 Svelte/React 双版本运行。
...
当前边界也必须保持明确：encrypted RAR/7z 在 session secret store 和安全密码通道完成前明确拒绝；方向改变后的选择性停止和跨 session 持久复用仍属于后续 Phase 4，不能把当前 provider 宣称为完整 RAR/7z 功能。
...
尚未完成的是内存/磁盘混合层、按方向停止未需要的后半段，以及完整结果按 archive fingerprint 跨 session 复用。这些优化必须建立在当前唯一顺序 extractor 上，不能再引入第二条解压主链。

`bun run benchmark:neoview-sevenzip` 会生成 3 页、48 MiB 的固定 solid CB7，并分别测量“首条后相邻页”和“直接请求末页”。2026-07-15 本机独占运行基线（Bun 1.4.0、Windows x64、系统 7-Zip 26.02、临时文件系统、OS cache 状态未指定）为：索引约 `46～47 ms`，第一页 entry-ready `67.73 ms`，随后相邻页 `0.23 ms`，新 provider 直接请求末页 `74.03 ms`；每个场景均只有一个 solid extractor CPU lease，关闭后临时目录残留为零。脚本禁用 archive 时间戳以保持 fixture hash 可重复，并输出运行时、7-Zip 路径/版本、样本与 archive SHA-256、payload/archive 大小和全部调度请求；这些数值是开发机回归基线，不是跨设备性能承诺，基准不得与测试或构建并行执行。
...
嵌套 archive 不会为了形式上的流式进入 JS Buffer。仍待补充的是嵌套加密密码作用域、跨 session fingerprint 复用，以及历史 path stack codec 与 UI 的进入/返回导航。
...
- 已验证 ZIP 流式读取、背压、取消、ETag、错误恢复和 session close；non-solid RAR/7z 已使用单 entry stdout 流，solid RAR/7z 已使用单进程顺序 materializer，并覆盖真实 CB7、CRC、预算、取消和临时目录清理；嵌套 ZIP/7z 已使用路径栈与物化 lease 打通，嵌套加密、历史/UI 导航与 PDF/EPUB 仍需同等级实现和测试。
...
- 已实现非 solid RAR/7z stdout stream、solid archive 单进程顺序预提取和嵌套 archive 物化 lease；继续补加密密码通道、嵌套历史/UI 导航、方向取消与持久复用；
