...
| --- | --- | --- | --- |
| U01 | `[x]` | 11 工具侧栏 | 节点 full 模式保留工具轨道；compact/collapsed 合理降级 |
...
宿主映射证据（2026-07-15）：full 与 compact 真实组件测试逐一按可访问名称定位 11 个扫描工具，collapsed 保留当前工具、状态和扫描/停止主动作；工具轨道宽度和最小化仍由节点布局状态管理。节点根层从会遮挡工作区的 `bg-background` 修正为 `bg-transparent text-foreground`，主题来源标记直接使用 `host.env.theme`，内部面板只消费 Xiranite 语义色，因此全局主题、自定义背景、模糊和遮罩均由宿主统一控制，不复制 fork 主题引擎。组件测试验证 dark 宿主映射、透明层级且节点内不存在最小化/最大化/关闭窗口动作；窗口控制继续完全属于 Xiranite 桌面壳。30 项组件测试通过。
...
Q11 验证证据（2026-07-15）：Windows x64 Release `.node` 为 21.32 MiB，`dav1d.dll` 为 1.70 MiB，二者的发布 ZIP 为 8.75 MiB；Bun smoke 返回 API v3 / Czkawka 10.0.0，覆盖 duplicate、basic、image/video media 三类入口，并真实观测 `CollectingFiles` 进度、取消和 session 清理。构建直接复用仓库 `native/vendor/dav1d-windows-x64.zip` 中的开发包，通过 `PKG_CONFIG_PATH` 定位 `dav1d.pc`，不安装 VCPKG；预编译 ZIP 与 SHA-256 manifest 已由 `packages/czkawka-native/prebuilt/win32-x64` 固化，embedded smoke 也成功加载 API v3。
