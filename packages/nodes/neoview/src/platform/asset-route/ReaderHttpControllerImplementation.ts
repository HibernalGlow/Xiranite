...
import {
  ...
  ReaderConfigPatchInvalid,
  ReaderShellRevisionConflict,
  boundedInteger,
  diagnosticsHistoryExportFormat,
  errorMessage,
  isPreloadOutcome,
  isSchedulerPoolSnapshot,
  isSharedSchedulerSnapshot,
  jsonResponse,
  lockedPageOrder,
  methodNotAllowed,
  pageMetadata,
  parseAdjacentBookRequest,
  parseArchivePasswords,
  parseEntryPaths,
  parseNonNegativeInteger,
  parseOptionalDiagnosticsInteger,
  parsePreloadPerformanceMetrics,
  parsePreloadViewportContext,
  parseReaderActivationProvenance,
  readControlJson,
  reloadTargetPage,
  requirePageIndex,
  safeDecode,
  safeStat,
  schedulerSnapshot,
  sharedSchedulerSnapshot,
  subtitleEtag,
  validPreloadCount,
  validPreloadDuration,
  validTime,
  videoProcessSnapshot,
  waitForSignal,
...
export class ReaderHttpController implements AsyncDisposable {
  ...
  constructor(options: ReaderHttpControllerOptions) {
    ...
    this.#imageProcessing = options.imageProcessing ?? {
      ...DEFAULT_NEOVIEW_IMAGE_PROCESSING_CONFIG,
    }
    ...
    this.#solidArchiveCache =
      options.solidArchiveCache ??
      new SolidArchiveCache({
        maxBytes: options.maxSolidArchiveCacheBytes,
      })
    const ownedVideoProcessScheduler = options.videoProcessScheduler === undefined ? new VideoProcessScheduler() : undefined
    ...
    const bookLoader = createPlatformReaderBookLoader({
      ...options,
      solidArchiveCache: this.#solidArchiveCache,
    ...
    const createImageTransformer = (enabled: () => boolean) =>
    ...
    const loadThumbnailImageTransformer = createImageTransformer(() => this.#imageProcessingPolicy.thumbnailTransformEnabled)
    const loadImageTransformer = createImageTransformer(() => this.#imageProcessingPolicy.sharpFallbackEnabled)
    const loadVideoThumbnailProvider =
      options.loadVideoThumbnailProvider ??
      (async () => {
    ...
    const loadSystemThumbnailProvider =
      options.loadSystemThumbnailProvider ??
      (process.platform === "win32"
        ? async () => {
    ...
    this.#bookTraversal = new ReaderHierarchicalBookTraversal(
      ...
      new ReaderFolderPenetrationResolver(traversalListingProvider, {
        mediaFormats: this.#mediaFormats,
      }),
    )
    ...
    this.#superResolutionArtifacts =
      options.superResolutionArtifactPages && options.superResolutionArtifactStore
    ...
    this.#thumbnailMaintenance = new ThumbnailMaintenanceRoute({
      token: options.token,
      thumbnailStore: options.thumbnailStore,
    })
    ...
    this.#fileOperations = new ReaderFileOperationHttpController(
      async () => {
        ...
        return new ReaderFileOperationService(
          new PlatformReaderFileMutationProvider({
    ...
    this.#mediaProgress = options.mediaProgressStore ? new ReaderMediaProgressService(options.mediaProgressStore) : undefined
  ...
  async handle(request: Request): Promise<Response | undefined> {
    ...
    if (url.pathname === "/reader/diagnostics" && request.method === "GET") {
      ...
      return jsonResponse({
        ...
        reader: {
          ...diagnostics.reader,
          sessionPreload: { generation: telemetry.generation, pages },
        },
      })
    }
  ...
  async #openSession(request: Request): Promise<Response> {
    ...
    if (initialPageValue !== undefined && (typeof initialPageValue !== "number" || !Number.isSafeInteger(initialPageValue) || initialPageValue < 0)) {
    ...
    if (entryPaths === "invalid") {
      return jsonResponse(
        {
          error: "entryPath/entryPaths must contain non-empty archive paths and cannot be combined",
        },
        400,
      )
    }
    ...
    if (archivePasswords === "invalid") {
      return jsonResponse(
        {
          error: "password/archivePasswords must contain valid, uniquely scoped password entries and cannot be combined",
        },
        400,
      )
    }
    ...
    if (body.provenance !== undefined && !provenance) {
      return jsonResponse(
        {
          error: "provenance must contain browserOriginPath and browserOriginEntryPath",
        },
        400,
      )
    }
    try {
      const source: ViewSource = entryPaths ? { kind: "archive", path: body.path, entryPaths } : { kind: "path", path: body.path }
      const session = await this.#service.openViewSource(source, {
        initialPage,
  ...
  async #patchShellConfig(request: Request): Promise<Response> {
    ...
    if (Object.hasOwn(body, "book")) {
      ...
      const operation = this.#configUpdateQueue.then(async () => {
        ...
        if (updated.lockedReadingDirection) {
          this.#sessionOptions = {
            ...this.#sessionOptions,
            direction: updated.lockedReadingDirection,
          }
    ...
    let parsed:
      | ReturnType<typeof parseNeoviewSidebarLayoutPatch>
      | ReturnType<typeof parseNeoviewCardLayoutPatch>
      | ReturnType<typeof parseNeoviewBoardLayoutPatch>
      | ReturnType<typeof parseNeoviewShellControlPatch>
    try {
  ...
  async #metadata(encodedSessionId: string, signal?: AbortSignal): Promise<Response> {
    ...
    const pageFilePath = page && !page.timestamps && !page.entryPath && page.sourcePath !== sourcePath ? page.sourcePath : undefined
  ...
  async #emmMetadataResponse(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    try {
      ...
      const updated = await this.#emmMetadata.update(path, body.expectedRevision as number, body.patch as Record<string, unknown>, request.signal)
  ...
  async #reportPreloadEvents(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    const demandedPageIds = new Set(
      session
        .preloadTelemetry()
        .outcomes.filter((event) => event.outcome === "started")
        .map((event) => event.pageId),
    )
    void this.#archivePreloadDemand.update(session.id, session.pages, session.preloadPlan(), demandedPageIds).catch(() => undefined)
  ...
  async #updatePreloadContext(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    try {
      ...
      const preload = session.updatePreloadContext({
        ...context,
        ...resources,
      })
  ...
  async #preloadAction(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    if (!session) return jsonResponse({ error: "Reader session not found" }, 404)
    const body = await readControlJson(request)
    if (!body || Object.keys(body).length !== 2 || body.confirmed !== true || (body.action !== "cancel-speculative" && body.action !== "release-retained")) {
      return jsonResponse(
        {
          error: "Preload action must be cancel-speculative or release-retained with confirmed=true",
        },
        400,
      )
    }
  ...
  async #updateSessionOptions(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    if (!body || !Object.keys(body).length || Object.keys(body).some((key) => key !== "direction" && key !== "layout")) {
      return jsonResponse(
        {
          error: "Reader session options must contain only direction or layout",
        },
        400,
      )
    }
    ...
    if (direction !== undefined && direction !== "left-to-right" && direction !== "right-to-left") {
      return jsonResponse(
        {
          error: "Reader session options.direction must be left-to-right or right-to-left",
        },
        400,
      )
    }
    ...
    if (layout !== undefined) {
      ...
      if (!Object.keys(record).length || Object.keys(record).some((key) => !layoutKeys.has(key))) {
        return jsonResponse(
          {
            error: "Reader session options.layout contains unsupported fields",
          },
          400,
        )
      }
      if (record.pageMode !== undefined && record.pageMode !== "single" && record.pageMode !== "double") {
        return jsonResponse(
          {
            error: "Reader session options.layout.pageMode must be single or double",
          },
          400,
        )
      }
  ...
  async #updateSessionPageOrder(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    if (!body || !Object.keys(body).length || Object.keys(body).some((key) => !allowed.has(key))) {
      return jsonResponse(
        {
          error: "Reader page order must contain only sortMode, mediaPriority or randomSeed",
        },
        400,
      )
    }
  ...
  async #handleBookSettings(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    try {
      if (request.method === "GET") {
        return jsonResponse({
          settings: await this.#bookSettings.read(session.book.id, defaults, request.signal),
        })
      }
      const body = await readControlJson(request)
      if (!body || Object.keys(body).some((key) => key !== "expectedRevision" && key !== "patch")) {
        return jsonResponse(
          {
            error: "Book settings request must contain expectedRevision and patch",
          },
          400,
        )
      }
  ...
  async #openAdjacentBook(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    if (!parsed)
      return jsonResponse(
        {
          error: "Adjacent-book request must contain a valid direction and optional sort rule",
        },
        400,
      )
    let replacement: ReaderSession | undefined
  ...
  async #handleMediaProgress(encodedSessionId: string, request: Request): Promise<Response> {
    ...
    if (request.method === "GET") {
      try {
        return jsonResponse({
          progress: (await this.#mediaProgress.get(session.book.id)) ?? null,
        })
      } catch (error) {
        if (request.signal.aborted) throw error
        return jsonResponse({ error: errorMessage(error) }, 500)
  ...
  async #subtitleAsset(encodedSessionId: string, encodedPageId: string, encodedAssetId: string, request: Request, url: URL): Promise<Response> {
    const session = this.#findSession(encodedSessionId)
    const pageId = safeDecode(encodedPageId)
  ...
  #runConfigMutation<T>(operation: () => Promise<T>): Promise<T> {
    ...
    this.#configUpdateQueue = result.then(
      () => undefined,
      () => undefined,
    )
    return result
