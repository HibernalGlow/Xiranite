...
use crate::common::dir_traversal::{
    common_get_entry_data, common_get_metadata_dir, common_read_dir, get_modified_time,
};
...
impl EmptyFolder {
    ...
    pub(crate) fn check_for_empty_folders(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
        let mut folders_to_check: Vec<PathBuf> =
            self.common_data.directories.included_directories.clone();
        ...
        while !folders_to_check.is_empty() {
            ...
            let segments: Vec<_> = folders_to_check
                ...
                .map(|current_folder| {
                    ...
                    for entry in read_dir {
                        let Some(entry_data) =
                            common_get_entry_data(&entry, &mut warnings, &current_folder)
                        else {
                            continue;
                        };
            ...
            let required_size = segments
                .iter()
                .map(|(segment, _, _, _)| segment.len())
                .sum::<usize>();
        ...
        let mut folder_entries: HashMap<String, FolderEntry> = HashMap::with_capacity(
            start_folder_entries.len()
                + new_folder_entries_list.iter().map(Vec::len).sum::<usize>(),
        );
    ...
    pub(crate) fn set_as_not_empty_folder(
        ...
        loop {
            ...
            if let Some(parent_path) = &d.parent_path {
                ...
                d = folder_entries
                    .get_mut(&cf)
                    .unwrap_or_else(|| panic!("Folder {cf} not found in folder_entries"));
                if d.is_empty == FolderEmptiness::No {
