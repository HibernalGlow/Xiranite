...
CLI 侧 14 个节点已完成 TOML 统一改造，但前端节点 UI 仍通过 `host.getData/patchData` 把所有输入/输出/日志混存在 `comp.data` 中，没有"默认配置"概念。前端必须同步接入 `xiranite.config.toml` 体系，否则 CLI 配置迁好了、节点 UI 还是各自一套输入框状态，会分裂。

前端配置统一方案详见 **[docs/frontend-node-config-plan.md](./frontend-node-config-plan.md)**。

核心原则：

- `xiranite.config.toml` 是人工配置主源，前端不直接读写文件，通过 backend config service 读写。
- 节点 UI 只关心自己的 section（`nodes.<nodeId>`）。
- 区分"本次输入"（`comp.data`）和"默认配置"（TOML）。
- 合并优先级：`comp.data` 配置字段 > `xiranite.config.toml [nodes.<nodeId>]` > 节点包默认值。
- 运行结果、日志、history 不进 TOML，继续存 `comp.data` 或 DB。
- 节点包通过 `host.getNodeConfig` / `host.saveNodeConfig` 间接访问，不直接依赖 `@xiranite/config`。
