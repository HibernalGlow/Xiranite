...
- 已知差异：L2 已复用引用计数 lease 保留 response 与 current/near frame 实际生成的每页最新 presentation；L3 复用 proper-lockfile 的 hashed per-key 短锁保护跨实例磁盘 get/put/remove，maintenance 对正在读取的 key 非阻塞跳过；diagnostics 与固定/真实图片基准已接入，current listing 预算降级及更多格式/尺寸/压力 corpus 校准仍待完成
