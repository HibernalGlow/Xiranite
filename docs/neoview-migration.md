...
| Phase 4：复杂格式与 CLI/TUI | 进行中（图片阅读原型可用） | EPUB/7z/RAR/nested provider；共享 HeadlessController；`xneoview` inspect/pages/frame/extract 支持本地或 `--connect` 常驻后端；持久 TUI 工作台通过共享 `TerminalImagePreview` 流式显示单/双页 frame | 真实 EPUB 语料、SIXEL/Kitty 终端验收、TUI 远程连接、缩略图 gallery、完整设置/feature 命令、平台能力诊断；PDF 不计入当前阶段验收 |
...
- 已增加共享 `ReaderHeadlessController`、`xneoview inspect/pages/frame/extract-page`、这些命令的 loopback `--connect` adapter 和持久 TUI Reader 工作台；继续补真实图片协议、TUI/diagnostics 远程连接、设置和后续 feature 命令，不复制 application 逻辑；
