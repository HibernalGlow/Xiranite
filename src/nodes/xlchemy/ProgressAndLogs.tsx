...
export function WorkbenchTelemetry(props: { format: string; phase: XlchemyPhase; progress: number; result: XlchemyData | null; running: boolean; threads: number }) {
  ...
  const converted = props.format === "dynar" ? props.result?.renamedCount ?? 0 : props.result?.convertedCount ?? 0
...
export function ProgressWorkbench(props: { data: XlchemyCardState; format: string; sourceCount: number; progress: number; result: XlchemyData | null; running: boolean; onPatch: (patch: Partial<XlchemyCardState>) => void }) {
  ...
  const progressCount = `${completed}/${hasUnknownEfuTotal ? "?" : total}`
  ...
  const encoder = props.format === "dynar" ? "filesystem" : props.format === "JPEG XL" ? "cjxl" : props.format === "AVIF" ? props.data.avifEncoder === "svt" ? "SVT-AV1" : props.data.avifEncoder === "slimg" ? `slimg ${props.data.slimgBackend === "cli" ? "CLI" : "DLL"}` : "AOM AV1" : props.format === "JPEG" ? props.data.jpegEncoder === "libjpeg" ? "libjpeg" : "JPEGLI" : props.format
  const action = props.format === "dynar" ? "重命名" : "转换"
  return <div className="flex flex-col gap-3" data-testid="xlchemy-progress-workbench"><div className="rounded-md border bg-card p-3"><div className="mb-3 flex items-center justify-between gap-3"><div><div className="text-[10px] uppercase tracking-wider text-muted-foreground">运行状态</div><div className="mt-1 text-sm font-semibold">{props.running ? `${action}进行中` : total > 0 ? `最近一次${action}已完成` : "等待开始"}</div></div><Badge variant={props.running ? "default" : total > 0 ? "secondary" : "outline"}>{props.running ? "运行中" : total > 0 ? "已完成" : "待机"}</Badge></div><Progress value={props.progress} /><div className="mt-3 grid grid-cols-3 gap-2">{showCounter && <Stat label="进度" value={progressCount} />}<Stat label="已用时间" value={formatDuration(elapsedMs)} />{showEta && <Stat label="ETA" value={props.running && !hasUnknownEfuTotal ? formatDuration(etaMs) : "--:--"} />}</div></div>{(props.running || elapsedMs > 0) && <div className="flex items-center gap-3 rounded-md border bg-card px-3 py-2 text-[11px] tabular-nums text-muted-foreground"><span className="inline-flex items-center gap-1"><Clock className="size-3" /><span className="text-foreground">{formatDuration(elapsedMs)}</span></span>{props.running && completed > 0 && <>{showEta && !hasUnknownEfuTotal && <span className="inline-flex items-center gap-1"><Timer className="size-3" />ETA {formatDuration(etaMs)}</span>}<span>{speed >= 1 ? `${speed.toFixed(1)}/s` : `${(speed * 60).toFixed(1)}/min`}</span></>}<span className="ml-auto">{props.progress}%</span></div>}{props.data.currentFile && (showCurrentFile || showSizeChange) && <div className="rounded-md border bg-card px-3 py-2">{showCurrentFile && <div className="truncate text-xs font-medium">{props.data.currentFile}</div>}{showSizeChange && <div className="mt-1 text-[11px] text-muted-foreground">{props.data.progressText}</div>}</div>}{showSummary && <div className="text-xs leading-5 text-muted-foreground">{props.data.progressText || "添加文件后，这里会显示当前进度、耗时、ETA 和输出摘要。"}</div>}{(showCounter || showFormat || showEncoder) && <div className="flex flex-wrap items-center gap-1.5 text-[10px] text-muted-foreground">{showCounter && <Badge variant="outline">{progressCount}</Badge>}{showFormat && <Badge variant="outline">{props.format}</Badge>}{showEncoder && <Badge variant="outline">{encoder}</Badge>}</div>}{props.data.showRawProgress !== false && <div className="rounded-md border bg-muted/40 p-2.5 font-mono text-[11px] text-muted-foreground"><div>{props.data.progressText || "—"}</div><div>{props.result ? `${props.result.outputBytes} bytes · ${props.result.errorCount} errors` : "—"}</div></div>}<div className="grid grid-cols-2 gap-2 rounded-md border bg-card p-3 text-[11px] @sm/xlchemy:grid-cols-4"><Option checked={showCounter} label="计数" onChange={(showProgressCounter) => props.onPatch({ showProgressCounter })} /><Option checked={showSummary} label="摘要" onChange={(showProgressSummary) => props.onPatch({ showProgressSummary })} /><Option checked={showEta} label="ETA" onChange={(showProgressEta) => props.onPatch({ showProgressEta })} /><Option checked={showFormat} label="格式" onChange={(showProgressFormat) => props.onPatch({ showProgressFormat })} /><Option checked={showEncoder} label="编码器" onChange={(showProgressEncoder) => props.onPatch({ showProgressEncoder })} /><Option checked={showCurrentFile} label="当前文件" onChange={(showProgressCurrentFile) => props.onPatch({ showProgressCurrentFile })} /><Option checked={showSizeChange} label="大小变化" onChange={(showProgressSizeChange) => props.onPatch({ showProgressSizeChange })} /><Option checked={props.data.showRawProgress ?? true} label="原始日志" onChange={(showRawProgress) => props.onPatch({ showRawProgress })} /></div></div>
}
...
export function ConversionLog(props: { logs: string[]; onClear: () => void; onCopy: (text: string) => void }) {
  ...
  const bottom = useRef<HTMLDivElement>(null), entries = props.logs.map(parseLog), filtered = entries.filter((entry) => levels.includes(entry.level) && (!query.trim() || entry.message.toLowerCase().includes(query.trim().toLowerCase())))
...
function parseLog(line: string, index: number): LogEntry { const lower = line.toLowerCase(), level: LogLevel = /error|failed|失败|错误/.test(lower) ? "error" : /warn|skip|警告|跳过/.test(lower) ? "warn" : /success|complete|完成|converted|renamed/.test(lower) ? "success" : "info"; return { time: /^\d\d:\d\d:\d\d/.exec(line)?.[0] ?? new Date(index * 1000).toISOString().slice(11, 19), level, message: line.replace(/^\d\d:\d\d:\d\d\s*/, "") } }
