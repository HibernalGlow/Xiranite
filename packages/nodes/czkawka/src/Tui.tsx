    ...
    <box flexGrow={1} minHeight={0} marginTop={1} flexDirection="row" gap={1}>
      <box width="27%" minWidth={24} flexDirection="column" gap={1}>
        ...
        <WorkbenchPanel title="▣ SCAN / OPERATION INPUT" description="扫描与共享文件操作" flexGrow={1}><ActionTabs id="czkawka-input-tabs" options={[{ value: "roots", label: "目录" }, { value: "filters", label: "过滤" }, { value: "algorithm", label: "算法" }, { value: "operations", label: "操作" }]} value={panel} focused={false} onFocus={() => undefined} onChange={(value) => setPanel(value as typeof panel)} />
          <scrollbox flexGrow={1} minHeight={0} marginTop={1}><box flexDirection="column" gap={1}>{panel === "roots" ? <><Field id="tool" /><Field id="includedDirectoriesText" /><Field id="excludedDirectoriesText" /><box height={3} flexShrink={0} flexDirection="row" gap={1}><box width="50%"><Field id="recursive" /></box><box flexGrow={1}><Field id="useCache" /></box></box></> : panel === "filters" ? <><Field id="excludedItemsText" /><Field id="allowedExtensions" /><Field id="excludedExtensions" /><Field id="filterText" /></> : panel === "algorithm" ? getCzkawkaToolOptions(tool).map((option) => <Field key={option.id} id={option.id} />) : <><Field id="selectedPathsText" />{action === "move" ? <><Field id="destinationDirectory" /><Field id="copyMode" /><Field id="preserveStructure" /><Field id="conflictPolicy" /></> : null}{action === "delete" ? <Field id="deleteMode" /> : null}{action === "save" ? <Field id="outputPath" /> : null}{action === "delete" || action === "move" ? <Field id="dryRun" /> : null}</>}</box></scrollbox>
        </WorkbenchPanel>
      </box>
      <WorkbenchPanel title={`▦ RESULT GROUPS · ${data?.groupCount ?? 0}`} description="组轨道 / 文件 / 大小 / 详情" flexGrow={1}>
        ...
        <scrollbox id="czkawka-results" flexGrow={1}>{data?.groups.length ? data.groups.flatMap((group) => group.entries.map((entry, index) => <box key={entry.id} height={2} flexShrink={0} flexDirection="row"><box width={8}><text fg={entry.status === "error" ? theme.colors.error : entry.status === "skipped" ? theme.colors.warning : groupColor(group.id, theme)}>{entry.status ? entry.status.slice(0, 7).toUpperCase() : `${index === 0 ? "◆" : index === group.entries.length - 1 ? "└" : "│"} ${String(group.id + 1).padStart(2, "0")}`}</text></box><box width="20%"><text>{entry.status ?? formatBytes(entry.size)}</text></box><box width="45%"><text fg={theme.colors.foreground}>{entry.path}</text></box><text fg={entry.error ? theme.colors.error : theme.colors.mutedForeground}>{entry.error ?? entry.secondaryPath ?? entry.detail ?? entry.properExtension ?? entry.similarity ?? ""}</text></box>)) : <text fg={theme.colors.mutedForeground}>选择工具与目录，然后点击“开始扫描”。</text>}</scrollbox>
      </WorkbenchPanel>
