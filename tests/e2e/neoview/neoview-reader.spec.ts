...
async function selectFolderToolbarAction(page: Page, folderCard: Locator, action: string): Promise<void> {
  if (action === "多选模式" || action === "退出多选") {
    const multi = folderCard.getByRole("button", { name: /多选/ })
    await multi.click()
    return
  }
  const direct = folderCard.getByRole("button", { name: action, exact: true })
  ...
  if (moreLabels.has(action) || action === "更多操作" || action === "更多") {
    await folderCard.getByRole("button", { name: "更多", exact: true }).click()
    if (action === "更多操作" || action === "更多" || action === "项目尺寸") return
    const mapped = action === "重载缩略图" ? "重载可见缩略图" : action
    const item = page.getByRole("menuitem", { name: mapped, exact: true })
    if (await item.count()) await item.click()
    else await page.getByRole("menuitemcheckbox", { name: mapped, exact: true }).click()
    return
  }
  if (action === "视图") {
    await folderCard.getByRole("button", { name: "视图", exact: true }).click()
    return
  }
  if (action === "排序") return
  if (action === "粘贴到当前目录") {
    await folderCard.getByRole("button", { name: /粘贴/ }).click()
    return
  }
  await folderCard.getByRole("button", { name: action, exact: true }).click()
}

async function selectFolderViewMode(page: Page, folderCard: Locator, view: string): Promise<void> {
  const direct = folderCard.getByRole("button", { name: view, exact: true })
  if (await direct.count()) {
    await direct.click()
    return
  }
...
async function openFolderMoreActions(page: Page, folderCard: Locator): Promise<void> {
  // Single-tab mode: create-tab sits on the breadcrumb next to path actions.
...
test("[neoview.react.cbz-e2e] [neoview.thumbnail.react-e2e] [neoview.shell.e2e] [neoview.folder.tree-layout-e2e] [neoview.folder.tree-pins-e2e] [neoview.folder.tree-roots-e2e] [neoview.folder.watch-live-e2e] decodes, virtualizes thumbnails and navigates a real CBZ", async ({ page }, testInfo) => {
  ...
  await selectFolderToolbarAction(page, folderCard, "多选模式")
  ...
  await selectFolderToolbarAction(page, folderCard, "文件树")
...
test("[neoview.folder.filter-e2e] filters the current directory without reopening the File Card", async ({ page }) => {
  ...
  try {
    ...
    await selectFolderToolbarAction(page, folderCard, "类型筛选")
...
test("[neoview.folder.panel-keepalive-e2e] keeps the File Card alive across panel and edge visibility changes", async ({ page }, testInfo) => {
  await page.addInitScript(({ baseUrl, token }) => {
    window.__XIRANITE_BACKEND__ = { baseUrl, token }
  }, { baseUrl: backend.url, token: backend.token })
  await page.goto(`/tests/e2e/neoview/neoview-harness.html?path=${encodeURIComponent(fixture.path)}`, { waitUntil: "domcontentloaded" })
  ...
  const folderCard = leftSidebar.locator('[data-neoview-folder-card="true"]')
  const viewButton = folderCard.getByRole("button", { name: "视图", exact: true })
  await viewButton.focus()
  await viewButton.press("Enter")
  const coverGridItem = page.getByRole("menuitemradio", { name: "封面网格", exact: true })
  await expect(coverGridItem).toBeVisible()
  await coverGridItem.press("Enter")
  await page.mouse.move(24, page.viewportSize()!.height / 2)
  const thumbnailEntry = folderCard.getByTitle(join(fixture.directory, "nested-search"), { exact: true })
  const thumbnailImage = thumbnailEntry.locator("img")
  await expect(thumbnailImage).toBeVisible({ timeout: 30_000 })
  await thumbnailEntry.click({ modifiers: ["Control"] })
  await expect(folderCard).toHaveAttribute("data-selection-count", "1")
  const thumbnailUrl = await thumbnailImage.getAttribute("src")
  const thumbnailBox = await thumbnailImage.boundingBox()
  await thumbnailImage.evaluate((element) => element.setAttribute("data-panel-keepalive-thumbnail", "stable"))
  await thumbnailEntry.evaluate((element) => element.setAttribute("data-file-card-instance", "stable"))
  ...
  await expect(thumbnailEntry).toHaveAttribute("data-file-card-instance", "stable")
  await expect(thumbnailEntry).toHaveAttribute("aria-selected", "true")
  await expect(folderCard).toHaveAttribute("data-selection-count", "1")
  await expect(thumbnailImage).toHaveAttribute("data-panel-keepalive-thumbnail", "stable")
  await expect(thumbnailImage).toHaveAttribute("src", thumbnailUrl ?? "")
  ...
  await page.mouse.move(1, page.viewportSize()!.height / 2)
  await expect(leftSidebar).toBeVisible()
  await expect(page.locator('[data-reader-edge="left"]')).toHaveAttribute("data-reader-edge-presentation", "visible")
  await page.waitForTimeout(180)
  await expect(thumbnailEntry).toHaveAttribute("data-file-card-instance", "stable")
  await expect(thumbnailImage).toHaveAttribute("data-panel-keepalive-thumbnail", "stable")
  await expect(thumbnailImage).toHaveAttribute("src", thumbnailUrl ?? "")
  expect(await thumbnailImage.boundingBox()).toEqual(thumbnailBox)
  await page.screenshot({ path: testInfo.outputPath("folder-panel-keepalive.png"), fullPage: false })
...
test("[neoview.folder.tabs-lifecycle-e2e] [neoview.folder.tabs-navigation-history-e2e] [neoview.folder.tabs-bulk-close-e2e] [neoview.folder.tabs-pin-duplicate-e2e] [neoview.folder.tabs-reopen-e2e] [neoview.folder.selection-tab-isolation-e2e] keeps Explorer folder tabs isolated and releases closed sessions", async ({ page }) => {
  ...
  try {
    ...
    const createTabButton = folderCard.getByRole("button", { name: "新建文件夹标签" })
    if (await createTabButton.count()) {
      await createTabButton.focus()
      await createTabButton.press("Enter")
    } else {
      await page.getByRole("menuitem", { name: "新建文件夹标签", exact: true }).click()
    }
    await expect(currentBreadcrumb()).toHaveAttribute("title", secondPath)
    await expect(folderCard.locator('[data-neoview-folder-selection-bar="true"]')).toHaveCount(0)
    await expect(folderCard).toHaveAttribute("data-selection-count", "0")
    ...
    await openFolderMoreActions(page, folderCard)
    const createTabButtonAgain = folderCard.getByRole("button", { name: "新建文件夹标签" })
    if (await createTabButtonAgain.count()) {
      await createTabButtonAgain.focus()
      await createTabButtonAgain.press("Enter")
    } else {
      await page.getByRole("menuitem", { name: "新建文件夹标签", exact: true }).click()
    }
    await expect(currentBreadcrumb()).toHaveAttribute("title", secondPath)
