...
function serializeNode(node: any, depth = 0, maxDepth = 10): any {
  ...
  // Text (sanitized 鈥?scene content is untrusted user data)
  try {
    ...
    try {
      ...
      switch (type) {
        ...
        case 'render': {
          ...
          import.meta.hot!.send('agent:state', {rendering: true});
          // Render asynchronously so the HTTP request can return immediately.
          rendererInstance
            .render(settings)
            .catch((err: any) => {
              if (import.meta.hot) {
            ...
            .finally(() => {
              if (import.meta.hot) {
                import.meta.hot.send('agent:state', {rendering: false});
              }
            });
        ...
        case 'get-threads': {
          ...
          const scene = playerInstance.playback.currentScene as any;
          if (!scene) throw new Error('No active scene');
          // Thread info is limited 鈥?we can report what's available
