...
test.beforeAll(async () => {
  ...
  await writeFile(configPath, [
    "[nodes.neoview]", "schema_version = 1",
    "[nodes.neoview.panels]", "left_sidebar_visible = true",
    "[nodes.neoview.panels.sidebar_control]", "enabled = false",
    "[nodes.neoview.panels.sidebars.left]", "pinned = true", "open = true", "width = 320",
    "[nodes.neoview.panels.edges.top]", "enabled = true", "initial_visible = true", "pinned = true", "trigger_size = 32", "lock_mode = \"locked-open\"",
    "[nodes.neoview.panels.edges.left]", "enabled = true", "initial_visible = true", "pinned = true", "trigger_size = 32", "lock_mode = \"locked-open\"", "",
  ].join("\n"), "utf8")
  backend = await startBackend({ token: "neoview-history-context-actions-e2e", repository: createMemoryWorkspaceRepository(), configPath, legacyThumbnailDatabasePath: databasePath })
})
...
test("[neoview.history.context-actions-e2e] [neoview.history.thumbnail-reload-e2e] reuses folder-style thumbnails for history file actions", async ({ page }) => {
  ...
  await readerImage.evaluate((node) => node.setAttribute("data-history-context-image", "stable"))

  const sidebar = page.locator('[data-reader-sidebar="left"]')
  await activateControl(sidebar.getByRole("button", { name: "历史记录", exact: true }))
  const card = sidebar.locator('[data-reader-card="历史记录"]')
  await activateControl(card.getByRole("button", { name: "内容" }))
  const row = card.locator('[data-history-id]').filter({ hasText: "fixture.cbz" }).first()
  await expect(row).toBeVisible()
  await expect(card.locator('[data-neoview-history-context-actions="ready"]')).toBeAttached()
  await expect(row).toHaveAttribute("data-context-menu", "neoview-history-entry")
  ...
  const rowButton = row.locator('[data-history-row-button]')
  ...
  const refreshResponse = page.waitForResponse((response) => {
    ...
    const body = response.request().postDataJSON() as { items?: Array<{ refresh?: boolean }> }
  ...
  expect(await readerImage.getAttribute("data-history-context-image")).toBe("stable")
...
async function openContextMenu(button: import("@playwright/test").Locator) {
  await button.focus()
  await expect(button).toBeFocused()
