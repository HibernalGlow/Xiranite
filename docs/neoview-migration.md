...
当前 `ReaderApp` 的常驻顶部路径栏、底部导航和缩略图条只是验证 Reader HTTP/DOM `<img>`/虚拟列表的 smoke scaffold，**不构成 `panels-toolbar-shell` 或 `card-windows-tabs` 的兼容实现**。正式 UI 重构必须先建立 `ReaderEdgeShell`、四个 edge slot、hover/pin 状态机及 registry contract，再逐 panel/card 从 AST scaffold 接入；完成前这两个 feature 一律保持 `pending`。
...
`XIRANITE_CHUNK_REPORT=1 vite build && bun run audit:neoview-build` 的 2026-07-15 最新生产构建证据为：包含原图预解码和虚拟缩略图的 NeoView React app 独立懒 chunk `25,715 bytes`，初始 chunk 中 NeoView 实现模块为 `0`，前端 `zip.js` 模块为 `0`。同次初始产物为 `529,176 bytes`，仍高于全仓通用 520,000 bytes 门槛，但模块报告证明超额不是 NeoView 实现泄漏；NeoView 专属门禁独立限制 app chunk 不超过 40 KiB，并禁止 Reader/zip.js 泄漏到首屏。sharp 只存在于 backend 二级动态 import，不参与前端构建。
