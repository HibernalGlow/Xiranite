...
import { ACTIONS, type ClassfAction } from "./constants"
...
export function ActionPicker(props: {
  ...
  onActionChange: (action: ClassfAction) => void
}) {
  return (
    <ToggleGroup
      aria-label="classf action"
      ...
      variant="outline"
      onValueChange={(value) => {
        if (value) props.onActionChange(value as ClassfAction)
      }}
    >
      {ACTIONS.map((item) => (
    ...
    <div className="flex min-w-0 flex-col gap-1.5">
      {!props.compact && (
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="classf-paths" className="flex items-center gap-1.5">
      ...
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-1.5">
        {props.compact ? (
          ...
          <Textarea
            ...
            placeholder={"每行一个归档或目录路径，例如：\nD:/Archives/pack1.zip\nD:/Archives/pack2.zip"}
            value={props.data.pathsText ?? ""}
            onChange={(event) => props.onPatch({ pathsText: event.currentTarget.value })}
          />
        )}
...
export function PathFields(props: {
  ...
  return (
    <div className="grid gap-2 @3xl/classf:grid-cols-2">
      <Input
        aria-label="classf 配置文件"
        disabled={props.disabled}
        placeholder="配置文件，可选"
      ...
      />
      <Input
        aria-label="classf Python 可执行文件"
        disabled={props.disabled}
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="classf 运行选项" disabled={props.disabled} size="icon-sm" variant="outline">
              <Settings2 />
  ...
  return (
    <Popover>
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="classf 默认配置" disabled={props.disabled} size="icon-sm" variant={props.configDirty ? "secondary" : "outline"}>
              <DatabaseZap />
      ...
      <PopoverContent align="end" className="w-72">
        <div className="mb-3">
          ...
          <p className="text-xs text-muted-foreground">保存 ClassF 的路径和运行选项。</p>
        </div>
        <div className="grid gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-xl">
              <DialogHeader>
                <DialogTitle>ClassF 配置</DialogTitle>
                <DialogDescription>当前 nodes.classf 默认值和配置文件位置。</DialogDescription>
              </DialogHeader>
...
function ConfigPreview(props: {
  config?: Partial<ClassfCardState>
  path?: string
}) {
