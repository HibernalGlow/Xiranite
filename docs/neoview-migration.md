...
| 子系统 | 状态 | 当前实现 | 未完成边界 |
| --- | --- | --- | --- |
| Reader domain/application | 可用但未全兼容 | `packages/nodes/neoview/src/domain` 与 `application`；GUI/CLI/TUI 共用 session/controller；显式页码优先、默认恢复上次进度，翻页写入不阻塞主链且关闭强制刷盘；共享 `ReaderLibraryService` 提供最近阅读、书签和列表契约，CLI/OpenTUI 已覆盖分页、添加、删除、列表和有界清理 | 完整阅读模式、历史/书签剩余 GUI 批量交互、无效路径同步、统计/播放列表和全部错误恢复 |
