...
import { normalizeXlchemyInput } from "@xiranite/node-xlchemy/core"
  ...
  async function execute(nextAction: XlchemyAction) {
    ...
    try {
      ...
      if (nextAction === "diagnose") patch({ environment: response.data?.environment?.length ? response.data.environment : unavailableEnvironment("运行端待刷新，请重新检测"), environmentCheckedAt: response.data?.environment?.length ? new Date().toISOString() : undefined, progressText: response.data?.environment?.length ? response.message : "运行端尚未加载新版工具检测，请刷新后重试。" })
        ...
        const next: Partial<XlchemyCardState> = { phase: cancelled ? "cancelled" : response.success ? "completed" : "error", progress: response.success ? 100 : cancelled ? dataRef.current.progress ?? 0 : 0, progressText: response.message, result: response.data ?? null }
...
function FormatControls({ props }: { props: ViewProps }) {
  const lossy = !(props.data.lossless ?? false), outputMode = props.data.outputMode ?? "source", fixedMode = props.format === "Lossless JPEG Transcoding" || props.format === "JPEG Reconstruction"
  return <div className="flex flex-col gap-2">
    <div className="grid grid-cols-[repeat(auto-fit,minmax(8rem,1fr))] gap-2">
      <Field className="gap-1"><FieldLabel className="text-[10px]">目标格式</FieldLabel><Select value={props.format} onValueChange={(format) => props.onPatch({ format: format as XlchemyFormat })}><SelectTrigger className="w-full" size="sm" aria-label="目标格式"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{FORMATS.map((item) => <SelectItem key={item.value} value={item.value}>{item.label} · {item.extension}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>
      {!fixedMode && <Field className="gap-1"><FieldLabel className="text-[10px]">压缩模式</FieldLabel><ToggleGroup type="single" value={lossy ? "lossy" : "lossless"} className="grid grid-cols-2" size="sm" onValueChange={(value) => value && props.onPatch({ lossless: value === "lossless" })}><ToggleGroupItem value="lossless">无损</ToggleGroupItem><ToggleGroupItem value="lossy">有损</ToggleGroupItem></ToggleGroup></Field>}
      <Field className="gap-1"><FieldLabel className="text-[10px]">输出位置</FieldLabel><ToggleGroup type="single" value={outputMode} className="grid grid-cols-2" size="sm" variant="outline" onValueChange={(value) => value && props.onPatch({ outputMode: value as "source" | "directory" })}><ToggleGroupItem value="source">源文件旁</ToggleGroupItem><ToggleGroupItem value="directory">指定目录</ToggleGroupItem></ToggleGroup></Field>
