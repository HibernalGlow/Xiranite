...
| Panel/Card | 兼容布局、折叠与设置态管理纵切 | 完整旧 panel ID metadata；Card lazy loader；关闭/折叠后内容 DOM=0；旧布局/card v14 经 TOML 规范化；新版状态写入 canonical `panel_state/card_state`；“卡片管理”按需加载 Dice UI Kanban；“边栏管理”复用旧 SidebarManagementPanel 行为，支持搜索、显隐、左右移动、同栏顺序和重置；两者都只在保存时一次 board PATCH；设置模块也是共享 registry Card，默认只在设置浮窗，显式停靠后可进入静态侧栏 | 其余全部业务卡片、Panel 高级组件设置、Card 高度拖动、独立 card window/tab |
...
- 前端 NeoView：13 个测试文件、35 项 component/hook/registry/shell/config/resize/card/settings/Kanban 测试通过；后端 NeoView 最近完整复跑为 36 个文件、Vitest 179 项 + Bun TUI 1 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖边栏草稿零写入、左右移动、顺序、隐藏、重置、未知未来 Panel 保留、两张设置 Card 默认未停靠及显式侧栏停靠；
- Reader pipeline 最近复跑：backend startup `11.50 ms`、open `56.90 ms`、navigation p95 `6.36 ms`、response headers p95 `2.80 ms`、first byte p95 `2.91 ms`、full stream p50 `42.80 ms`、throughput p50 `66.33 MiB/s`、RSS delta `0 MiB`；全部通过预算，设置管理没有进入图片数据热路径；
- 真实 Chromium 路径已覆盖桌面与 `420×360`：默认边栏页修改 Panel 位置时不发请求，保存后仅一次 board PATCH；“卡片管理”再将两张默认隐藏设置 Card 停靠到设置 Panel 并仅一次 PATCH；关闭浮窗后左栏静态加载同一两张共享 Card，原图 `<img>` 节点身份不变；最终热翻页为 desktop `57.8/83.9 ms`、card `65.9/85.6 ms`（navigation/decode），仍明显低于 `150/200 ms` 硬门槛；
- 当前生产构建报告：NeoView entry `32,117 bytes`，ReaderSidebar `6,315 bytes`，设置浮窗 `3,272 bytes`，SidebarManagementSettingsCard `5,942 bytes`，PanelLayoutEditor `3,903 bytes`，Dice UI Kanban/dnd-kit runtime `58,547 bytes`；两张设置 Card 均保持延迟 chunk，Kanban 仍为“卡片管理”二级延迟 chunk，未进入初始 chunk、NeoView entry 或普通 ReaderSidebar；PageNavigationCard `1,233 bytes`、BookInformationCard `1,015 bytes`，首屏 NeoView module `0`、前端 zip.js module `0`。
