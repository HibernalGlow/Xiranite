...
`folderMain` 前三条核心纵切已经接入：GUI/CLI/TUI 可复用的 `CoreReaderDirectoryBrowser` 维护稳定目录快照、前进/后退/上级/刷新和 generation；鉴权 HTTP 控制路由提供 128 项任意 cursor 分页、session 级排序 PATCH 和显式释放；后端单层浏览继续使用可取消的原生 `opendir()`，真正的递归索引/搜索收口到独立 `readdirp` stream adapter，文件树增量事件收口到按需动态导入的 `@parcel/watcher` adapter。排序在分页前由唯一应用层排序器完成，八字段契约已冻结，name/type/path/random 即时重排，date/size 通过 Node/Bun 原生 `Readable.map({ concurrency, signal })` 有界并发 metadata adapter 按需 `stat`；重排后返回焦点路径的新索引，前端直接请求对应稀疏页。React `FolderMainCard` 是独立 lazy chunk，仅在可见时打开浏览会话；当前已使用 React Virtuoso 驱动最多 12 个批次的有界稀疏 catalog，提供 compact/cover-grid、稳定选择、前进/后退快照、返回父目录建议索引与远端批次自动定位，并把可见项按最多 64 项注册到现有 opaque library thumbnail route；折叠/卸载后取消分页/缩略图并释放 context/session。当前仍标记 `partial`，因为 cover/mosaic list、mosaic grid、details、完整多选/键盘导航、多标签、搜索、上下文文件操作、目录递归大小、EMM/评分批量 hydration，以及排序记忆/默认值/四级优先级仍待迁移。
...
本轮聚焦证据：后端排序/目录会话/route/opendir/metadata/scanner/watcher 7 文件 12 项通过；前端 client/catalog/Card 3 文件 12 项通过，其中覆盖排序 PATCH、焦点路径新索引、可见缩略图批次与卸载 context 释放；NeoView package build 与根 `typecheck:app` 通过；`@parcel/watcher` 在当前 Windows/Bun 环境完成真实临时目录 `create` 事件并 unsubscribe。尚未运行生产 chunk、10K/100K corpus、Playwright 和完整性能审计，因此不得据此宣称 `folderMain` 完成。

#### `folderMain` 完整兼容清单（2026-07-16）

权威机器清单为 `migration/neoview/folder-main-compatibility.json`，共 74 项：`partial=21`、`pending=53`、`complete=0`。每项固定包含 `sourceEvidence`、`targetContract`、`surfaces`、`status`、`testIds` 和 `notes`；本文表格是便于阅读的索引，发生冲突时以源码证据与 JSON 契约为准。`complete` 必须同时满足功能、原版 UI、设置、快捷键、异常态、生命周期、自动化测试和适用的 GUI/CLI/TUI 共用契约。
...
| Phase 2：目录/归档/唯一数据链 | 进行中（主阅读链较完整） | 目录/单图、ZIP、non-solid/solid 7z/RAR、嵌套包、AES ZIP、loopback HTTP stream、ETag/Range/cancel、图片探测、惰性 sharp、L2 cache、缩略图生成/批写/失败退避/在线维护 | PDF/EPUB、RAR/7z 安全密码通道、L3 cache、缩略图离线维护与旧版本迁移、更多真实语料 |
...
| Phase 4：复杂格式与 CLI/TUI | 进行中（图片阅读原型可用） | 7z/RAR/nested provider；共享 HeadlessController；`xneoview` inspect/pages/frame/extract；持久 TUI 工作台通过共享 `TerminalImagePreview` 流式显示单/双页 frame | PDF/EPUB、真实 SIXEL/Kitty 终端验收、缩略图 gallery、`--connect`、完整设置/feature 命令、平台能力诊断 |
...
读写 adapter 已接入同一 `platform` 懒工厂：连接生命周期内缓存 statement，提供严格的 `key + category` 单条查询和最多 512 key 的去重批量查询，不为普通命中逐次更新日期；原始 blob 零额外复制返回，`LZ4\0 + lz4_flex size-prepended block` 通过 `lz4-napi` 异步解压并在解压前校验声明长度。writer 以 50 ms/32 项阈值合并、`BEGIN IMMEDIATE`、busy 指数退避和只更新缩略图列的 UPSERT 持久化规范 WebP，不清空 EMM/评分/AI/标签；成功写入同时删除对应失败记录。`bun run benchmark:neoview-thumbnails -- --iterations 1000 --batch 100` 只输出聚合值；当前真实 2.4 库连续两轮单条命中为 `0.0725～0.0728 ms`，100 条批量为 `19.24～21.15 ms/批`（`0.192～0.212 ms/条`）。
