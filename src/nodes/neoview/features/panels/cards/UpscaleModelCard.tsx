...
export default function UpscaleModelCard({ client, session, disabled, superResolution, onSuperResolutionConfigChange, pickDirectory }: ReaderPanelContext) {
  ...
  useEffect(() => {
    ...
    commit({ defaultModelId: model.id, defaultScale: model.scales.includes(2) ? 2 : model.scales[0] })
  ...
  return <div className="space-y-3 text-xs" data-neoview-upscale-model="true">
    {config?.provider === "disabled" ? <div className="py-4 text-center text-muted-foreground"><p>超分功能不可用</p><p className="mt-1 text-[10px]">请启用 OpenComic 超分 provider</p></div> : <>
      <div className="space-y-1"><Label htmlFor="neoview-upscale-model">模型</Label><div className="flex gap-1"><select id="neoview-upscale-model" className="h-7 min-w-0 flex-1 rounded border border-input bg-background px-2 text-xs" value={selectedModel} disabled={disabled || models.length === 0} onChange={(event) => { const model = models.find((candidate) => candidate.id === event.currentTarget.value); commit({ defaultModelId: event.currentTarget.value, ...(model ? { defaultScale: model.scales.includes(2) ? 2 : model.scales[0] } : {}) }) }}>{models.length ? models.map((model) => <option key={model.id} value={model.id}>{model.displayName}</option>) : <option value="">{session ? "未发现可用模型" : "打开书籍后探测模型"}</option>}</select><Button type="button" variant="outline" size="icon" className="size-7" title="刷新模型" aria-label="刷新模型" disabled={disabled || refreshing || !session} onClick={refresh}>{refreshing ? <Loader2 className="size-3.5 animate-spin" /> : <RefreshCw className="size-3.5" />}</Button></div>{capabilities && !capabilities.available ? <p className="text-[10px] text-destructive">模型探测失败：{capabilities.reason}</p> : null}</div>
