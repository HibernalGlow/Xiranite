...
`folderMain` 前四条核心纵切已经接入：GUI/CLI/TUI 可复用的唯一 `ReaderFileTreeService` 维护稳定目录快照、前进/后退/上级/刷新、generation、递归搜索和订阅生命周期，旧 `CoreReaderDirectoryBrowser` 已删除；鉴权 HTTP 控制路由提供 128 项任意 cursor 分页、session 级排序/偏好 PATCH、`watch:true` 显式监听、NDJSON 搜索流和异步释放。后端单层浏览继续使用可取消的原生 `opendir()`；递归搜索通过同一 service 委托唯一 `readdirp` stream adapter，以直接依赖的 `picomatch` 处理 glob、`ignore` 处理 gitignore 语义剪枝，并支持文本/路径、类型、大小写、深度、结果数和扫描数硬上限。搜索流输出 `meta/entry/complete`，每次 Web Stream pull 只推进一个 event，首条结果不等待后续扫描；consumer cancel、Request abort、session DELETE 和 service dispose 都关闭同一 `ReaderFileTreeSearchHandle`。scanner 持有宿主 `reader.file-tree.scan` I/O lease，交互搜索使用 `view` 优先级，结束和取消幂等释放。文件树事件通过按需动态导入的 `@parcel/watcher` adapter，只在显式监听的 session 建立订阅；当前目录直接子项变化会使快照变脏，下一次分页请求以单飞方式重读、重新 hydration/排序并递增 generation。排序在分页前由唯一应用层排序器完成，八字段契约已冻结，name/type/path/random 即时重排，date/size 通过 Node/Bun 原生 `Readable.map({ concurrency })` 有界并发 metadata adapter 按需 `stat`，每个 mapper 在 I/O 前后检查 AbortSignal；不把跨 realm 的 Web signal 直接传入 Node stream。重排后返回焦点路径的新索引，前端直接请求对应稀疏页。排序偏好严格执行“临时规则 > 文件夹记忆 > 标签默认 > 全局默认”，全局/标签默认和最多 1000 项文件夹记忆写入原 NeoView Reader SQLite 的独立 `xr_reader_*` 表，临时规则只存在 browser session；搜索 exclusion 当前只随请求存在，不新增数据库或擅自持久化。React `FolderMainCard` 是独立 lazy chunk，仅在可见时打开浏览会话；当前已使用 React Virtuoso 驱动最多 12 个批次的有界稀疏 catalog，并把可见项按最多 64 项注册到现有 opaque library thumbnail route。当前仍标记 `partial`，因为前端尚未默认请求 watcher/搜索流，树 UI、搜索历史、持久 exclusion policy、EMM 标签搜索、穿透 single/all 与 CLI/TUI 命令 surface 未完成；完整多选/键盘导航、上下文文件操作、目录递归大小和原版视觉验收也仍待迁移。

数据边界保持不变：上述 `xr_reader_*` 表只能位于 `%APPDATA%/NeoView/thumbnails.db`；搜索 exclusion 当前只随请求存在，不写 `xiranite.db`、不新增第二个 NeoView 主库，也不擅自持久化。节点设置仍只写 `xiranite.config.toml` 的 `[nodes.neoview]`。
