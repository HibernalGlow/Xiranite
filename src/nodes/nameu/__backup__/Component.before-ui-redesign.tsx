...
import { ACTIONS, NODE_META, type NameuAction } from "./constants"
import {
  ActionIconButton,
  ActionPicker,
...
import { NameuResultTabs, NameuStatsPanel } from "./results"
...
export function Component({ compId, host }: NodeComponentProps<NameuCardState>) {
  ...
  const [defaults, setDefaults] = useState<Partial<NameuCardState> | undefined>(undefined)
  ...
  function resetOverride() {
    const empty: Partial<NameuCardState> = {}
  ...
  const commonProps = {
    ...
    running,
    status,
    onActionChange: (value: NameuAction) => patch({ action: value }),
    onCopyLogs: copyLogs,
  ...
  return (
    <TooltipProvider>
      <div ref={surface.ref} className="@container/nameu relative flex h-full min-h-0 w-full overflow-hidden">
...
function CollapsedView(props: ViewProps) {
  ...
  return (
    <div data-testid="nameu-collapsed-view" className="relative flex h-full min-h-0 items-center gap-2 overflow-hidden rounded-xl border bg-background/85 px-3 py-2 shadow-sm">
    ...
    <div data-testid="nameu-compact-view" className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 px-3 pb-3">
        ...
        <div className="min-h-0 flex-1">
          <NameuResultTabs compact logs={props.logs} result={props.result} running={props.running} onCopyLogs={props.onCopyLogs} onCopyResults={props.onCopyResults} />
        </div>
      </div>
    </div>
  )
...
function FullView(props: ViewProps) {
  return (
    <div data-testid="nameu-full-view" className="flex min-h-0 flex-1 flex-col gap-3 p-3">
      <div className="flex shrink-0 flex-col gap-3 @4xl/nameu:flex-row @4xl/nameu:items-center @4xl/nameu:justify-between">
        <div className="flex min-w-0 flex-col gap-2 @4xl/nameu:flex-row @4xl/nameu:items-center">
          <HeaderLine actionMeta={props.actionMeta} status={props.status} subtitle={props.data.progressText || summaryText(props)} />
          <div data-testid="nameu-header-toolbar" className="flex min-w-0 flex-wrap items-center gap-2">
            <ActionPicker action={props.action} disabled={props.running} triggerClassName="@4xl/nameu:w-80" onActionChange={props.onActionChange} />
        ...
        <NameuStatsPanel result={props.result} />
      </div>

      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 @5xl/nameu:grid-cols-[minmax(320px,380px)_minmax(0,1fr)]">
        <section className="flex min-h-0 flex-col gap-3 overflow-auto pr-1">
          <div className="grid gap-3 border-b pb-3">
            <div>
              <div className="text-sm font-semibold">路径</div>
              <div className="text-xs text-muted-foreground">归档目录，每行一条。查看配置不需要路径。</div>
            </div>
        ...
        <div className="min-h-0">
          <NameuResultTabs logs={props.logs} result={props.result} running={props.running} onCopyLogs={props.onCopyLogs} onCopyResults={props.onCopyResults} />
        </div>
      </div>
    </div>
  )
...
function RunActionButton({ compact, props }: { compact?: boolean; props: ViewProps }) {
  if (props.running) {
    return (
      <Button aria-label="nameu running" disabled size={compact ? "icon-sm" : "sm"} variant="secondary">
        <Square />
  ...
  if (destructive) {
    return (
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>确认执行重命名？</AlertDialogTitle>
            <AlertDialogDescription>
              当前已关闭预演，会调用 NameU 模块执行真实重命名，可能会修改归档目录名称。请确认配置文件和归档路径无误。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>取消</AlertDialogCancel>
            <AlertDialogAction variant="destructive" onClick={() => props.onExecute(props.action)}>确认重命名</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    )
...
function buildInput(action: PackuToolAction, data: NameuCardState, spec: PackuToolSpec): PackuToolInput {
...
function statusFromState(data: NameuCardState, running: boolean): NameuStatusMeta {
  if (running || data.phase === "running") {
    return {
      label: "运行中",
      description: data.progressText || "NameU 正在生成重命名计划或调用模块。",
      tone: "running",
  ...
  return {
    label: "就绪",
    description: "选择动作后查看配置、预览重命名或执行重命名。",
    tone: "idle",
