...
use handsome_logger::{
    ColorChoice, CombinedLogger, ConfigBuilder, FormatText, SharedLogger, TermLogger, TerminalMode,
    TimeFormat, WriteLogger,
};
...
pub fn setup_logger(
    disabled_terminal_printing: bool,
    app_name: &str,
    filtering_messages_func: fn(&Record) -> bool,
) {
    log_panics::init();

    let terminal_log_level = if disabled_terminal_printing
        && ![Ok("1"), Ok("true")].contains(&env::var("ENABLE_TERMINAL_LOGS_IN_CLI").as_deref())
    {
        LevelFilter::Off
    } else {
        LevelFilter::Info
    ...
    let combined_logger = (|| {
        ...
        let cache_logs_path = config_cache_path
            .cache_folder
        ...
        let combined_logs: Vec<Box<dyn SharedLogger>> =
            if [Ok("1"), Ok("true")].contains(&env::var("DISABLE_FILE_LOGGING").as_deref()) {
                vec![TermLogger::new_from_config(term_config.clone())]
            } else {
                vec![
                    TermLogger::new_from_config(term_config.clone()),
                    WriteLogger::new(file_config, write_rotater),
                ]
            };
        ...
        CombinedLogger::init(combined_logs).ok().inspect(|()| {
            info!(
                ...
                cache_logs_path.to_string_lossy()
            );
        })
    })();

    if combined_logger.is_none() {
        TermLogger::init(term_config, TerminalMode::Mixed, ColorChoice::Always)
...
pub fn print_version_mode(app: &str) {
    ...
    let debug_release = if cfg!(debug_assertions) {
        "debug"
    } else {
        "release"
    };
    ...
    if cfg!(debug_assertions) {
        warn!(
            "You are running debug version of app which is a lot of slower than release version."
        );
    }

    if option_env!("USING_CRANELIFT").is_some() {
        warn!(
            "You are running app with cranelift which is intended only for fast compilation, not runtime performance."
        );
    }

    if cfg!(panic = "abort") {
        warn!(
            "You are running app compiled with panic='abort', which may cause panics when processing untrusted data."
        );
    }
}
