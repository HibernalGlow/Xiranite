...
local headless transport 现在由 `ReaderHeadlessController.upscalePage()` 暴露当前 session 的单页手动执行：按页索引取得真实 `ReaderPage` 后直接委托上述 pages service，返回值移除内部 `sourcePath`，但保留 decision、输出路径、模型、尺寸和耗时。`LazySuperResolutionPagePort` 在首次执行前不加载 fork runtime、不探测系统 CLI、不启动进程，并将并发首次调用合并到一个 capability load；Reader 打开、分页、导航和原图提取仍为零超分初始化。controller dispose 会关闭已经加载的唯一 `SuperResolutionService`，从而复用 Provider 的 daemon/短进程清理。`xneoview upscale-page <book> --index N --output PATH` 只提供显式 manual 入口，模型、scale、noise、tile、TTA 和 GPU 仍全部来自 `[nodes.neoview.super_resolution.preferences]`；默认拒绝覆盖现有输出，必须显式 `--force`。当前命令只允许 local headless，`--connect` 在鉴权 HTTP 路由与 remote controller 共享同一契约前明确拒绝，不能把本地调用冒充完整远程 transport。
