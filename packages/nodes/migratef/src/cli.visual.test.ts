...
describe("migratef guided CLI visual capture", () => {
  test("captures the Clack guided entry screen as ANSI, HTML, and PNG artifacts", async () => {
    ...
    expect(capture.plainText).toContain("Xiranite Migratef")
    expect(capture.plainText).toContain("文件迁移工具")
    expect(capture.plainText).toContain("preserve/flat/direct")
    expect(capture.plainText).toContain("留空进入任务选择")
    expect(capture.plainText).not.toContain("migratef guided")
    expect(capture.plainText).not.toContain("Source paths")
