      ...
      <div className="flex items-center justify-between gap-2">
        <label className="flex min-w-0 items-center gap-2 text-xs">
          <Checkbox
            ...
            disabled={disabled}
            onCheckedChange={(value) => void store.update({ enabled: value === true })}
          />
          <span className="truncate">启用翻页动画</span>
        </label>
        <Button
          type="button"
          variant="ghost"
      ...
      {settings.enabled ? (
        <>
          <SelectField
            ...
            value={settings.type}
            disabled={disabled}
            options={READER_PAGE_TRANSITION_TYPES.map((value) => ({ value, label: READER_PAGE_TRANSITION_TYPE_LABELS[value] }))}
            onChange={(value) => void store.update({ type: value as ReaderPageTransitionType })}
          />

          <div className="space-y-1">
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">动画时长</span>
              <output className="tabular-nums">{settings.duration}ms</output>
            </div>
            <input
              type="range"
              min={0}
          ...
          <SelectField
            ...
            disabled={disabled}
            options={READER_PAGE_TRANSITION_EASINGS.map((value) => ({ value, label: READER_PAGE_TRANSITION_EASING_LABELS[value] }))}
            onChange={(value) => void store.update({ easing: value as ReaderPageTransitionEasing })}
          />

          <div className="border-t border-border pt-2">
            <span className="text-xs text-muted-foreground">预览</span>
            <div className="relative mt-2 h-16 overflow-hidden rounded bg-muted/30">
              <button
                type="button"
                className="absolute inset-2 flex items-center justify-center rounded bg-primary/20 text-xs text-muted-foreground"
