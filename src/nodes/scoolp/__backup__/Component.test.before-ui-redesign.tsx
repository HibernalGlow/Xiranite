...
describe("app-owned scoolp Component", () => {
  test.each(NODE_SURFACE_TEST_MODES)(
    ...
    (mode) => {
      setSurface(mode)
      render(<Component compId="comp-scoolp" host={createHost({ configText: "[scoop]\nroot = \"D:/scoop\"" })} />)
      ...
      if (mode === "compact") {
        ...
        expect(screen.getByRole("button", { name: "scoolp advanced options" })).toBeTruthy()
        ...
        expect(screen.getByText("路径与包名")).toBeTruthy()
  ...
  test("switches action to cache delete and runs a scan through host.actions.run", async () => {
    setSurface("regular")
    const host = createHost({ configText: "", cachePath: "D:/scoop/cache", logs: [] })
    ...
    expect(host.state.logs?.at(-1)).toBe("Found 2 obsolete cache file(s), 1.00 KB.")
  })

  test("uses confirmation before deleting cache files when dry run is disabled", async () => {
    setSurface("regular")
    const host = createHost({ cachePath: "D:/scoop/cache", dryRun: false, logs: [] })
    render(<Component compId="comp-scoolp" host={host} />)
    const user = userEvent.setup()
  ...
  test("catches thrown runner errors and appends the message to logs", async () => {
    setSurface("regular")
    const host = createHost({ cachePath: "D:/scoop/cache", logs: [] }, { runError: new Error("backend offline") })
    render(<Component compId="comp-scoolp" host={host} />)
    const user = userEvent.setup()
  ...
  test("saves, restores, clears, and opens default config controls", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "scoolp defaults" })).toBeTruthy())
    await user.click(screen.getByRole("button", { name: "scoolp defaults" }))
    ...
    expect(host.state.configText).toBeUndefined()
...
function createHost(initial: ScoolpCardState, options: HostOptions = {}): TestHost {
  const host: TestHost = {
    state: { ...initial },
    runCalls: [],
    copiedText: "",
    ...
    actions: {
      run: async <TInput, TData>(
        ...
        onEvent?.({ type: "progress", progress: 30, message: "Scanning cache." })
...
const scoolpData: ScoolpData = {
  ...
  cache: {
    ...
    obsoleteSize: 1024,
    obsoletePackages: [
      { name: "7zip", version: "23.0", size: 512, filename: "7zip#23.0#x64", path: "D:/scoop/cache/7zip#23.0#x64" },
      { name: "git", version: "2.40.0", size: 512, filename: "git#2.40.0#x64", path: "D:/scoop/cache/git#2.40.0#x64" },
    ],
  },
}
