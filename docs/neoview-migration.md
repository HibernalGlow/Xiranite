...
EMM 新写入同样不得回写旧 `thumbs` 行。XR 只读合并旧 `thumbs.rating_data/emm_json/manual_tags`，用户在 XR 修改评分、手动标签或译名时写入同库独立表 `xr_reader_emm_overrides(path_key, display_path, overrides_json, revision, updated_at)`；`null` patch 表示恢复继承旧值，空覆盖仍保留 revision tombstone，避免 reset 后 ABA。GUI 使用鉴权 `GET/PATCH /reader/s/{id}/emm-metadata`，本地与远程 Headless facade 复用同一个 `ReaderEmmMetadataService` 和 Zod wire schema；更新译名后主动失效 Book Information metadata cache。FolderMain 另以 `PATCH /reader/browser/s/{id}/emm-metadata` 提交最多 64 项的 generation-bound 编辑：Zod 在任何写入前验证完整命令并拒绝重复路径，`ReaderFileTreeService` 将路径限制为当前稳定 listing 成员，`p-map` 只提供最多 8 路有界 admission，每项仍由同一 EMM service 串行/CAS；成功项批量回读现有 metadata provider、复用唯一排序器并只推进一次目录 generation，冲突按 item 返回且响应不回传路径。导航导致 generation 过期时拒绝写入；写入后并发导航则保留已确认数据并返回 `refreshRequired`，由 GUI 拉取新目录快照。旧 NeoView V1、V3、V4 实际共用同一 `thumbs` 表，V4 也复用 V3 的数据库/生成器；它们是 service 版本而不是三套待搬运 schema，因此只保留一个兼容 codec/store，不做无意义的 V1/V3/V4 BLOB 迁移。当前 EMM feature 仍为 `pending`：FolderMain/独立 EMM GUI 编辑器、收藏/collect tag、翻译字典、批量同步和完整 CLI/TUI 命令尚未完成。
