import { FolderOpen, HardDrive, Loader2, RefreshCw, RotateCcw, Trash2 } from "lucide-react"
...
export default function UpscaleCacheCard({ client, session, disabled, panelActive = true, superResolution, onSuperResolutionConfigChange, pickDirectory }: ReaderPanelContext) {
  ...
  const resetDirectory = () => {
    setDirectory("")
    commitConfig({ artifactCache: { directory: "" } })
  }
  ...
  return <div className="space-y-3 text-xs" data-neoview-upscale-cache="true">
    <div className="space-y-1"><Label htmlFor="neoview-upscale-cache-directory">超分缓存目录</Label><div className="flex gap-1"><Input id="neoview-upscale-cache-directory" className="h-7 min-w-0 text-[10px]" value={directory} placeholder="Xiranite\\nodes\\neoview\\upscale-artifacts（默认）" disabled={disabled} onChange={(event) => setDirectory(event.currentTarget.value)} onBlur={saveDirectory} onKeyDown={(event) => { if (event.key === "Enter") saveDirectory() }} /><Button type="button" variant="outline" size="icon" className="size-7" title="恢复默认超分缓存目录" aria-label="恢复默认超分缓存目录" disabled={disabled || !directory.trim()} onClick={resetDirectory}><RotateCcw className="size-3.5" /></Button><Button type="button" variant="outline" size="icon" className="size-7" title="选择超分缓存目录" aria-label="选择超分缓存目录" disabled={disabled || !pickDirectory} onClick={() => void chooseDirectory()}><FolderOpen className="size-3.5" /></Button></div><p className="text-[10px] text-muted-foreground">留空使用默认目录；目录更改在重启 Reader 后生效。</p></div>
