...
`historyList/bookmarkList` 已接入统一 Reader library：两张 Card 各自保持独立 lazy loader，列表共同复用 100 项分页、`@tanstack/react-virtual` 可视窗口与 AbortController 生命周期；历史项可恢复原 source 并删除，书签支持 `all/default/favorites` 与自定义列表筛选、当前书籍添加、收藏切换、列表创建/删除。GUI 只通过鉴权 `/reader/library/*` 控制路由读写元数据，CLI/TUI 可直接复用 `ReaderLibraryService`，不维护第二套 store。两张仍为 `partial`，剩余旧 localStorage 数据导入、批量选择/上下文文件操作、缩略图/评分呈现以及 CLI/TUI 命令界面。
...
历史/书签纵切已遵守该边界：最近阅读不复制第二张 history 表，而是按 `xr_reader_progress.updated_at` 倒序分页；书签写入 `xr_reader_bookmarks`，自定义列表写入 `xr_reader_bookmark_lists`，多对多关系写入 `xr_reader_bookmark_memberships`。进度恢复和 ReaderLibrary 不再各自打开一条可写连接或维护重复 schema，默认 GUI composition 只创建一个 `SqliteReaderDataStore`，由它同时实现 `ReaderProgressStore` 与 `ReaderLibraryStore`；CLI/TUI 的独立 composition 也复用同一 adapter。`all/default/favorites` 是 `ReaderLibraryService` 合成的系统视图，不向数据库写保留列表行；`default` 表示没有自定义列表成员关系的书签，`favorites` 由 bookmark 的 `starred` 字段筛选。书签和成员关系的复合更新使用短 `BEGIN IMMEDIATE` 事务及最多三次有界 busy retry，清理历史使用带 limit 的小批删除。HTTP 控制面仅传 JSON 元数据，位于鉴权后的 `/reader/library/recents`、`/reader/library/bookmarks` 和 `/reader/library/bookmark-lists`，不进入页面 asset/翻页热路径；GUI 的 `history-list/bookmark-list` 只消费该契约，并复用一个分页虚拟列表内核。当前仍未迁移旧 `neoview-history`、`neoview-bookmarks`、`neoview-bookmark-lists-v2` localStorage 数据，也未完成 CLI/TUI 命令界面，因此相关 feature 继续保持 partial/pending。
