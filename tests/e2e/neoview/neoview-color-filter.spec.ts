...
test("[neoview.color-filter.ui-1920x1080] [neoview.color-filter.image-stability] color filter previews on the active image and persists once at slider end", async ({ page }, testInfo) => {
  ...
  await page.getByText("上色", { exact: true }).click()
  await expect(page.getByLabel("上色预设", { exact: true })).toBeVisible()
