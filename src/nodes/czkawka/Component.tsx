...
export function Component({ compId, host }: NodeComponentProps<CzkawkaCardState>) {
  ...
  async function executeOperation(action: CzkawkaAction, overrides: Partial<CzkawkaInput> = {}) {
    ...
    try {
      const input = { ...createCzkawkaOperationInput(action as Exclude<CzkawkaAction, "scan">, { ...data, tool, selectedPaths }), ...overrides }
...
function OperationsCard(props: View) {
  ...
  return <div className="grid gap-3"><div className="text-xs text-muted-foreground">删除和移动默认只生成可检查的逐项计划。</div><SwitchLine label="仅预演操作" checked={props.data.dryRun ?? true} onChange={(dryRun) => props.patch({ dryRun })} /><Field label="删除方式"><Select value={props.data.deleteMode ?? "trash"} onValueChange={(deleteMode) => props.patch({ deleteMode: deleteMode as CzkawkaCardState["deleteMode"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="trash">移入回收站</SelectItem><SelectItem value="permanent">永久删除</SelectItem></SelectContent></Select></Field><Field label="移动或复制到"><div className="flex gap-1"><Input value={props.data.destinationDirectory ?? ""} placeholder="D:/Review" onChange={(event) => props.patch({ destinationDirectory: event.currentTarget.value })} /><Button aria-label="move selected" disabled={!props.selectedPaths.length || !props.data.destinationDirectory || props.running} size="icon-sm" variant="outline" onClick={() => void props.executeOperation("move")}><MoveRight /></Button></div></Field><SwitchLine label="复制而非移动" checked={props.data.copyMode ?? false} onChange={(copyMode) => props.patch({ copyMode })} /><SwitchLine label="保留原目录结构" checked={props.data.preserveStructure ?? false} onChange={(preserveStructure) => props.patch({ preserveStructure })} /><Field label="目标冲突"><Select value={props.data.conflictPolicy ?? "skip"} onValueChange={(conflictPolicy) => props.patch({ conflictPolicy: conflictPolicy as CzkawkaCardState["conflictPolicy"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="skip">跳过</SelectItem><SelectItem value="overwrite">覆盖</SelectItem><SelectItem value="rename">自动改名</SelectItem><SelectItem value="error">报告错误</SelectItem></SelectContent></Select></Field>{props.tool === "similar-images" ? <div className="grid gap-2 rounded-md border p-2"><Field label="相似组子目录模板"><Input value={props.data.organizeSubfolderTemplate ?? "variants_{groupId}"} onChange={(event) => props.patch({ organizeSubfolderTemplate: event.currentTarget.value })} /></Field><SwitchLine label="跳过仅一个文件的来源目录" checked={props.data.organizeSkipSingleFileFolders ?? true} onChange={(organizeSkipSingleFileFolders) => props.patch({ organizeSkipSingleFileFolders })} /><AlertDialog><AlertDialogTrigger asChild><Button disabled={!organizePlan.items.length || props.running} size="sm" variant="outline"><ArchiveX />整理相似组（{organizePlan.items.length}）</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{props.data.dryRun ?? true ? "生成相似组整理计划？" : "执行相似组整理？"}</AlertDialogTitle><AlertDialogDescription>将 {organizePlan.selectedGroupCount} 组、{organizePlan.items.length} 项整理到 {organizePlan.targetFolderCount} 个子目录。{props.data.dryRun ?? true ? "当前不会修改文件。" : "将立即执行真实文件操作。"}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>取消</AlertDialogCancel><AlertDialogAction onClick={() => void props.executeOperation("move", { selectedPaths: organizePlan.items.map((item) => item.path), destinationItems: organizePlan.items })}>确认整理</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div> : null}<Field label="导出结果"><div className="flex gap-1"><Input value={props.data.outputPath ?? ""} placeholder="D:/result.json" onChange={(event) => props.patch({ outputPath: event.currentTarget.value })} /><Button aria-label="save selected" disabled={!props.selectedPaths.length || !props.data.outputPath || props.running} size="icon-sm" variant="outline" onClick={() => void props.executeOperation("save")}><Save /></Button></div></Field><AlertDialog><AlertDialogTrigger asChild><Button disabled={!props.selectedPaths.length || props.running} variant="destructive"><Trash2 />删除已选</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{props.data.dryRun ?? true ? "生成删除计划？" : props.data.deleteMode === "permanent" ? "永久删除已选文件？" : "将已选文件移入回收站？"}</AlertDialogTitle><AlertDialogDescription>{props.data.dryRun ?? true ? "当前是预演模式，不会修改文件。" : liveDeleteDescription}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>取消</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => void props.executeOperation("delete")}>确认</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>{props.data.operation ? <OperationResultDetails data={props.data.operation} /> : null}</div>
}
