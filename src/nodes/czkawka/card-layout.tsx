  ...
  return (
    <div data-testid={`czkawka-card-stack-${panel}`} className="grid content-start gap-2" onDragOver={(event) => event.preventDefault()} onDrop={dropAtEnd}>
      {cards.map((card, index) => (
        <CzkawkaLayoutCard key={card.id} card={card} first={index === 0} last={index === cards.length - 1} onChange={onChange} layout={layout}>
          {renderCard(card.id)}
        </CzkawkaLayoutCard>
      ))}
      {cards.length ? null : <div className="rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">该面板没有可见卡片</div>}
    </div>
  )
}

function CzkawkaLayoutCard({ card, first, last, layout, onChange, children }: { card: CzkawkaCardConfig; first: boolean; last: boolean; layout: CzkawkaCardLayout; onChange: (layout: CzkawkaCardLayout) => void; children: ReactNode }) {
  ...
  return (
    <article data-card-id={card.id} draggable onDragStart={startDrag} onDragOver={(event) => event.preventDefault()} onDrop={dropBefore} className="flex min-h-0 flex-col overflow-hidden rounded-md border bg-card" style={card.collapsed ? undefined : { height: card.height }}>
      <header className="flex shrink-0 items-center gap-1 border-b bg-muted/20 px-1 py-1">
        ...
        <Button aria-label={`上移${definition.title}`} disabled={first} size="icon-xs" variant="ghost" onClick={() => onChange(moveCzkawkaCardBy(layout, card.id, -1))}>
          <ArrowUp />
        </Button>
        <Button aria-label={`下移${definition.title}`} disabled={last} size="icon-xs" variant="ghost" onClick={() => onChange(moveCzkawkaCardBy(layout, card.id, 1))}>
          <ArrowDown />
        </Button>
      </header>
      {card.collapsed ? null : (
        <>
          ...
          <input
            aria-label={`调整${definition.title}高度`}
            title="拖动调整高度，双击恢复默认"
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button aria-label="管理卡片" size="icon-sm" variant="ghost">
          <LayoutPanelTop />
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>卡片管理</DialogTitle>
          <DialogDescription>配置卡片可见性、面板归属和顺序。拖动卡片也可跨面板移动。</DialogDescription>
        </DialogHeader>
            ...
            return (
              <div key={definition.id} className="grid grid-cols-[minmax(0,1fr)_130px_auto] items-center gap-2 rounded-md border p-2">
                <div className="min-w-0">
                  <div className="truncate text-sm font-medium">{definition.title}</div>
                  <div className="text-[10px] text-muted-foreground">{card.collapsed ? "已折叠" : `${card.height}px`}</div>
                </div>
                <Select value={card.panel} onValueChange={(panel) => onChange(moveCzkawkaCard(layout, card.id, panel as CzkawkaCardPanelId, layout.cards.filter((item) => item.panel === panel).length))}>
                  <SelectTrigger aria-label={`${definition.title}面板`}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="source">扫描条件</SelectItem>
                    <SelectItem value="analysis">分析操作</SelectItem>
                  </SelectContent>
                </Select>
                <Button
                  aria-label={`${card.visible ? "隐藏" : "显示"}${definition.title}`}
                  size="icon-sm"
