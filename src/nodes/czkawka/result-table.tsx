...
import { AlertTriangle, ArrowDown, ArrowUp, ChevronsUpDown, CircleStop, ClipboardCopy, Copy, ExternalLink, FolderOpen, ListFilter, MousePointer2, RefreshCw } from "lucide-react"
...
export function CzkawkaResultTable(props: CzkawkaResultTableProps) {
  ...
  const previewItems = buildLocalImagePreviewItems(rows)
  const videoPreviewItems = buildLocalVideoPreviewItems(rows)
  const audioPreviewItems = buildLocalAudioPreviewItems(rows, { checkType: props.musicCheckType ?? "tags", maximumDifference: props.musicMaximumDifference ?? "10", minimumFragmentDuration: props.musicMinimumFragmentDuration ?? "15", similarTitlesOnly: props.musicCompareFingerprintsOnlyWithSimilarTitles ?? true })
  ...
  const rowHeight = Math.max(RESULT_ROW_HEIGHT, previewSize + 16)
  const gridTemplateColumns = [SELECT_WIDTH, previewWidth, GROUP_WIDTH, ...columns.map((item) => toolWidths[item.id] ?? defaultColumnWidth(item.id))].map((width) => `${width}px`).join(" ")
  const tableWidth = SELECT_WIDTH + previewWidth + GROUP_WIDTH + columns.reduce((total, item) => total + (toolWidths[item.id] ?? defaultColumnWidth(item.id)), 0)
  ...
  const notice = props.phase === "error" ? { icon: AlertTriangle, text: props.statusMessage || "扫描失败，请检查扫描条件后重试。", destructive: true } : props.phase === "stopped" ? { icon: CircleStop, text: props.statusMessage || "扫描已停止，已保留返回的结果。", destructive: false } : null
  const emptyText = props.running ? "正在分析文件…" : deferredFilter ? "没有匹配当前筛选的结果。" : props.phase === "error" ? props.statusMessage || "扫描失败，请重试。" : props.phase === "stopped" ? "扫描已停止，没有返回结果。" : props.phase === "completed" ? "扫描完成，未发现匹配项。" : "添加目录并开始扫描。"

  return <section className="flex min-h-0 flex-col rounded-md border bg-card" data-testid="czkawka-result-table" aria-busy={filter !== deferredFilter}>{notice ? <div role={notice.destructive ? "alert" : "status"} className={cn("flex items-center gap-2 border-b px-2 py-1.5 text-xs", notice.destructive ? "bg-destructive/10 text-destructive" : "bg-muted/50 text-muted-foreground")}><notice.icon className="size-3.5 shrink-0" /><span className="min-w-0 flex-1 truncate">{notice.text}</span><Button disabled={!props.onRetry || props.running} size="xs" variant="ghost" onClick={() => void props.onRetry?.()}><RefreshCw />重新扫描</Button></div> : null}<div className="flex items-center justify-between gap-2 border-b px-2 py-1.5"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em]"><ListFilter className="size-3.5 text-primary" />结果组</div><Input aria-label="filter results" className="h-7 w-48 text-xs" placeholder="过滤当前工具结果" value={filter} onChange={(event) => changeFilter(event.currentTarget.value)} /></div><div className="min-h-0 flex-1 overflow-auto" data-testid="czkawka-result-viewport" onScroll={updateViewport}><TableComponent className="grid text-xs" style={{ minWidth: tableWidth }}><TableHeader className="sticky top-0 z-10 grid bg-card"><TableRow className="grid" style={{ gridTemplateColumns }}><TableHead /><TableHead className="relative">预览<span role="separator" aria-label="调整预览列宽" aria-orientation="vertical" className="absolute inset-y-0 right-0 w-1.5 cursor-col-resize touch-none hover:bg-primary/40" onPointerDown={startPreviewResize} onPointerMove={resizePreview} onPointerUp={finishPreviewResize} onPointerCancel={finishPreviewResize} /></TableHead><TableHead>组</TableHead>{columns.map((item) => <TableHead key={item.id} className={cn("relative overflow-hidden", item.align === "right" && "text-right")} style={{ width: toolWidths[item.id] ?? defaultColumnWidth(item.id) }}><Button className="h-7 max-w-[calc(100%-6px)] px-1 text-xs" variant="ghost" onClick={() => changeSort(item.id)}>{item.label}{sort.id !== item.id ? <ChevronsUpDown className="size-3 opacity-40" /> : sort.descending ? <ArrowDown className="size-3" /> : <ArrowUp className="size-3" />}</Button><span role="separator" aria-label={`调整${item.label}列宽`} aria-orientation="vertical" className="absolute inset-y-0 right-0 w-1.5 cursor-col-resize touch-none hover:bg-primary/40" onPointerDown={(event) => startResize(event, item.id)} onPointerMove={resizeColumn} onPointerUp={finishResize} onPointerCancel={finishResize} /></TableHead>)}</TableRow></TableHeader><TableBody className="relative grid select-none" style={{ height: rows.length * rowHeight }} onPointerDown={startBoxSelection} onPointerMove={moveBoxSelection} onPointerUp={finishBoxSelection} onPointerCancel={finishBoxSelection}>{renderedRows.map((virtualRow) => { const row = rows[virtualRow.index]!; return <VirtualResultRow key={row.entry.id} row={row} columns={columns} gridTemplateColumns={gridTemplateColumns} selectedSet={selectedSet} groupSelection={groupSelections.get(row.group)!} getFileUrl={props.getFileUrl} onCopyText={props.onCopyText} onOpenPath={props.onOpenPath} onRevealPath={props.onRevealPath} onPreview={changeActivePreview} onSelect={select} onSelectGroup={selectGroup} previewSize={previewSize} rowHeight={rowHeight} start={virtualRow.start} /> })}{boxSelection ? <div data-testid="czkawka-selection-box" className="pointer-events-none absolute left-0 z-20 w-full border border-primary bg-primary/10" style={{ top: Math.min(boxSelection.startY, boxSelection.currentY), height: Math.abs(boxSelection.currentY - boxSelection.startY) }}><MousePointer2 className="size-3 text-primary" /></div> : null}</TableBody></TableComponent>{rows.length === 0 ? <div className="grid h-56 place-items-center text-xs text-muted-foreground">{emptyText}</div> : null}</div><LocalImagePreviewDialog items={previewItems} activePath={activePreviewPaths[props.tool]} getFileUrl={props.getFileUrl} onActivePathChange={changeActivePreview} /><LocalVideoPreviewDialog items={videoPreviewItems} activePath={activePreviewPaths[props.tool]} getFileUrl={props.getFileUrl} onActivePathChange={changeActivePreview} /><LocalAudioPreviewDialog items={audioPreviewItems} activePath={activePreviewPaths[props.tool]} getFileUrl={props.getFileUrl} onActivePathChange={changeActivePreview} /></section>
}

function VirtualResultRow({ row, columns, gridTemplateColumns, selectedSet, groupSelection, getFileUrl, onCopyText, onOpenPath, onRevealPath, onPreview, onSelect, onSelectGroup, previewSize, rowHeight, start }: { row: ResultRowItem; columns: readonly CzkawkaResultColumn[]; gridTemplateColumns: string; selectedSet: Set<string>; groupSelection: GroupSelection; getFileUrl?: (path: string) => string; onCopyText?: (text: string) => Promise<void>; onOpenPath?: (path: string) => Promise<void>; onRevealPath?: (path: string) => Promise<void>; onPreview: (path: string) => void; onSelect: (entry: CzkawkaEntry, checked: boolean, event: MouseEvent) => void; onSelectGroup: (group: CzkawkaGroup, selection: GroupSelection) => void; previewSize: number; rowHeight: number; start: number }) {
...
function buildLocalAudioPreviewItems(rows: ResultRowItem[], settings: MusicPreviewSettings): LocalAudioPreviewItem[] {
  return rows.filter((row) => isLocalAudioPath(row.entry.path)).map(({ entry, group }) => ({
    path: entry.path,
    name: entry.name,
    metadata: [
      { label: "判断方式", value: settings.checkType === "fingerprint" ? "音频指纹" : "标签" },
      ...(settings.checkType === "fingerprint" ? [
        { label: "最大指纹差异", value: settings.maximumDifference },
        { label: "最小片段", value: `${settings.minimumFragmentDuration} s` },
        { label: "标题限制", value: settings.similarTitlesOnly ? "仅相似标题" : "不限标题" },
      ] : []),
      { label: "标题", value: entry.title || "—" },
      { label: "艺术家", value: entry.artist || "—" },
      { label: "流派", value: entry.genre || "—" },
      { label: "年份", value: entry.year || "—" },
      { label: "码率", value: entry.bitrate ? `${entry.bitrate} kbps` : "—" },
      { label: "时长", value: entry.length || "—" },
      { label: "大小", value: formatBytes(entry.size) },
      { label: "组", value: String(group.id + 1).padStart(2, "0") },
    ],
  }))
