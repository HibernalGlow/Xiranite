...
function definition(defaults: XlchemyNodeConfig, language: TerminalLanguage): TerminalInteractionDefinition<XlchemyInput, XlchemyResult> { return { schema: createXlchemyInteractionSchema(interactionDefaults(defaults), language), run: (input, onEvent) => runXlchemy(input, createNodeXlchemyRuntime(), onEvent) } }
async function runPipe(args: string[], host: CliHost) { const json = args.includes("--json"); const { config } = await loadNodeConfigWithHints<XlchemyNodeConfig>("xlchemy", { env: host.env, cwd: host.cwd, hintSink: { stderr: host.stderr }, jsonMode: json }); let paths = parseXlchemyPositional(args); if (paths.includes("-") || (!paths.length && hasPipedInput(host.stdin))) paths = paths.filter((path) => path !== "-").concat(await readStdinLines(host.stdin)); const input = buildPipeInput(args, config ?? {}, paths); const result = await runXlchemy(input, createNodeXlchemyRuntime()); if (json) writeJson(host, result); else { writeLine(host, result.message); for (const tool of result.data?.environment ?? []) writeLine(host, `${tool.runnable ? "ready" : "missing"}\t${tool.label}\t${tool.path ?? "-"}\t${tool.version ?? tool.detail ?? ""}`); for (const file of result.data?.files ?? []) writeLine(host, `${file.status}\t${file.sourcePath}\t->\t${file.outputPath}`) } if (!result.success) process.exitCode = 1 }
