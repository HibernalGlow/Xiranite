...
export default function FolderClipmDialog({ open, name, work, loading, error, onClose, onSave }: FolderClipmDialogProps) {
  const [label, setLabel] = useState<"P" | "N">(work?.label ?? "P")
  const [score, setScore] = useState(work?.score ?? 0)
  const [saving, setSaving] = useState(false)
  const editedRef = useRef(false)

  useEffect(() => {
    if (!work || editedRef.current) return
    setLabel(work.label)
    setScore(work.score)
  }, [work])
          ...
          <div className="grid gap-4">
            {loading ? (
              <div className="flex items-center gap-2 text-xs text-muted-foreground" role="status">
                <LoaderCircle className="size-3.5 animate-spin" />正在确认作品记录…
              </div>
            ) : null}
            ...
            <label className="grid gap-1.5 text-sm">
              ...
              <ToggleGroup type="single" value={label} onValueChange={(value) => {
                if (value !== "P" && value !== "N") return
