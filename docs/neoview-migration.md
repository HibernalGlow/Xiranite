...
| 超分 | 后端契约、Page workflow、local headless/CLI 与 Windows 三 CLI 小样本实测可用，生产依赖未锁定 | 共享 policy/service、文件页零复制与归档页受 lease materialize、GPU lease、TOML composition、旧 preferences/conditions 导入、系统 CLI/daemon capability 探测、薄 Provider、惰性 runtime/headless port、输出校验、daemon/短进程清理及可注册模型；`xneoview upscale-page` 复用当前 Reader session 与同一 policy，不嵌入 Python/exe | fork 推送/锁版本、鉴权 HTTP/remote headless、TUI/GUI/Card、更多真实页/驱动/显存与派生进程树、MangaJaNai 旧目录 manifest 补全 |
