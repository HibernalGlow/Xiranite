...
import { createCzkawkaOptionHelpFields } from "./tool-options.js"
...
export const help = {
  ...
  commands: [
    { title: "Duplicates", command: "xiranite czkawka scan duplicate-files D:/media", description: "Find content duplicates using BLAKE3.", examples: [{ label: "No cache", command: "xiranite czkawka scan duplicate-files D:/media --no-cache --hash blake3", description: "Run a real duplicate scan without reading or writing the native cache." }] },
    ...
    { title: "Safe delete", command: "xiranite czkawka delete D:/old.tmp", description: "Preview moving a path to the recycle bin; add --live to execute or --permanent for explicit permanent deletion.", examples: [{ label: "Empty folders", command: "xiranite czkawka delete D:/empty --tool empty-folders --permanent --live", description: "Recheck that the complete directory tree contains no files immediately before deletion." }] },
  ...
  fields: createCzkawkaOptionHelpFields("en"),
  ...
  translations: { zh: { fields: createCzkawkaOptionHelpFields("zh") } },
} satisfies NodeHelp
