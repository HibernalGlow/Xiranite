...
impl Search for BadExtensions {
    ...
    fn search(
        ...
        if self.look_for_bad_extensions_files(stop_flag, progress_sender)
...
impl DeletingItems for BadExtensions {
    fn delete_files(
        ...
        _progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
        todo!()
    }
}
...
impl PrintResults for BadExtensions {
    fn write_results<T: Write>(&self, writer: &mut T) -> std::io::Result<()> {
        ...
        writeln!(
            writer,
            "Found {} files with invalid extension.\n",
        ...
        for file_entry in &self.bad_extensions_files {
            writeln!(
                ...
                file_entry.proper_extensions_group
            )?;
        }
