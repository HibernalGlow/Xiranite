...
describe("embedded native asset cache", () => {
  test("extracts, verifies, and reuses a versioned binding", () => {
    ...
    const bindingName = `xiranite-test.${process.platform}-${process.arch}.node`
    ...
    writeFileSync(join(assetRoot, "test.zip"), archive)
    ...
    const first = extractEmbeddedNativeBinding(assetRoot, cacheRoot, "test")
    const second = extractEmbeddedNativeBinding(assetRoot, cacheRoot, "test")
