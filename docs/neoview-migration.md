...
`folderMain` 第一条核心纵切已经接入：GUI/CLI/TUI 可复用的 `CoreReaderDirectoryBrowser` 维护稳定目录快照、前进/后退/上级/刷新和 generation；平台层用有界 `opendir` 收集目录与文件，不逐项串行 `stat`，目录优先并自然排序；鉴权 HTTP 控制路由提供 128 项默认分页和显式 session 释放；React `FolderMainCard` 是独立 lazy chunk，仅在可见时打开浏览会话，使用 `@tanstack/react-virtual` 渲染可见行，折叠/卸载后取消请求并关闭 session。当前仍标记 `partial`，因为旧版多标签、搜索/排序切换、选择、上下文文件操作、评分与 Bookmark/History 共享虚拟源尚未完成。
