  ...
  it("[neoview.react.control] sends token-authenticated open, navigation and close requests", async () => {
    const fetchMock = vi.fn(async (request: RequestInfo | URL, _init?: RequestInit) => {
      ...
      if (url.endsWith("/reader/config")) return Response.json({
        ...
        folderView: { viewMode: "compact", previewCount: 4, details: { columnOrder: ["name"], hiddenColumns: [], pinnedLeft: ["name"], pinnedRight: [], columnWidths: READER_FOLDER_DETAIL_DEFAULT_WIDTHS } },
    ...
    expect(await client.updateFolderView!({ folderView: { viewMode: "details", previewCount: 9, details: { columnWidths: { name: 300 } } } })).toMatchObject({ viewMode: "compact", previewCount: 4 })
    ...
    expect(JSON.parse(String(fetchMock.mock.calls[5]?.[1]?.body))).toEqual({ folderView: { viewMode: "details", previewCount: 9, details: { columnWidths: { name: 300 } } } })
