...
- 性能基准：`cache-memory-budget`、`presentation-retention`、`solid-cross-session`
- 已知差异：L2 已复用引用计数 lease 保留 response 与 current/near frame 实际生成的每页最新 presentation；diagnostics 从 L1/L2/L3/solid owner 单次快照派生统一 memory/disk/lease 视图且不复制计数器；固定 1 MiB corpus 的 retention 硬预算/淘汰/释放基准已接入，current listing 预算降级、跨进程 lease 与真实图片 corpus 校准仍待完成
...
- 已知差异：当前提供无副作用瞬时快照、宿主资源池 lease/queue wait、archive/browser owner payload 及预读性能累计指标；GPU 利用率、时间序列采样、算法对照和报告导出仍待迁移
