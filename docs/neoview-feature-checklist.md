...
- 已知差异：viewport/resource context 与 TTFB/decode/retained bytes/active lease application 遥测已完成；HTTP/React 实时字段上报仍待接入
...
- 已知差异：当前提供无副作用瞬时快照、宿主资源池 lease/queue wait 及预读 TTFB/decode/retained bytes/lease 累计指标；GPU 利用率、时间序列采样、算法对照和报告导出仍待迁移
