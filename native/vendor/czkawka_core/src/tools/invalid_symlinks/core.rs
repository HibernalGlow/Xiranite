...
use crate::tools::invalid_symlinks::{
    ErrorType, Info, InvalidSymlinks, MAX_NUMBER_OF_SYMLINK_JUMPS, SymlinkInfo,
};
        ...
        match result {
            DirTraversalResult::SuccessFiles {
                ...
                self.invalid_symlinks = grouped_file_entries
                    ...
                    .flatten()
                    .filter_map(|e| {
                        let (destination_path, type_of_error) =
                            Self::check_invalid_symlinks(&e.path)?;
                        Some(e.into_symlinks_entry(SymlinkInfo {
                            destination_path,
                            type_of_error,
                        }))
                    })
