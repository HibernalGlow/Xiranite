...
`LegacyNeoViewDataLocator.inspect()/discover()` 现已提供无副作用的 canonical/custom 候选报告：它只检查配置目录下的已知 `thumbnails.db` 路径，canonical 数据库始终保持 active；自定义数据库即使在 canonical 缺失时也只标记为 secondary compatibility source，不自动挂载、复制、合并或创建新库。SQLite schema/WAL 兼容性仍由既有 `LegacyThumbnailDatabaseInspector` 在显式 inspect/迁移流程中负责，CLI/GUI 自动确认与一次性合并仍保持 pending。
