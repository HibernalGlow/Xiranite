...
排序偏好继续执行“临时规则 > 文件夹记忆 > 标签默认 > 全局默认”：全局/标签默认和最多 1000 项文件夹记忆写入原 NeoView Reader SQLite 的独立 `xr_reader_*` 表，临时规则只存在 browser session。React `FolderMainCard` 保持独立 lazy chunk 和有界稀疏 catalog。整体仍为 `partial`：树 GUI、搜索历史、EMM 标签搜索、穿透 single/all、其余 CLI/TUI feature 命令、完整多选/键盘导航、上下文文件操作、目录递归大小和原版视觉验收仍待迁移。
