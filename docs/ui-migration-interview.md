...
状态：本轮产品问题已回答；当前以 Windows、2560 宽、60 Hz 为交付验收基线，MacBook Air M5 为后续适配目标。目标、功能复用边界、独立会话、共享续读位置、窗口关闭后的服务存续与 i18n 全量重构均已确认。2026-09-14 核查本地 `vendor/neoxide`，其 HEAD 为 `f2380d2b`，另有未提交改动。它是根仓库忽略的独立 checkout，origin 为 `HibernalGlow/neoxide`，上游为 `MikageSawatari/mimageviewer`。
...
- 浏览器主要在局域网使用，并需要支持可配置的公网远程入口；关闭桌面窗口后，后台核心／远程服务继续使浏览器可用。
- 同一本书共用续读位置，重新打开时接续共享记录；已经打开的会话保持各自当前页面，不互相跳页。
- 当前仍在 Windows 开发，以 2560 宽、60 Hz 验收；后续可能转用 MacBook Air M5。4K 图片和图片／视频流畅切换仍是代表负载，用户所写“AVFif”按上下文暂记为 AVIF。

## 当前性能与平台验收基线

| 维度 | 当前约束 |
| --- | --- |
| 正式开发与验收 | Windows；保留 Neoxide 已实现的行为、媒体算法与 GPU 快路径。 |
| 显示目标 | 用户最终指定 `2560@60Hz`，交互帧预算约 `16.7ms`；测量时记录实际窗口高度、backing pixels、DPI／缩放、显示模式与 GPU。 |
| 源素材 | 4K AVIF 图片、连续翻页／缩放及图片与视频之间的切换；4K 指源图片负载，不擅自把显示目标改回 4K。 |
| Mac | MacBook Air M5 为后续适配目标；本轮不要求在尚未使用的 Mac 上完成实机验收，也不移除跨平台架构要求。 |
| 网络 | 局域网优先，同时设计可配置的公网 HTTPS 入口与认证边界；冷加载的网络／解码等待单独度量。 |
| 基线比较 | 原 Neoxide 在相同素材、显示条件、画质、缓存状态和后台负载下的行为与性能；用户认为已有体验良好，不能把 UI 重做当作重写媒体管线的理由。 |

先前的 120 帧目标已经由用户明确调整，不再作为当前交付门槛。60 Hz 目标描述连续交互和呈现节奏，不表示每秒必须解码 60 张不同的 AVIF，也不改变视频源帧率。验收分别记录帧时间分布、输入响应、冷／热首显、下一媒体首帧、切换空白与掉帧；不能仅用平均 FPS 宣称流畅。
...
| 公网入口需明确 TLS 与代理边界 | remote-web 的 [`main.rs`](../vendor/neoxide/crates/remote-web/src/main.rs) 通过 `Server::http` 启动；`--url=https://…` 仅公布 URL。[`http.rs`](../vendor/neoxide/crates/remote-web/src/http.rs) 的 scheme／来源 IP 入口直接采用 forwarding headers；公网设计需要限定可信代理来源与头覆盖规则。 |
| 认证可复用，不能假设由 Tailscale 代管 | [`auth.rs`](../vendor/neoxide/crates/remote-web/src/auth.rs) 使用 PIN／Argon2、签名 Cookie 或 Bearer；失败锁定当前为全局状态。需明确公网登录限流、会话撤销、同源写操作及 Host／Origin 边界。 |
| AVIF 原生能力当前依赖 Windows | [`canonical_image_loader.rs`](../vendor/neoxide/src/canonical_image_loader.rs) 的完整图片加载在 `image` 失败后尝试 WIC、再尝试 Susie；[`wic_decoder.rs`](../vendor/neoxide/src/wic_decoder.rs) 的非 Windows 分支返回 `None`，当前只取首帧并转换为 BGRA。它不能作为 Mac AVIF、动画 AVIF 或高位深显示已经实现的证据。 |
| 原媒体性能机制可以保留 | 原生 [`app.rs`](../vendor/neoxide/src/app.rs) 已有前后页预取、延迟纹理销毁、当前目标优先的上传 backlog；[`gpu_lanczos.rs`](../vendor/neoxide/src/gpu_lanczos.rs) 缓存 GPU 输出。[`app/native_video.rs`](../vendor/neoxide/src/app/native_video.rs) 已区分视频 fast swap 与视频→图片，并验证目标／窗口／pending 状态。 |
...
已补充独立的[NeoView 外壳代码图与逐控件清单](neoxide-ui-reference.md)，包含真实模式分支、默认面板入口、控件选项、持久化差异、已有测试范围和关键源码指纹。它覆盖第一版外壳，业务 Card 内部与运行时几何继续补充。
...
公网入口是可选部署能力，复用应用自身认证；TLS 终止、外部 URL、可信代理、登录限流、会话撤销及同源写入在网关边界定义。局域网其他设备访问也需要满足 WebGPU 的安全上下文要求。媒体算法与共享业务写入仍由现有 core 承担，浏览器逐帧绘制不依赖网络往返。
...
| Q3 | 体验复刻的验收标准 | 采用 NeoView 布局与交互；性能优先，原生功能保留，以既有体验为比较基线 | 已确认；当前显示目标见 Q14 |
| Q4 | 框架与源码 | egui；目标已定位到 `vendor/neoxide` | 已确认 |
| Q5 | 原生 GPU 要求 | 保留 Neoxide 已有原生 GPU 路径；源码已明确视频和 HUD 两条链 | 目标已确认，性能预算后续量化 |
| Q6 | Mac 设备与平台适配顺序 | 用户指定 MacBook Air M5，但随后明确当前仍在 Windows 开发／验收，Mac 为后续可能使用的平台 | 已回答；保留后续适配目标 |
...
| Q10 | 浏览器实际使用环境 | 局域网为主，并考虑开放公网入口远程使用；具体浏览器版本作为测试环境事实登记 | 网络范围已确认 |
| Q11 | 关闭桌面后的服务存续 | 关闭桌面窗口后，浏览器及提供能力的后台核心／远程服务继续可用；停止服务有明确生命周期操作 | 已确认；不能只保留失去 core 的 HTTP 进程 |
| Q12 | 同本内容再次打开时的阅读进度 | 各端共享一个续读位置；当前会话仍独立，书签与评分共享 | 已确认；取代此前各端独立续读的建议 |
| Q13 | 性能代表负载 | 4K 图片，格式按上下文记为 AVIF，图片／视频切换保持原 Neoxide 的流畅体验 | 已确认；显示目标由 Q14 调整 |
| Q14 | 当前显示与帧率验收 | Windows、2560 宽、60 Hz；MacBook Air M5 后续适配，120 帧不作为当前交付门槛 | 已明确回答 |
| L1 | 语言范围与机制 | 全量重构调用点与基础设施，中英日正式消息目录、稳定语义 key、类型化参数；旧查表和包装路径退出 | 已确认，用户再次强调；不再作为待选问题 |

Q6、Q10–Q14 已按用户实际回答更新，未把此前建议值当成答案。当前没有需要继续泛问的产品选择；具体平台缺口或不可兼容行为出现时，先给出源码证据与可行替代。其他工程选择由源码核查和测试解决，不反复让用户确认既定要求。

本仓库术语表中“外部浏览请求”也用于 NeoView 目录标签导航；本文使用“浏览器客户端”指本次 WASM 界面，避免与目录导航混用。这是设计文档术语，不另设产品命名问题。

## 后续实施与验证依赖

本轮答案已经确定以下交付边界；框架内部 API、消息库验证、缓存策略和采集工具等工程选择由源码核查与测试解决：

- 已确认的 Q3/Q7 → 从现有 NeoView 提取主题、密度、状态及交互证据，建立 Neoxide 独有功能入口映射和逐控件验收清单。
- Q6/Q14 → Windows 当前门禁；后续 MacBook Air M5 的系统版本、快捷键/输入法/触控板/窗口行为、平台专属实现、分发与实机验收。
- Q10 → 浏览器与设备矩阵、窄屏/触摸交互、HTTPS 入口和实际网络条件；服务端解码与浏览器本地绘制仍可组合。
- Q11/Q12 → 无桌面窗口时的核心生命周期、同本共享续读位置的有序写入与接续；独立会话及共享业务数据不再重新选择。预取完成和关闭时陈旧快照不能覆盖较新的续读记录。
- Q13/Q14 与已确认的 Q5/Q9 → 4K AVIF／视频代表素材、2560@60Hz 原生基线、传输/复制/上传成本、双端并发资源预算与端到端性能验证。

## 实施顺序与完成证据

1. **固化行为基线和 UI 证据。** 以本轮外壳代码图为起点，逐模块补齐 Card、菜单、快捷键与 Neoxide 独有功能映射；为固定主题／密度／状态采集布局数据和可读 SVG。代码图描述结构，运行时数据证明几何，两者分别留存覆盖范围和源码版本。
2. **建立会话与运行时边界。** 分离各端当前显示状态和共享业务状态，使后台核心生命周期不随桌面窗口关闭而结束；沿用单一业务 writer，给共享续读、任务取消、完成、纹理租约和释放建立明确所有者。跨会话、异步和窗口结构先独立审查，再以状态／生命周期测试验证。
3. **完整替换 i18n。** 验证语义消息库的 native／WASM 适配、资源初始化与绘制开销，随后迁移全部应用文案调用点、中英日目录及动态参数；固定 widget ID，并退出旧查表／包装工具。允许按模块提交，但最终完整性按 ADR-0062 验收。
4. **实现共享 egui 界面及客户端适配。** 在应用层复刻泳道、侧栏、工具条和动态状态；原生调用保留已有 GPU／媒体快路径，WASM 使用独立会话和本地 WebGPU 绘制。复用 core 业务、媒体与 remote 认证能力；只为实际缺口调整框架 patch 或平台 adapter。
5. **验证 Windows 与浏览器交付。** 串行执行相关自动化检查，比较原生 2560@60Hz 基线、4K AVIF 冷／热路径、图片↔视频／视频↔视频切换、双端并发及桌面窗口关闭后的持续使用。准备可运行验证构建；互动验证按仓库既有隔离数据流程安排。后续 Mac 以同一契约补齐平台实现与实机证据。

当前完成的是需求记录和第一版源码证据，尚未实现布局导出器、共享新 UI、运行时拆分或 i18n 替换，也未宣称已有性能测试结果。
