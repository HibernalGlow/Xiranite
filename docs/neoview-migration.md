...
| Phase 3：React Reader | 进行中（当前主线） | 原始 asset URL `<img>`、相邻页 `Image.decode()`、64 页批次虚拟缩略图；四边 EdgeShell、Panel/Card、fit/zoom/rotation 与首批 TopToolbar 控制 | 完整 TopToolbar、左右全部 panel/card、单双页切换/全景/连续长图、拖拽/捏合手势、窗口/tab |
...
| 图片显示与热翻页 | fit/zoom/rotation 纵切可用且已过硬门槛 | 唯一 DOM `<img src=assetUrl>`；共享 TS presentation geometry；适应窗口/填满/宽/高/原始大小、0.1～8 倍手动缩放、90° 旋转；稳定 wrapper、完整 scroll extent、零重复 asset 请求；相邻页有界预解码 | 双页切换/全景/连续模式、拖拽/捏合、旧 zoom 设置导入与持久化、8K/16K/GPU 长测 |
...
| Panel/Card | 兼容布局、折叠、Card 高度与设置态管理纵切 | 完整旧 panel ID metadata；Card lazy loader；关闭/折叠后内容 DOM=0；旧布局/card v14 经 TOML 规范化；新版状态写入 canonical `panel_state/card_state`；Card 高度拖动仅用 ref/DOM style，pointerup 单次 PATCH，取消恢复，双击或标题按钮恢复自动高度；“卡片管理”按需加载 Dice UI Kanban；“边栏管理”支持搜索、显隐、左右移动、同栏顺序和重置；设置模块也是共享 registry Card，显式停靠后可进入静态侧栏 | 其余全部业务卡片、Panel 高级组件设置、独立 card window/tab |
| 完整阅读工具与设置 UI | 设置浮窗、两张共享设置 Card 与首批视图工具可用 | 路径打开、前后/页码/关闭；fit mode 菜单、缩放、旋转、reset 及 `+/-/R/0` 快捷键；按 `setting.png` 建立独立设置浮窗，“边栏管理”和“卡片管理”已是可复用 registry Card | 原 TopToolbar 其余功能、其余设置分类业务卡、文件浏览、历史/书签、超分、AI、benchmark、完整快捷操作 |

当前验证证据（数值随复跑波动，以预算和原始输出为准）：

- 前端 NeoView：14 个测试文件、42 项 component/hook/registry/shell/config/resize/card/settings/presentation 测试通过；后端 NeoView 串行完整复跑为 39 个文件、Vitest 192 项 + Bun TUI 1 项通过，Backend 26 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖五种 fit mode、双页 frame 尺寸、旋转归一化、0.1～8 倍缩放边界、工具条状态、活动 `<img>` 身份和取消打开后的 session 回收；
- Reader pipeline 最近复跑：backend startup `22.15 ms`、open `75.53 ms`、navigation p95 `8.33 ms`、response headers p95 `3.48 ms`、first byte p95 `3.63 ms`、full stream p50 `36.20 ms`、throughput p50 `78.42 MiB/s`、RSS delta `0 MiB`；全部通过预算，presentation 不进入图片 HTTP 数据处理链；
- 真实 Chromium 路径已覆盖桌面与 `420×360`：原始大小产生完整可滚动区域，reset 后 fit 回到视口内，旋转与 110% 手动缩放保持同一个 `<img>`、同一原始 asset URL、零重复 active asset 请求且无 Canvas；最终热翻页为 desktop `41.8/66.8 ms`、card `44.9/70.9 ms`（navigation/decode），仍明显低于 `150/200 ms` 硬门槛；
- 当前生产构建报告：NeoView entry `21,666 bytes`，ReaderFrame/PageImage `2,269 bytes`，ReaderViewToolbar `2,858 bytes`，ReaderSidebar `8,353 bytes`，设置浮窗 `3,272 bytes`，SidebarManagementSettingsCard `5,942 bytes`，PanelLayoutEditor `3,903 bytes`，Dice UI Kanban/dnd-kit runtime `58,547 bytes`；presentation UI 与 backend open 并行加载且受 16 KiB 独立门禁，未打开书籍时不进入 NeoView entry；首屏 NeoView module `0`、前端 zip.js module `0`。
...
`ReaderFrame/PageImage` 与 `ReaderViewToolbar` 是打开书籍时才加载的两个独立 chunk；动态 import 与 backend `open` 同时开始，不形成串行 waterfall。UI chunk 失败时关闭已经创建的 session，组件卸载后迟到的 open 结果也会立即回收。当前 fit/manual scale/rotation 是每次成功打开书籍后重置的 presentation 会话态；旧 `zoomMode` 默认值导入、TOML 写回和跨书保留属于后续设置迁移，不能用 localStorage 再建立第二套状态。
