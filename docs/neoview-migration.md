...
可逆 copy/move/rename/create-directory 会由 platform 返回带 `dev/ino/size/type/mtime/ctime` 的 receipt，service 维护最多 50 个 session transaction 并严格逆序撤销；目标在操作后发生变化时返回 `ESTALE`，不会盲删或覆盖。部分撤销失败只移除已成功 receipt，失败项与尚未执行项保留待重试。`GET /reader/files/operations` 暴露 capability/state，`POST /reader/files/undo` 与 `file-ui` undo 都要求显式确认；永久 delete、覆盖既有目标及 trash 因无法可靠恢复而不生成 receipt。当前纵切没有写入 `xiranite.db` 或 Reader SQLite，也不自行实现跨盘算法或 Windows 回收站协议。安全 journal 目前是 Reader HTTP/OpenTUI 会话态；跨进程持久化、trash restore 与 Explorer/open-with 仍待完成，feature 继续保持 `pending`。
