...
> 最后更新：2026-07-15；最近已提交基线：`897886a`（原图阅读/预解码/虚拟缩略图）、`880ccff`（复杂 Shell/Card AST scaffold）、`1e4f70c`（懒加载四边 Shell 与首批 Panel/Card）、`5064b82`（只读 Shell 配置链）、`444bdbc`（React Compiler 友好的侧栏 resize/drag 与 TOML PATCH 原子持久化）。本节必须随每个 NeoView 实现提交同步更新；代码、feature matrix 和测试证据优先于文字。
...
- 前端 NeoView：9 个测试文件、24 项 component/hook/registry/shell/config/resize 测试通过；后端 NeoView 最近完整复跑为 35 个文件、Vitest 170 项 + Bun TUI 1 项通过；Backend CORS 17 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖 parser PATCH 边界、TOML 原子回写、HTTP 鉴权/脱敏/串行 PATCH、客户端请求、异步配置应用、活动图片节点不重挂，以及 100 次 pointermove 只产生一次布局提交；
- Reader pipeline 最近复跑：backend startup `22.88 ms`、open `122.42 ms`、navigation p95 `6.29 ms`、response headers p95 `3.92 ms`、first byte p95 `4.04 ms`、full stream p50 `35.77 ms`、throughput p50 `79.37 MiB/s`、RSS delta `0 MiB`；全部通过预算。open 相比上轮波动仍远低于 `500 ms` 门槛，翻页和数据面无性能回退；
- 四边 Shell + 左右 lazy card 的真实 Chromium 路径已覆盖桌面与 `420×360`：关闭侧栏/底栏后 DOM 为 0，底栏卸载时预解码仍继续；本轮加入真实 PATCH/resize 后热翻页为 desktop `67.7/92.8 ms`、card `42.2/62.6 ms`（navigation/decode），仍明显低于 `150/200 ms` 硬门槛；40 步 pointermove 仅 1 次 PATCH，原图 `<img>` 节点身份保持不变；
- 本轮生产构建报告：NeoView entry `33,259 bytes`，ReaderSidebar `7,916 bytes`，PageNavigationCard `2,132 bytes`，BookInformationCard `2,020 bytes`；首屏 NeoView module `0`、前端 zip.js module `0`。entry 仍低于 40 KiB，Panel/Card 仍低于 32 KiB 并保持独立延迟加载；由于 resize 逻辑只进入 ReaderSidebar deferred chunk，没有进入首屏入口。
