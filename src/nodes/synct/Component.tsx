...
export function Component({ compId, host }: NodeComponentProps<SynctCardState>) {
  ...
  async function execute(nextAction: SynctAction = action) {
    ...
    if (!splitLines(dataRef.current.pathsText).length) {
      const message = "Add at least one source path before running Synct."
  ...
  return (
    <TooltipProvider>
      <div ref={surface.ref} className="@container/synct flex h-full min-h-0 w-full overflow-hidden bg-card">
...
function CollapsedView(props: ViewProps) {
  ...
  return (
    <div data-testid="synct-collapsed-view" className="flex h-full min-h-0 w-full items-center gap-2 overflow-hidden rounded-xl border bg-card px-3 py-2 shadow-sm">
    ...
    <div data-testid="synct-full-view" className="flex min-h-0 flex-1 flex-col gap-2 p-3">
      ...
      <div className="grid min-h-0 flex-1 gap-2 @2xl/synct:grid-cols-[minmax(250px,330px)_minmax(0,1fr)] @4xl/synct:grid-cols-[minmax(250px,330px)_minmax(0,1fr)_minmax(270px,340px)]">
        <section className="flex min-h-0 flex-col gap-2 overflow-auto rounded-lg border bg-card p-2">
          <ZoneTitle icon={FolderInput} label="Sources and format" />
...
function SourceMode(props: { disabled?: boolean; value: SynctSourceMode; onChange: (value: SynctSourceMode) => void }) {
  return (
    <ToggleGroup type="single" value={props.value} disabled={props.disabled} onValueChange={(value) => value && props.onChange(value as SynctSourceMode)} className="grid grid-cols-2" size="sm">
      {SOURCE_MODES.map((item) => <ToggleGroupItem key={item.value} value={item.value} className="min-w-0 gap-1"><item.icon className="size-3.5" /><span className="truncate text-xs">{item.label}</span></ToggleGroupItem>)}
    </ToggleGroup>
  )
}
...
function FormatPicker(props: { disabled?: boolean; value: SynctFormatKey; onChange: (value: SynctFormatKey) => void }) {
  return (
    <div className="grid gap-1.5">
      <ZoneTitle icon={GitCompareArrows} label="Target format" />
...
function PathInput(props: { compact?: boolean; data: SynctCardState; disabled?: boolean; onPaste: () => void; onPatch: (patch: Partial<SynctCardState>) => void }) {
  return (
    <div className="grid gap-1.5">
      {!props.compact && <Label htmlFor="synct-paths" className="text-xs">Source paths</Label>}
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-1.5">
        <Textarea id="synct-paths" aria-label="synct paths" className={cn("min-h-0 resize-none font-mono text-xs", props.compact ? "h-14" : "h-28")} disabled={props.disabled} placeholder={"One file or folder per line\nD:/downloads"} value={props.data.pathsText ?? ""} onChange={(event) => props.onPatch({ pathsText: event.currentTarget.value })} />
...
function SwitchPanel(props: { compact?: boolean; data: SynctCardState; disabled?: boolean; onPatch: (patch: Partial<SynctCardState>) => void }) {
  return (
    <div className={cn("grid gap-2", props.compact ? "grid-cols-1" : "grid-cols-[repeat(auto-fit,minmax(8rem,1fr))]")}>
      <SwitchRow checked={props.data.dryRun ?? true} disabled={props.disabled} icon={ShieldAlert} label="Dry run" onCheckedChange={(dryRun) => props.onPatch({ dryRun })} />
      <SwitchRow checked={props.data.recursive ?? false} disabled={props.disabled || props.data.sourceMode === "folders"} icon={FolderTree} label="Recursive files" onCheckedChange={(recursive) => props.onPatch({ recursive })} />
      <SwitchRow checked={props.data.archiveFolder ?? false} disabled={props.disabled} icon={Archive} label="Use archive folder" onCheckedChange={(archiveFolder) => props.onPatch({ archiveFolder })} />
      <SwitchRow checked={props.data.fallbackToCreatedTime ?? true} disabled={props.disabled} icon={CalendarClock} label="Fallback time" onCheckedChange={(fallbackToCreatedTime) => props.onPatch({ fallbackToCreatedTime })} />
      <SwitchRow checked={props.data.syncFolderFileTimes ?? true} disabled={props.disabled || props.data.sourceMode !== "folders"} icon={ListChecks} label="Sync folder files" onCheckedChange={(syncFolderFileTimes) => props.onPatch({ syncFolderFileTimes })} />
    </div>
  )
}
  ...
  return (
    <section className={cn("flex shrink-0 flex-col gap-2 rounded-lg border bg-card p-2", live && "border-destructive/50 bg-destructive/[0.03]")}>
      ...
      <div className="grid gap-2 @sm/synct:grid-cols-2">
        ...
        <SwitchRow checked={props.data.archiveFolder ?? false} disabled={props.running} icon={Archive} label="Use archive folder" onCheckedChange={(archiveFolder) => props.onPatch({ archiveFolder })} />
      </div>
...
function RunButton({ compact, props }: { compact?: boolean; props: ViewProps }) {
  if (props.running) return <Button aria-label="synct running" disabled size={compact ? "icon-sm" : "sm"} variant="secondary"><Square />{!compact && <span>Running</span>}</Button>
  ...
  if (live) {
    return (
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Confirm live archive?</AlertDialogTitle><AlertDialogDescription>Synct will move ready files or folders. Existing targets are skipped as conflicts, but the source paths should be checked before continuing.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => props.onExecute(props.action)}>Confirm archive</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    )
  }
...
function PlanRows(props: { items: SynctPlanItem[]; paths: string[] }) {
  if (!props.items.length) {
    const text = props.paths.length ? "Run a plan to show archive destinations." : "Add source paths to preview the archive route."
        ...
        {props.items.slice(0, 180).map((item, index) => {
          ...
          return (
            <div key={`${item.sourcePath}:${index}`} className={cn("grid gap-1 rounded-md border px-2 py-1.5", (item.status === "conflict" || item.status === "error") && "border-destructive/40", item.status === "unchanged" && "opacity-75")}>
              ...
              <div className="truncate text-[11px] text-muted-foreground">{item.timestamp ? formatDate(item.timestamp) : "No timestamp"}{item.reason ? ` / ${item.reason}` : ""}</div>
            </div>
          )
        })}
      </div>
...
function ResultTabs(props: { compact?: boolean; logs: string[]; result: SynctData | null; onCopyLogs: () => void; onCopyResults: () => void }) {
  return (
    <Tabs defaultValue="plan" className="flex h-full min-h-0 flex-col">
      <TabsList variant="line" className="shrink-0"><TabsTrigger value="plan"><Map className="size-3.5" />Plan</TabsTrigger><TabsTrigger value="issues"><AlertTriangle className="size-3.5" />Issues</TabsTrigger><TabsTrigger value="logs"><Terminal className="size-3.5" />Log</TabsTrigger></TabsList>
...
function PlanPanel(props: { compact?: boolean; result: SynctData | null; onCopy: () => void }) {
  return (
    <section className="flex h-full min-h-0 flex-col rounded-lg border bg-card">
      <div className={props.compact ? "flex shrink-0 items-center justify-between gap-2 px-2 py-1.5" : "flex shrink-0 items-center justify-between gap-2 px-3 py-2"}><div className="flex min-w-0 items-center gap-2 text-xs font-medium text-muted-foreground"><Map className="size-3.5" /><span>{props.result?.items.length ? `${props.result.items.length} items` : "Waiting for plan"}</span></div><Button disabled={!props.result?.items.length} size="xs" variant="ghost" onClick={props.onCopy}><Copy data-icon="inline-start" />Copy</Button></div>
      <Separator />
...
function StatsPanel(props: { paths: string[]; progress: number; result: SynctData | null }) {
  const stats = [
    { label: "Sources", value: props.paths.length },
    { label: "Scanned", value: props.result?.scannedCount ?? 0 },
...
function statusFromState(data: SynctCardState, running: boolean, result: SynctData | null): SynctStatusMeta {
  if (running || data.phase === "running") return { label: "Running", description: data.progressText || "Synct is scanning or moving archive items.", tone: "running", badgeVariant: "secondary", iconClass: "bg-primary text-primary-foreground" }
  if (data.phase === "error" || result?.errorCount) return { label: "Failed", description: data.progressText || result?.errors[0] || "Last Synct run failed. Check issues.", tone: "error", badgeVariant: "destructive", iconClass: "bg-destructive text-destructive-foreground" }
  if (data.phase === "completed") return { label: "Done", description: data.progressText || "Last Synct run completed.", tone: "success", badgeVariant: "default", iconClass: "bg-primary text-primary-foreground" }
  return { label: "Ready", description: "Add paths and preview timestamp archive destinations.", tone: "idle", badgeVariant: "outline", iconClass: "bg-secondary text-secondary-foreground" }
}
...
function itemStatusMeta(status: SynctPlanItem["status"]) {
  ...
  if (status === "skipped") return { icon: AlertTriangle, label: "Skipped", variant: "outline" as const }
  return { icon: CheckCircle2, label: "Same", variant: "outline" as const }
}

function summaryText(props: ViewProps): string {
  if (props.data.progressText) return props.data.progressText
  if (props.result) return `${props.result.items.length} items / ready ${props.result.readyCount} / conflicts ${props.result.conflictCount}`
  if (props.paths.length) return `${props.paths.length} source path(s) / ${props.actionMeta.shortLabel}`
