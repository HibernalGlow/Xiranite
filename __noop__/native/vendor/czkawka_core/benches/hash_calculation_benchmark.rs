...
fn benchmark_hash_calculation_vec<const FILE_SIZE: u64, const BUFFER_SIZE: usize>(
    c: &mut Criterion,
) {
...
fn benchmark_hash_calculation_arr<const FILE_SIZE: u64, const BUFFER_SIZE: usize>(
    c: &mut Criterion,
) {
    let file_entry = get_file_entry(FILE_SIZE);
