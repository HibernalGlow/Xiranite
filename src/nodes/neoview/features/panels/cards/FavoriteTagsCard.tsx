...
function FavoriteTagsContent({ client }: { client: ReaderPanelContext["client"] }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (requestGeneration.current === generation) refreshPending.current = false
    })
  ...
  if (state.error) return <div role="alert" className="grid min-h-20 justify-items-center gap-2 text-center text-[11px] text-destructive"><span>{state.error}</span><Button size="sm" variant="outline" onClick={refresh}>重试</Button></div>
  ...
  return (
    <div className="space-y-2 text-[11px]" data-favorite-tags-card="true">
      <div className="flex items-center justify-between"><span className="inline-flex items-center gap-1 text-muted-foreground"><Tags className="size-3" />{state.tags.length} 个标签</span><Button type="button" size="icon-sm" variant="ghost" aria-label="刷新收藏标签" disabled={state.loading} onClick={refresh}><RefreshCw /></Button></div>
