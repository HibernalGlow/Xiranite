...
| D11 | `[x]` | 左右多 Tab 工作区 | 宽屏条件栏和分析栏按卡片注册表生成 Tab，不再纵向堆叠 |
...
卡片系统证据（更新于 2026-07-16）：`packages/nodes/czkawka/src/card-layout.ts` 继续提供六张卡片的注册表、`source`/`analysis` 面板、可见性、顺序和跨面板归属。宽屏 GUI 参考 xlchemy 改为 `CzkawkaCardTabs`：左右栏分别由当前面板的可见卡片生成 Tab，只挂载活动内容，活动 Tab 写入 `sourcePanelTab`/`analysisPanelTab`；头部管理器仍可切换可见性、面板归属和恢复默认布局。`CzkawkaCardStack` 只保留给浮动分析面板和兼容路径，避免主工作区继续纵向堆叠。
...
布局与迁移证据（更新于 2026-07-16）：`CzkawkaWorkspaceLayout` v1 继续兼容旧工具轨道字段，运行时消费扫描条件和分析面板宽度及最小化状态；两个桌面 separator 支持指针、方向键、Shift 步进和双击复位，compact/portrait 不应用桌面列宽。宽屏左右栏已从卡片上下堆叠改为注册表驱动的多 Tab，compact/portrait 仍保留条件/结果/统计顶层 Tab。结果区使用项目现有 NikoTable：小结果使用标准表体，200 行起切虚拟表体；名称单元格通过共享 `LocalMediaPreview` 和宿主 `getFileUrl` 直接显示图片/视频/音频，不再维护独立预览列。测试覆盖 Niko 引擎、万级虚拟化、图片直通、缩略图关闭、组/行选择、右键宿主动作、固定预览和左右 Tab 持久化。
...
Q11 验证证据（2026-07-15）：Windows x64 Release API v4 `.node` 为 30.77 MiB，`dav1d.dll` 为 1.70 MiB，二者的发布 ZIP 为 10.52 MiB；Bun smoke 返回 API v4 / Czkawka 10.0.0，覆盖 duplicate、basic、image/video media 三类入口，并真实观测 `CollectingFiles` 进度、取消和 session 清理。构建直接复用仓库 `native/vendor/dav1d-windows-x64.zip` 中的开发包，通过 `PKG_CONFIG_PATH` 定位 `dav1d.pc`，不安装 VCPKG；Windows 构建使用 thin-LTO 与 8 codegen units，避免 full-LTO 的 LLVM 内存峰值。预编译 ZIP 与 SHA-256 manifest 已由 `packages/czkawka-native/prebuilt/win32-x64` 固化，embedded smoke 也成功加载 API v4。
