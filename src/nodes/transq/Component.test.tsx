...
describe("app-owned transq Component", () => {
  test.each(NODE_SURFACE_TEST_MODES)(
    "renders the %s surface with TransQ-specific UI",
    (mode) => {
      setSurface(mode)
      render(<Component compId="comp-transq" host={createHost({ pathsText: "D:/archives/a.zip" })} />)
...
const packuData: PackuToolData = {
  spec: NODE_META.spec,
  command: {
    label: "python -m transq",
    command: "python",
    args: ["-m", "transq", "D:/archives/a.zip"],
