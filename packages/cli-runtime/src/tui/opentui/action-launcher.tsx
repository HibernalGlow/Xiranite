...
export function ActionLauncher<Result>({ field, session, disabled, id = `field-${field.id}`, options = field.options ?? [] }: { field: InteractionField; session: TerminalUiSession<Result>; disabled?: boolean; id?: string; options?: readonly InteractionOption[] }) {
  return <ActionTabs id={id} options={options} value={session.values[field.id]} focused={session.focusedControlId === field.id} disabled={disabled ?? session.phase === "running"} onFocus={() => session.focus(field.id)} onChange={(value) => void session.requestAction(field.id, value)} />
}
