...
describe("loadNeoviewSessionOptions", () => {
  ...
  it("loads shell options from the same TOML snapshot as reader defaults", async () => {
    ...
    await writeFile(configPath, [
      ...
      "width = 444",
      "height = \"half\"",
      "[nodes.neoview.panels.swimlane]",
      "lane_order = [\"left\", \"reader\", \"right\"]",
      "active_lane = \"left\"",
      "reader_solo = true",
      "reader_width_ratio = 0.58",
      "edge_reveal_delay_ms = 360",
      "reader_focus_on_hover = true",
      "reader_focus_hover_delay_ms = 750",
      "left = { width = 520, collapsed = false, active_panel_id = \"pageList\" }",
      "reader = { width = 1600, collapsed = false }",
      "right = { width = 360, collapsed = true, active_panel_id = \"info\" }",
    ...
    expect((await loadNeoviewRuntimeConfig({ configPath })).shellOptions).toMatchObject({
      ...
      workspace: {
        ...
        swimlane: {
          activeLane: "left",
          readerSolo: true,
          readerWidthRatio: 0.58,
          edgeRevealDelayMs: 360,
          readerFocusOnHover: true,
          readerFocusHoverDelayMs: 750,
          lanes: {
            left: { width: 520, collapsed: false, activePanelId: "pageList" },
            reader: { width: 1600, collapsed: false },
            right: { width: 360, collapsed: true, activePanelId: "info" },
          },
        },
      },
