...
`ArchiveProvider.updatePreloadDemand()` 现在提供了明确的 provider-demand 契约：需求携带 generation、阅读方向、方向置信度和 provider-local entry IDs，`SevenZipArchiveProvider` 只按这份显式需求驱动 solid materializer。materializer 在已验证 entry 边界停止不需要的后半段；新的 generation 请求更远 entry 时复用同一个 materializer 实例并从新的顺序 extractor 继续，`openEntry` 只提升硬性可见 entry 目标，不推断方向。该路径继续使用现有 `SolidArchiveCache`、内存/磁盘层和唯一资源 lease，不引入第二条解压主链。剩余接线是由 ReaderPreloadCoordinator 将 page candidate 按 provider 分组后调用该契约；在接线完成前，HTTP/React 不主动吞读相邻原图。
