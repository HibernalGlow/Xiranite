...
| Panel/Card | 兼容布局、折叠与设置态 Kanban 纵切 | 完整旧 panel ID metadata；Card lazy loader；关闭/折叠后内容 DOM=0；旧布局/card v14 经 TOML 规范化；新版状态写入 canonical `panel_state/card_state`；独立设置浮窗的“卡片管理”按需加载 Dice UI Kanban，Panel 为列、Card 为 item，含“隐藏/未停靠”列并一次批量 PATCH；设置模块也是共享 registry Card，默认只在设置浮窗，显式停靠后可进入静态侧栏 | 其余全部业务卡片、边栏管理写回、Card 高度拖动、独立 card window/tab |
| 完整阅读工具与设置 UI | 设置浮窗骨架纵切 | 路径打开、前后/页码/关闭等 smoke controls；按 `setting.png` 建立独立大尺寸设置浮窗、左侧分类导航、默认“边栏管理”和独立“卡片管理”页 | 原 TopToolbar 全功能、其余设置分类业务卡、文件浏览、历史/书签、超分、AI、benchmark、快捷操作 |

当前验证证据（数值随复跑波动，以预算和原始输出为准）：

- 前端 NeoView：12 个测试文件、33 项 component/hook/registry/shell/config/resize/card/settings/Kanban 测试通过；后端 NeoView 最近完整复跑为 35 个文件、Vitest 174 项 + Bun TUI 1 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖完整 board patch、单次批量保存、设置浮窗分类结构、默认页零 Kanban、modal Escape 保护、设置 Card 默认未停靠、精确目标菜单及显式侧栏停靠；
- Reader pipeline 最近复跑：backend startup `12.64 ms`、open `47.40 ms`、navigation p95 `3.63 ms`、response headers p95 `5.09 ms`、first byte p95 `5.16 ms`、full stream p50 `32.96 ms`、throughput p50 `86.14 MiB/s`、RSS delta `4.77 MiB`；全部通过预算，设置/Kanban 没有进入图片数据热路径；
- 真实 Chromium 路径已覆盖桌面与 `420×360`：设置浮窗默认边栏页不挂载编辑器，切换卡片管理后加载 Kanban，将默认隐藏设置 Card 精确停靠到设置 Panel 后一次保存只发一个 board PATCH，关闭后左栏静态加载同一共享 Card，原图 `<img>` 节点身份不变；最终热翻页为 desktop `45.6/73.0 ms`、card `41.7/61.2 ms`（navigation/decode），仍明显低于 `150/200 ms` 硬门槛；设置窗口桌面宽度大于视口 65%、高度大于 80%，并输出截图证据；
- 当前生产构建报告：NeoView entry `32,117 bytes`，ReaderSidebar `6,315 bytes`，设置浮窗 `4,725 bytes`，PanelLayoutEditor `3,903 bytes`，Dice UI Kanban/dnd-kit runtime `58,547 bytes`；Kanban 为“卡片管理”二级延迟 chunk，未进入初始 chunk、NeoView entry、设置浮窗基础 chunk或普通 ReaderSidebar；PageNavigationCard `1,233 bytes`、BookInformationCard `1,015 bytes`，首屏 NeoView module `0`、前端 zip.js module `0`。
