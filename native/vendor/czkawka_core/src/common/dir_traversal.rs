...
impl<'b, F> DirTraversalBuilder<'b, F> {
    ...
    pub(crate) fn build(self) -> DirTraversal<'b, F> {
        DirTraversal {
            ...
            stop_flag: self
                .stop_flag
                .expect("Stop flag must be always initialized"),
            progress_sender: self.progress_sender,
            checking_method: self.checking_method,
...
pub(crate) fn common_get_entry_data<'a>(
    entry: &'a Result<DirEntry, std::io::Error>,
    warnings: &mut Vec<String>,
    current_folder: &Path,
) -> Option<&'a DirEntry> {
...
pub(crate) fn common_get_metadata_dir(
    entry_data: &DirEntry,
    warnings: &mut Vec<String>,
    current_folder: &Path,
) -> Option<Metadata> {
...
mod tests {
    ...
    fn test_traversal() -> io::Result<()> {
        ...
        let secs = NOW
            ...
            .expect("Cannot fail calculating duration since epoch")
            .as_secs();

        let mut common_data = CommonToolData::new(ToolType::SimilarImages);
        common_data
            .directories
            .set_included_directory([dir.path().to_owned()].to_vec());
    ...
    fn test_traversal_group_by_inode() -> io::Result<()> {
            ...
            .run()
        {
            DirTraversalResult::SuccessFiles {
                ...
                let actual: HashSet<_> = grouped_file_entries
                    .into_iter()
                    .flat_map(take_1_per_inode)
                    .collect();
                assert_eq!(
                    HashSet::from([
                        FileEntry {
    ...
    fn test_traversal_group_by_inode() -> io::Result<()> {
        ...
        let secs = NOW
            .duration_since(SystemTime::UNIX_EPOCH)
            .expect("Cannot fail duration from epoch")
            .as_secs();
