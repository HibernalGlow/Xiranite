...
文件操作采用同样的 presentation-neutral service/port 边界。`ReaderFileOperationService` 对最多 256 项的批次提供逐项成功、失败与取消结果，默认并发 4、最大 8；每项 platform mutation 都取得宿主 interactive I/O lease。Node 内建 `fs.cp/rm/mkdir` 承担复制、永久删除和建目录，维护中的 `move-file@4` 承担跨卷文件/目录移动与同目录 rename，`trash@10` 承担回收站适配；默认 `overwrite=false`，目录复制也先明确检查目标冲突。统一鉴权路由 `POST /reader/files/operations` 在首次调用时才加载 platform adapter，delete/trash 必须显式 `confirmed=true`。当前纵切没有写入 `xiranite.db` 或 Reader SQLite，也不自行实现跨盘算法或 Windows 回收站协议；安全 undo journal、回收站恢复、Explorer/open-with 及 CLI/TUI 命令面仍待完成，feature 继续保持 `pending`。
