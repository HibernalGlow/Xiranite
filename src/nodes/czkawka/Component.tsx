...
export function Component({ compId, host }: NodeComponentProps<CzkawkaCardState>) {
  ...
  async function cancelScan() {
    ...
    addActivityLog({ kind: "system", level: "warning", message: "正在请求停止 Czkawka 扫描…" })
    await cancel()
  }
...
function ScanPresetManager(props: View) {
  ...
  function apply(id: string) { const preset = presets.find((item) => item.id === id); if (!preset) return; setName(preset.name); setError(""); props.patch({ ...(czkawkaScanPresetToValues(preset) as Partial<CzkawkaCardState>), activeScanPresetId: preset.id }) }
  function save(overwrite: boolean) { try { const saved = czkawkaScanPresetFromValues(name, props.data as Record<string, unknown>, { presets, id: overwrite ? active?.id : undefined }); setName(saved.preset.name); setError(""); props.patch({ scanPresets: saved.presets, activeScanPresetId: saved.preset.id }) } catch (caught) { setError(message(caught)) } }
  function remove() { if (!active) return; props.patch({ scanPresets: deleteCzkawkaScanPreset(presets, active.id), activeScanPresetId: undefined }); setName(""); setError("") }
  function importPresets() { try { const imported = importCzkawkaScanPresets(transferText, presets); props.patch({ scanPresets: imported }); setError("") } catch (caught) { setError(message(caught)) } }
  return <details className="rounded-md border bg-muted/20 p-2"><summary className="cursor-pointer text-xs font-medium">扫描配置预设 · {presets.length}</summary><div className="mt-2 grid gap-2">{presets.length ? <Field label="活动预设"><Select value={active?.id ?? ""} onValueChange={apply}><SelectTrigger aria-label="active scan preset"><SelectValue placeholder="选择预设" /></SelectTrigger><SelectContent>{presets.map((preset) => <SelectItem key={preset.id} value={preset.id}>{preset.name} · {toolMeta(preset.tool).label}</SelectItem>)}</SelectContent></Select></Field> : null}<Field label="预设名称"><Input aria-label="scan preset name" value={name} onChange={(event) => setName(event.currentTarget.value)} /></Field><div className="grid grid-cols-3 gap-1"><Button size="xs" variant="outline" onClick={() => save(false)}>新建</Button><Button disabled={!active} size="xs" variant="outline" onClick={() => save(true)}>覆盖</Button><Button disabled={!active} size="xs" variant="ghost" onClick={remove}>删除</Button></div><Field label="导入 / 导出 JSON"><Textarea aria-label="scan preset transfer" className="min-h-24 font-mono text-[10px]" value={transferText} onChange={(event) => setTransferText(event.currentTarget.value)} /></Field><div className="grid grid-cols-2 gap-1"><Button disabled={!presets.length} size="xs" variant="outline" onClick={() => { setTransferText(exportCzkawkaScanPresets(presets)); setError("") }}>导出</Button><Button disabled={!transferText.trim()} size="xs" variant="outline" onClick={importPresets}>导入合并</Button></div>{error ? <div role="alert" className="text-xs text-destructive">{error}</div> : null}<div className="text-[10px] text-muted-foreground">活动预设及已应用字段随节点状态持久化，重新打开时自动恢复。</div></div></details>
}
