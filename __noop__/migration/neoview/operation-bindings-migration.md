...
原 NeoView 的操作绑定同时覆盖动作目录、键盘/鼠标/滚轮/触控、上下文路由、输入录制、径向菜单和旧配置导入。Xiranite 当前已经具备共享 DTO、规范 TOML 持久化、冲突检测、上下文隔离、九宫格与六类设备/轨迹运行时；剩余主体是完整动作目录、径向菜单、旧动作级转换和 CLI/TUI 投影。
...
- 鼠标：按钮 0–7、单击/双击/按下/长按、持续时间与移动容差。
- 鼠标轨迹：最多 16 段去重方向序列、即时/长按触发与可视化录制。
- 滚轮：上/下与四种修饰键。
- 触控：左/右/上/下滑、点击、长按与 1–3 指。
- 手柄：标准按钮 0–31。
...
- 鼠标、滚轮、触控、轨迹和手柄录制器分别由 `@use-gesture/react` 与 `gamepad.js` 提供设备生命周期，复用同一 DTO，不另建持久化格式。
- 一个操作可拥有任意多条不同设备或上下文绑定；扁平记录仅以唯一 `id` 标识输入，不按动作去重。
...
以下能力仍按兼容矩阵保持 `pending`，不计入本轮完成：径向菜单、语音、旧动作级转换、CLI/TUI 上下文投影，以及尚无业务 dispatch 的原项目动作。它们必须复用本轮动作目录、上下文和持久化模型。

## 验收标准

- 键盘、鼠标、轨迹、滚轮、触控、手柄与九宫格的既有 DTO 字段均能在设置卡中编辑。
- 可通过录制按钮捕获键盘组合键，并可取消录制。
- 一个操作的多条绑定、0–7 鼠标键、轨迹序列、滚轮修饰键和 1–3 指触控可以保存并通过严格后端解析。
