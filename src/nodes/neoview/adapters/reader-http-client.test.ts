...
describe("reader-http-client", () => {
  it("[neoview.react.control] sends token-authenticated open, navigation and close requests", async () => {
    ...
    expect(String(fetchMock.mock.calls[8]?.[0])).toContain("/reader/s/reader-1/navigate")
    expect(JSON.parse(String(fetchMock.mock.calls[9]?.[1]?.body))).toEqual({ action: "goTo", pageIndex: 17 })
    expect(String(fetchMock.mock.calls[10]?.[0])).toContain("/reader/s/reader-1/options")
    expect(JSON.parse(String(fetchMock.mock.calls[10]?.[1]?.body))).toEqual({ layout: { pageMode: "double" } })
    expect(fetchMock.mock.calls[11]?.[1]).toMatchObject({ method: "DELETE", keepalive: true })
  })
