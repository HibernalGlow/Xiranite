...
export function ActionSelect(props: {
  ...
  return (
    <Select value={props.action} disabled={props.disabled} onValueChange={(value) => props.onActionChange(value as GifuAction)}>
      <SelectTrigger aria-label="gifu action" className={cn("min-w-0", props.triggerClassName)}>
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        {ACTIONS.map((item) => (
  ...
  return (
    <div className="grid gap-2 @3xl/gifu:grid-cols-2">
      <Select
        ...
        disabled={props.disabled}
        onValueChange={(format) => props.onPatch({ format: format as GifuFormat })}
      >
        <SelectTrigger aria-label="gifu 输出格式">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
      ...
      <Select
        ...
        disabled={props.disabled}
        onValueChange={(outMode) => props.onPatch({ outMode: outMode as GifuOutputMode })}
      >
        <SelectTrigger aria-label="gifu 输出模式">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
