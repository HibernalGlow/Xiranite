...
- [x] `folder.tabs.lifecycle` 多标签创建、切换与关闭
  ...
  - 计划测试：无
  - 备注：GUI 使用最多 8 个标签的有界宿主，与 backend browser session 上限一致；新标签打开共享 Home，最后一个标签不可关闭。默认布局下单标签仍渲染完整 TabBar，确保复制、固定、新建、重开与布局动作可达；仅用户显式选择 layout=none 时折叠为恢复布局按钮。每个标签独立保存当前目录、browser session、导航历史、renderer、选择、焦点与 Virtuoso 视口，切换不重复 POST，关闭会卸载 pane 并释放 watcher、缩略图 context 与 browser session。普通 list/grid/details 始终只显示各标签当前目录的直接子项；Folder Tree 是独立 companion view，不把树层级或磁盘根注入普通列表。标签宿主与标签栏保持二级延迟 chunk，desktop/420x360 Chromium 已验证状态隔离、关闭 DELETE 与活动阅读图片身份稳定。
...
- [x] `emm-raw-data.filter` 字段和值过滤
  ...
  - 源码：`src/lib/cards/properties/EmmRawDataCard.svelte`
  - 测试：`neoview.emm-sync.direct-source`、`neoview.emm-raw-data.filter-empty`
  - 计划测试：无
  - 备注：Filtering is local, case-insensitive across key, label, raw and formatted value, preserves the source DTO and exposes a distinct filtered-empty state.
- [x] `emm-raw-data.sort` 字段和值排序
  - 六维：`core=N/A transport=N/A gui=C cli=N/A tui=N/A evidence=C`；阻塞：无
