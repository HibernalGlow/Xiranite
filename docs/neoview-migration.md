...
> 最后更新：2026-07-15；最近已提交基线：`897886a`（原图阅读/预解码/虚拟缩略图）、`880ccff`（复杂 Shell/Card AST scaffold）、`1e4f70c`（懒加载四边 Shell 与首批 Panel/Card）。当前工作纵切已把旧 `panels` 设置接入 TOML → 后端 → React，只读配置链验证通过，待本次提交后补充提交号。本节必须随每个 NeoView 实现提交同步更新；代码、feature matrix 和测试证据优先于文字。
...
| Phase 3：React Reader | 进行中（当前主线） | 原始 asset URL `<img>`、相邻页 `Image.decode()`、64 页批次虚拟缩略图；四边 EdgeShell、首批左右 Panel/Card 正在本轮接入 | 完整 TopToolbar、左右全部 panel/card、单双页/全景/连续长图、缩放/旋转/手势、窗口/tab |
...
| 设置与数据迁移 | 部分完成 | LegacySettingsCodec、TOML inspect/import、确认/备份/原子写入；Reader runtime 同一快照解析阅读默认值和受限 Shell DTO，鉴权 `/reader/config` 只返回规范化 UI 数值 | Shell 调整 PATCH/原子回写、GUI/TUI 确认、专用数据库数据、导出/自动备份/Gist transport |
...
| 图片显示与热翻页 | 可用且已过硬门槛 | 唯一 DOM `<img src=assetUrl>`；零 Base64/Blob/Canvas/sharp 默认链；相邻页有界预解码 | 双页/全景/连续模式、旋转/fit layout engine、8K/16K/GPU 长测 |
| 缩略图 | 只读显示纵切可用 | 64 页 cursor 批次、横向虚拟窗口、1000 页约 20 个 tile DOM、旧 DB URL | 生成/维护、竖向 PageList、筛选/区域/边栏模式 |
| 四边 Shell | 进行中（配置读取纵切完成） | AST provenance 的 top/right/bottom/left slot；trigger、显隐、pinned、延迟、透明度、模糊、左右宽度/高度/垂直对齐/水平偏移均从 Xiranite TOML 的规范化 DTO 读取；IME/浮层、Escape、零挂载；配置异步到达不重挂活动 `<img>` | resize/drag、空白点击策略、布局 PATCH 原子回写 TOML、完整旧 sidebar store 导入 |
| Panel/Card | 首个纵切 | 完整旧 panel ID metadata；Card lazy loader；左“页面导航”和右“书籍信息”；关闭后 DOM=0 | 其余全部卡片、显隐/折叠持久化、排序、跨 panel、独立 window/tab |
| 完整阅读工具与设置 UI | 大部分未迁移 | 路径打开、前后/页码/关闭等 smoke controls | 原 TopToolbar 全功能、设置、文件浏览、历史/书签、超分、AI、benchmark、快捷操作 |

当前验证证据（数值随复跑波动，以预算和原始输出为准）：

- 前端 NeoView：8 个测试文件、22 项 component/hook/registry/shell/config 测试通过；后端 NeoView 最近完整复跑为 35 个文件、Vitest 169 项 + Bun TUI 1 项通过；新增覆盖 parser 边界、TOML 同快照加载、HTTP 鉴权/脱敏 DTO、客户端请求、异步配置应用和活动图片节点不重挂；
- Reader pipeline 最近复跑：backend startup `19.33 ms`、open `57.04 ms`、navigation p95 `6.75 ms`、response headers p95 `2.94 ms`、first byte p95 `3.03 ms`、full stream p50 `27.85 ms`、throughput p50 `101.96 MiB/s`、RSS delta `2.34 MiB`；全部通过预算；
- 四边 Shell + 左右 lazy card 的真实 Chromium 路径已覆盖桌面与 `420×360`：关闭侧栏/底栏后 DOM 为 0，底栏卸载时预解码仍继续；本轮接入 TOML Shell 配置后的热翻页为 desktop `41.9/66.7 ms`、card `50.8/80.3 ms`（navigation/decode），仍明显低于 `150/200 ms` 硬门槛；
- 本轮生产构建报告：NeoView entry `32,916 bytes`，ReaderSidebar `6,044 bytes`，PageNavigationCard `2,132 bytes`，BookInformationCard `2,020 bytes`；首屏 NeoView module `0`、前端 zip.js module `0`。相对上一提交 entry 增加 `1,176 bytes`、ReaderSidebar 增加 `800 bytes`，仍分别低于 40 KiB/32 KiB 硬门槛；配置 parser/HTTP DTO 保持后端链，Panel/Card 继续独立延迟加载。
...
当前最小 React viewer 的 `PageImage` 保持唯一 DOM `<img>` 主链，`src` 始终是后端返回的原始 `assetUrl`。普通翻页不测量视口、不构造 transform URL，也不经 sharp 解码再编码。双页模式只消费 backend `FrameSnapshot` 给出的可见页；后续 fit、旋转、全景等布局由 `FrameLayoutEngine` 计算 CSS 几何，不改变图片的数据主链。
