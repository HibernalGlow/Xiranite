...
- 已知差异：GUI 已直接消费后端 direction/tier/admission preload plan，每代按后端顺序最多保留 4 个预解码图片，旧后端无 plan 时才回退相邻页；Image load/decode 生命周期按 session+generation 以 50ms、最多 64 条批量上报 TTFB/decode/retained bytes/active leases，不写 React state；viewport context 仅在会话建立或窗口 focus/visibility 变化时更新，普通翻页不新增 context PATCH；CLI 直接投影同一 Headless plan，OpenTUI 标记 Near/Ahead/Background 候选和 generation/admission，不另开页面流或复制 scheduler；core 与端到端 evidence 仍待按全部旧行为清单收口
