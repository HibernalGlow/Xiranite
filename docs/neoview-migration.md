...
从 `folderMain` 开始，77 张 Card 统一执行“清单先于实现”的完成门禁，机器可读模板固定在 `migration/neoview/card-acceptance-contract.json`，基础审计执行 `bun run audit:neoview-card-checklists`，发布门禁追加 `--require-complete`。每张 Card 开工前必须从冻结源码列全：用户能力、UI 层级和控件、所有交互状态、设置/默认值/优先级、快捷键与可访问性、GUI/CLI/TUI 共用数据契约、加载/休眠/释放生命周期、性能预算、测试 ID 和有意偏离。仅有后端 API、一个 smoke control 或一条 happy path 均不能标记 `complete`；没有源码证据、自动化测试和视觉/几何证据的项保持 `pending/partial`。这份仓库内 JSON 与本文档是跨任务的持久迁移记忆，后续实现不得依赖聊天上下文记住遗漏。
...
- `folderMain` 只有 74 项全部 `complete` 后才能把 `migration/neoview/card-compatibility.json` 中对应 Card 从 `partial` 改为完成；随后其余 76 张 Card 逐张建立同结构的专用清单。
