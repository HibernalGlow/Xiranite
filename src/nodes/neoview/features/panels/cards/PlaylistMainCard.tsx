...
export default function PlaylistMainCard({ client, disabled, panelActive = true, onOpen }: ReaderPanelContext) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col" data-neoview-card="playlist-main">
    <header className="border-b p-2"><div className="flex items-center gap-2"><ListMusic className="size-4"/><span className="text-sm font-semibold">播放列表</span><div className="ml-auto flex gap-1"><Input aria-label="新播放列表名称" className="h-7 w-32 text-xs" value={name} onChange={(event) => setName(event.currentTarget.value)} onKeyDown={(event) => { if (event.key === "Enter") void create() }} placeholder="新列表" disabled={disabled || pending}/><Button size="icon-sm" aria-label="新建播放列表" disabled={disabled || pending || !name.trim()} onClick={() => void create()}><Plus/></Button></div></div><div className="mt-2 flex gap-1 overflow-x-auto" data-scrollbar="hidden">{playlists.map((item) => <Button key={item.id} size="sm" variant={item.id === activeId ? "default" : "secondary"} className="h-7 shrink-0 text-xs" onClick={() => setActiveId(item.id)}>{item.name}</Button>)}</div></header>
