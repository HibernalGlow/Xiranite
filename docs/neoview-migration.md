...
- ZIP/CBZ/EPUB entry stream 与压缩包页 presentation 已和 7-Zip 一样接入宿主 scheduler，并共享单个解压/变换 lease；library source stat 与 folder representative 扫描也已按 view/background 进入宿主 I/O 池；继续审计 SQLite 预热/维护、超分和其他高负载节点，补 queue/cache 指标与 benchmark；
