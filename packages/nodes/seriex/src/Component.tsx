...
export function Component({ compId, host }: NodeComponentProps) {
  ...
  async function execute(action: SeriexInput["action"], dryRun = false) {
    if (running) return
    const runNativeAction = createUnavailableNativeAction("Native action is unavailable in the shell-less Component. Use the xiranite-seriex CLI for filesystem actions.")
    setRunning(true)
    patch({ phase: "running" })
