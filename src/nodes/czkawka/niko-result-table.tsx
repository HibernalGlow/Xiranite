  ...
  const columns = useMemo<DataTableColumnDef<ResultRow>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const openPreview = () => {
          if (mediaKind === "image" && props.imageComparison && !props.previewPanelEnabled) props.imageComparison.open(entry.path)
          else setActivePreviewPath(entry.path)
        }
        return <div className="flex min-w-0 items-center gap-2">{props.thumbnailEnabled === false ? null : mediaKind ? <button type="button" aria-label={mediaKind === "image" ? t("result.previewEntry", "预览 {{name}}", { name: entry.name }) : t("result.playEntry", "播放 {{name}}", { name: entry.name })} className="shrink-0" onClick={openPreview}>{preview}</button> : preview}<div className="min-w-0"><div className={cn("text-xs font-medium", props.wrapText ? "line-clamp-2 break-all" : "truncate")} title={entry.path}>{entry.name}</div>{entry.isReference ? <Badge variant="secondary" className="mt-0.5 h-4 px-1 text-[9px]">{t("result.reference", "参考")}</Badge> : null}</div></div>
      },
    },
...
function mediaMetadata(entry: CzkawkaEntry, group: CzkawkaGroup, t: Translate) { return [{ label: t("result.columns.size", "大小"), value: formatBytes(entry.size) }, { label: t("result.columns.dimensions", "分辨率"), value: entry.width && entry.height ? `${entry.width}×${entry.height}` : "—" }, ...(entry.fps === undefined ? [] : [{ label: t("result.columns.fps", "帧率"), value: `${Number(entry.fps.toFixed(2))} fps` }]), ...(entry.codec ? [{ label: t("result.columns.codec", "编码"), value: entry.codec }] : []), ...(entry.bitrate === undefined ? [] : [{ label: t("result.columns.bitrate", "码率"), value: `${entry.bitrate} kbps` }]), ...(entry.length ? [{ label: t("result.columns.length", "时长"), value: entry.length }] : []), { label: t("result.columns.modified", "修改时间"), value: formatDate(entry.modifiedDate) }, { label: t("result.columns.similarity", "相似度"), value: entry.similarity || "—" }, { label: t("result.group", "组"), value: String(group.id + 1).padStart(2, "0") }] }
