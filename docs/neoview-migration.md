...
| Phase 1：领域核心与契约 | 进行中（核心纵切可用） | Page/Frame/ReaderSession、自然排序、单双页基础、generation/cancel、统一 ReaderService、共享阅读进度恢复/异步合并写/关闭刷盘、characterization/conformance tests | 完整 layout、历史列表、书签、统计、设置/数据迁移契约和所有旧行为 |
...
| 子系统 | 状态 | 当前实现 | 未完成边界 |
| --- | --- | --- | --- |
| Reader domain/application | 可用但未全兼容 | `packages/nodes/neoview/src/domain` 与 `application`；GUI/CLI/TUI 共用 session/controller；显式页码优先、默认恢复上次进度，翻页写入不阻塞主链且关闭强制刷盘 | 完整阅读模式、历史列表/书签/统计/播放列表和全部错误恢复 |
