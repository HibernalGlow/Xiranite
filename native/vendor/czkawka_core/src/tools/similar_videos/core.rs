...
use crate::common::cache::{
    CACHE_VIDEO_VERSION, extract_loaded_cache, load_cache_from_file_generalized_by_path,
...
impl SimilarVideos {
    ...
    pub(crate) fn check_for_similar_videos(
        &mut self,
        ...
        self.common_data
            .extensions
            .set_and_validate_allowed_extensions(VIDEO_FILES_EXTENSIONS);
        ...
        match result {
            DirTraversalResult::SuccessFiles {
                ...
                self.videos_to_check = grouped_file_entries
                    ...
                    .map(|fe| {
                        (
                            fe.path.to_string_lossy().to_string(),
                            fe.into_videos_entry(),
                        )
                    })
                    .collect();
                self.common_data.text_messages.warnings.extend(warnings);
                debug!(
                    "check_files - Found {} video files.",
                    self.videos_to_check.len()
                );
    ...
    fn load_cache_at_start(
        &mut self,
    ) -> (
        BTreeMap<String, VideosEntry>,
        BTreeMap<String, VideosEntry>,
        BTreeMap<String, VideosEntry>,
    ) {
        let loaded_hash_map;
        ...
        if self.common_data.use_cache {
            let (messages, loaded_items) = load_cache_from_file_generalized_by_path::<VideosEntry>(
                &get_similar_videos_cache_file(
                    self.params.skip_forward_amount,
                    self.params.duration,
                    self.params.crop_detect,
                ),
    ...
    fn check_video_file_entry(&self, mut file_entry: VideosEntry) -> VideosEntry {
        ...
        let vhash =
            match VideoHashBuilder::from_options(creation_options).hash(file_entry.path.clone()) {
                Ok(t) => t,
                Err(e) => {
    ...
    pub(crate) fn sort_videos(
        ...
        for file_entry in &vec_file_entry {
            ...
            if file_entry.error.is_empty() {
                hashmap_with_file_entries.insert(
                    file_entry.vhash.src_path().to_string_lossy().to_string(),
                    file_entry.clone(),
                );
    ...
    fn save_cache(
        &mut self,
        vec_file_entry: Vec<VideosEntry>,
        loaded_hash_map: BTreeMap<String, VideosEntry>,
    ) {
        if self.common_data.use_cache {
    ...
    fn match_groups_of_videos(
        &mut self,
        vector_of_hashes: Vec<VideoHash>,
        hashmap_with_file_entries: &HashMap<String, VideosEntry>,
    ) {
        ...
        let match_group = vid_dup_finder_lib::search(
            vector_of_hashes,
            self.get_params().tolerance as f64 / 40.0f64,
        );
...
pub fn get_similar_videos_cache_file(
    ...
    format!(
        "cache_similar_videos_{CACHE_VIDEO_VERSION}__skip_{skip_forward_amount}__dur_{duration}__cd_{crop_detect_str}.bin"
    )
}
