...
interface SharedViewProps { dense?: boolean; data: VertCardState; browserFiles: File[]; browserOutputs: VertBrowserOutput[]; drop: ReturnType<typeof useLocalFileDrop>; engine: VertEnginePreference; paths: string[]; routes: VertConversionRoute[]; running: boolean; target: string; onBrowserPick: () => void; onChooseNative: () => void; onEngine: (value: VertEnginePreference) => void; onGroupChange: (key: string, config: VertConversionGroupConfig) => void; onReset: () => void; onRun: () => void }
function Compact(props: SharedViewProps) { return <div data-testid="vert-compact-view" className={cn("flex min-h-0 flex-1 flex-col p-2", props.dense ? "gap-1.5" : "gap-2")}><Header data={props.data} engine={props.engine} running={props.running} /><UploadPanel compact dense={props.dense} {...props} /><ConversionTopology compact dense={props.dense} routes={props.routes} onChange={props.onGroupChange} /><div className="flex items-center gap-2">{props.dense ? null : <EngineSelect value={props.engine} onChange={props.onEngine} />}<Button className="flex-1" size="sm" disabled={props.running || !props.routes.length} onClick={props.onRun}>{props.running ? <LoaderCircle data-icon="inline-start" className="animate-spin" /> : <RefreshCw data-icon="inline-start" />}转换 {props.routes.length} 组 · {props.engine === "auto" ? "CLI 优先" : props.engine.toUpperCase()}</Button></div><Progress value={props.data.progress ?? 0} label="VERT progress" /></div> }
