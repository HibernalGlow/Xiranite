...
1. Go unit/integration tests cover valid ZIP/CBZ indexing, duplicate paths, corrupted archives, encrypted and nested members, unsupported and unsafe archives, source-fingerprint invalidation, v1-to-v2 schema migration, same-stat central-directory reconciliation, anomaly cohorts, task recovery, request-id idempotency, deep retry, and every FFI error envelope.
2. `packages/findz-native` tests reject a compatible-ABI DLL that omits a required Worker capability. DLL smoke tests load both development and embedded artifacts, complete the ABI handshake, create a temporary Library database, scan a fixture, paginate a query, and release native response buffers.
3. Bun tests cover Worker routing, pointer cleanup, watcher coalescing, file-size/mtime stability retries, degraded reconciliation, cancellation hand-off, and Worker failure cleanup without accessing the user's `xiranite.db`.
4. Vitest Browser Mode covers rule-tree field mapping, expandable folder/archive hierarchy, stale-query suppression, table/treemap selection and scroll synchronization, retained treemap zoom, sort and metric changes, manual analysis controls, deep retry, pagination, empty/error/unsupported states, and Chinese localization.
