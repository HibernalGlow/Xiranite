...
describe("SoundW direct OpenTUI screen", () => {
  test("uses mouse actions and renders animated command state without a real SoundSwitch process", async () => {
    ...
    try {
      await act(async () => setup.renderOnce()); const first = setup.captureCharFrame(); expect(first).toContain("SOUNDW // RECORDING ROUTE"); expect(first).toContain("设备矩阵"); expect(first).toContain("预设中心")
