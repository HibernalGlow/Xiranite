...
pub struct DuplicateFinder {
    ...
    files_with_identical_size_names_referenced:
        BTreeMap<(u64, String), (DuplicateEntry, Vec<DuplicateEntry>)>,
    // File Size, File Entry
    ...
    files_with_identical_hashes_referenced:
        BTreeMap<u64, Vec<(DuplicateEntry, Vec<DuplicateEntry>)>>,
...
impl DuplicateFinder {
    ...
    pub const fn get_files_sorted_by_size_name(
        &self,
    ) -> &BTreeMap<(u64, String), Vec<DuplicateEntry>> {
    ...
    pub fn get_files_with_identical_hashes_referenced(
        &self,
    ) -> &BTreeMap<u64, Vec<(DuplicateEntry, Vec<DuplicateEntry>)>> {
        &self.files_with_identical_hashes_referenced
    }

    pub fn get_files_with_identical_name_referenced(
        &self,
    ) -> &BTreeMap<String, (DuplicateEntry, Vec<DuplicateEntry>)> {
        &self.files_with_identical_names_referenced
    }

    pub fn get_files_with_identical_size_referenced(
        &self,
    ) -> &BTreeMap<u64, (DuplicateEntry, Vec<DuplicateEntry>)> {
        &self.files_with_identical_size_referenced
    }

    pub fn get_files_with_identical_size_names_referenced(
        &self,
    ) -> &BTreeMap<(u64, String), (DuplicateEntry, Vec<DuplicateEntry>)> {
...
pub(crate) fn hash_calculation_limit(
    buffer: &mut [u8],
    ...
    let mut file_handler = match File::open(&file_entry.path) {
        ...
        Err(e) => {
            ...
            return Err(format!(
                "Unable to check hash of file {:?}, reason {e}",
                file_entry.path
            ));
        }
    };
    ...
    let n = match file_handler.read(&mut buffer[..limit as usize]) {
        ...
        Err(e) => {
            return Err(format!(
                "Error happened when checking hash of file {:?}, reason {}",
                file_entry.path, e
            ));
        }
    };
...
mod tests {
    ...
    fn test_hash_calculation_limit() -> io::Result<()> {
        ...
        let r1 = hash_calculation_limit(&mut buf, &e, HashType::Blake3, 1, &size_counter_1)
            .expect("hash_calculation failed");
        let r2 = hash_calculation_limit(&mut buf, &e, HashType::Blake3, 2, &size_counter_2)
            .expect("hash_calculation failed");
        let r3 = hash_calculation_limit(&mut buf, &e, HashType::Blake3, 1000, &size_counter_3)
            .expect("hash_calculation failed");
        assert_ne!(r1, r2);
        assert_eq!(r2, r3);
    ...
    fn test_hash_calculation_invalid_file() -> io::Result<()> {
        ...
        .expect_err("hash_calculation succeeded");
        assert!(!r.is_empty());
        Ok(())
    }
}
