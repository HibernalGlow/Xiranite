...
| Panel/Card | 兼容布局读取纵切 | 完整旧 panel ID metadata；Card lazy loader；左“页面导航”和右“书籍信息”；关闭后 DOM=0；旧 `panelsLayout.sidebarConfig.panels` 的 visible/order/position 经 TOML 规范化，未知未来 panel 原样保留，已实现卡片可用 panel 的显隐、排序和跨左右栏呈现 | 其余全部卡片、管理 UI 与写回、折叠持久化、拖拽排序、独立 window/tab |
...
- Reader pipeline 最近复跑：backend startup `21.33 ms`、open `40.50 ms`、navigation p95 `3.38 ms`、response headers p95 `2.67 ms`、first byte p95 `2.72 ms`、full stream p50 `30.15 ms`、throughput p50 `94.17 MiB/s`、RSS delta `5.41 MiB`；全部通过预算，open、翻页与数据面均优于上一轮；
- 四边 Shell + 左右 lazy card 的真实 Chromium 路径已覆盖桌面与 `420×360`：关闭侧栏/底栏后 DOM 为 0，底栏卸载时预解码仍继续；加入旧 panel layout 读取后的热翻页为 desktop `42.5/74.0 ms`、card `35.3/62.9 ms`（navigation/decode），仍明显低于 `150/200 ms` 硬门槛；40 步 pointermove 仅 1 次 PATCH，原图 `<img>` 节点身份保持不变；
- 本轮生产构建报告：NeoView entry `33,259 bytes`，ReaderSidebar `8,048 bytes`，PageNavigationCard `2,132 bytes`，BookInformationCard `2,020 bytes`；首屏 NeoView module `0`、前端 zip.js module `0`。panel layout 增量仅进入 ReaderSidebar deferred chunk（相对上轮 `+132 bytes`），entry 零增长，Panel/Card 仍低于 32 KiB 并保持独立延迟加载。
