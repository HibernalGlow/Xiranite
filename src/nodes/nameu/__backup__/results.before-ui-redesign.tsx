...
export function NameuStatsPanel({ result }: { result: PackuToolData | null }) {
  const stats = [
  ...
  return (
    <div data-testid="nameu-stats-panel" className="grid shrink-0 grid-cols-3 gap-1">
...
export function NameuResultTabs(props: {
  compact?: boolean
  logs: string[]
    ...
    <section className="flex h-full min-h-0 flex-col rounded-lg border bg-background/70">
      ...
      <ScrollArea className="min-h-0 flex-1">
        {hasCommand && command ? (
          ...
          <EmptyState compact={props.compact} icon={Terminal} title="等待命令" description="运行预览重命名后会显示命令计划。" />
        )}
      </ScrollArea>
    </section>
  )
    ...
    <section className="flex h-full min-h-0 flex-col rounded-lg border bg-background/70">
      ...
      <ScrollArea className="min-h-0 flex-1">
        {rows.length ? (
          ...
          <EmptyState compact={props.compact} icon={Settings2} title="等待集成" description="运行查看配置后会显示源码目录、模块名和配置候选。" />
        )}
      </ScrollArea>
    </section>
  )
