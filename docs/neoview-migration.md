...
> 最后更新：2026-07-15；最近已提交基线：`897886a`（原图阅读/预解码/虚拟缩略图）、`880ccff`（复杂 Shell/Card AST scaffold）、`1e4f70c`（懒加载四边 Shell 与首批 Panel/Card）、`5064b82`（旧 `panels` 设置接入 TOML → 后端 → React 的只读 Shell 配置链）。本节必须随每个 NeoView 实现提交同步更新；代码、feature matrix 和测试证据优先于文字。
