...
pub(crate) fn prepare_thread_handler_common(
    ...
    assert_ne!(
        tool_type,
        ToolType::None,
        "Cannot send progress data for ToolType::None"
    );
    ...
    let progress_thread_sender = if let Some(progress_sender) = progress_sender.cloned() {
        ...
        thread::spawn(move || {
            ...
            let mut time_since_last_send = Instant::now()
                .checked_sub(Duration::from_secs(10u64))
                .unwrap_or_else(Instant::now);

            loop {
                if time_since_last_send.elapsed().as_millis()
                    > SEND_PROGRESS_DATA_TIME_BETWEEN as u128
                {
                    ...
                    progress_sender
                        .send(progress_data)
                        .expect("Cannot send progress data");
...
pub(crate) fn send_info_and_wait_for_ending_all_threads(
    ...
    progress_thread_handle
        .join()
        .expect("Cannot join progress thread - quite fatal error, but happens rarely");
}
