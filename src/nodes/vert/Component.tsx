...
export function Component({ compId, host }: NodeComponentProps<VertCardState>) {
  ...
  function changeConversionGroup(key: string, config: VertConversionGroupConfig) { patch({ conversionGroups: { ...dataRef.current.conversionGroups, [key]: config } }) }
      ...
      for (let index = 0; index < routes.length; index += 1) {
        ...
        if (route.group.paths.length) {
          if (engine === "wasm") aggregate.errors.push(`.${route.group.extension}: 本地路径不能直接交给浏览器 Wasm，请使用“Wasm 文件”重新选择。`)
            ...
            usedCli = true
            const input: VertInput = { action, paths: route.group.paths, targetFormat: route.config.targetFormat, outputDirectory: dataRef.current.outputDirectory, engine, overwrite: dataRef.current.overwrite ?? false, quality: dataRef.current.quality ?? 90 }
...
interface SharedViewProps { dense?: boolean; data: VertCardState; browserFiles: File[]; browserOutputs: VertBrowserOutput[]; drop: ReturnType<typeof useLocalFileDrop>; engine: VertEnginePreference; paths: string[]; routes: VertConversionRoute[]; running: boolean; target: string; onBrowserPick: () => void; onChooseNative: () => void; onEngine: (value: VertEnginePreference) => void; onGroupChange: (key: string, config: VertConversionGroupConfig) => void; onRemoveBrowserFile: (file: File) => void; onRemoveGroup: (group: VertInputFileGroup) => void; onRemovePath: (path: string) => void; onReset: () => void; onRun: () => void }
function Compact(props: SharedViewProps) { return <div data-testid="vert-compact-view" className={cn("flex min-h-0 flex-1 flex-col p-2", props.dense ? "gap-1.5" : "gap-2")}><Header data={props.data} engine={props.engine} running={props.running} /><UploadPanel compact dense={props.dense} {...props} /><ConversionTopology compact dense={props.dense} routes={props.routes} running={props.running} onChange={props.onGroupChange} onRemoveFile={props.onRemoveBrowserFile} onRemoveGroup={props.onRemoveGroup} onRemovePath={props.onRemovePath} /><div className="flex items-center gap-2">{props.dense ? null : <EngineSelect value={props.engine} onChange={props.onEngine} />}<Button className="flex-1" size="sm" disabled={props.running || !props.routes.length} onClick={props.onRun}>{props.running ? <LoaderCircle data-icon="inline-start" className="animate-spin" /> : <RefreshCw data-icon="inline-start" />}转换 {props.routes.length} 组 · {props.engine === "auto" ? "CLI 优先" : props.engine.toUpperCase()}</Button></div><Progress value={props.data.progress ?? 0} label="VERT progress" /></div> }
...
function buildConversionRoutes(groups: VertInputFileGroup[], data: VertCardState): VertConversionRoute[] {
  return groups.map((group) => {
    const configured = data.conversionGroups?.[group.key]
    if (configured && compatibleCategories(group.category).includes(configured.outputCategory)) return { group, config: configured }
    const outputCategory = compatibleCategories(group.category)[0]
    return { group, config: { outputCategory, targetFormat: defaultFormat(outputCategory) } }
  })
}
