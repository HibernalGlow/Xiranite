...
impl Directories {
    ...
    pub(crate) fn set_included_directory(&mut self, included_directory: Vec<PathBuf>) -> Messages {
        ...
        if included_directory.is_empty() {
            messages
                .errors
                .push(flc!("core_missing_no_chosen_included_directory"));
            return messages;
        }
        ...
        if checked_directories.is_empty() {
            messages
                .warnings
                .push(flc!("core_included_directory_zero_valid_directories"));
            return messages;
        }
    ...
    fn canonicalize_and_clear_path(
        directory: &Path,
        is_excluded: bool,
    ) -> (Option<PathBuf>, Messages) {
    ...
    pub(crate) fn optimize_directories(&mut self, recursive_search: bool) -> Messages {
        ...
        if cfg!(target_family = "windows") {
            self.included_directories = self
                .included_directories
                .iter()
                .map(normalize_windows_path)
                .collect();
            self.excluded_directories = self
                .excluded_directories
                .iter()
                .map(normalize_windows_path)
                .collect();
            self.reference_directories = self
    ...
    pub(crate) fn filter_reference_folders<T>(
        &self,
        entries_to_check: Vec<Vec<T>>,
    ) -> Vec<(T, Vec<T>)>
    where
        T: ResultEntry,
    {
        entries_to_check
            ...
            .filter_map(|vec_file_entry| {
                let (mut files_from_referenced_folders, normal_files): (Vec<_>, Vec<_>) =
                    vec_file_entry
                        .into_iter()
                        .partition(|e| self.is_in_referenced_directory(e.get_path()));
