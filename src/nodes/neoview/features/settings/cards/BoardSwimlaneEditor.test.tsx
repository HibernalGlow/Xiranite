...
import { applyBoardDrop, applyBoardDropAt, createBoardLanes, createBoardPatch } from "./BoardSwimlaneEditor"
...
describe("BoardSwimlaneEditor layout moves", () => {
  ...
  it("does not hide required cards when their host panel is in the hidden lane", () => {
    ...
    const playlist = lanes.hidden.find((panel) => panel.id === "playlist") ?? lanes.left.find((panel) => panel.id === "playlist") ?? lanes.right.find((panel) => panel.id === "playlist")
  ...
  it("places a card after the hovered card when the landing preview says after", () => {
    ...
    expect(settings?.cards.slice(0, 3).map((card) => card.id)).toEqual(["slideshow-settings", "media-settings", "book-information"])
  })
  ...
  it("places a panel after the hovered panel when the landing preview says after", () => {
    const lanes = createBoardLanes(shellConfig())
    const moved = applyBoardDrop(lanes, "settings", "info", true)

    expect(moved.right.slice(0, 2).map((panel) => panel.id)).toEqual(["info", "settings"])
  })

  it("commits the exact final card index frozen by the landing preview", () => {
    const lanes = createBoardLanes(shellConfig())
    const moved = applyBoardDropAt(lanes, "emm-tags", { kind: "card", panelId: "properties", index: 2 })

    expect(moved.right.find((panel) => panel.id === "properties")?.cards.slice(0, 3).map((card) => card.id)).toEqual(["book-settings", "folder-ratings", "emm-tags"])
  })

  it("commits the exact final panel index frozen by the landing preview", () => {
    const lanes = createBoardLanes(shellConfig())
    const moved = applyBoardDropAt(lanes, "settings", { kind: "panel", lane: "right", index: 1 })
