...
| Phase 2：目录/归档/唯一数据链 | 进行中（主阅读链较完整） | 目录/单图、ZIP、EPUB 图片资源、non-solid/solid 7z/RAR、嵌套包、AES ZIP、loopback HTTP stream、ETag/Range/cancel、图片探测、惰性 sharp、L2/L3 cache、缩略图生成/批写/失败退避/在线维护 | RAR/7z 安全密码通道、缩略图离线维护与旧版本迁移、更多真实 EPUB/归档语料；PDF 明确 deferred |
...
嵌套 archive 不会为了形式上的流式进入 JS Buffer。AES ZIP 的嵌套密码作用域和 solid 跨 session fingerprint 复用已完成；RAR/7z 密码仍保持 secure boundary：系统 7-Zip 没有可审计的非交互 stdin 密码协议，把 `-p<password>` 放入 argv 会向同机进程暴露凭据，因此不能为了格式覆盖率降级安全。后续必须使用不暴露 argv 的 native adapter 或明确的安全 broker；历史 path stack codec 与 UI 的进入/返回导航也仍待完成。
...
`ReaderFileTreeService` 也已在 owner 内公开 current listing、back/forward navigation path 与 random seed 的逻辑 payload 快照：字符串按 UTF-8 bytes，数字/布尔按定宽字段累计，不 JSON 序列化、不遍历文件系统、不把结果冒充 V8 对象总 heap。内存压力仍保留 current listing 以保证首屏和选择身份，同时释放可重建 tree index、目录大小任务和 random seed；session close 后所有计数归零。当前 browser route/diagnostics 尚未聚合该快照，后续接线必须直接读取 owner，不能复制计数器。
...
`ReaderDiagnosticsService` 现已把上述宿主 scheduler snapshot 与 Reader 自有的 session/preload telemetry、transform singleflight、L2 字节 LRU、thumbnail coordinator、内存压力/admission、L3 `cacache`、solid archive cache 和 Node 原生进程指标聚合成 schema v1 DTO。它不创建采样线程、不自行测 CPU 百分比、不写 benchmark report、SQLite 或 TOML，也不包含 source path、HTTP token、cache key、page ID 或密码。鉴权 `GET /reader/diagnostics` 读取常驻 GUI backend 的真实状态；`xneoview diagnostics [--json]` 不带连接参数时复用同一 application service并显示独立 CLI 进程，带 `--connect <loopback-url>` 时直接读取常驻 GUI backend 的真实 session/cache/scheduler/process DTO，且不会初始化本地 Reader、SQLite、cache 或 scheduler。快照使用 `process.memoryUsage()`、`process.cpuUsage()`、`availableMemory()` 和 `constrainedMemory()`，scheduler/preload/thumbnail/cache/pressure 则直接复用各 owner 的成熟 `snapshot()`，禁止复制计数器。GPU 实际利用率、时间序列、pipeline latency、算法对照和报告导出仍待迁移，所以 performance feature 保持 `pending`。
...
当前已完成 application 计划层：`ReaderPreloadCoordinator` 复用唯一的 `buildFrameSnapshot`，按逻辑 frame 生成 near/view、ahead/ahead 和反向 background 三层候选；单双页、首尾单页、宽图单页和 RTL 不另写配对算法。ReaderSession 初始化、导航和 layout 变更各生成一次单调 `preloadGeneration`，计划包含 frame generation、方向、置信度、opaque page ID/索引和现有 `ResourcePriority`，通过 GUI HTTP DTO 与 HeadlessController 共用；它不打开 PageContent、不读取 archive、不创建队列/Worker/Buffer，也不取得 scheduler lease。`ReaderSession.updatePreloadContext()` 现接受调用方上报的 paged/continuous/scrub、pages/s 速度、稳定时长、焦点、宿主 queue wait 与 memory pressure 事实，不启动后台 timer：未聚焦、scrub、稳定不足 120 ms、连续滚动超过 4 pages/s、elevated/critical pressure 或 queue wait 超过默认 100 ms 时暂停 speculative candidates；稳定的中速连续滚动只保留 near + 1 ahead 且取消反向保留，150 ms 后的慢速状态恢复配置窗口。`deriveReaderPreloadResourceContext()` 是 GUI/CLI/TUI 共用的唯一资源快照映射：从 CPU/I/O/GPU 三池选当前最大 `oldestQueuedWaitMs`，复用现有 memory-pressure level，兼容旧 snapshot 缺字段并限制异常值；调用端不得从累计 max/average 自行推断。viewport 更新只增加 preload generation，不伪造 frame generation，后续导航沿用最后 context；默认不传 context 时保持旧行为。`ReaderPreloadTelemetry` 随 session 维护 current generation、三层候选计数、active/started/ready/failed/cancelled/evicted，以及 stale/rejected/duplicate reports；可选有界 performance metrics 累计 TTFB/decode sample、total/max、retained bytes 和 active leases，不自行计时，也不保留 page ID、URL、路径或 bytes。非法 NaN/负值/超限样本整项拒绝且不污染统计，新 plan 与 close 自动把未结束项收口为 cancelled。鉴权 `POST /reader/s/{sessionId}/preload-events` 当前每批只接受 1..64 个 `{pageId,outcome}`，旧 generation 返回 409，未知 page 返回 400；metrics 的 HTTP 解析及 React `PerformanceResourceTiming`/decode 上报仍待 Controller 接线。当前仍是 partial：HTTP/React viewport 与实时资源/性能上报，以及当前 React `Image.decode()` hook 消费 plan 尚未接入；在这些完成前，后端不得为“预热”主动吞读相邻原图。
...
- 已验证 ZIP 流式读取、背压、取消、ETag、错误恢复和 session close；EPUB 已复用 ZIP provider，完成有界 container/OPF 解析、路径安全、取消、关闭和统一 asset stream 测试；non-solid RAR/7z 已使用单 entry stdout 流，solid RAR/7z 已使用单进程顺序 materializer，并覆盖真实 CB7、CRC、预算、取消和临时目录清理；嵌套 ZIP/7z 已使用路径栈与物化 lease 打通，AES ZIP 根级/嵌套密码已使用 session scope；RAR/7z 加密、历史/UI 导航与真实 EPUB 语料回归仍需补齐，PDF 不计入当前缺口。
...
- 已增加共享 `ReaderHeadlessController`、`xneoview inspect/pages/frame/extract-page/diagnostics`、这些命令与持久 TUI Reader 工作台的 loopback `--connect` adapter；继续补真实图片协议、设置和后续 feature 命令，不复制 application 逻辑；
