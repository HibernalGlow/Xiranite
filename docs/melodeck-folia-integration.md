...
- 浮窗默认封面 tab 在曲名、歌手和专辑下方直接显示 Folia Remote Control 共用的上一首、播放/暂停、下一首和进度控制，并常驻显示其逐字滚动歌词；播放顺序按钮复用底栏同一组件与 `off -> all -> one -> random -> off` 状态机。它们只投影共享 Provider 的状态与动作，不创建第二套播放状态或第二个 `<audio>`。
