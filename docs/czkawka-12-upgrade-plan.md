...
Status: Czkawka release validation is complete. On 2026-07-28, `bun run --cwd packages/nodes/czkawka test` passed 183 package and 4 TUI compatibility tests; `bun run typecheck:app`, `bun run package:node-app czkawka`, and `bun run wails:build:incremental` all passed. The staged Czkawka manifest executes `scripts/smoke-node-app-czkawka.ts`, the generic staged verification, and the packaged EXE smoke before publishing. The verified D-drive prebuilt ZIP remains `7816a12d5529f111387d13083982c28a0bcff6236615593b348ce705ff5d1742`, matching the prebuilt and embedded manifests without a native rebuild.

The final rendered gate, `bun run test:browser -- src/nodes/czkawka/Component.browser.test.tsx`, passes all 28 scenarios. The two progress-layout mounts run as independent browser tests so React and Radix own normal unmounting; the suite does not manually remove portal DOM nodes between tests. `bun run audit:node-architecture` passes.

The full-repository `bun run check:source-size` is currently blocked by concurrent non-Czkawka files: `src/nodes/neoview/features/panels/cards/folder/FolderBrowserPane.tsx` (1433 lines), `src/nodes/neoview/app/ReaderApp.tsx` (1009 lines), and existing XLchemy size debt. This is not treated as a green Czkawka gate. Every Czkawka and build-script file touched by this upgrade remains below 300 lines; the repository-wide size gate must be resolved by the owning work before a whole-worktree release claim.
