...
impl BadExtensions {
    ...
    pub(crate) fn look_for_bad_extensions_files(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
        ...
        self.bad_extensions_files = self.verify_extensions(
            files_to_check,
    ...
    fn verify_extension_of_file(
        ...
        let (mut all_available_extensions, valid_extensions) =
            Self::check_for_all_extensions_that_file_can_use(
                hashmap_workarounds,
                &current_extension,
                proper_extension,
            );
