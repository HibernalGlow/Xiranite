...
describe("reader-http-client", () => {
  it("[neoview.react.control] sends token-authenticated open, navigation and close requests", async () => {
    ...
    expect(await client.updateFolderView!({ folderView: { viewMode: "details", previewCount: 9, details: { columnWidths: { name: 300 } }, search: { includeSubfolders: false, searchInPath: true } } })).toMatchObject({ viewMode: "compact", previewCount: 4 })
    ...
    expect(JSON.parse(String(fetchMock.mock.calls[5]?.[1]?.body))).toEqual({ folderView: { viewMode: "details", previewCount: 9, details: { columnWidths: { name: 300 } }, search: { includeSubfolders: false, searchInPath: true } } })
