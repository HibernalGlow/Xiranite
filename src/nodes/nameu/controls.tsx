...
import { Archive, Clipboard, DatabaseZap, Eye, Info, Settings2 } from "lucide-react"
...
import { ACTIONS, type NameuAction } from "./constants"
...
export function ActionPicker(props: {
  ...
  onActionChange: (action: NameuAction) => void
}) {
  return (
    <ToggleGroup
      aria-label="nameu action"
      ...
      variant="outline"
      onValueChange={(value) => {
        if (value) props.onActionChange(value as NameuAction)
      }}
    >
      {ACTIONS.map((item) => (
    ...
    <div className="flex min-w-0 flex-col gap-1.5">
      {!props.compact && (
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="nameu-paths" className="flex items-center gap-1.5">
      ...
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-1.5">
        {props.compact ? (
          <Input
            id="nameu-paths"
            aria-label="nameu 归档目录"
            disabled={props.disabled}
            className="font-mono text-xs"
            placeholder="每行一个归档目录路径"
...
export function PathFields(props: {
  ...
  return (
    <div className="grid gap-2 @3xl/nameu:grid-cols-2">
      <Input
        aria-label="nameu 配置文件"
        disabled={props.disabled}
        placeholder="配置文件，可选"
  ...
  return (
    <div className="grid gap-2 @3xl/nameu:grid-cols-2">
      <SwitchRow
        ...
        icon={Eye}
        label="预演"
        description="只生成重命名计划，不真正调用 NameU 模块。"
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="nameu 运行选项" disabled={props.disabled} size="icon-sm" variant="outline">
              <Settings2 />
  ...
  return (
    <Popover>
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="nameu 默认配置" disabled={props.disabled} size="icon-sm" variant={props.configDirty ? "secondary" : "outline"}>
              <DatabaseZap />
      ...
      <PopoverContent align="end" className="w-72">
        <div className="mb-3">
          ...
          <p className="text-xs text-muted-foreground">保存 NameU 的路径和运行选项。</p>
        </div>
        <div className="grid gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-xl">
              <DialogHeader>
                <DialogTitle>NameU 配置</DialogTitle>
                <DialogDescription>当前 nodes.nameu 默认值和配置文件位置。</DialogDescription>
              </DialogHeader>
