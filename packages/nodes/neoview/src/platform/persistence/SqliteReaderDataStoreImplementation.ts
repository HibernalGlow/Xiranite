...
import type { ReaderPlaylistEntryRecord, ReaderPlaylistRecord, ReaderPlaylistStore } from "../../ports/ReaderPlaylistStore.js"
...
export class SqliteReaderDataStore
  implements
    ReaderDataStore,
    ReaderPlaylistStore,
    ...
    ReaderEmmOverrideStore
{
  ...
  async importBookSettings(
    ...
    const normalized = records.map((record) => {
      ...
      return {
        ...
        overrides: BookSettingsOverridesSchema.parse(record.overrides),
      }
    })
    return this.#transaction(() => {
      const result: ReaderBookSettingsImportResult = {
        inserted: 0,
        updated: 0,
        unchanged: 0,
      }
      for (const record of normalized) {
        ...
        const overrides = strategy === "merge" ? { ...record.overrides, ...current?.overrides } : record.overrides
  ...
  async deleteOldestRecent(limit: number): Promise<{ selectedIds: readonly string[]; deleted: number }> {
    ...
    return this.#transaction(() => {
      const selectedIds = this.database
        .all(
          `SELECT book_id FROM xr_reader_progress
      ...
      const deleted = this.database.run("DELETE FROM xr_reader_progress WHERE book_id IN (SELECT value FROM json_each(?1))", encodedIds).changes
  ...
  async clearRecentBefore(timestamp: number, limit: number): Promise<number> {
    ...
    return this.#transaction(() => {
      ...
      this.database.run("DELETE FROM xr_reader_path_stacks WHERE book_id IN (SELECT value FROM json_each(?1))", encodedIds)
      this.database.run("DELETE FROM xr_reader_media_progress WHERE book_id IN (SELECT value FROM json_each(?1))", encodedIds)
      return this.database.run("DELETE FROM xr_reader_progress WHERE book_id IN (SELECT value FROM json_each(?1))", encodedIds).changes
    })
  }

  async clearByPathPrefix(collection: ReaderLibraryCollection, normalizedPrefix: string): Promise<number> {
    this.#assertOpen()
    if ((collection !== "recents" && collection !== "bookmarks") || !normalizedPrefix || normalizedPrefix.length > 32_768 || normalizedPrefix.includes("\0")) {
    ...
    return this.#transaction(() => {
      ...
      return this.database.run(`DELETE FROM xr_reader_bookmarks WHERE ${matchesPrefix}`, normalizedPrefix).changes
    })
  }
  ...
  async findBookmarkByPath(path: string): Promise<ReaderBookmarkRecord | undefined> {
    ...
    const memberships = this.database
      .all("SELECT list_id FROM xr_reader_bookmark_memberships WHERE bookmark_id = ?1 ORDER BY list_id ASC", id)
  ...
  async upsertBookmark(bookmark: ReaderBookmarkRecord): Promise<void> {
    ...
    await this.#transaction(() => {
      ...
      for (const listId of bookmark.listIds) {
        if (listId === "default") {
          this.database.run("INSERT OR IGNORE INTO xr_reader_bookmark_memberships (bookmark_id, list_id) VALUES (?1, 'default')", bookmark.id)
        } else {
          this.database.run(
  ...
  async updateBookmark(id: string, update: ReaderBookmarkUpdate): Promise<ReaderBookmarkRecord | undefined> {
    ...
    return this.#transaction(() => {
      ...
      if (update.listIds !== undefined) {
        ...
        for (const listId of update.listIds) {
          this.database.run("INSERT INTO xr_reader_bookmark_memberships (bookmark_id, list_id) VALUES (?1, ?2)", id, listId)
        }
      }
      this.database.run(
  ...
  async deleteOldestBookmark(limit: number): Promise<{ selectedIds: readonly string[]; deleted: number }> {
    ...
    return this.#transaction(() => {
      ...
      if (!selectedIds.length) return { selectedIds, deleted: 0 }
      const encodedIds = JSON.stringify(selectedIds)
      this.database.run("DELETE FROM xr_reader_bookmark_memberships WHERE bookmark_id IN (SELECT value FROM json_each(?1))", encodedIds)
      const deleted = this.database.run("DELETE FROM xr_reader_bookmarks WHERE id IN (SELECT value FROM json_each(?1))", encodedIds).changes
      return { selectedIds, deleted }
    })
  }
  ...
  async clearBookmarkBefore(timestamp: number, limit: number): Promise<number> {
    ...
    return this.#transaction(() => {
      ...
      const encodedIds = JSON.stringify(selectedIds)
      this.database.run("DELETE FROM xr_reader_bookmark_memberships WHERE bookmark_id IN (SELECT value FROM json_each(?1))", encodedIds)
      return this.database.run("DELETE FROM xr_reader_bookmarks WHERE id IN (SELECT value FROM json_each(?1))", encodedIds).changes
    })
  }
  ...
  async getPlaylist(id: string): Promise<ReaderPlaylistRecord | undefined> {
    this.#assertOpen()
    return parsePlaylist(this.database.get(`SELECT id, name, created_at, updated_at FROM xr_reader_playlists WHERE id = ?1`, id))
  }
  ...
  async appendPlaylistEntries(playlistId: string, entries: readonly ReaderPlaylistEntryRecord[], updatedAt: number): Promise<void> {
    ...
    await this.#transaction(() => {
      ...
      const countRow = this.database.get("SELECT COUNT(*) AS count FROM xr_reader_playlist_entries WHERE playlist_id = ?1", playlistId)
  ...
  async replacePlaylistEntryOrder(playlistId: string, entryIds: readonly string[], updatedAt: number): Promise<void> {
    ...
    await this.#transaction(() => {
      const current = this.database
        .all("SELECT id FROM xr_reader_playlist_entries WHERE playlist_id = ?1 ORDER BY position ASC, id ASC", playlistId)
      ...
      for (const [position, id] of entryIds.entries()) {
        this.database.run("UPDATE xr_reader_playlist_entries SET position = ?3 WHERE playlist_id = ?1 AND id = ?2", playlistId, id, position)
      }
      this.database.run("UPDATE xr_reader_playlists SET updated_at = ?2 WHERE id = ?1", playlistId, updatedAt)
    })
  }
  ...
  async setFolderRule(pathKey: string, path: string, sort: ReaderDirectorySortRule, updatedAt: number): Promise<void> {
    this.#assertOpen()
  ...
  async clearFolderRules(pathKey?: string): Promise<number> {
    ...
    return this.#write(() =>
      pathKey
        ? this.database.run("DELETE FROM xr_reader_folder_sort_rules WHERE path_key = ?1", requireSortIdentity(pathKey, "folder sort path key")).changes
  ...
  async readDirectoryEmmRecords(paths: readonly string[], signal?: AbortSignal): Promise<ReadonlyMap<string, ReaderDirectoryEmmRecord>> {
    this.#assertOpen()
  ...
  async recordSearchHistory(record: Omit<ReaderSearchHistoryRecord, "useCount">, maximumEntries: number): Promise<ReaderSearchHistoryRecord> {
    this.#assertOpen()
  ...
  deleteSearchHistory(scope: string, query: string): Promise<boolean> {
    ...
    return this.#write(() => this.database.run("DELETE FROM xr_reader_search_history WHERE scope_id = ?1 AND query = ?2", scope, query).changes > 0)
  }

  clearSearchHistory(scope: string): Promise<number> {
    this.#assertOpen()
    assertSearchHistoryScope(scope)
    return this.#write(() => this.database.run("DELETE FROM xr_reader_search_history WHERE scope_id = ?1", scope).changes)
  }
  ...
  removeFileUndoTransaction(id: string): Promise<boolean> {
    ...
    return this.#write(() => this.database.run("DELETE FROM xr_reader_file_undo_transactions WHERE id = ?1", id).changes > 0)
  }
...
function bookmarkListCondition(listId: string | undefined): {
  sql: string
  bindings: string[]
} {
...
function parseDirectorySort(row: Record<string, unknown> | undefined): ReaderDirectorySortRule | undefined {
  ...
  return {
    field,
    order,
    directoriesFirst: directoriesFirst === 1 || directoriesFirst === 1n,
  }
}
...
function mergeEmmRecord(legacy: ReaderDirectoryEmmRecord | undefined, override: ReaderEmmOverrideRecord | undefined): ReaderDirectoryEmmRecord {
...
function librarySearchPredicate(
...
const BookmarkBatchStoreUpdateSchema = z
  .array(
    z
      .object({
        id: z
      ...
      .strict()
      .refine((update) => update.starred !== undefined || update.listIds !== undefined, {
...
function parseBookSettings(row: Record<string, unknown>): ReaderBookSettingsRecord {
  ...
  if (row.reading_direction !== null && row.reading_direction !== undefined) {
    if (row.reading_direction !== "left-to-right" && row.reading_direction !== "right-to-left")
      throw new Error("Stored reader book settings direction is invalid.")
  ...
  if (row.horizontal_book !== null && row.horizontal_book !== undefined)
    overrides.horizontalBook = requireBooleanInteger(row.horizontal_book, "book settings horizontal book")
  return {
