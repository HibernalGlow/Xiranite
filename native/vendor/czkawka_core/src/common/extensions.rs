...
impl Extensions {
    ...
    pub(crate) fn filter_extensions(mut file_extensions: String) -> (HashSet<String>, Messages) {
        ...
        file_extensions = file_extensions.replace(
            "VIDEO",
            "mp4,flv,mkv,webm,vob,ogv,gifv,avi,mov,wmv,mpg,m4v,m4p,mpeg,3gp",
        );
        ...
        for mut extension in extensions {
            ...
            if extension.contains('.') {
                messages.warnings.push(format!(
                    "{extension} is not valid extension because contains dot inside"
                ));
                continue;
            }
            ...
            if extension.contains(' ') {
                messages.warnings.push(format!(
                    "{extension} is not valid extension because contains empty space inside"
                ));
                continue;
            }
    ...
    pub(crate) fn check_if_entry_have_valid_extension(&self, entry_data: &DirEntry) -> bool {
        ...
        if !self.allowed_extensions_hashset.is_empty() {
            if extension.chars().all(|c| c.is_ascii_lowercase()) {
                ...
                self.allowed_extensions_hashset
    ...
    fn extend_allowed_extensions(&mut self, file_extensions: &[&str]) {
        for extension in file_extensions {
            ...
            self.allowed_extensions_hashset
                .insert(extension_without_dot.to_string());
        }
    }
    ...
    fn union_allowed_extensions(&mut self, file_extensions: &[&str]) {
        ...
        for extension in file_extensions {
            ...
            if !self
                ...
                .contains(extension_without_dot)
            {
