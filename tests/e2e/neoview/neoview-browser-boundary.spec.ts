...
test("[neoview.browser-boundary] loads the workspace module entry without Node runtime modules", async ({ page }) => {
  ...
  page.on("request", (request) => modulePaths.push(decodeURIComponent(new URL(request.url()).pathname)))
  ...
  expect(modulePaths.filter((path) => /(?:__vite-browser-external|node:(?:path|crypto)|\/dist\/(?:index|core|ui-core)\.js|\/src\/core\.ts|ReaderDirectorySort|ReaderFileTree(?:Service|Index))/i.test(path))).toEqual([])
