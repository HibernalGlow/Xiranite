...
旧版 `src/lib/cards/registry.ts` 中的 77 张 Card 已冻结到 `migration/neoview/card-compatibility.json`，并由 `bun run audit:neoview-cards` 校验 source hash、逐项 feature 映射、当前 manifest 对应关系和六维聚合；发布阶段使用 `--require-complete` 禁止任何 Card 留有未完成的适用维度。当前 Card 数量、派生总体及 `core/transport/gui/cli/tui/evidence` 分布只以生成的 `docs/neoview-card-functional-checklist.md` 和审计输出为准，本文不再手工复制实时数字。实现顺序明确以核心阅读 Card 为先：`folderMain`、`pageListMain`、`bookInfo/imageInfo/storage/time`、`historyList`、`bookmarkList`、`preloadStatus`，slideshow overlay、动画和其余设置 Card 延后。
...
`folderMain` 的目录、排序、搜索、过滤、监听和树节点后端纵切已经收口到 GUI/CLI/TUI 可复用的唯一 `ReaderFileTreeService`，旧 `CoreReaderDirectoryBrowser` 已删除。单层浏览使用可取消的原生 `opendir()`；八字段排序与偏好在分页前统一处理；`all/archive/directory/video` 类型过滤同样在稳定 listing 分页前执行，filter 是 browser session 状态并在 clone、reopen、前后导航和目录切换中保留。切换 filter 只增加 generation 并重新切片内存快照，不重新 `opendir/stat`、不复制第二份 catalog；archive 分类复用唯一 `platformReaderBookFileKind`，video 分类复用同一 TOML media registry，因此自定义视频扩展在 GUI 与 Headless 结果一致。鉴权 `PATCH /reader/browser/s/{id}/filter` 使用 Zod 严格命令，响应携带 filter/filterOptions/过滤后 total；同一 session 的递归搜索自动继承 filter，先用 filter 收紧 scanner 的 file/directory 输出，再在已有 AsyncIterable 中对 archive/video 应用同一 classifier，NDJSON meta 回显 filter，未复制 scanner、队列或取消逻辑。History/Bookmark 虚拟源现在也通过同一领域 filter 在原 NeoView SQLite 的 `LIMIT/OFFSET` 前筛选，EPUB 归入 archive；书签列表 membership 与类型条件使用括号化 predicate 组合，避免 `OR` 优先级让星标项绕过筛选。HTTP、Headless、`folder-search`、library CLI 和两个 OpenTUI 工作台均复用该契约；只剩 TypeFilterBar GUI 与虚拟源前端接线。递归搜索使用唯一 `readdirp` stream adapter，并把 glob 与 gitignore 语义分别交给 `picomatch` 和 `ignore`。NDJSON 每次 Web Stream pull 只推进一个事件，consumer cancel、Request abort、session DELETE 和 service dispose 都关闭同一搜索 handle；scanner 全程持有宿主 `reader.file-tree.scan` I/O lease。`@parcel/watcher` 只在显式 `watch:true` 的 session 动态加载，并使目录快照及受影响树节点缓存失效。持久排除规则已经统一应用到树与搜索，请求级 exclusion 仍只影响单次搜索。

GUI 关闭“包含子目录”时继续调用同一搜索端点并显式发送 `depth=0`；`ReaderFileTreeService` 此时通过 `ReaderDirectoryListingScanner` 把当前 session 已排序的稳定 listing 适配到既有 `ReaderFileTreeScanner` port，不再启动 `readdirp` 或重复访问磁盘。text/glob 匹配、`ignore` 排除、类型 filter、generation meta、结果上限、NDJSON 背压和取消契约仍由同一搜索应用层执行；只有递归搜索才取得平台 scanner 和宿主 I/O lease。该优化不新增配置项、不创建数据库表，也不改变 GUI/CLI/TUI 契约。
...
排序偏好继续执行“临时规则 > 文件夹记忆 > 标签默认 > 全局默认”：全局/标签默认和最多 1000 项文件夹记忆写入原 NeoView Reader SQLite 的独立 `xr_reader_*` 表，临时规则只存在 browser session。搜索历史的 `folder/file/bookmark/history` 四个 scope 统一由 `ReaderSearchHistoryService` 管理，相同 query 去重置顶、累计使用次数且每 scope 默认最多 20 项；运行数据非破坏性写入原 `%APPDATA%/NeoView/thumbnails.db` 的 `xr_reader_search_history`，不修改旧表、旧索引、`metadata.version`、`user_version` 或 journal mode。HTTP 与 Headless/CLI/TUI 复用这一 service，独立文件树控制器仅在首次历史操作时打开 SQLite；`LegacySearchHistoryCodec` 使用 `zod` 兼容完整导出和四个 raw localStorage key，`search-history-inspect/import` 提供显式确认的 merge/overwrite 导入。GUI 历史下拉仍待完成。React `FolderMainCard` 保持独立 lazy chunk 和有界稀疏 catalog。整体仍为 `partial`：树 GUI、搜索历史 GUI、收藏/随机标签交互、穿透 single/all、其余 CLI/TUI feature 命令、完整多选/键盘导航、上下文文件操作、目录大小结果的前端重排/滚动锚点和原版视觉验收仍待迁移。

EMM 标签搜索后端纵切已接入现有搜索主链：`tag`/`excludeTag` 支持最多 64 个规范化标签，`tagMode=all|any` 明确组合语义，允许 tag-only 空文本查询。`ReaderMetadataHydratingScanner` 作为现有 scanner port 的 decorator，以 128 项有界批次调用唯一 `ReaderDirectoryMetadataProvider`；当前目录复用 session listing，递归搜索继续复用 `readdirp`，旧库读取继续由 `SqliteReaderDataStore.readDirectoryEmmRecords()` 的 256-key 查询承担。它不新增 Reader 表、不建立第二套标签索引、不逐文件查询 SQLite，并保留 generation、NDJSON 背压、maximumEntries 与取消链。`folder-search` CLI 已支持重复 `--tag`/`--exclude-tag`、`--tag-mode=all|any` 和 tag-only 查询；共享 OpenTUI 使用相同结构化参数。旧组合修饰键 GUI 仍需在同一契约上接线。

收藏/随机标签 suggestion 也已收口到共享 application port。`SqliteReaderDataStore.sampleEmmTags()` 复用 SQLite JSON1：先按旧版行为从 `thumbs.emm_json` 随机取最多 50 个有效 JSON 行，再只对该有界集合执行 `json_each`、大小写去重和结果限量；不会把 JSON BLOB 批量拉入 JS，也不创建旧表索引或第二张 tag catalog。`ReaderEmmTagSuggestionService` 并行读取这个 catalog 与唯一 `PlatformEmmCollectTagSource`，使用可测试的无放回采样最多混入 3 个收藏标签，最终通过 `GET /reader/browser/emm-tags/suggestions?count=1..32` 返回不含路径的结构化结果。真实 `%APPDATA%/NeoView/thumbnails.db` 只读探针为 13,959 行、当前 `emm_json` 非空 0 行；查询热态 43～50 ms，普通随机标签为空并正确降级到收藏标签，不写数据库或失败黑名单。

标签翻译由独立 `PlatformEmmTranslationSource` 负责，而不是在 route 或 React 中加载整份字典。它按旧版顺序惰性发现 `portable`、`%APPDATA%`、`%LOCALAPPDATA%` 的 `db.text.json`，限制为 8 MiB，支持完整/缩写命名空间，按 mtime 缓存解析结果并只返回本次最多 32 个建议所需的 `translatedTag`；坏文件或缺失键降级为原 tag。Reader memory pressure、route dispose 都清空字典。真实 6,130,776-byte 字典首次解析/查两个 tag 为 42.6 ms、热缓存 1.6 ms、清理后重载 26.9 ms，首次 heap 增量约 10.5 MiB，因此禁止在 XR 启动时预加载或把完整字典传给 GUI。`folder-emm-tags` CLI 与 OpenTUI suggestion action 均复用该惰性翻译链；完整 GUI 仍待完成。

本地 `createReaderFileTreeController()` 现已与 GUI HTTP composition 复用同一 EMM application/port，而不是维护一套 Headless 搜索：`LazyReaderDirectoryMetadataProvider` 只在首次需要 date/size/EMM hydration 或 tag-only 搜索时加载真实 provider，失败会释放 singleflight 以允许重试；`LazyReaderDataStoreResource` 让 metadata、`ReaderSearchHistoryService` 和 `ReaderEmmTagSuggestionService` 共用一个惰性 `SqliteReaderDataStore` 并在 controller dispose 时 exactly-once 关闭。普通打开/分页不会触碰 SQLite；`legacyThumbnailDatabasePath=false` 时仍保留普通文件浏览和 stat metadata，但 tag 搜索/建议明确报告 unavailable。Headless facade 已暴露 `suggestEmmTags()`，`xneoview folder-emm-tags` 已复用该 controller 完成参数与文本/JSON 呈现接线；OpenTUI 和完整 GUI suggestion surface 仍待完成，不复制 JSON1、收藏混入或翻译逻辑。
...
权威机器清单为 `migration/neoview/folder-main-compatibility.json`，固定包含 74 个源码级能力项和 19 组逐控件 UI 库存。实时的派生总体、六维分布及阻塞项只由 `bun run generate:neoview-card-checklist` 输出到 `docs/neoview-card-functional-checklist.md`，本文不再复制易漂移的状态数字。每个源码控件库存组必须提供旧文件证据并映射到一个或多个验收 ID；每个验收项固定包含 `sourceEvidence`、`targetContract`、`surfaces`、`capabilityStatus`、`testIds`、`plannedTestIds` 和 `notes`。`capabilityStatus` 固定为 `core/transport/gui/cli/tui/evidence` 六维，overall 仅由适用维度派生；未来证据必须放入 `plannedTestIds`，不能冒充已经存在的测试。本文表格只是索引，完整控件名单由生成文档展示；发生冲突时以旧源码证据、schema v2 JSON 契约和审计结果为准。
...
高级清理剩余的 Bookmark oldest/date 与两类 clear-all 也已进入相同边界：oldest 以 `created_at ASC, id ASC` 稳定选择最多 500 项，date 以旧 `createdAt < cutoff` 语义有界删除，clear-all 必须由 HTTP 显式 `confirmed=true`。Bookmark membership 与主体原子删除但保留自定义列表定义；History 的单项、批量、oldest、date、folder 和 all 六条删除路径统一在同一事务中清除 `xr_reader_progress`、`xr_reader_media_progress` 与 `xr_reader_path_stacks`，保留独立书籍设置和书签，避免历史已清理但重开仍恢复媒体位置或嵌套路径。
...
跨书导航后端已从旧 `bookStore -> FolderPanel UI cache -> browseDirectory` 链收口为共享 `ReaderAdjacentBookService`。它以当前书籍父目录为范围，复用 `PlatformDirectoryListingProvider`、`PlatformDirectoryMetadataProvider` 和唯一 `sortReaderDirectoryEntries`，按当前 `name/date/size/type/random/rating/path/collectTagCount` 规则只 hydrate 必需字段；候选保持旧行为的“文件夹、归档、视频”，单图模式不切书，PDF deferred 不进入候选。找不到当前项时 next 取第一项、previous 取末项，越界返回无候选且不循环。鉴权 `POST /reader/s/{id}/adjacent-book` 先完整打开 replacement 并建立 frame，成功后才释放旧 session；解析、解密或打开失败保留旧 session。local headless 与 `--connect` remote headless 暴露同一 `openAdjacent()`，不复制扫描和排序。headless composition 同时修正为读取完整 `[nodes.neoview]` runtime config，并与 GUI 共用动态图片/视频格式 registry；这项能力不新增数据库表、不写 `xiranite.db` 或 `thumbnails.db`。GUI 的 next/previous book 按钮、尾页 `next-book` 自动触发、切换提示和 FolderMain 焦点同步仍由前端任务接线，因此五种尾页行为尚不能标记 complete。
...
| 完整阅读工具与设置 UI | 设置浮窗、共享设置 Card 与首批视图/幻灯片工具可用 | 路径打开、前后/页码/关闭；fit mode 菜单、单双页 segmented control、缩放、旋转、reset 及 `+/-/R/0` 快捷键；fit/page mode 默认值写入 Xiranite TOML 并立即作用于当前视图与同进程后续书籍；工具栏、设置窗与停靠 Card 共用单一串行写队列，按服务端确认快照处理失败回滚；共享 TS `ReaderSlideshow` 以单个 deadline timeout 提供播放/暂停、1～60 秒间隔、循环、随机、剩余时间与慢导航串行化；`interval/loop/random/fadeTransition` 兼容旧 `reader.slideshow` 与 `book.autoPageTurnInterval`，统一规范化到 `[nodes.neoview.slideshow]`，工具栏经同一后端/前端串行写队列持久化并按确认快照回滚；按 `setting.png` 建立独立设置浮窗，“视图默认值”与合并后的“布局”Card 由同一 registry 按 section 二级动态加载，布局 Card 以左/右/隐藏三泳道统一管理 Panel 与 Card，也可显式停靠到侧栏；TUI 复用共享 slideshow runtime，并通过本地 TOML 或远端 HTTP 持久化；旧版未消费的 `fadeTransition` 在 XR 中定义为 decode 提交后的 180ms compositor fade | slideshow 完整 overlay、原 TopToolbar 其余功能、阅读方向/尾页等尚无严格写入契约的视图设置、其余设置分类业务卡、文件浏览、历史/书签、超分、AI、benchmark、完整快捷操作 |
...
- `folder-main`、`history-list`、`bookmark-list` 在共享 Card manifest 中显式声明 `exclusivePanel`：布局泳道同时在 Card 与所属 Panel 标注“独占面板”，可见状态下禁止与其他 Card 共用 Panel；Reader 仅在声明的独占 Card 确实单独占用 Panel 时启用全尺寸无框渲染，不能再以“当前只有一张 Card”推断；三张 Card 的根节点、内容壳与虚拟列表视口必须保持 `min-h-0 + flex-1` 高度链，不得以固定列表高度截断独占面板；
...
最小 React 纵切已接入 `src/nodes/neoview` 的 app/feature/adapter 边界：节点 `entry.ts` 与 `Component.tsx` 保持薄，`ReaderApp` 只持有 session/frame/交互小状态，HTTP client adapter 负责控制面；主显示链使用原始 asset URL 的 DOM `<img decoding="async">`，相邻页只通过浏览器 `Image.decode()` 做有界预解码，不引入 Svelte runtime、Canvas、Blob、Base64 或默认 sharp 转换。缩略图列表按 64 页批次拉取并虚拟化，1000 页时只挂载可见窗口。真实 CBZ Playwright E2E 会启动独立 loopback backend，在桌面与 `420×360` 卡片视口中验证图片解码、热翻页、有限缩略图 DOM、零 Canvas 和关闭后 session 释放。鉴权 `POST /reader/s/{sessionId}/reload` 现提供 GUI 可直接消费的原子替换控制面：先完整打开 replacement session，再按原 page source identity 恢复锚点，最后释放旧 session 的进度、临时物化、页面信息、asset retention、metadata load 和书籍资源；打开、定位或取消失败时关闭 replacement 并保持旧 session/画面有效。加密包仍只接受现有 password DTO，不在后端持久保存密码。

GUI 的源变化控制面使用鉴权 `GET /reader/s/{sessionId}/source-changes?after=<revision>`。首次请求才懒加载现有 `@parcel/watcher` native adapter；目录会话监听书籍根目录，单图、视频和 archive 会话监听父目录但只接收目标文件事件。application service 将原生批次收口为单调 revision、变化种类和计数，不返回路径、文件名或 watcher 错误细节；25 秒无变化返回 `204`，请求取消只移除该 waiter，不关闭同一会话共享监听。close、reload 和 controller dispose 都会等待 unsubscribe，避免 Reader 关闭后残留 native watcher。GUI 仍需订阅该端点并决定“提示用户”或“自动调用 reload”的交互策略，因此打开/关闭/reload feature 继续保持 `pending`；后端不得在 watcher callback 中直接重载，以免用户编辑文件时频繁替换正在阅读的 session。
...
- 小 entry 现在通过 `SolidArchiveCache` 共享的现有 `lru-cache` 做 host-level 有界内存 tier：默认总预算 32 MiB、单 entry 默认 8 MiB；首次从已校验 temp file 读取，后续请求直接返回内存流，超出预算或需要路径的嵌套场景透明回退到 temp file。key 使用 archive fingerprint + entry id，源失效只清理对应前缀，caller 取消不会中止共享读取，关闭/压力回收会清空或 trim tier；不增加 extractor、CAS 或第二个 SQLite。
- 同一宿主 cache owner 现在还共享 7-Zip L0 index tier：最多 32 个已验证 descriptor，key 包含 canonical source revision、7-Zip executable path/version 和 listing byte limit；并发 provider 只启动一次 `7z l -slt`，返回 descriptor 副本避免跨 provider mutation。密码归档不进入该缓存，source revision 改变自然换 key，cache close 清理 index；ZIP 仍由 zip.js provider 自己持有 reader/index。

`ArchiveProvider.updatePreloadDemand()` 现在提供了明确的 provider-demand 契约：需求携带 generation、阅读方向、方向置信度和 provider-local entry IDs，`SevenZipArchiveProvider` 只按这份显式需求驱动 solid materializer。materializer 在已验证 entry 边界停止不需要的后半段；新的 generation 请求更远 entry 时复用同一个 materializer 实例并从新的顺序 extractor 继续，`openEntry` 只提升硬性可见 entry 目标，不推断方向。共享 materializer 对 provider scope 分别维护 demand，避免一个 session 的 generation 覆盖另一个 session。`ReaderArchivePreloadDemandBridge` 已接入 session frame 生命周期，但只有客户端对当前 generation 明确上报 `started` 的 candidate 才转成 archive demand；规划本身不会主动吞读相邻原图。该路径继续使用现有 `SolidArchiveCache`、内存/磁盘层和唯一资源 lease，不引入第二条解压主链。

`bun run benchmark:neoview-sevenzip` 会生成 3 页、48 MiB 的固定 solid CB7，并分别测量“首条后相邻页”“直接请求末页”和“关闭后跨 session 命中”。2026-07-18 本机独占运行基线（Bun 1.4.0、Windows x64、系统 7-Zip 26.02、临时文件系统、OS cache 状态未指定）为：索引 `98.06 ms`，首个 entry-ready `116.57 ms`、首块 `1.82 ms`，随后相邻页 entry-ready `0.47 ms`、首块 `2.22 ms`；新 provider 直接请求末页 entry-ready `146.70 ms`、首块 `0.52 ms`；跨 session 场景冷请求末页 `148.79 ms`，第二个 provider warm 命中第一页 `0.51 ms`，两个 session 合计只有一个 solid extractor CPU lease，并保留 `50,331,648` bytes。样本 SHA-256 为 `ad642359be259fdb7a1bfa9cc8a82197054573cd3890c176d52db87cd8136ee9`，archive SHA-256 为 `e33e75c3ff923c7b53d7e811e66034b688cc08d57c64e1375c773e74492f5eed`；关闭宿主 cache 后临时目录残留为零。脚本禁用 archive 时间戳以保持 fixture hash 可重复，并输出运行时、7-Zip 路径/版本、样本与 archive SHA-256、payload/archive 大小和全部调度请求；这些数值是开发机回归基线，不是跨设备性能承诺，基准不得与测试或构建并行执行。
...
`ReaderDiagnosticsService` 现已把上述宿主 scheduler snapshot 与 Reader 自有的 session/preload telemetry、archive provider runtime resources、FileTree listing/navigation/random-seed 逻辑 payload、transform singleflight、L2 字节 LRU、thumbnail coordinator、内存压力/admission、L3 `cacache`、solid archive cache、composition-owned ffmpeg/ffprobe process pool 和 Node 原生进程指标聚合成 schema v1 DTO。archive/browser/video 数据直接读取各 owner 的无副作用快照，会话关闭后归零，不为诊断重扫目录或压缩包。Controller 默认创建并关闭自己的单槽视频 process scheduler；注入的 scheduler 由调用方继续拥有，ffmpeg 与 ffprobe 默认 provider 共享同一实例。它不创建采样线程、不自行测 CPU 百分比、不写 benchmark report、SQLite 或 TOML，也不包含 source path、HTTP token、cache key、page ID 或密码。跨进程消费统一经过 core 导出的 `ReaderDiagnosticsWireSchema` / `parseReaderDiagnosticsSnapshot()`：复用现有 Zod，对 CLI/TUI 实际读取的嵌套计数、字节、耗时、scheduler、cache 和 pressure 字段做 finite/non-negative 验证；所有对象 passthrough 保留未来字段，旧 v1 缺少新增可选 `cache`、solid lease 或 video process 字段仍兼容，畸形嵌套值以统一错误拒绝且不回显 payload。鉴权 `GET /reader/diagnostics` 读取常驻 GUI backend 的真实状态；`GET /reader/diagnostics/history/export?format=json|csv` 复用同一 bounded history 输出无路径、no-store 的诊断报告，CSV 既保留常用数值列也保留各嵌套 DTO 的 JSON 列和 `droppedSamples`，不会因扁平化丢失证据。远程 headless adapter 通过 `fetchRemoteReaderDiagnosticsHistory()` 以同一 loopback/token/schema 获取有界 JSON history，不打开 Reader session；`xneoview diagnostics-history-export --connect <loopback-url> [--format json|csv] [--since-ms N] [--limit N] [--output path|-]` 直接复用该 adapter 和同一 serializer，不初始化本地 Reader、SQLite、cache 或 scheduler。`xneoview diagnostics [--json]` 仍只输出单次 snapshot；TUI 的 history 导出入口尚待接线。快照使用 `process.memoryUsage()`、`process.cpuUsage()`、`availableMemory()` 和 `constrainedMemory()`，scheduler/preload/thumbnail/cache/pressure/video process 则直接复用各 owner 的成熟 `snapshot()`，禁止复制计数器。React Storage Card 当前仍消费自己的窄 diagnostics DTO，切换到共享 parser 后才能声明 GUI/CLI/TUI wire 校验完全统一。GPU 实际利用率、自动时间序列采样、pipeline latency 和算法对照仍待迁移，所以 performance feature 保持 `pending`。

诊断历史的 OpenTUI 入口现已接通：`xneoview diagnostics-history-ui --connect <loopback-url>` 复用同一远程 adapter 和 serializer，在终端中查看 JSON 或 CSV 的有界历史，默认 100、最多 1,000 条样本；它不初始化本地 Reader、SQLite、cache 或 scheduler。
...
截至 2026-07-19，`HibernalGlow/opencomic-ai-bin` 的 system-only fork 已形成五个隔离提交：`b86caac` 将包收口为 `@hibernalglow/opencomic-ai-system@0.1.0`，增加宿主 `binaryResolver`，移除生产入口的 YOLO/ONNX/Potrace 和包内 `win/linux/mac` 路径，并通过 `package.json#files` 与 pack 审计禁止发布 exe、动态库、原生 `.node`、ONNX、模型 `.bin/.param` 和 Python；`610e34b` 修复 daemon 参数拆分、跨 chunk `Ready>`、`Error -> Ready>` 成功误判、Windows/Unicode/引号 quoting，以及 process error/exit/close、主动 close 时当前和排队任务悬空；`5102092` 统一跟踪 daemon 与 waifu2x/realcugan 等短进程，并通过 `closeAllProcesses()` 释放直接子进程，避免 Provider 取消时只关闭 daemon；`cd23bc0` 在原 `ModelObject/parseModels/getModels/pipeline` 主链上增加原子 `registerModels()`/`unregisterModel()`，自定义 manifest 必须声明安全相对路径、license、input/output blob 和每个文件的 SHA-256，本地缺失文件不会隐式联网，现有/下载文件用流式 hash 校验并按 path+size+mtime+checksum 做 256 项有界缓存；`5c5dace` 将同一 `closeAllProcesses()` 的 Windows 分支升级为 `taskkill /pid <pid> /t /f` 进程树终止，CLI 自行派生的 GPU worker 孙进程不再遗留，`taskkill` 失败或非 Windows 继续回退到 `ChildProcess.kill()`。fork 自身 22 项测试通过，pack 审计仍仅包含 8 个文件、29,141 bytes。上述提交已合并到远端 `main`；`cc83499` 额外跟踪三个构建入口，使 Bun 可从 Git commit 直接安装，而 Xiranite 仍不得从 `ref/` import。

Xiranite 侧的共享边界已经由 `f4d58552`、`19d31807`、`8e42df71`、`95cc96e7`、`557200bf` 和 `8ebabc6d` 建立：`SuperResolutionService` 位于 NeoView application/ports 层，GUI、CLI、TUI 后续只复用这一实例；模型 manifest 可注册，任务在进入 Provider 前完成 scale/path 校验、同一输出路径互斥和 `AbortSignal` 检查，并通过宿主 `ResourceScheduler` 获取 GPU lease，构造与列模型都保持零进程。`SystemSuperResolutionCliResolver` 位于 platform 层，按“显式路径 -> PATH -> 可信候选”惰性探测 canonical path、版本、架构与 daemon capability；显式路径失效不会静默回退，同一 engine 共享缓存，单 caller 取消不会破坏其他等待者。`OpenComicAiSystemProvider` 是注入 runtime 的薄 adapter，不复制模型下载、参数生成、daemon 或进程队列；runtime 初始 daemon 数固定为 0，只有 UpScayl help 明确声明 `-d/daemon` 时才启用配置值，旧构建自动使用同一 fork 的受控短进程。首次真实任务才配置模型目录和已验证的 CLI resolver，取消/超时调用 fork 的统一进程释放，完成后复用现有 `FilePageContent + StreamingImageMetadataProbe` 检查输出文件非空、路径一致且尺寸等于请求 scale。composition 现在只从 `[nodes.neoview.super_resolution]` 读取 provider、三条显式路径、模型目录、资源预算、版本化 preferences/conditions 和最多 64 个 `custom_models` manifest，不访问 `xiranite.db` 或 Reader SQLite；空路径保留自动发现语义，未配置模型目录时复用 `%APPDATA%/NeoView/models`，禁用 provider 时不加载包、不探测 CLI。每个 custom manifest 必须声明安全相对目录、模型文件、scale、engine、license、input/output blob 和逐文件 SHA-256；额外 checksum、未声明 scale alias、路径逃逸和非 HTTPS 下载地址会在加载 TOML 时拒绝。composition 将验证后的 manifest 幂等注册到 fork 的唯一 runtime registry，内建或其他所有者占用同 ID 时拒绝，不维护第二份模型表。模型清单随后仍直接从 fork runtime 的 `modelsList/model()` 派生，并完整保留 license、逐文件 checksums 与 input/output blob；runtime loader 同时验证 register/unregister API，拒绝旧的不兼容 fork。`loadOpenComicSystemRuntime()` 使用变量 specifier 惰性加载 `@hibernalglow/opencomic-ai-system`；包缺失或版本不兼容只返回 composition unavailable，不阻止 Reader 启动。fork 已从 `HibernalGlow/opencomic-ai-bin` 的完整 commit `cc83499463e68cc893c8b67d12c6e0bf375081a7` 锁定为生产依赖；该 commit 跟踪可直接导入的 `dist`，Bun 安装不依赖 `prepare`，Xiranite 也不从 `ref/` import。真实 UpScayl 小样本 benchmark 已完成；更大真实页/驱动覆盖仍未完成。MangaJaNai 已由当前统一超分能力取代，旧目录继续只作为 migration report 的不可执行来源，不再迁移为 runtime model。
...
local headless transport 现在由 `ReaderHeadlessController.upscalePage()` 暴露当前 session 的单页手动执行：按页索引取得真实 `ReaderPage` 后直接委托上述 pages service，返回值移除内部 `sourcePath`，但保留 decision、输出路径、模型、尺寸和耗时。`LazySuperResolutionPagePort` 在首次执行前不加载 fork runtime、不探测系统 CLI、不启动进程，并将并发首次调用合并到一个 capability load；Reader 打开、分页、导航和原图提取仍为零超分初始化。controller dispose 会关闭已经加载的唯一 `SuperResolutionService`，从而复用 Provider 的 daemon/短进程清理。`xneoview upscale-page <book> --index N --output PATH` 只提供显式 manual 入口，模型、scale、noise、tile、TTA 和 GPU 仍全部来自 `[nodes.neoview.super_resolution.preferences]`；默认拒绝覆盖现有输出，必须显式 `--force`。`xneoview upscale-capabilities` 不打开书籍或 Reader SQLite，只通过同一 lazy port 列出 runtime 注册模型并刷新经 resolver 验证的 engine path/version/availability；可选 runtime 缺失时返回结构化 unavailable。当前执行命令只允许 local headless，`--connect` 在鉴权 HTTP 路由与 remote controller 共享同一契约前明确拒绝，不能把本地调用冒充完整远程 transport。

旧 `ProgressiveUpscaleCard` 的逐控件事实源现已冻结到 `migration/neoview/progressive-upscale-compatibility.json`：覆盖预超分/渐进开关、页数 `1/2/3/5/10/20`、并发 `1..4`、dwell `1/2/3/5/10/15/30s`、最大页数 `5/10/20/50/100/999=全部`、完成/排队计数、进度条、running/countdown/imminent/idle 状态，以及 page change/unmount 的 timer/task 生命周期。`SuperResolutionPreloadService` 只消费现有 `ReaderPreloadPlan`，按候选顺序去重并用成熟 `p-map@4` 执行 `background_concurrency` 有界批次；每页仍进入同一个 `SuperResolutionPageService`，因此 policy、archive materialize、I/O/GPU lease、输出互斥和 Provider 进程队列均保持唯一。渐进模式使用 `node:timers/promises` 的 abortable dwell，随后严格向当前页之后扩展；保留旧 `999=全部剩余页` 语义。nearby/progressive 使用独立的 session context + generation，较新 generation 取消旧批次，dispose 等待已启动任务退出，observer 错误不会改变任务结果。`SuperResolutionPageService.plan()/runPlanned()` 现在先由唯一 policy 生成真实 model/scale/cache decision，再允许 platform 基于该 decision 构造 opaque fingerprint，禁止 GUI 或 resolver 复制条件逻辑。`SuperResolutionArtifactPageService` 在命中时返回流式 lease，miss 时把同一 page workflow 作为 producer 交给专用 `CacacheSuperResolutionArtifactStore`；该 store 使用 `cacache@20` 的临时目录、stream put/get、内容寻址与原子 index publish，不复用会整块读取且只有 24 MiB 单项上限的 presentation cache。它按真实唯一内容字节执行 20 GiB 默认预算、512 MiB 单项上限、年龄/当前书籍/全部清理、低磁盘拒绝、图片头和 cacache integrity 校验；active lease 延迟删除，相同 key singleflight，单 caller 取消不破坏其他 waiter，最后一个 demand 释放会取消同一 Provider signal。64 MiB 实际文件门禁同时约束 JS heap 与 RSS，证明生成和读取不会把高分辨率输出整体装入 JS。HTTP composition 默认在 `%APPDATA%/NeoView/upscale-artifacts` 构造同一 store 和 `LazySuperResolutionPagePort`，构造阶段不创建目录、不加载 runtime、不探测 CLI；也允许调用方成对注入 port/store，避免 producer 与 reader 指向不同缓存。鉴权 `POST /reader/s/{session}/pages/{page}/upscale-artifact` 只返回 session-scoped opaque 256-bit digest URL，不暴露路径或 cache key 前缀；`GET/HEAD` 直接流式取得 lease，支持 immutable cache、ETag、304、`nosniff`、consumer cancel 和 stale version。session release 会取消 active generation 并使 URL 404，controller dispose 聚合回收 lazy capability 与 store，但持久 artifact 不随关书删除。live snapshot 使用 `lru-cache` 最多保留 128 个 session/mode 记录，公开 planned/settled/failed/cancelled/pending/progress 与 queued/countdown/running/completed/disabled/empty/paused/cancelled/failed 状态；鉴权 GET/start/pause/retry 控制面直接调用同一 preload service。retry 重交最近 canonical input，已生成页通过 artifact hit 避免重复 GPU；pause 取消当前 demand 并等待退出，session release 删除 snapshot 与请求引用，正在加载的可选 runtime 不阻塞关书。remote client、GUI/TUI 和用户维护控件仍待接入，因此 Card 整体继续为 `partial`，GUI/CLI/TUI 未完成维度不变。

2026-07-24 路径契约更新：上段记录的 `%APPDATA%/NeoView/upscale-artifacts` 已废弃且不迁移。默认根目录改为 Xiranite 数据目录下的 `nodes/neoview/upscale-artifacts`；`cacache` 索引保持内部格式，真实图片同步暴露在 `images-v1` 并保留 `.png`、`.jpg` 或 `.webp` 后缀，条目清理时同步删除。

当前状态更新：本地 headless 与 Remote Headless/`--connect` CLI 已覆盖 preload status/start/pause/retry；两者都复用当前 session 的唯一 plan、artifact store、generation 取消和 context release，关闭或换书不会遗留预载任务。仍待的是 GUI/OpenTUI 的长期 session 消费和原版 Card 呈现。超分 artifact cache 维护同样只操作运行中 HTTP composition 持有的唯一 `CacacheSuperResolutionArtifactStore`：鉴权 session route 提供严格 stats 与显式确认的 age/current-book/all 清理，current-book 在 backend 内映射到 opaque `book.id`，不会向客户端暴露 cache key 或内部 metadata。Remote Headless 使用 Zod 验证完整计数/字节/清理 DTO，`upscale-cache-stats`、`upscale-cache-cleanup --kind age|book|all --yes --connect ...` 与 `upscale-cache-ui --connect ...` 复用该 route，并拒绝本地/offline 双开 cache owner。OpenTUI 每次操作使用临时远程 session，所有 mutation 进入通用危险确认，取消中止同一 fetch，finally exactly-once 释放；GUI 维护控件仍待迁移。
...
不能照搬上游 `opencomic-ai-bin` 当前 wrapper 的所有实现细节。system-only fork 与 Xiranite 薄 Provider 已覆盖参数拆分、协议分块、Error 状态、quoting、直接进程清理、取消/超时、输出校验和宿主 GPU lease；真实短进程 CLI 已验证，未来支持 daemon 的 UpScayl build 和派生孙进程仍需继续覆盖。完整门禁如下：
...
- daemon 启动失败、处理中 error/exit/close 和主动/idle quit 都必须 settle 当前 Promise 并清空队列；fork 已覆盖基础生命周期与直接子进程，Provider 已接 `AbortSignal` 和超时；当前 Scoop UpScayl help 不含 `-d`，resolver 标记 `daemonSupported=false` 并自动降级短进程，而不是尝试 daemon 后以 255 退出；真实 CLI 若派生孙进程仍需 Windows tree termination 测试；
...
2026-07-18 新增可重复入口 `bun run benchmark:neoview-super-resolution -- --assert --runtime <module> --runs 2`；`--probe-only` 只验证 CLI、模型布局和 transient manifest，不启动 GPU。脚本复用正式 resolver/composition/provider，使用 SHA-256 固定的 256×384 PNG，直接 junction 到 Scoop 安装模型，不复制或下载权重；UpScayl 临时 manifest 仍执行逐文件 hash 和 blob 校验。RTX 4060 Laptop（驱动 610.47、8188 MiB）正式短进程结果为：UpScayl Lite 4× cold/warm `1422.60/1363.93 ms`，Waifu2x CUnet 2× `978.78/1014.16 ms`，RealCUGAN SE 2× `1299.93/1299.34 ms`。六个输出均达到精确尺寸且同模型两次 SHA-256 一致；宿主 RSS 增量 14.3 MiB，任务期间 250 ms 采样的 GPU memory 增量峰值 144 MiB，dispose 后 `upscayl-bin/waifu2x/realcugan` 进程数相对基线增量均为 0。该小样本证明三 CLI 纵切与清理契约可运行，不替代多张真实漫画页、8K/16K、多个驱动版本和并发显存压力门禁。
...
`ReaderFrame/PageImage` 与 `ReaderViewToolbar` 是打开书籍时才加载的两个独立 chunk；动态 import 与 backend `open` 同时开始，不形成串行 waterfall。UI chunk 失败时关闭已经创建的 session，组件卸载后迟到的 open 结果也会立即回收。fit mode 与单双页默认值已从旧 `defaultZoomMode/doublePageView` 兼容读取，统一经 `/reader/config` 严格 PATCH 写回 Xiranite TOML；`/reader/s/{sessionId}/options` 仅保留临时 session 更新。持久的本书覆盖统一走 `/reader/s/{sessionId}/book-settings`：GET 返回 version/revision/overrides/effective/inherited，PATCH 使用 expectedRevision 拒绝陈旧写入，方向、单双页和横版本子策略立即重建当前 frame，持久化失败则恢复最后确认 frame。`ReaderHeadlessController.getBookSettings/updateBookSettings` 现复用同一个 `ReaderBookSettingsService`、Zod patch、per-book queue、CAS 和 rollback；GUI HTTP 与 headless composition 的默认值都由 `readerBookSettingsDefaults(sessionOptions)` 派生。CLI 的 `book-settings-get/set` 只把 `true|false|inherit`、`1..5|inherit` 和枚举参数投影为同一 nullable patch，set 强制显式 expected revision；本地和 `--connect` 均复用该 controller，远程 snapshot 通过共享 Zod wire schema 完整校验。OpenTUI 的 `book-settings-ui` 复用通用 `TerminalInteractionDefinition` 表单渲染器，set 先读取当前 revision 再提交同一 CAS update，字段只提供保持、继承和规范显式值，不直接读写 SQLite，也不在持久 Reader TUI 中复制设置状态机。`CoreReaderService` 在开书时按规范 book id 读取同一 store，显式 open options 仍优先于持久覆盖，覆盖再优先于全局 TOML 默认。favorite/rating 只写元数据，不重建活动媒体。旧 `neoview-book-settings` 已通过鉴权 `/reader/book-settings/migration/inspect|import` 建立一次性迁移纵切；manual scale/rotation 仍是每次成功打开书籍后重置的 presentation 会话态，最近视图记忆后续仍须走统一契约，不能用 localStorage 再建立第二套状态。
...
`splitHorizontalPages` 与上述书籍级 `horizontalBook` 是两个独立契约。XR 的规范字段为 `ReaderLayout.splitWidePages`，默认关闭，只在非全景的单页模式且已取得有效横向尺寸时生效；`FrameSnapshot.anchorPart` 和 `FramePage.part` 用 `0=左半、1=右半` 表示物理半页，并随 frame 传输 typed crop insets。LTR 顺序为左→右，RTL 为右→左，`goTo()` 总是重置到阅读方向的第一半，反向导航先回到同一物理页的前一半。GUI 继续复用同一个 Page DTO、asset URL、DOM `<img>`/`<video>` 和解码链，半页切换只改变 typed crop、有效尺寸和合成位移；preload generation 在同一物理页切 half 时保持不变。规范 TOML 写入 `[nodes.neoview.reader].split_wide_pages`，读取继续兼容旧 `reader.view.page_layout.split_horizontal_pages` 与 camelCase 变体。验证覆盖 core 正反向导航/frame window、legacy codec、canonical config、HTTP/CLI transport、React 媒体身份，以及 Chromium desktop/420×360 单请求 E2E。

悬停滚动现已使用 Reader 现有滚动节点完成纵切。规范设置写入 `[nodes.neoview.reader].hover_scroll_enabled` 与 `hover_scroll_speed`（0.5..10），读取继续兼容旧 `image.hoverScrollEnabled` / `image.hoverScrollSpeed` 及 snake_case 变体，旧设置导入直接投影到规范字段。GUI 保留左键开关、右键展开、启用状态与倍率滑条；`viewer.toggle-hover-scroll` 也接入共享输入 action。普通模式绑定 Reader viewport，全景模式通过 `react-virtuoso` 的 `scrollerRef` 绑定真实虚拟列表滚动节点；两者只监听 passive pointer move，指针和动画状态不进入 React state。行为沿用 NEEView 的内容溢出映射：鼠标位置决定每个溢出轴的目标 offset，倍率控制平滑追踪速度，外侧 20% 饱和到真实 0/max 边界，确保上下左右都能看完整；RTL 使用 Chromium 的负 `scrollLeft` 语义保持物理方向正确。仅在存在溢出且尚未到达目标时维持唯一 rAF，禁用、切换物理页/半页或全景布局、离开 viewport 或卸载会同步取消。它不查询 DOM selector、不使用 MutationObserver、不改 Page DTO/媒体 URL/解码/预加载/翻页 generation。focused unit、canonical TOML/HTTP、input action、1920×1080 工具栏及 Chromium desktop/420×360 普通/全景原生滚动证据共同验收。
...
方向预读由独立 `useReaderImagePreloader` 消费 `ReaderSession.preload`：React 按后端 direction/tier/admission 候选顺序取得原始 URL 并调用浏览器 `Image.decode()`，每代最多保留 4 张；paused admission、session/generation 切换和卸载会取消或释放旧 URL，无 plan 的旧 transport 才回退相邻页。它不创建隐藏 React 页面树，不生成 Blob/Base64，不调用 sharp。Image 生命周期按 session+generation 以 50ms、每批最多 64 条上报 TTFB、decode duration、retained bytes 和 active leases，普通翻页不写 React state，也不新增 viewport-context PATCH。普通 Reader 候选预算统一读取 canonical `[nodes.neoview.performance].preload_pages`，兼容迁移期 `preload_items`、`preLoadSize`、`image.preloadCount` 与旧 `reader.book.preloadPages`；canonical 值优先，输入允许 `0..1000`，执行端硬限制为最多 32 页，且只裁剪唯一 coordinator 的候选，不复活旧 `PreloadPipeline`/`Preloader` 重复队列。真实 `2000×3000` 高熵 JPEG CBZ 的 Chromium E2E 同时验证 asset URL 不含 transform 参数、热翻页 decode 门槛、桌面/卡片视口和零 Canvas；显式 transform route 只保留给浏览器不支持格式、用户主动转换和导出。`xneoview preload-plan <path>` 直接投影同一 Headless snapshot，OpenTUI 在页面列表标记 Near/Ahead/Background 并显示 generation/admission 计数；二者不另开页面流、不复制 scheduler，也不会让终端投机解码抢占当前翻页。
...
数据所有权是硬边界，三类数据不能混用：NeoView 节点设置沿用其他节点的配置机制，经 `NeoviewConfigStore` 原子合并到 `xiranite.config.toml` 的 `[nodes.neoview]`；规范写入保留 `reader`、`panels` 等一级业务分区和最多一层相关项分组，`card_state` 等集合中的每个相关对象各占一行 inline table，对象数组每个对象一行，读取端继续兼容旧深层嵌套表、上一版全量 `config` envelope 及混合格式，完整格式与验收命令见 `docs/neoview-config-format.md`。`xiranite.db` 只属于 Xiranite 项目本身，用于工作区和 XR 运行数据，NeoView 迁移不得因为可以拿到 `databasePath` 就向其中追加 Reader 业务表；NeoView 的缩略图、阅读进度、历史、书签、评分、标签及兼容元数据统一沿用原 `%APPDATA%/NeoView/thumbnails.db`，以 `xr_` 命名空间独立表和带备份、可回滚的 schema migration 扩展。扩展必须是向后兼容且非破坏性的：不得修改旧 `thumbs`、`failed_thumbnails`、`metadata` 表及其索引，不得改写 `metadata.version`、`PRAGMA user_version` 或 journal 设置；旧 NeoView 应忽略新增表并可与 XR 同时读写，XR 遇到 busy/locked 只能有界退避或跳过本次进度写入，不能通过 checkpoint、vacuum 或强制迁移干扰旧进程。这里的“统一”表示一个 NeoView 主库，不表示把所有数据塞进旧 `thumbs` 表；同时禁止在 Xiranite 数据目录再创建第二个 NeoView SQLite 主库。
...
EMM 新写入同样不得回写旧 `thumbs` 行。XR 只读合并旧 `thumbs.rating_data/emm_json/manual_tags`，用户在 XR 修改评分、手动标签或译名时写入同库独立表 `xr_reader_emm_overrides(path_key, display_path, overrides_json, revision, updated_at)`；`null` patch 表示恢复继承旧值，空覆盖仍保留 revision tombstone，避免 reset 后 ABA。GUI 使用鉴权 `GET/PATCH /reader/s/{id}/emm-metadata`，本地与远程 Headless facade 复用同一个 `ReaderEmmMetadataService` 和 Zod wire schema；更新译名后主动失效 Book Information metadata cache。FolderMain 另以 `PATCH /reader/browser/s/{id}/emm-metadata` 提交最多 64 项的 generation-bound 编辑：Zod 在任何写入前验证完整命令并拒绝重复路径，`ReaderFileTreeService` 将路径限制为当前稳定 listing 成员，`p-map` 只提供最多 8 路有界 admission，每项仍由同一 EMM service 串行/CAS；成功项批量回读现有 metadata provider、复用唯一排序器并只推进一次目录 generation，冲突按 item 返回且响应不回传路径。导航导致 generation 过期时拒绝写入；写入后并发导航则保留已确认数据并返回 `refreshRequired`，由 GUI 拉取新目录快照。Headless file-tree composition 现在按首次编辑惰性创建同一个 editor 和原库 store；`folder-emm-edit` CLI 与 OpenTUI 只接收 `{updates, concurrency?}` JSON 并注入当前 generation，不复制校验、CAS、批处理或 refresh 逻辑。旧 NeoView V1、V3、V4 实际共用同一 `thumbs` 表，V4 也复用 V3 的数据库/生成器；它们是 service 版本而不是三套待搬运 schema，因此只保留一个兼容 codec/store，不做无意义的 V1/V3/V4 BLOB 迁移。当前剩余 EMM 缺口集中在 FolderMain/独立 EMM GUI 编辑器、乐观回滚和收藏标签快捷应用。

旧 `neoview-book-settings` localStorage 迁移复用 `LegacySettingsCodec` 的 backup/full-export envelope 识别，并由 `LegacyBookSettingsCodec` 用 Zod 逐字段校验直接 map 或 rawLocalStorage JSON；最多 10,000 本，坏字段单独计数，未知字段报告但不写入。`LegacyBookSettingsImporter` 使用现有 `resolveLegacyReaderSource()` 进行 realpath/格式识别和 `stableOpaqueId`，通过 `p-map@4` 并发 8 解析，缺失路径只记为 best-effort identity；别名解析到同一 book id 时确定性保留最后一项。所有身份解析成功后才调用一次 `SqliteReaderDataStore.importBookSettings()`：merge 保留现有显式覆盖并补入旧字段，overwrite 替换整条 overrides，未变化记录不递增 revision。鉴权 inspect/import 响应只返回计数、未解析数、重复 identity 数和写入统计，不回传旧路径；import 必须 `confirmed=true`，请求取消发生在事务前。共享 `ReaderBookSettingsMigrationFileController` 使用 Node `FileHandle` 完成 64 MiB 有界读取；inspect 只加载同一 codec、坏 JSON 与未确认 import 均零 SQLite，confirmed import 才通过 platform factory 独占并 exactly-once 关闭同一 `SqliteReaderDataStore`。`book-settings-legacy-inspect/import` 与 `book-settings-migration-ui` 复用该 controller，CLI 要求 `--yes`，OpenTUI import 使用通用 dangerous confirmation，二者都支持 merge/overwrite 与显式原库路径。真实临时 current-schema 库验证写入后旧 `metadata.version`、`user_version` 和 WAL 不变。共享 codec/importer/service 已从 core 导出，GUI HTTP、CLI 与 TUI 后端纵切均完成。
...
`LegacyNeoViewDataLocator` 在 Windows 固定解析 `%APPDATA%\NeoView\thumbnails.db`，并显式返回同目录 WAL/SHM；macOS/Linux 使用对应 Tauri app-data 约定，绝不读取 TOML 的 `paths.thumbnail_directory` 作为主库。`LegacyThumbnailDatabaseInspector` 先以只读/query-only 模式检查文件、sidecar、`user_version`、`journal_mode`、表、列、索引和 `metadata.version`，按 `current`、`legacy-upgrade-required`、`newer-read-only`、`incompatible` 分类；只有 `current` 才允许 `WritableLegacyThumbnailStore` 取得单 writer。fixture 覆盖冻结的 2.4 schema、活动 WAL/SHM、旧版缺列、较新版本、无关 SQLite 和损坏文件；当前机器的真实原版库也已只读验证为 2.4/current。2026-07-18 真实主库只读基准覆盖 14,258 行、816,513,264 Blob bytes，数据库 835,448,832 bytes，SHA-256 为 `2d53dc58e4a02d378af0d5da15f8c7eff952ace1feb7fb26495a5638fcaa6ede`，5,000 次单条平均 0.1248 ms、512 条 raw/cooperative 批次平均 56.91/60.11 ms，且 400 个 I/O lease 全部释放。该 feature 仍保持 `pending`，直到不同存储介质与真实 reader thumbnail-system corpus 的性能验收完成。

2026-07-19 在同一机器对当前主库再次执行只读基准：数据库 `977,899,520` bytes、`16,264` 行、`955,968,628` Blob bytes、SHA-256 `29406a5eb9ca51d5d07dbba36929940a1f9b58581b3c41d799591cca2dc9be5b`；schema 仍为 2.4/current，WAL/SHM 分别为 `20,632/32,768` bytes，空 BLOB 为 0。5,000 次单条读取平均 `0.0674 ms`；512 条 raw/cooperative 批次平均 `33.25/35.66 ms`。维护统计用时 `70.88 ms`，65 个统计 I/O lease 与 400 个合作读取 I/O lease 全部释放，队列归零。该基准不输出缩略图 key 或 BLOB，只用于同机回归；当前结果继续支持最多 64 条合作块，不支持为此引入 Worker/Piscina 或第二 SQLite binding。
...
GUI library thumbnail 注册、旧库只读批量解码和可写库无效路径检查已统一使用项目固定依赖的 `p-map@4`，显式保留有序结果、并发上限和首错停止语义；三处本地 worker-loop/cursor helper 已删除。缩略图任务的资源优先级与生成 singleflight 仍由唯一 `ThumbnailCoordinator`/宿主 `ResourceScheduler` 管理，`p-map` 只负责这些有界批次的 fan-out，不形成第二套队列或资源预算。

这仍不是完整 thumbnail feature：横向 React 列表已消费 `thumbnailUrl`，Page/file/folder/video/归档内视频生成、失败退避、单 writer 批写、统计、清空空 BLOB、保留 folder 的过期清理、无效路径清理和失败黑名单清理已完成。GUI 当前目录/选中项预热复用纯 application `ReaderLibraryThumbnailWarmupService`；鉴权 `POST /reader/library/thumbnails/prewarm` 接受最多 256 个 file/folder demand，使用 `p-map` 做 1～8 的有界 fan-out（默认 2），真正 admission、singleflight 和宿主资源预算仍只由 `ThumbnailCoordinator`/`ResourceScheduler` 负责。响应以 NDJSON 输出 `start`、逐项 `completed|failed` 和 `complete`，客户端断开、Card 关闭或 route dispose 会取消同一 `AbortSignal` 并释放临时 context，不建立 job 表或第二套后台队列。`mode=ensure` 只保证命中或生成；`mode=refresh` 则走 pipeline 的显式 refresh demand，跳过旧 SQLite BLOB、失败退避、Windows shell cache 与 page thumbnail 复用，生成成功后等待现有 writer 的 `BEGIN IMMEDIATE`/UPSERT 真正提交，再切换该 source/profile 独立的 library L1 epoch。epoch 使用已有 `lru-cache` 限制为 4096 个被刷新来源；重载一个选中项不会让其他可见缩略图重新查库，只有超过上限淘汰历史 epoch 时才提升 base epoch 防止回落到陈旧 L1。生成、取消或提交失败都不会先删除旧记录，也不会把 SQLite busy 误写入失败黑名单；真实双连接写锁测试证明 replacement 失败后同一 key 仍返回旧 WebP。五类维护统一经过纯 application `ReaderThumbnailMaintenanceService`，HTTP 与 CLI 不再各自计算 cutoff 或直接编排 store；无效路径每批最多删除 500 项、最多扫描 2000 项，暂时离线或不可访问的卷必须保留。维护 port 使用标准 `AbortSignal`，GUI 断开或主动取消会贯穿 HTTP、application、lazy adapter 和 SQLite store；无效路径扫描在每次异步状态探测后及删除事务前检查取消，取消不删除记录、也不推进批次游标，前端可用每批 `scanned/deleted/unavailableVolumeRowsPreserved/wrapped` 累计进度，无需后端再维护第二套 job registry。默认隐藏、无需 Reader session 的 `thumbnail-maintenance` Card 现已复用上述鉴权 HTTP，只在显式停靠且 Panel 激活时读取一次聚合统计，所有清理单飞并在卸载时取消；Card 独立延迟 chunk 为 `7,397 bytes`，未进入 initial、NeoView entry 或 `ReaderSidebar`。旧版按路径前缀删除、VACUUM 与路径规范化没有作为在线按钮迁入，继续由确认式离线 CLI 承担。SQLite 原生一致性备份、XR writer/维护进程锁、显式离线 checkpoint/optimize/vacuum、验证备份恢复与 V1/V3/V4 调度行为收口已经完成；剩余核心项是真实数据库 Chromium 操作证据与大规模性能验收，因此 Card 与整体 thumbnail feature 均保持 `partial`。这些离线操作不得混入活动 Reader 的在线维护 API。
...
`LegacyNeoViewDataLocator.inspect()/discover()` 现已提供无副作用的 canonical/custom 候选报告：它只检查配置目录下的已知 `thumbnails.db` 路径，canonical 数据库始终保持 active；自定义数据库即使在 canonical 缺失时也只标记为 secondary compatibility source，不自动挂载、复制、合并或创建新库。SQLite schema/WAL 兼容性仍由既有 `LegacyThumbnailDatabaseInspector` 在显式 inspect/迁移流程中负责。CLI 的 `thumbnail-db-merge-plan [canonical] --source <secondary.db>` 先只读统计两库行数、冲突、按较新缩略图的胜出项及可从 secondary 填充的 EMM/评分/翻译/手工标签字段；只有两个库都是 2.4/current 且非同一路径时才 eligible。实际合并必须显式调用 `thumbnail-db-merge-secondary [canonical] --source <secondary.db> --backup <canonical-backup.db> --offline --yes`：它对 canonical/secondary 取得稳定顺序锁，只 checkpoint canonical，以 `VACUUM INTO` 先验证 canonical 回滚备份和 canonical 目录内的 secondary 一致快照，随后在一条短事务中字段级合并。缩略图冲突以较新 `date` 胜出，兼容扩展字段只从 non-null secondary 值补 canonical 空值；失败记录保留较新错误并取较高 retry count。secondary 原库及 WAL/SHM 不被 checkpoint 或写入，临时快照在完成/失败后删除。GUI 自动确认与自动合并继续保持 pending。

动态格式 registry 的历史待办现已完成：`image.supportedFormats` 与 `image.videoFormats` 已收口为 controller-scoped `ReaderMediaFormatRegistryRef`。标准扩展使用唯一 MIME 表；自定义扩展必须同时提供 `[nodes.neoview.image.media_mime_types]` 的显式 `image/*` 或 `video/*` 映射，未知 MIME、图片/视频重叠扩展、孤立 override 和类型不匹配在配置提交前拒绝。扩展名统一去点、小写、去重并限制为 128 项和 16 字符；默认 JPEG/PNG/WebP/GIF/AVIF/JXL/TIFF/BMP 与 MP4/MKV/WebM/MOV/AVI 等格式保持不退化，旧 `.nov` 继续明确映射为 `video/mp4`。目录、单文件、archive/EPUB loader、GUI 目录 listing/媒体 metadata 和文件夹缩略图代表项共用同一 resolver；配置 PATCH 后 revision 原子切换，新 session、新 listing 和新缩略图缓存键立即生效，已打开 session 的页 MIME/媒体种类保持冻结，避免中途变更页面契约。该设置只非破坏性写入 `xiranite.config.toml` 的 `[nodes.neoview.image]`，不写 `xiranite.db` 或 `%APPDATA%/NeoView/thumbnails.db`；PDF 继续 deferred。
...
library source 的 `realpath/stat` 与 folder representative 的 `readdir/stat` 已分别进入宿主 I/O 池：可见注册使用 `view`，后台 warmup 使用 `background`。代表图索引保持 singleflight，多个等待者只占一个 `neoview.thumbnail.folder-representative` lease；最后一个等待者取消会从宿主队列撤销或中止扫描。这样即使 GUI 一次注册 64 项、warmup 并发配置为 8，也不会绕过 XR 的全局 I/O 上限。
...
- 4K JPEG PageThumbnail 的冷生成 p95 初始目标不高于 250 ms，AVIF/JXL、视频和 solid archive 分开报告，不能混合稀释；`benchmark:neoview-thumbnail-system -- --video-source <path>` 复用当前 pipeline/ffmpeg provider 报告视频冷生成、L1 命中和 release 状态，不在未提供真实视频语料时加载 ffmpeg；
...
interface FeatureCompatibilityEntry {
  ...
  disposition: "migrate" | "preserved" | "host-replaced" | "import-only" | "removed-with-approval"
  capabilityStatus: {
    core: "pending" | "partial" | "complete" | "not-applicable"
    transport: "pending" | "partial" | "complete" | "not-applicable"
    gui: "pending" | "partial" | "complete" | "not-applicable"
    cli: "pending" | "partial" | "complete" | "not-applicable"
    tui: "pending" | "partial" | "complete" | "not-applicable"
    evidence: "pending" | "partial" | "complete" | "not-applicable"
  }
...
- 任一适用维度没有对应 tracked 测试证据时，该维度不得标为 `complete`；尚未实现的证据名称只能放入 `plannedTestIds`；
