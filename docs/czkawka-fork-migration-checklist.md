...
| R09 | `[-]` | 分组轨道/分隔 | 分组结构可见；参考项和普通项语义明确 |
| R10 | `[-]` | 右键菜单 | 选中/取消整组、复制路径、复制文件、打开路径 |
| R11 | `[x]` | 行操作 | 打开、定位、复制路径/名称、预览 |
| R12 | `[ ]` | 搜索命中联动 | 顶栏搜索实时过滤当前工具且保持分组合法 |
| R13 | `[x]` | 每工具状态隔离 | 数据、排序、过滤、选择在切换工具后保留 |
| R14 | `[ ]` | 空态/错误态/停止态 | 三类状态均有明确文案与恢复入口 |

验证证据（2026-07-14）：`src/nodes/czkawka/result-table.tsx` 为 11 个工具定义独立列集，并以工具 ID 隔离排序、过滤、列宽、滚动窗口和选择锚点；固定 52px 行高的纯 TS 虚拟窗口仅渲染可视范围加 overscan。拖框按虚拟行全局坐标求交，支持替换、Ctrl/Shift 追加、Alt 移除且跳过参考项。每行右键菜单提供整组选择/取消、复制路径/名称、系统打开和文件管理器定位；文件对象剪贴板在通用 host 尚无该能力时明确禁用。`result-table.test.ts` 覆盖列映射、列排序、Shift/Ctrl 选择、框选三种模式/参考项和 10,000 行窗口计算；`result-table.component.test.tsx` 覆盖 520,000px 总滚动高度、滚动后窗口由第 0 行更新至第 73 行且 DOM 始终少于 80 行、列宽由 160px 拖至 240px 时行高不变、真实拖框与右键 Host 回调以及跨工具过滤/排序恢复；`Component.test.tsx` 覆盖跨工具结果和选择恢复。
...
| ID | 状态 | 能力 | 验收标准 |
| --- | --- | --- | --- |
| F01 | `[-]` | 快速文本筛选 | 当前已有路径/详情文本过滤；补正则、大小写与字段范围 |
| F02 | `[ ]` | 标记状态筛选 | 已选、未选、全部 |
| F03 | `[ ]` | 分组数量筛选 | 按组内项目数范围 |
| F04 | `[ ]` | 分组体积筛选 | 按组总大小范围 |
| F05 | `[ ]` | 文件大小筛选 | 带 B/KB/MB/GB/TB 单位 |
| F06 | `[ ]` | 扩展名筛选 | include/exclude 与实时格式统计联动 |
| F07 | `[ ]` | 日期筛选 | 预设和自定义范围 |
| F08 | `[ ]` | 路径筛选 | contains/starts/ends/regex 等模式 |
| F09 | `[ ]` | 相似度筛选 | 相似图片/视频差异范围 |
| F10 | `[ ]` | 分辨率/宽高比筛选 | 图片和视频维度筛选 |
| F11 | `[ ]` | 只看已选/未选 | 与表格选择实时联动 |
| F12 | `[ ]` | 过滤组内显示全部 | 命中组后可显示组内未命中项 |
| F13 | `[ ]` | 筛选预设 | 预设、重置、导入/导出、持久化 |
| F14 | `[ ]` | 格式 Badge 筛选 | 图片/视频/音频/文档/压缩包/文件夹类别及单格式选择 |
| F15 | `[ ]` | 筛选统计 | 原始/命中组数、文件数、体积实时显示 |
| F16 | `[ ]` | 快捷键 | 展开、重置、应用等与 fork 一致的快捷操作 |
...
| A04 | `[ ]` | 4 种组选择模式 | 替换、添加、移除、交集语义与 fork 对等 |
| A05 | `[ ]` | 多级排序条件 | 可增删/拖动优先级，作用于组内候选 |
| A06 | `[ ]` | 文本规则 | 文件名/路径字段、匹配模式、包含/排除 |
| A07 | `[ ]` | 目录规则 | 包含目录、排除目录、reference 规则 |
| A08 | `[ ]` | 选择历史 | undo/redo，切换工具隔离 |
| A09 | `[-]` | 清空/反选/全选 | 当前有清空/全选；补反选和可见范围语义 |
| A10 | `[ ]` | 选择统计 | 已选数量、大小、预计回收空间 |
| A11 | `[ ]` | 配置持久化 | 助手规则和展开状态持久化 |
| A12 | `[ ]` | 选择快捷键 | 应用、撤销、重做、清空 |
...
| ID | 状态 | 能力 | 验收标准 |
| --- | --- | --- | --- |
| P01 | `[~]` | 通用本地图片直通 | `LocalImagePreview` 供所有节点复用；支持 AVIF/JXL；失败回退有测试 |
| P02 | `[-]` | 结果表图片缩略图 | 当前只有简单直通；补动态尺寸、行高与预加载 |
| P03 | `[ ]` | 视频缩略图 | 不依赖 fork 本地 HTTP server，改用 Xiranite 通用本地媒体 URL |
| P04 | `[ ]` | 图片点击预览 | 对话框/侧栏/浮动卡片至少一种完整模式，支持上一个/下一个 |
| P05 | `[ ]` | 视频播放器 | 横竖屏自适应、自定义进度、播放状态、跳转、音量、全屏 |
| P06 | `[ ]` | 音频预览 | 重复音频可播放并显示 tag/fingerprint 信息 |
| P07 | `[ ]` | 侧栏预览开关 | 当前工具独立状态和持久化 |
| P08 | `[ ]` | 图片/视频信息卡 | 路径、大小、分辨率、修改时间、相似度等 |
| P09 | `[~]` | 缩略图缓存 | 优先共享 host/cache 能力；若无需缓存必须用性能证据证明 |
| P10 | `[ ]` | 预览列覆盖 | 重复文件、相似图片、相似视频、音频及其他媒体结果统一动态 preview cell |
...
| D02 | `[ ]` | 格式占比 | donut chart 与数据表一致 |
| D03 | `[ ]` | 格式体积分布 | bar chart 与过滤后数据联动 |
| D04 | `[ ]` | 相似度分布 | 图片/视频按等级和差异区间统计 |
| D05 | `[ ]` | 选择统计 | 数量、体积、回收空间实时联动 |
...
| Q04 | `[ ]` | `LocalImagePreview` 共享组件成功/失败/禁用测试 |
| Q05 | `[ ]` | 11 工具参数 contract → native DTO 映射测试 |
| Q06 | `[ ]` | 高级筛选 property tests 或等价覆盖 |
| Q07 | `[ ]` | 智能选择规则与历史 property tests 或等价覆盖 |
