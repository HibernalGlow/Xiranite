...
- NeoView TOML 规范写入格式为 `[nodes.neoview]` 下的单行 `config = { ... }` inline table；读取端必须继续兼容旧 `[nodes.neoview.*]` 嵌套表和迁移期混合格式，混合冲突时 `config` 优先。验收与告警命令见 `docs/neoview-config-format.md`。
