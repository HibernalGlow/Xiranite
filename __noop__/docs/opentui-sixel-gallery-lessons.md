...
`sixel@0.16.0` 的 `image2sixel` 内置 indexed encoder 尚未正确处理透明像素，
而 `contain` 缩放会在竖图两侧产生大量透明留白。两者组合后，量化索引会污染
不透明页面区域；实际表现是漫画灰阶被压成高反差噪点，虽然 AVIF/归档解码本身
完全正常。

共享编码器必须先复制 RGBA，把透明和半透明像素合成到不透明背景，再以 256 色
上限编码。不能把缓存帧直接传给 `image2sixel`，因为其内置误差扩散会原地修改
输入数组。这个处理属于 terminal-image adapter，NeoView、EngineV 等节点不得
各自实现一份。
...
### 6. 原始字节不能直接交给 OpenTUI 私有 writer
