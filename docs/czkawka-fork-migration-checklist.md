...
T06 阶段证据（2026-07-15）：fork 原先在 Tauri Rust 中按父目录计数并硬编码阈值 2，前端再按设置二次过滤。Xiranite 已将该逻辑迁为 `@xiranite/node-czkawka/similar-folders` 纯 TS 模块，一次生成路径、图片数、体积、涉及组数和非 reference 优先预览图，并按本次 `similarImagesFolderThreshold` 直接写入共享 `CzkawkaData.similarFolders`。CLI、guided 结果和 OpenTUI 分析卡已消费同一字段；纯函数及 core 测试覆盖 Windows/POSIX 路径、阈值、排序、reference 预览替换和结果挂载。T06 暂保持部分完成，直至 GUI 图片/文件夹视图切换与真实媒体参数矩阵完成。
...
| U02 | `[x]` | 可调侧栏宽度 | 节点内部可调并持久化，不影响端口/resize |
...
布局与迁移证据（2026-07-15）：`CzkawkaWorkspaceLayout` v1 统一保存工具轨道、扫描条件和分析面板宽度及最小化状态，宽度经过边界归一化；三个桌面 separator 支持指针、方向键、Shift 步进和双击复位，compact/portrait 不应用桌面列宽。卡片栈继续作为条件、设置、日志、助手和操作面板的唯一布局实现，高度滑杆支持双击恢复注册表默认值，折叠状态保持结果区可用。`CzkawkaCardState.schemaVersion` 与迁移补丁会补齐并修复旧状态而不丢失业务字段；纯函数、状态迁移和组件交互测试共覆盖默认值、clamp、持久化、最小化/恢复及双击复位。
...
Q11 验证证据（2026-07-15）：Windows x64 Release `.node` 为 21.30 MiB，`dav1d.dll` 为 1.70 MiB，二者的发布 ZIP 为 8.74 MiB；Bun smoke 返回 API v3 / Czkawka 10.0.0，覆盖 duplicate、basic、media 三类入口，并真实观测 `CollectingFiles` 进度、取消和 session 清理。构建直接复用仓库 `native/vendor/dav1d-windows-x64.zip` 中的开发包，通过 `PKG_CONFIG_PATH` 定位 `dav1d.pc`，不安装 VCPKG；预编译 ZIP 与 SHA-256 manifest 已由 `packages/czkawka-native/prebuilt/win32-x64` 固化，embedded smoke 也成功加载 API v3。
