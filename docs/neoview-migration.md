...
> 最后更新：2026-07-15；最近已提交基线：`897886a`（原图阅读/预解码/虚拟缩略图）、`880ccff`（复杂 Shell/Card AST scaffold）。本节必须随每个 NeoView 实现提交同步更新；代码、feature matrix 和测试证据优先于文字。
...
| 设置与数据迁移 | 部分完成 | LegacySettingsCodec、TOML inspect/import、确认/备份/原子写入 | GUI/TUI 确认、专用数据库数据、导出/自动备份/Gist transport |
...
| 四边 Shell | 进行中（本轮） | AST provenance 的 top/right/bottom/left slot；32px trigger、延迟、pinned、IME/浮层、Escape、零挂载；top/bottom 已接阅读 UI | 宽高/对齐/偏移/resize/drag、空白点击策略、布局 TOML 持久化 |
...
- 前端 NeoView：8 个测试文件、21 项 component/hook/registry/shell 测试通过；后端 NeoView 最近完整复跑为 35 个文件、Vitest 165 项 + Bun TUI 1 项通过；
- Reader pipeline 最近复跑：open `53.36 ms`、navigation p95 `3.72 ms`、first byte p95 `2.57 ms`、throughput p50 `81.96 MiB/s`；
- 四边 Shell + 左右 lazy card 的真实 Chromium 路径已覆盖桌面与 `420×360`：关闭侧栏/底栏后 DOM 为 0，底栏卸载时预解码仍继续；热翻页已观测 desktop `44.9/69.6 ms`、card `43.4/66.7 ms`（navigation/decode），另一轮含冷 dev chunk 转换的 desktop 为 `72.1/121.4 ms`，仍低于 `150/200 ms` 硬门槛但需生产构建继续优化；
- 本轮先用“当前 HEAD + 仅 NeoView staged diff”的隔离 worktree避开并行 Czkawka 半成品完成生产构建，随后在主工作区再次得到一致报告：NeoView entry `31,740 bytes`，ReaderSidebar `5,244 bytes`，PageNavigationCard `2,132 bytes`，BookInformationCard `2,020 bytes`；首屏 NeoView module `0`、前端 zip.js module `0`。entry 相对上一基线增加 `6,025 bytes`，仍低于 40 KiB 硬门槛，Panel/Card 保持独立延迟 chunk。
