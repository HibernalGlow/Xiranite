...
| 旧缩略图数据库 | 主运行纵切可用、迁移未完成 | 沿用 `%APPDATA%/NeoView/thumbnails.db`；惰性单 writer、批量命中、生成/持久化、失败退避、统计与有界在线清理；SQLite 原生一致性备份与显式离线 checkpoint/optimize/vacuum | V1/V3/V4 迁移、外部 writer epoch 协调、损坏恢复 |
