...
describe("app-owned xlchemy Component", () => {
  ...
  test("combines file settings and uses the width-adaptive shared tabs layout", async () => {
    ...
    const keepOriginal = within(settingsTabs).getByRole("switch", { name: "转换后未变小时保留原图" })
    const copyOriginal = within(settingsTabs).getByRole("switch", { name: "未变小时复制原图到输出目录" })
    expect(keepOriginal).toBeTruthy()
    expect(copyOriginal).toBeTruthy()
    expect(within(settingsTabs).getByText("转换后未变小时保留原图").closest("[title]")?.getAttribute("title")).toBe("转换结果大于或等于原图时，删除转换结果并保留原图。")
    expect(within(settingsTabs).getByText("未变小时复制原图到输出目录").closest("[title]")?.getAttribute("title")).toBe("转换结果未变小时，用原图替代输出目录中的转换结果；开启时会同时启用保留原图。")
    await userEvent.setup().click(copyOriginal)
    expect(host.cardState).toMatchObject({ keepIfLarger: true, copyIfLarger: true })
    view.rerender(<Component compId="xlchemy-card" host={host} />)
    await userEvent.setup().click(within(settingsTabs).getByRole("switch", { name: "转换后未变小时保留原图" }))
    expect(host.cardState).toMatchObject({ keepIfLarger: false, copyIfLarger: false })
  ...
  test("previews, compares and explicitly copies a clipboard conversion from one workbench", async () => {
    ...
    expect(within(workbench).getByText("校准矩阵")).toBeTruthy()
    for (const tab of ["参数", "转换", "文件", "常规"]) expect(within(workbench).getByRole("tab", { name: tab })).toBeTruthy()
    const configuration = within(workbench).getByTestId("xlchemy-clipboard-configuration")
    expect(within(configuration).getAllByRole("combobox")[0]?.textContent).toContain("WebP")
    expect(within(workbench).getByRole("slider", { name: "质量" }).getAttribute("aria-valuenow")).toBe("74")
    expect(within(workbench).getByText("并行线程")).toBeTruthy()
    expect(within(workbench).queryByRole("slider", { name: /change the position of the slider/i })).toBeNull()
    ...
    const comparison = within(workbench).getByRole("slider", { name: /change the position of the slider/i })
    expect(comparison.getAttribute("aria-valuenow")).toBe("50")
    const comparisonRoot = workbench.querySelector<HTMLElement>('[data-rcs="root"]')!
    expect(comparisonRoot).toBeTruthy()
    comparisonRoot.getBoundingClientRect = () => ({ bottom: 100, height: 100, left: 0, right: 100, top: 0, width: 100, x: 0, y: 0, toJSON: () => ({}) })
    const pointerDown = new Event("pointerdown", { bubbles: true })
    Object.defineProperties(pointerDown, { button: { value: 0 }, pageX: { value: 80 }, pageY: { value: 50 } })
    fireEvent(comparisonRoot, pointerDown)
    expect(comparison.getAttribute("aria-valuenow")).toBe("80")
    fireEvent.pointerUp(window, { button: 0, clientX: 80, clientY: 50, pageX: 80, pageY: 50 })
    expect(writeImage).not.toHaveBeenCalled()
    await user.click(within(workbench).getByRole("button", { name: "复制图片" }))
    ...
    expect(host.cardState.progressText).toBe("转换结果已复制为兼容图片。")
  })
  ...
  test("automatically writes a converted image when clipboard auto-copy is enabled", async () => {
    const host = createHost({ clipboardAutoCopy: false, clipboardCopyMode: "image", clipboardFormat: "WebP" })
    const writeImage = vi.fn(async () => undefined)
    host.clipboard!.readImage = vi.fn(async () => ({ base64: "cG5n", mimeType: "image/png" }))
    host.clipboard!.writeImage = writeImage
    ...
    await user.click(screen.getByRole("button", { name: "打开剪贴板图片工作台" }))
    const workbench = await screen.findByTestId("xlchemy-clipboard-workbench")
    const autoCopy = within(workbench).getByRole("button", { name: "转换后自动写入剪贴板" })
    expect(autoCopy.getAttribute("aria-pressed")).toBe("false")
    await user.click(autoCopy)
    expect(host.cardState.clipboardAutoCopy).toBe(true)
    await user.click(within(workbench).getByRole("button", { name: "转换" }))
    await waitFor(() => expect(writeImage).toHaveBeenCalledWith({ base64: "d2VicA==", mimeType: "image/webp" }))
    expect(within(workbench).getByRole("button", { name: "已复制图片" })).toBeTruthy()
  })
})
