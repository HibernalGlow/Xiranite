...
export function Component({ compId, host }: NodeComponentProps<SnfCardState>) {
  ...
  async function execute(nextAction: SnfAction = action) {
    ...
    if (!splitLines(dataRef.current.pathsText).length) {
      const message = "请先输入至少一个库目录或作者目录。"
  ...
  const props: ViewProps = {
    action, actionMeta, configDirty, data, defaults, logs, paths, progress, result, running, status,
...
function CompactView(props: ViewProps) {
  return (
    <div data-testid="snf-compact-view" className="flex min-h-0 flex-1 flex-col">
      <div className="flex shrink-0 items-start justify-between gap-2 p-3 pb-2"><HeaderLine status={props.status} subtitle={props.data.progressText || summaryText(props)} /><div className="flex shrink-0 items-center gap-1"><ActionTools {...props} compact /><RunButton compact props={props} /></div></div>
      <div className="flex min-h-0 flex-1 flex-col gap-2 px-3 pb-3"><ActionMode value={props.action} disabled={props.running} onChange={props.onActionChange} /><ModeToggle value={props.data.mode ?? "library"} disabled={props.running} onChange={(mode) => props.onPatch({ mode })} /><PathInput compact data={props.data} disabled={props.running} onPaste={props.onPastePaths} onPatch={props.onPatch} /><SwitchPanel compact data={props.data} disabled={props.running} onPatch={props.onPatch} /><div className="min-h-0 flex-1"><ResultTabs compact logs={props.logs} result={props.result} onCopyLogs={props.onCopyLogs} onCopyResults={props.onCopyResults} /></div></div>
    </div>
  )
}

function PortraitView(props: ViewProps) {
  return <div data-testid="snf-portrait-view" className="flex h-full min-h-0 flex-col gap-2 p-2"><div className="flex shrink-0 items-start justify-between gap-2"><HeaderLine status={props.status} subtitle={props.data.progressText || summaryText(props)} /><RunButton compact props={props} /></div><ActionMode value={props.action} disabled={props.running} onChange={props.onActionChange} /><ModeToggle value={props.data.mode ?? "library"} disabled={props.running} onChange={(mode) => props.onPatch({ mode })} /><PathInput compact data={props.data} disabled={props.running} onPaste={props.onPastePaths} onPatch={props.onPatch} /><SwitchPanel compact data={props.data} disabled={props.running} onPatch={props.onPatch} /><div className="min-h-0 flex-1"><ResultTabs compact logs={props.logs} result={props.result} onCopyLogs={props.onCopyLogs} onCopyResults={props.onCopyResults} /></div></div>
}

function FullView(props: ViewProps) {
  return (
    <div data-testid="snf-full-view" className="flex min-h-0 flex-1 flex-col gap-2 p-3">
      <div className="flex shrink-0 flex-col gap-2 @3xl/snf:flex-row @3xl/snf:items-center @3xl/snf:justify-between"><div className="flex min-w-0 flex-col gap-2 @3xl/snf:flex-row @3xl/snf:items-center"><HeaderLine status={props.status} subtitle={props.data.progressText || summaryText(props)} /><div data-testid="snf-header-toolbar" className="flex min-w-0 flex-wrap items-center gap-1"><ActionTools {...props} /></div></div><StatsPanel result={props.result} paths={props.paths} progress={props.progress} /></div>
      ...
      <div className="grid min-h-0 flex-1 gap-2 @2xl/snf:grid-cols-[minmax(250px,320px)_minmax(0,1fr)] @4xl/snf:grid-cols-[minmax(250px,320px)_minmax(0,1fr)_minmax(260px,320px)]">
        <section className="flex min-h-0 flex-col gap-2 overflow-auto rounded-lg border bg-card p-2"><ZoneTitle icon={FolderInput} label="路径和模式" /><PathInput data={props.data} disabled={props.running} onPaste={props.onPastePaths} onPatch={props.onPatch} /><ModeToggle value={props.data.mode ?? "library"} disabled={props.running} onChange={(mode) => props.onPatch({ mode })} /><SwitchPanel data={props.data} disabled={props.running} onPatch={props.onPatch} /></section>
        <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border bg-card"><div className="flex shrink-0 items-center justify-between gap-2 px-3 py-2"><ZoneTitle icon={GitCompare} label="序号计划" /><Badge variant="outline">{props.result?.items.length ?? props.paths.length}</Badge></div><Separator /><PlanRows items={props.result?.items ?? []} paths={props.paths} /></section>
        <div className="grid min-h-0 gap-2 grid-rows-[auto_minmax(0,1fr)] @2xl/snf:col-span-2 @4xl/snf:col-span-1"><ExecutionGate {...props} /><ResultTabs logs={props.logs} result={props.result} onCopyLogs={props.onCopyLogs} onCopyResults={props.onCopyResults} /></div>
      </div>
    </div>
  )
}
...
function ActionTools(props: ViewProps & { compact?: boolean }) {
  return <div className="flex min-w-0 items-center gap-1">{!props.compact && <ActionMode value={props.action} disabled={props.running} onChange={props.onActionChange} />}<IconButton disabled={props.running} active={props.configDirty} icon={DatabaseZap} label="保存默认" onClick={props.onSaveDefault} /><IconButton disabled={props.running || !props.defaults} icon={Settings2} label="恢复默认" onClick={props.onRestoreDefault} /><IconButton icon={RotateCcw} label="清空状态" onClick={props.onReset} /></div>
}

function ActionMode(props: { disabled?: boolean; value: SnfAction; onChange: (value: SnfAction) => void }) {
  return <ToggleGroup type="single" value={props.value} disabled={props.disabled} onValueChange={(value) => value && props.onChange(value as SnfAction)} className="grid grid-cols-3" size="sm">{ACTIONS.map((item) => <ToggleGroupItem key={item.value} value={item.value} className="min-w-0 gap-1"><item.icon className="size-3.5" /><span className="truncate text-xs">{item.shortLabel}</span></ToggleGroupItem>)}</ToggleGroup>
}

function ModeToggle(props: { disabled?: boolean; value: SnfMode; onChange: (value: SnfMode) => void }) {
  return <ToggleGroup type="single" value={props.value} disabled={props.disabled} onValueChange={(value) => value && props.onChange(value as SnfMode)} className="grid grid-cols-2" size="sm">{MODES.map((item) => <ToggleGroupItem key={item.value} value={item.value} className="min-w-0 gap-1"><item.icon className="size-3.5" /><span className="truncate text-xs">{item.label}</span></ToggleGroupItem>)}</ToggleGroup>
}

function PathInput(props: { compact?: boolean; data: SnfCardState; disabled?: boolean; onPaste: () => void; onPatch: (patch: Partial<SnfCardState>) => void }) {
  return <div className="grid gap-1.5">{!props.compact && <Label htmlFor="snf-paths" className="text-xs">库目录或作者目录</Label>}<div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-1.5"><Textarea id="snf-paths" aria-label="snf paths" className={cn("min-h-0 resize-none font-mono text-xs", props.compact ? "h-14" : "h-28")} disabled={props.disabled} placeholder={"每行一个目录\nD:/archives"} value={props.data.pathsText ?? ""} onChange={(event) => props.onPatch({ pathsText: event.currentTarget.value })} /><div className="grid content-start gap-1.5"><IconButton disabled={props.disabled} icon={Clipboard} label="粘贴路径" onClick={props.onPaste} /><IconButton disabled={props.disabled || !props.data.pathsText} icon={Trash2} label="清空路径" onClick={() => props.onPatch({ pathsText: "" })} /></div></div></div>
}

function SwitchPanel(props: { compact?: boolean; data: SnfCardState; disabled?: boolean; onPatch: (patch: Partial<SnfCardState>) => void }) {
  return <div className={cn("grid gap-2", props.compact ? "grid-cols-1" : "grid-cols-[repeat(auto-fit,minmax(8rem,1fr))]")}><SwitchRow checked={props.data.dryRun ?? true} disabled={props.disabled} icon={ShieldAlert} label="预览" onCheckedChange={(dryRun) => props.onPatch({ dryRun })} /><SwitchRow checked={props.data.keepTimestamp ?? true} disabled={props.disabled} icon={ListOrdered} label="保留时间" onCheckedChange={(keepTimestamp) => props.onPatch({ keepTimestamp })} /></div>
}

function ExecutionGate(props: ViewProps) {
  const live = props.action === "rename" && !(props.data.dryRun ?? true)
  return <section className={cn("flex shrink-0 flex-col gap-2 rounded-lg border bg-card p-2", live && "border-destructive/50 bg-destructive/[0.03]")}><div className="flex items-center justify-between gap-2"><ZoneTitle icon={live ? AlertTriangle : ShieldAlert} label="执行" tone={live ? "danger" : "default"} /><Badge variant={live ? "destructive" : "outline"}>{props.data.dryRun ?? true ? "预览" : "写入"}</Badge></div><ActionMode value={props.action} disabled={props.running} onChange={props.onActionChange} /><SwitchPanel data={props.data} disabled={props.running} onPatch={props.onPatch} /><RunButton props={props} /></section>
}

function RunButton({ compact, props }: { compact?: boolean; props: ViewProps }) {
  if (props.running) return <Button aria-label="snf running" disabled size={compact ? "icon-sm" : "sm"} variant="secondary"><Square />{!compact && <span>运行中</span>}</Button>
  const label = actionLabel(props.action)
  const live = props.action === "rename" && !(props.data.dryRun ?? true)
  if (live) return <AlertDialog><AlertDialogTrigger asChild><Button aria-label={label} size={compact ? "icon-sm" : "sm"} variant="destructive"><Play />{!compact && <span>{label}</span>}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>确认执行修复？</AlertDialogTitle><AlertDialogDescription>当前会重命名编号目录。请先确认路径、模式和冲突列表。</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>取消</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => props.onExecute(props.action)}>确认执行</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
...
function PlanRows(props: { items: SnfPlanItem[]; paths: string[] }) {
  if (!props.items.length) return <div className="flex min-h-32 flex-1 items-center justify-center p-4 text-center text-sm text-muted-foreground">{props.paths.length ? "运行预览后显示序号修复计划。" : "输入目录后预览序号修复计划。"}</div>
  return <ScrollArea className="min-h-0 flex-1"><div className="grid gap-1.5 p-3">{props.items.slice(0, 180).map((item, index) => { const meta = itemStatusMeta(item.status); const StatusIcon = meta.icon; return <div key={`${item.sourcePath}:${index}`} className={cn("grid gap-1 rounded-md border px-2 py-1.5", (item.status === "conflict" || item.status === "error") && "border-destructive/40", item.status === "unchanged" && "opacity-75")}><div className="flex min-w-0 items-center gap-2"><ListOrdered className="size-4 shrink-0 text-muted-foreground" /><div className="min-w-0 flex-1"><div className="truncate text-xs font-medium">{item.sourceName}</div><div className="truncate font-mono text-[11px] text-muted-foreground">{"->"} {item.targetName}</div></div><Badge variant={meta.variant} className="gap-1"><StatusIcon className="size-3" />{meta.label}</Badge></div><div className="truncate text-[11px] text-muted-foreground">{item.sequence ? `#${item.sequence}` : "未编号"}{item.reason ? ` / ${item.reason}` : ""}</div></div> })}</div></ScrollArea>
}

function ResultTabs(props: { compact?: boolean; logs: string[]; result: SnfData | null; onCopyLogs: () => void; onCopyResults: () => void }) {
  return <Tabs defaultValue="plan" className="flex h-full min-h-0 flex-col"><TabsList variant="line" className="shrink-0"><TabsTrigger value="plan">计划</TabsTrigger><TabsTrigger value="issues">问题</TabsTrigger><TabsTrigger value="logs">日志</TabsTrigger></TabsList><TabsContent value="plan" className="min-h-0 flex-1"><PlanPanel compact={props.compact} result={props.result} onCopy={props.onCopyResults} /></TabsContent><TabsContent value="issues" className="min-h-0 flex-1"><TextPanel empty="暂无问题" lines={[...(props.result?.errors ?? []), ...(props.result?.items ?? []).filter((item) => item.reason && item.status !== "ready").map((item) => `${item.sourcePath}: ${item.reason}`)]} /></TabsContent><TabsContent value="logs" className="min-h-0 flex-1"><TextPanel actionLabel="复制" empty="运行日志会显示在这里。" icon={Terminal} lines={props.logs} onAction={props.onCopyLogs} /></TabsContent></Tabs>
}

function PlanPanel(props: { compact?: boolean; result: SnfData | null; onCopy: () => void }) {
  return <section className="flex h-full min-h-0 flex-col rounded-lg border bg-card"><div className={props.compact ? "flex shrink-0 items-center justify-between gap-2 px-2 py-1.5" : "flex shrink-0 items-center justify-between gap-2 px-3 py-2"}><div className="flex min-w-0 items-center gap-2 text-xs font-medium text-muted-foreground"><GitCompare className="size-3.5" /><span>{props.result?.items.length ? `${props.result.items.length} 项` : "等待运行"}</span></div><Button disabled={!props.result?.items.length} size="xs" variant="ghost" onClick={props.onCopy}><Copy data-icon="inline-start" />复制</Button></div><Separator /><PlanRows items={props.result?.items ?? []} paths={[]} /></section>
}

function TextPanel(props: { actionLabel?: string; empty: string; icon?: LucideIcon; lines: string[]; onAction?: () => void }) {
  const Icon = props.icon
  return <section className="flex h-full min-h-0 flex-col rounded-lg border bg-card"><div className="flex shrink-0 items-center justify-between gap-2 px-3 py-2"><span className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground">{Icon && <Icon className="size-3.5" />}{props.lines.length ? `${props.lines.length} 行` : props.empty}</span>{props.onAction && <Button disabled={!props.lines.length} size="xs" variant="ghost" onClick={props.onAction}>{props.actionLabel ?? "复制"}</Button>}</div><Separator /><ScrollArea className="min-h-0 flex-1">{props.lines.length ? <pre className="p-3 text-xs leading-5 text-muted-foreground">{props.lines.join("\n")}</pre> : <div className="flex min-h-24 items-center justify-center p-4 text-sm text-muted-foreground">{props.empty}</div>}</ScrollArea></section>
}

function HeaderLine(props: { status: SnfStatusMeta; subtitle: string }) {
  const Icon = NODE_ICON
  return <div className="min-w-0"><div className="flex min-w-0 items-center gap-2"><div className={cn("grid size-8 shrink-0 place-items-center rounded-lg", props.status.iconClass)}><Icon /></div><div className="min-w-0"><div className="flex min-w-0 items-center gap-2"><h3 className="truncate text-sm font-semibold leading-none">SNF</h3><Badge variant={props.status.badgeVariant}>{props.status.label}</Badge></div><p className="mt-1 truncate text-xs text-muted-foreground">{props.subtitle}</p></div></div></div>
}

function StatsPanel(props: { paths: string[]; progress: number; result: SnfData | null }) {
  const stats = [{ label: "路径", value: props.paths.length }, { label: "作者", value: props.result?.artistCount ?? 0 }, { label: "待改", value: props.result?.readyCount ?? 0 }, { label: "已改", value: props.result?.renamedCount ?? 0 }, { label: "冲突", value: props.result?.conflictCount ?? 0 }, { label: "进度", value: props.progress, suffix: "%" }]
...
function statusFromState(data: SnfCardState, running: boolean, result: SnfData | null): SnfStatusMeta {
  if (running || data.phase === "running") return { label: "运行中", description: data.progressText || "SNF 正在扫描或修复序号。", tone: "running", badgeVariant: "secondary", iconClass: "bg-primary text-primary-foreground" }
  ...
  if (data.phase === "completed") return { label: "完成", description: data.progressText || "上次 SNF 任务已完成。", tone: "success", badgeVariant: "default", iconClass: "bg-primary text-primary-foreground" }
  return { label: "就绪", description: "输入目录后预览序号修复计划。", tone: "idle", badgeVariant: "outline", iconClass: "bg-secondary text-secondary-foreground" }
}

function itemStatusMeta(status: SnfPlanItem["status"]) {
...
function actionLabel(action: SnfAction): string {
