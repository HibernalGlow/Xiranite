...
import { ACTIONS, type ClassqAction } from "./constants"
...
export function ActionPicker(props: {
  ...
  onActionChange: (action: ClassqAction) => void
}) {
  return (
    <ToggleGroup
      aria-label="classq action"
      ...
      variant="outline"
      onValueChange={(value) => {
        if (value) props.onActionChange(value as ClassqAction)
      }}
    >
      {ACTIONS.map((item) => (
    ...
    <div className="flex min-w-0 flex-col gap-1.5">
      {!props.compact && (
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="classq-paths" className="flex items-center gap-1.5">
...
export function PathFields(props: {
  ...
  return (
    <div className="grid gap-2 @3xl/classq:grid-cols-2">
      <Input
        aria-label="classq 配置文件"
        disabled={props.disabled}
        placeholder="配置文件，可选"
      ...
      />
      <Input
        aria-label="classq Python 可执行文件"
        disabled={props.disabled}
  ...
  return (
    <div className="grid gap-2 @3xl/classq:grid-cols-2">
      <SwitchRow
        ...
        icon={Eye}
        label="预演"
        description="只生成分类计划，不真正调用 ClassQ 模块。"
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="classq 运行选项" disabled={props.disabled} size="icon-sm" variant="outline">
              <Settings2 />
  ...
  return (
    <Popover>
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="classq 默认配置" disabled={props.disabled} size="icon-sm" variant={props.configDirty ? "secondary" : "outline"}>
              <DatabaseZap />
      ...
      <PopoverContent align="end" className="w-72">
        <div className="mb-3">
          ...
          <p className="text-xs text-muted-foreground">保存 ClassQ 的路径和运行选项。</p>
        </div>
        <div className="grid gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-xl">
              <DialogHeader>
                <DialogTitle>ClassQ 配置</DialogTitle>
                <DialogDescription>当前 nodes.classq 默认值和配置文件位置。</DialogDescription>
              </DialogHeader>
...
function ConfigPreview(props: {
  config?: Partial<ClassqCardState>
  path?: string
}) {
