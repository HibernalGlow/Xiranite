...
use crate::common::cache::{
    CACHE_VERSION, extract_loaded_cache, load_cache_from_file_generalized_by_path,
...
use crate::tools::same_music::{
    GroupedFilesToCheck, Info, MusicEntry, MusicSimilarity, SameMusic, SameMusicParameters,
};
...
impl SameMusic {
    ...
    pub(crate) fn check_files(
        ...
        self.common_data
            .extensions
            .set_and_validate_allowed_extensions(AUDIO_FILES_EXTENSIONS);
    ...
    fn load_cache(
        ...
        if self.common_data.use_cache {
            let (messages, loaded_items) = load_cache_from_file_generalized_by_path::<MusicEntry>(
                &get_similar_music_cache_file(checking_tags),
    ...
    fn save_cache(
        &mut self,
        vec_file_entry: Vec<MusicEntry>,
        loaded_hash_map: BTreeMap<String, MusicEntry>,
        checking_tags: bool,
    ) {
        if !self.common_data.use_cache {
            return;
    ...
    pub(crate) fn calculate_fingerprint(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
        ...
        if self.params.compare_fingerprints_only_with_similar_titles {
            let grouped_by_title: BTreeMap<String, Vec<MusicEntry>> =
                Self::get_entries_grouped_by_title(mem::take(&mut self.music_entries));
            ...
            self.music_to_check = mem::take(&mut self.music_entries)
                .into_iter()
                .map(|e| (e.path.to_string_lossy().to_string(), e))
                .collect();
        }
    ...
    pub(crate) fn check_for_duplicate_tags(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
        ...
        if (self.params.music_similarity & MusicSimilarity::TRACK_TITLE)
        ...
        if (self.params.music_similarity & MusicSimilarity::TRACK_ARTIST)
        ...
        if self.common_data.use_reference_folders {
            self.duplicated_music_entries_referenced = self
                .common_data
                .directories
                .filter_reference_folders(mem::take(&mut self.duplicated_music_entries));
        }
    ...
    fn split_fingerprints_to_base_and_files_to_compare(
        &self,
        music_data: Vec<MusicEntry>,
    ) -> (Vec<MusicEntry>, Vec<MusicEntry>) {
        if self.common_data.use_reference_folders {
            music_data.into_iter().partition(|f| {
                self.common_data
                    .directories
                    .is_in_referenced_directory(f.get_path())
            })
        } else {
    ...
    fn get_entries_grouped_by_title(
        music_data: Vec<MusicEntry>,
    ) -> BTreeMap<String, Vec<MusicEntry>> {
    ...
    fn split_fingerprints_to_check(&mut self) -> Vec<GroupedFilesToCheck> {
        if self.params.compare_fingerprints_only_with_similar_titles {
            let entries_grouped_by_title: BTreeMap<String, Vec<MusicEntry>> =
                Self::get_entries_grouped_by_title(mem::take(&mut self.music_entries));

            entries_grouped_by_title
                .into_iter()
                .filter_map(|(_title, entries)| {
                    let (base_files, files_to_compare) =
                        self.split_fingerprints_to_base_and_files_to_compare(entries);
                    ...
                    if base_files.is_empty()
                        ...
                        || (base_files.len() == 1
                            && files_to_compare.len() == 1
                            && (base_files[0].path == files_to_compare[0].path))
                    {
                        return None;
                    }

                    Some(GroupedFilesToCheck {
                        base_files,
                        files_to_compare,
                    })
                })
        ...
        for f_entry in base_files {
            ...
            let (mut collected_similar_items, errors): (Vec<_>, Vec<_>) = files_to_compare
                ...
                .map(|e_entry| {
                    ...
                    let mut segments = match match_fingerprints(
                        &f_entry.fingerprint,
                        &e_entry.fingerprint,
                        configuration,
                    ) {
                        ...
                        Err(e) => {
                            return Some(Err(format!("Error while comparing fingerprints: {e}")));
                        }
                    };
                    segments.retain(|s| {
                        s.duration(configuration) > minimum_segment_duration
                            && s.score < maximum_difference
                    });
    ...
    pub(crate) fn check_for_duplicate_fingerprints(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
        ...
        let base_files_number = grouped_files_to_check
            .iter()
            .map(|g| g.base_files.len())
            .sum::<usize>();
        ...
        for group in grouped_files_to_check {
            let GroupedFilesToCheck {
                base_files,
                files_to_compare,
            } = group;
            let Some(temp_music_entries) = self.compare_fingerprints(
                stop_flag,
                progress_handler.items_counter(),
                base_files,
                &files_to_compare,
...
fn calc_fingerprint_helper(
    ...
    .unwrap_or_else(|_| {
        let message = create_crash_message(
            "Symphonia",
...
fn read_single_file_tags(path: &str, mut music_entry: MusicEntry) -> Option<MusicEntry> {
    ...
    let Some(tagged_file) = possible_tagged_file else {
...
fn get_simplified_name_internal(what: &str, ignore_numbers: bool) -> String {
    ...
    for character in what
        .chars()
        .map(|e| if e.is_whitespace() { ' ' } else { e })
    {
        match character {
            '(' | '[' => {
                tab_number += 1;
            ...
            ch => {
                if tab_number == 0 {
                    if ch.is_ascii_alphabetic() || (!ignore_numbers && ch.is_numeric()) {
                        ...
                        let new_items = deunicode::deunicode_char(character).map_or_else(
                            || vec![character; 1],
                            |e| e.trim().to_string().chars().collect::<Vec<_>>(),
                        );
...
mod tests {
    ...
    fn test_simplified_names() {
        ...
        for (input, expected) in cases {
            ...
            assert_eq!(
                res, expected,
                "Input: {input}, Expected: {expected}, Got: {res}"
            );
        }
    }
}
