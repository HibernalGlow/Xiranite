...
export function ActionPicker(props: {
  ...
  onActionChange: (action: PackuToolAction) => void
}) {
  return (
    <ToggleGroup
      aria-label="packu action"
      ...
      variant="outline"
      onValueChange={(value) => {
        if (value) props.onActionChange(value as PackuToolAction)
      }}
    >
      {ACTIONS.map((item) => (
    ...
    <div className="flex min-w-0 flex-col gap-1.5">
      {!props.compact && (
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="packu-paths" className="flex items-center gap-1.5">
...
export function PathFields(props: {
  ...
  return (
    <div className="grid gap-2 @3xl/packu:grid-cols-2">
      <Input
        aria-label="packu 配置文件"
        disabled={props.disabled}
        placeholder="配置文件，可选"
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="packu 运行选项" disabled={props.disabled} size="icon-sm" variant="outline">
              <Settings2 />
  ...
  return (
    <Popover>
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="packu 默认配置" disabled={props.disabled} size="icon-sm" variant={props.configDirty ? "secondary" : "outline"}>
              <DatabaseZap />
      ...
      <PopoverContent align="end" className="w-72">
        <div className="mb-3">
          ...
          <p className="text-xs text-muted-foreground">保存 PackU 的路径和运行选项。</p>
        </div>
        <div className="grid gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-xl">
              <DialogHeader>
                <DialogTitle>PackU 配置</DialogTitle>
                <DialogDescription>当前 nodes.packu 默认值和配置文件位置。</DialogDescription>
              </DialogHeader>
