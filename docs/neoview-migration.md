...
旧版 `src/lib/cards/registry.ts` 中的 77 张 Card 已冻结到 `migration/neoview/card-compatibility.json`，并由 `bun run audit:neoview-cards` 校验 source hash、逐项 feature 映射和当前 manifest 对应关系；发布阶段使用 `--require-complete` 禁止任何 Card 保持 `pending/partial`。当前 manifest 为 9 张 Card：9 张均为 `partial`，剩余 68 张待迁移。实现顺序明确以核心阅读 Card 为先：`folderMain`、`pageListMain`、`bookInfo/imageInfo/storage/time`、`historyList`、`bookmarkList`、`preloadStatus`，slideshow overlay、动画和其余设置 Card 延后。
...
| Phase 1：领域核心与契约 | 进行中（核心纵切可用） | Page/Frame/ReaderSession、自然排序、单双页基础、generation/cancel、统一 ReaderService、共享阅读进度恢复/异步合并写/关闭刷盘；ReaderLibrary 复用进度表提供最近阅读，书签/自定义列表使用规范化持久化与鉴权 HTTP 控制面 | 完整 layout、历史/书签 Card 与 CLI/TUI 命令、旧 localStorage 数据导入、统计和所有旧行为 |
...
历史/书签后端第一条纵切已遵守该边界：最近阅读不复制第二张 history 表，而是按 `xr_reader_progress.updated_at` 倒序分页；书签写入 `xr_reader_bookmarks`，自定义列表写入 `xr_reader_bookmark_lists`，多对多关系写入 `xr_reader_bookmark_memberships`。`all/default/favorites` 是 `ReaderLibraryService` 合成的系统视图，不向数据库写保留列表行；`default` 表示没有自定义列表成员关系的书签，`favorites` 由 bookmark 的 `starred` 字段筛选。书签和成员关系的复合更新使用短 `BEGIN IMMEDIATE` 事务及最多三次有界 busy retry，清理历史使用带 limit 的小批删除。HTTP 控制面仅传 JSON 元数据，位于鉴权后的 `/reader/library/recents`、`/reader/library/bookmarks` 和 `/reader/library/bookmark-lists`，不进入页面 asset/翻页热路径。当前仍未迁移旧 `neoview-history`、`neoview-bookmarks`、`neoview-bookmark-lists-v2` localStorage 数据，也未完成 GUI Card 与 CLI/TUI 命令，因此相关 feature 继续保持 partial/pending。
