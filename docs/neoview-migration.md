...
| 完整阅读工具与设置 UI | 设置浮窗、两张共享设置 Card 与首批视图工具可用 | 路径打开、前后/页码/关闭；fit mode 菜单、单双页 segmented control、缩放、旋转、reset 及 `+/-/R/0` 快捷键；fit/page mode 默认值写入 Xiranite TOML 并立即作用于同进程后续书籍；按 `setting.png` 建立独立设置浮窗，“边栏管理”和“卡片管理”由同一 Card registry 按 section 动态加载，也可显式停靠到侧栏 | 原 TopToolbar 其余功能、其余设置分类业务卡、文件浏览、历史/书签、超分、AI、benchmark、完整快捷操作 |

当前验证证据（数值随复跑波动，以预算和原始输出为准）：

- 前端 NeoView：14 个测试文件、46 项 component/hook/registry/shell/config/resize/card/settings/presentation 测试通过；后端 NeoView 最近稳定单 worker/低 worker 复跑为 41 个文件、Vitest 204 项 + Bun TUI 1 项通过，Backend 26 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖设置窗口 registry 动态 host、无书籍 session 的显式停靠设置 Card、业务 Card capability 过滤、共享 manifest 默认值、Card 放置策略、连续 board revision 与陈旧保存 `409`；
