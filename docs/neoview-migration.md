...
| 完整阅读工具与设置 UI | 设置浮窗与两张共享设置 Card 可用 | 路径打开、前后/页码/关闭等 smoke controls；按 `setting.png` 建立独立大尺寸设置浮窗、左侧分类导航；“边栏管理”和“卡片管理”已是可复用 registry Card | 原 TopToolbar 全功能、其余设置分类业务卡、文件浏览、历史/书签、超分、AI、benchmark、快捷操作 |

当前验证证据（数值随复跑波动，以预算和原始输出为准）：

- 前端 NeoView：13 个测试文件、39 项 component/hook/registry/shell/config/resize/card/settings/Kanban 测试通过；后端 NeoView 最近完整复跑为 38 个文件、Vitest 185 项 + Bun TUI 1 项通过，Backend 26 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖 40 次 pointermove 零回调、pointerup 单次提交、50px 下限、cancel 恢复、折叠零 resize DOM，以及 `height = "auto"` 覆盖旧 v14 数字高度；
- Reader pipeline 最近复跑：backend startup `17.01 ms`、open `53.50 ms`、navigation p95 `4.35 ms`、response headers p95 `2.39 ms`、first byte p95 `2.48 ms`、full stream p50 `39.91 ms`、throughput p50 `71.13 MiB/s`、RSS delta `1.84 MiB`；全部通过预算，Card 高度调整没有进入图片数据热路径；
- 真实 Chromium 路径已覆盖桌面与 `420×360`：Card 连续 40 次 move 期间零 PATCH，pointerup 仅一次鉴权 PATCH/TOML 原子写入，双击后以显式 `height: null` 恢复自动高度；拖动、写回和 reset 均不重挂原图 `<img>`；最终热翻页为 desktop `41.8/61.0 ms`、card `28.7/51.3 ms`（navigation/decode），仍明显低于 `150/200 ms` 硬门槛；
- 当前生产构建报告：NeoView entry `32,557 bytes`，ReaderSidebar `8,353 bytes`，设置浮窗 `3,272 bytes`，SidebarManagementSettingsCard `5,942 bytes`，PanelLayoutEditor `3,903 bytes`，Dice UI Kanban/dnd-kit runtime `58,547 bytes`；Card resize 仍只随 ReaderSidebar 延迟加载，Kanban 仍为“卡片管理”二级延迟 chunk，未进入初始 chunk或 NeoView entry；PageNavigationCard `1,233 bytes`、BookInformationCard `1,015 bytes`，首屏 NeoView module `0`、前端 zip.js module `0`。
