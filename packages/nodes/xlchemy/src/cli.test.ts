...
describe("Xlchemy pipe CLI", () => {
  test("maps complete format, file, metadata, resize, RAM, and custom argument flags", () => {
    const input = buildPipeInput([
      "convert", "D:/images", "--format", "avif", "--avif-encoder", "slimg", "--slimg-backend", "cli", "--avif-bit-depth", "10", "--quality", "72", "--threads", "12",
  ...
  test("accepts documented short format aliases and both modern and legacy TOML keys", () => {
    ...
    expect(buildPipeInput(["convert", "D:/a.webp", "--format", "dynar", "--no-skip-animated"])).toMatchObject({ format: "dynar", animationDetectionFormats: ["webp"] })
