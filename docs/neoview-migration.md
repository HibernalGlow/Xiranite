...
- 小 entry 现在在同一个 `SolidArchiveMaterializer` 内使用现有 `lru-cache` 做有界内存 tier：默认 32 MiB、单 entry 默认 8 MiB；首次从已校验 temp file 读取，后续请求直接返回内存流，超出预算或需要路径的嵌套场景透明回退到 temp file。内存加载按 fingerprint 所属 materializer 生命周期清理，caller 取消不会中止共享读取，关闭/失效会清空 tier；不增加 extractor、CAS 或第二个 SQLite。
