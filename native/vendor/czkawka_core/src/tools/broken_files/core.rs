...
use crate::common::consts::{
    AUDIO_FILES_EXTENSIONS, IMAGE_RS_BROKEN_FILES_EXTENSIONS, PDF_FILES_EXTENSIONS,
    ZIP_FILES_EXTENSIONS,
};
...
use crate::tools::broken_files::{
    BrokenEntry, BrokenFiles, BrokenFilesParameters, CheckedTypes, Info, TypeOfFile,
};
...
impl BrokenFiles {
    ...
    pub(crate) fn check_files(
        ...
        let images_extensions = IMAGE_RS_BROKEN_FILES_EXTENSIONS
            .iter()
            .collect::<HashSet<_>>();
        ...
        self.common_data
            .extensions
            .set_and_validate_allowed_extensions(&extensions);
            ...
            DirTraversalResult::SuccessFiles {
                ...
                self.files_to_check = grouped_file_entries
                    ...
                    .map(|fe| {
                        ...
                        (
                            broken_entry.path.to_string_lossy().to_string(),
                            broken_entry,
                        )
                    })
                    .collect();
                ...
                debug!(
                    "check_files - Found {} files to check.",
    ...
    fn check_broken_image(&self, mut file_entry: BrokenEntry) -> BrokenEntry {
        ...
        .unwrap_or_else(|_| {
            let message = create_crash_message(
                "Image-rs",
                &file_entry_clone.path.to_string_lossy(),
    ...
    fn load_cache(
        ...
        if self.common_data.use_cache {
            let (messages, loaded_items) = load_cache_from_file_generalized_by_path::<BrokenEntry>(
                &get_broken_files_cache_file(),
    ...
    pub(crate) fn look_for_broken_files(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
    ...
    fn save_to_cache(
        &mut self,
        vec_file_entry: &[BrokenEntry],
        loaded_hash_map: BTreeMap<String, BrokenEntry>,
    ) {
        if self.common_data.use_cache {
...
fn check_extension_availability(
    ...
    let Some(extension_idx) = file_name_str.rfind('.') else {
        return TypeOfFile::Unknown;
    };
    ...
    if images_extensions.contains(&extension_lowercase.as_str()) {
        ...
        error!(
            "File with unknown extension: \"{}\" - {extension_lowercase}",
            full_name.to_string_lossy()
        );
        debug_assert!(
            false,
