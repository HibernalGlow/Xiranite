...
export function createMelodeckInteractionSchema(defaults: Partial<Values> = {}, language: TerminalLanguage = "zh"): TerminalInteractionSchema<MelodeckInput, MelodeckResult> {
  ...
  const labels: Record<MelodeckAction, string> = {
    status: zh ? "状态" : "Status",
    play: zh ? "播放" : "Play",
    pause: zh ? "暂停" : "Pause",
    toggle: zh ? "播放/暂停" : "Play/Pause",
    stop: zh ? "停止" : "Stop",
    next: zh ? "下一首" : "Next",
  ...
  return {
    id: "melodeck",
    title: "Melodeck",
    description: zh ? "基于 mpv 的本地音乐播放器" : "Local music player backed by mpv",
    initialValues,
    fields: [
    ...
    toInput: (values) => ({
      action: values.action as MelodeckAction,
      paths: String(values.paths ?? "").split(/[\r\n;]+/).map((path) => path.trim()).filter(Boolean),
      volume: Number(values.volume ?? 80),
      seekSeconds: Number(values.seekSeconds ?? 0),
      mpvPath: String(values.mpvPath ?? "").trim() || undefined,
      ipcPath: String(values.ipcPath ?? "").trim() || undefined,
    }),
    validate: (_values, input) => {
      if ((input.action === "play" || input.action === "add") && !input.paths?.length) return zh ? "至少输入一个音频文件。" : "Provide at least one audio file."
      if (input.action === "seek" && (!Number.isFinite(input.seekSeconds) || input.seekSeconds === 0)) return zh ? "跳转秒数不能为 0。" : "Seek offset must be non-zero."
      return null
    },
    preview: (input) => [labels[input.action ?? "status"], `${input.paths?.length ?? 0} track(s)`, `Volume ${input.volume ?? 80}%`],
    isDangerous: () => false,
    result: (result) => {
      ...
      return {
        ...
        lines: status ? [
          status.running ? (status.paused ? "Paused" : "Playing") : "Stopped",
          status.title || status.path || "Melodeck",
          `${formatTime(status.position)} / ${formatTime(status.duration)} | ${Math.round(status.volume)}%`,
          ...status.playlist.slice(0, 4),
        ] : [],
      }
    },
  }
