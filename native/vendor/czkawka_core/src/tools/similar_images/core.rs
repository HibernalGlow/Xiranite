...
use crate::common::cache::{
    CACHE_IMAGE_VERSION, extract_loaded_cache, load_cache_from_file_generalized_by_path,
    save_cache_to_file_generalized,
};
use crate::common::consts::{
    HEIC_EXTENSIONS, IMAGE_RS_SIMILAR_IMAGES_EXTENSIONS, JXL_IMAGE_EXTENSIONS, RAW_IMAGE_EXTENSIONS,
};
use crate::common::dir_traversal::{
    DirTraversalBuilder, DirTraversalResult, inode, take_1_per_inode,
};
...
use crate::tools::similar_images::{
    Hamming, ImHash, ImagesEntry, SIMILAR_VALUES, SimilarImages, SimilarImagesParameters,
    SimilarityPreset,
};
...
impl SimilarImages {
    ...
    pub(crate) fn check_for_similar_images(
        &mut self,
        ...
        if cfg!(feature = "heif") {
            self.common_data
                .extensions
                .set_and_validate_allowed_extensions(
                    &[
                        IMAGE_RS_SIMILAR_IMAGES_EXTENSIONS,
                        RAW_IMAGE_EXTENSIONS,
                        JXL_IMAGE_EXTENSIONS,
                        HEIC_EXTENSIONS,
                    ]
                    .concat(),
                );
        ...
        match result {
            DirTraversalResult::SuccessFiles {
                ...
                self.images_to_check = grouped_file_entries
                    .into_par_iter()
                    .flat_map(if self.get_params().ignore_hard_links {
                        |(_, fes)| fes
                    } else {
                        take_1_per_inode
                    })
    ...
    fn hash_images_load_cache(
        &mut self,
    ) -> (
        BTreeMap<String, ImagesEntry>,
        BTreeMap<String, ImagesEntry>,
        BTreeMap<String, ImagesEntry>,
    ) {
        let loaded_hash_map;
        ...
        if self.common_data.use_cache {
            ...
            self.get_text_messages_mut()
                .extend_with_another_messages(messages);
            ...
            debug!(
                ...
                format_size(
                    records_already_cached.values().map(|e| e.size).sum::<u64>(),
                    BINARY
                ),
            );
        } else {
    ...
    pub(crate) fn hash_images(
        ...
        let (mut vec_file_entry, errors): (Vec<ImagesEntry>, Vec<String>) =
            non_cached_files_to_check
                .into_par_iter()
                .map(|(_s, file_entry)| {
        ...
        for file_entry in &vec_file_entry {
            ...
            if !(file_entry.hash.is_empty()
                || file_entry.hash.iter().all(|e| *e == 0)
                || file_entry.hash.iter().all(|e| *e == 255))
            {
                self.image_hashes
    ...
    fn save_to_cache(
        &mut self,
        vec_file_entry: Vec<ImagesEntry>,
        loaded_hash_map: BTreeMap<String, ImagesEntry>,
    ) {
        if self.common_data.use_cache {
    ...
    fn collect_image_file_entry(&self, mut file_entry: ImagesEntry) -> Result<ImagesEntry, String> {
        ...
        let hasher_config = HasherConfig::new()
            .hash_size(
                self.get_params().hash_size as u32,
                self.get_params().hash_size as u32,
            )
    ...
    fn split_hashes(
        ...
        if self.common_data.use_reference_folders {
            let mut files_from_referenced_folders: HashMap<ImHash, Vec<ImagesEntry>> =
                HashMap::new();
    ...
    fn compare_hashes_with_non_zero_tolerance(
        ...
        for chunk in base_hashes_chunks {
            let partial_results = chunk
                ...
                .map(|hash_to_check| {
                    ...
                    let mut found_items = self
                        ...
                        .filter(|(similarity, compared_hash)| {
                            *similarity != 0
                                && !hashes_parents.contains_key(*compared_hash)
                                && !hashes_with_multiple_images.contains(*compared_hash)
                        })
                        .filter(|(similarity, compared_hash)| {
                            if let Some((_, other_similarity_with_parent)) =
                                hashes_similarity.get(*compared_hash)
                            {
                ...
                .while_some()
                .filter(|(original_hash, vec_similar_hashes)| {
                    !vec_similar_hashes.is_empty()
                        || hashes_with_multiple_images.contains(*original_hash)
                })
                .collect::<Vec<_>>();
    ...
    fn connect_results(
        ...
        for (original_hash, vec_compared_hashes) in partial_results {
            ...
            for (similarity, compared_hash) in vec_compared_hashes {
                ...
                if let Some((current_parent_hash, current_similarity_with_parent)) =
                    hashes_similarity.get(original_hash)
                {
                    if *current_similarity_with_parent > similarity {
                        need_to_check = true;

                        *hashes_parents
                            .get_mut(current_parent_hash)
                            .expect("Cannot find parent hash") -= 1;
                        if hashes_parents.get(current_parent_hash) == Some(&0)
                            && !hashes_with_multiple_images.contains(current_parent_hash)
                        {
                            hashes_parents.remove(current_parent_hash);
                        }
                        hashes_similarity.remove(original_hash).expect(
                            "This should never fail, because we are iterating over this hash",
                        );
                    }
                } else {
                    need_to_check = true;
                ...
                if need_to_add {
                    hashes_similarity
                        .insert(compared_hash.clone(), (original_hash.clone(), similarity));
    ...
    pub(crate) fn find_similar_hashes(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
        ...
        } else if self.compare_hashes_with_non_zero_tolerance(
            &all_hashed_images,
            &mut collected_similar_images,
            progress_sender,
            stop_flag,
            tolerance,
    ...
    #[fun_time(
        message = "remove_multiple_records_from_reference_folders",
        level = "debug"
    )]
            ...
            self.similar_referenced_vectors = mem::take(&mut self.similar_vectors)
                ...
                .filter_map(|vec_file_entry| {
                    let (mut files_from_referenced_folders, normal_files): (Vec<_>, Vec<_>) =
                        vec_file_entry.into_iter().partition(|e| {
                            self.common_data
                                .directories
                                .is_in_referenced_directory(e.get_path())
                        });
    ...
    pub(crate) fn verify_duplicated_items(
        &self,
        collected_similar_images: &HashMap<ImHash, Vec<ImagesEntry>>,
    ) {
...
pub fn get_string_from_similarity(similarity: &u32, hash_size: u8) -> String {
    ...
    if *similarity == 0 {
        ...
        panic!(
            "Invalid similarity value {similarity} for hash size {hash_size} (index {index_preset})"
        );
    }
}

pub fn return_similarity_from_similarity_preset(
...
pub fn get_similar_images_cache_file(
    hash_size: &u8,
    hash_alg: &HashAlg,
...
mod tests {
    ...
    use crate::tools::similar_images::{
        Hamming, ImHash, ImagesEntry, SimilarImages, SimilarImagesParameters,
    };
    ...
    fn test_compare_tolerance_0_normal_mode() {
        for _ in 0..100 {
            ...
            add_hashes(
                &mut similar_images.image_hashes,
                vec![
                    fe1.clone(),
                    fe2.clone(),
                    fe3.clone(),
            ...
            let first_group = similar_images.get_similar_images()[0]
                .iter()
                .map(|e| &e.path)
                .collect::<Vec<_>>();
            let second_group = similar_images.get_similar_images()[1]
                .iter()
                .map(|e| &e.path)
                .collect::<Vec<_>>();
    ...
    fn test_simple_referenced_same_group() {
        for _ in 0..100 {
            ...
            similar_images.common_data.directories.reference_directories =
                vec![PathBuf::from("/home/rr/")];
    ...
    fn test_simple_normal_union_of_similarity() {
        for _ in 0..100 {
            ...
            if res[0].len() == 3 {
                ...
                assert!(
                    path == vec!["abc.txt".to_string(), "bcd.txt".to_string()]
                        || path == vec!["bcd.txt".to_string(), "rrd.txt".to_string()]
                );
            } else {
    ...
    fn test_reference_similarity_only_one() {
        for _ in 0..100 {
            ...
            let fe1 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b0001], "/home/rr/abc.txt");
            let fe2 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b0011], "/home/kk/bcd.txt");
    ...
    fn test_reference_too_small_similarity() {
        for _ in 0..100 {
            ...
            let fe1 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b0001], "/home/rr/abc.txt");
            let fe2 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b0010], "/home/kk/bcd.txt");
    ...
    fn test_reference_minimal() {
        for _ in 0..100 {
            ...
            let fe2 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b0011], "/home/kk/bcd.txt");
            let fe3 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b0100], "/home/kk/bcd2.txt");
            let fe4 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b1100], "/home/rr/krkr.txt");
    ...
    fn test_reference_union() {
        for _ in 0..100 {
            ...
            let fe0 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b1000], "/home/rr/abc2.txt");
            let fe1 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b0001], "/home/rr/abc.txt");
            let fe2 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b1110], "/home/kk/bcd.txt");
            let fe3 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b0100], "/home/kk/bcd2.txt");
            let fe4 =
                create_random_file_entry(vec![1, 1, 1, 1, 1, 1, 1, 0b1100], "/home/rr/krkr.txt");
    ...
    fn test_tolerance() {
        ...
        bktree.add(fe1);
        let (similarity, _hash) = bktree
            .find(&fe2, 100)
            .next()
            .expect("No similar images found");
