...
Q11 验证证据（2026-07-15）：Windows x64 Release API v4 `.node` 为 24.45 MiB，`dav1d.dll` 为 1.70 MiB，二者的发布 ZIP 为 9.37 MiB；Bun smoke 返回 API v4 / Czkawka 10.0.0，覆盖 duplicate、basic、image/video media 三类入口，并真实观测 `CollectingFiles` 进度、取消和 session 清理。构建直接复用仓库 `native/vendor/dav1d-windows-x64.zip` 中的开发包，通过 `PKG_CONFIG_PATH` 定位 `dav1d.pc`，不安装 VCPKG；Windows 构建使用 thin-LTO 与 8 codegen units，避免 full-LTO 的 LLVM 内存峰值。预编译 ZIP 与 SHA-256 manifest 已由 `packages/czkawka-native/prebuilt/win32-x64` 固化，embedded smoke 也成功加载 API v4。
