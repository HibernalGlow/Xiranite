...
表格型、文件清单型、画廊型节点应参考数据库组件的显示状态和调节方式，而不是把布局写死：

- 每行显示数量、密度、是否显示路径/元数据/缩略图/状态徽标等，应尽量做成用户可调设置。
- 默认值可以随容器尺寸自动变化，但用户应能覆盖，例如 1-6 列、紧凑/舒适密度、显示/隐藏路径。
- 调节入口应贴近对应视图，例如画廊工具条、Popover 或局部设置区；不要为此额外增加常驻侧栏。
- 调节项本身也要遵循空间策略：宽屏可用图标按钮 + Tooltip，紧凑态用 Popover，避免把两行工具栏挤出来。
- 长列表、长日志和大批量结果可以局部滚动；核心开关和当前显示状态应留在可见区域。
...
### 7.1 语义切换必须使用项目官方 Tabs

节点中的互斥视图、工作模式和流程阶段（例如“扫描 / 预览 / 改名”“库目录 / 单个作者”“计划 / 问题 / 日志”）必须使用项目共享的 `Tabs`、`TabsList`、`TabsTrigger` 组合；不得用 `Button` 组、`ToggleGroup`、自定义 `div` 加 active class 或手工下划线伪装成 Tab。

- 每一组切换器只保留一个信息源和一个渲染位置；执行按钮应紧邻其对应的流程 Tab，不能在顶栏和执行区重复一套动作按钮。
- 选择官方 Tabs 的 `default` 或 `line` 变体时，应以该组的导航语义和当前主题为准，不能通过覆盖背景、边框或伪元素把组件改回按钮外观。
- Tab 触发器必须位于 `TabsList` 内，并提供能说明这组切换含义的可访问名称；窄尺寸可压缩文字但不得替换为手搓控件。
- 配置入口使用共享 `NodeConfigPopover`，统一提供保存、恢复、重新读取、查看和打开配置文件的流程；节点只负责声明可持久化字段。

### 8. 参考 aestivus，但必须为 Xiranite 重构布局

迁移节点时可以参考 `D:\1VSCODE\Projects\aestivus\src` 已有实现，尤其是原节点的功能语义、状态拆分、组件用途、关键按钮、日志/预览/结果块，以及用户已经习惯的操作顺序。
...
- 空间不足时，优先把可被图标准确表达的动作压缩为纯图标按钮，并配套 `Tooltip`、`aria-label` 和 `sr-only` 文本；不要让两行工具栏挤占核心内容。
- 图标化只适合明确动作，例如启动、扫描、筛选、复制、重置、导出、删除、打开配置。含义不明确或高风险的操作仍要保留确认文案。
- 开关项默认只常驻“图标 + 短名称 + switch/checkbox”。解释性长文案放到 `Info` 图标的 Tooltip、Popover 或详情区中，不要让“预演模式”“复制模式”这类说明在主界面长期占两行高度。宽屏可以保留名称，窄屏可以只保留图标、短标签和悬停说明。
...
每个重写节点优先使用便当矩阵截图一次性验证多个断点，避免一张张手动截图：

- collapsed：极短/极窄信息条，必须保留状态、摘要和完整操作入口。
- compact：普通小卡片，常用操作直接可用，低频参数进入 Popover。
- portrait：偏手机竖屏/窄高卡片，日志、列表、结果等长内容应自然下沉到下方。
- expanded：大卡片，完整表单、预览、日志和结果分区显示。

推荐命令：

```bash
bun run qa:card -- <node-id> matrix --screenshot
```
