...
| 设置与数据迁移 | 部分完成 | LegacySettingsCodec、共享 ReaderSettingsMigrationService、CLI 与鉴权 HTTP legacy inspect/import；当前 `[nodes.neoview]` 的 Xiranite/NeoViewConfig v1 portable 导出/回读；确认/备份/跨进程锁/原子写入；Reader runtime 同一快照解析阅读默认值、Shell 与旧 panel layout | GUI/TUI 确认界面、自动备份、Reader SQLite 业务数据联合备份、Gist transport |
