...
The first benchmark pass does not justify another Bun process for common workloads. On the test host, the steady XLchemy-only sample kept event-loop p99 and health p95 within roughly one millisecond of its idle baseline after slimg stopped multiplying scheduler grants. A measured combined pass reported 11.7 ms event-loop p99 and 15.4 ms health p95. A Reader-only CPU profile attributed self time to zip.js codec and CRC work, but the measured workload event-loop p99 remained 9.55 ms. Repeated 16 MiB `lz4js` runs reported 30.43, 38.93, and 24.62 ms p99, so they did not cross the relative latency gate three consecutive times; smaller 4 MiB output stayed below 10 ms.
