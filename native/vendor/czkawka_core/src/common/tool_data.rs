...
impl<T: ResultEntry + Sized + Send + Sync> DeleteItemType<T> {
    fn calculate_size_to_delete(&self) -> u64 {
        match &self {
            Self::DeletingFiles(items) | Self::DeletingFolders(items) => {
    ...
    fn calculate_entries_to_delete(&self) -> usize {
        match &self {
            ...
            Self::HardlinkingFiles(items) => {
                items.iter().map(|(_original, files)| files.len()).sum()
            }
        }
    }
}
...
pub trait CommonData {
    ...
    fn set_reference_directory(&mut self, reference_directory: Vec<PathBuf>) {
        let messages = self
            .get_cd_mut()
            .directories
            .set_reference_directory(&reference_directory);
        self.get_cd_mut()
            .text_messages
    ...
    fn set_exclude_other_filesystems(&mut self, exclude_other_filesystems: bool) {
        self.get_cd_mut()
            .directories
            .set_exclude_other_filesystems(exclude_other_filesystems);
    }
    ...
    fn set_included_directory(&mut self, included_directory: Vec<PathBuf>) {
        let messages = self
            .get_cd_mut()
            .directories
            .set_included_directory(included_directory);
        self.get_cd_mut()
            .text_messages
            .extend_with_another_messages(messages);
    }
    ...
    fn set_excluded_directory(&mut self, excluded_directory: Vec<PathBuf>) {
        let messages = self
            .get_cd_mut()
            .directories
            .set_excluded_directory(excluded_directory);
        self.get_cd_mut()
            .text_messages
            .extend_with_another_messages(messages);
    }
    ...
    fn set_excluded_items(&mut self, excluded_items: Vec<String>) {
        let messages = self
            .get_cd_mut()
            .excluded_items
            .set_excluded_items(excluded_items);
        self.get_cd_mut()
            .text_messages
            .extend_with_another_messages(messages);
    }
    ...
    fn prepare_items(&mut self) {
        ...
        let messages = self
            .get_cd_mut()
            .directories
            .optimize_directories(recursive_search);
        self.get_cd_mut()
            .text_messages
            .extend_with_another_messages(messages);
    }
    ...
    fn delete_advanced_elements_and_add_to_messages<
        T: ResultEntry + Sized + Send + Sync + Clone,
    >(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
        ...
        let sorting_by_size = matches!(
            delete_method,
            DeleteMethod::AllExceptBiggest
                | DeleteMethod::AllExceptSmallest
                | DeleteMethod::OneBiggest
                | DeleteMethod::OneSmallest
        );
        let sort_items = |mut input: Vec<T>| -> Vec<T> {
            input.sort_unstable_by_key(if sorting_by_size {
                ResultEntry::get_size
            } else {
                ResultEntry::get_modified_date
            });
            input
        };
        ...
        let delete_results = if delete_method == DeleteMethod::HardLink {
            ...
            self.delete_elements(
                stop_flag,
                progress_sender,
                DeleteItemType::HardlinkingFiles(res),
            )
        } else {
            let res = files_to_process
                .into_iter()
                .flat_map(|values| {
                    ...
                    match delete_method {
                        ...
                        DeleteMethod::HardLink | DeleteMethod::None => {
                            unreachable!("HardLink and None should be handled before")
                        }
                    }
                    .to_vec()
                })
            ...
            self.delete_elements(
                stop_flag,
                progress_sender,
                DeleteItemType::DeletingFiles(res),
            )
        };
    ...
    fn delete_elements<T: ResultEntry + Sized + Send + Sync>(
        ...
        let delayed_sender =
            progress_sender.map(|e| DelayedSender::new(e.clone(), Duration::from_millis(200)));
        ...
        let res = match delete_item_type {
            DeleteItemType::DeletingFiles(ref items)
            | DeleteItemType::DeletingFolders(ref items) => items
                .into_par_iter()
                .map(|e| {
                    if stop_flag.load(std::sync::atomic::Ordering::Relaxed) {
                        return None;
                    ...
                    progress_tmp.bytes_checked = bytes_processed
                        .fetch_add(e.get_size(), std::sync::atomic::Ordering::Relaxed);
                    progress_tmp.entries_checked =
                    ...
                    {
                        fs::remove_file(e.get_path()).map_err(|err| {
                            format!(
            ...
            DeleteItemType::HardlinkingFiles(ref items) => items
                ...
                .map(|(original, files)| {
                    ...
                    let mut progress_tmp = progress;
                    progress_tmp.bytes_checked = bytes_processed.fetch_add(
                        files.iter().map(|e| e.get_size()).sum(),
    ...
    fn debug_print_common(&self) {
        ...
        println!(
            "Warnings size - {}",
            self.get_cd().text_messages.warnings.len()
        );
        println!(
            "Messages size - {}",
            self.get_cd().text_messages.messages.len()
        );
    }
}
