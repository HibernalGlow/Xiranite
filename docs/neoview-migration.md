...
| Panel/Card | 兼容布局、折叠、Card 高度与设置态管理纵切 | Panel/Card 默认值和能力约束已收口为节点包纯 TS manifest，后端默认配置/校验与 React lazy loader 共用；关闭/折叠后内容 DOM=0；旧布局/card v14 经 TOML 规范化；新版状态写入 canonical `panel_state/card_state`；全量 board 携带 `expectedRevision`，后端在串行写队列内以 `409` 拒绝陈旧快照；固定 Card 不能隐藏，未实现的 floating/bottom host 不能接收已知 Card；Card 高度拖动仅用 ref/DOM style，pointerup 单次 PATCH；管理 UI 仍按需加载 Dice UI Kanban | 其余全部业务卡片、设置页统一动态 registry host、无 session 设置 Card、Panel 高级组件设置、独立 card window/tab |
| 完整阅读工具与设置 UI | 设置浮窗、两张共享设置 Card 与首批视图工具可用 | 路径打开、前后/页码/关闭；fit mode 菜单、单双页 segmented control、缩放、旋转、reset 及 `+/-/R/0` 快捷键；fit/page mode 默认值写入 Xiranite TOML 并立即作用于同进程后续书籍；按 `setting.png` 建立独立设置浮窗，“边栏管理”和“卡片管理”已是可复用 registry Card | 原 TopToolbar 其余功能、其余设置分类业务卡、文件浏览、历史/书签、超分、AI、benchmark、完整快捷操作 |

当前验证证据（数值随复跑波动，以预算和原始输出为准）：

- 前端 NeoView：14 个测试文件、44 项 component/hook/registry/shell/config/resize/card/settings/presentation 测试通过；后端 NeoView 单 worker/低 worker 完整复跑为 41 个文件、Vitest 204 项 + Bun TUI 1 项通过，Backend 26 项通过；React Compiler boundary audit、typecheck、迁移漂移和节点架构检查均通过；新增覆盖共享 manifest 默认值、Card 放置策略、连续 board revision、陈旧保存 `409`、前端冲突刷新，以及旧 `fitHeight`/单双页默认值兼容；
