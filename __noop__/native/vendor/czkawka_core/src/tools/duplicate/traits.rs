...
impl DeletingItems for DuplicateFinder {
    ...
    fn delete_files(
        ...
        self.delete_advanced_elements_and_add_to_messages(
            stop_flag,
            progress_sender,
            files_to_delete,
        )
        ...
        match self.get_params().check_method {
            CheckingMethod::Name => {
                self.common_data.stopped_search =
                    self.check_files_name(stop_flag, progress_sender) == WorkContinueStatus::Stop;
                if self.common_data.stopped_search {
                    return;
                }
            }
            CheckingMethod::SizeName => {
                self.common_data.stopped_search = self
                    .check_files_size_name(stop_flag, progress_sender)
            ...
            CheckingMethod::Size => {
                self.common_data.stopped_search =
                    self.check_files_size(stop_flag, progress_sender) == WorkContinueStatus::Stop;
                if self.common_data.stopped_search {
                    return;
                }
            }
            CheckingMethod::Hash => {
                ...
                self.common_data.stopped_search =
                    self.check_files_hash(stop_flag, progress_sender) == WorkContinueStatus::Stop;
                if self.common_data.stopped_search {
                    return;
                }
            }
...
impl DebugPrint for DuplicateFinder {
    ...
    fn debug_print(&self) {
        ...
        println!(
            ...
            self.information.number_of_duplicated_files_by_name,
            self.information.number_of_groups_by_name
        );
        println!(
        ...
        println!(
            "Hashed Files list size - {}",
            self.files_with_identical_hashes.len()
        );
        println!(
            "Files with identical names - {}",
            self.files_with_identical_names.len()
        );
        println!(
            "Files with identical size names - {}",
            self.files_with_identical_size_names.len()
        );
        println!(
            "Files with identical names referenced - {}",
            self.files_with_identical_names_referenced.len()
        );
        println!(
            "Files with identical size names referenced - {}",
            self.files_with_identical_size_names_referenced.len()
        );
        println!(
            "Files with identical size referenced - {}",
            self.files_with_identical_size_referenced.len()
        );
        println!(
            "Files with identical hashes referenced - {}",
            self.files_with_identical_hashes_referenced.len()
        );
...
impl PrintResults for DuplicateFinder {
        ...
        match self.get_params().check_method {
            CheckingMethod::Name => {
                if !self.files_with_identical_names.is_empty() {
                    ...
                    for (name, (file_entry, vector)) in
                        self.files_with_identical_names_referenced.iter().rev()
                    {
                        writeln!(writer, "Name - {} - {} files ", name, vector.len())?;
            ...
            CheckingMethod::SizeName => {
                if !self.files_with_identical_names.is_empty() {
                    ...
                    writeln!(
                        ...
                        self.information.number_of_duplicated_files_by_size_name,
                        self.information.number_of_groups_by_size_name,
                    )?;
                    ...
                    for ((size, name), (file_entry, vector)) in
                        self.files_with_identical_size_names_referenced.iter().rev()
                    {
                        writeln!(
                            writer,
                            "Name - {}, {} - {} files ",
            ...
            CheckingMethod::Size => {
                if !self.files_with_identical_size.is_empty() {
                    ...
                    for (size, (file_entry, vector)) in
                        self.files_with_identical_size_referenced.iter().rev()
                    {
                        writeln!(
                            writer,
                            "\n---- Size {} ({}) - {} files",
            ...
            CheckingMethod::Hash => {
                if !self.files_with_identical_hashes.is_empty() {
                    ...
                    for (size, vectors_vector) in
                        self.files_with_identical_hashes_referenced.iter().rev()
                    {
    ...
    fn save_results_to_file_as_json(&self, file_name: &str, pretty_print: bool) -> io::Result<()> {
        if self.get_use_reference() {
            match self.get_params().check_method {
                CheckingMethod::Name => self.save_results_to_file_as_json_internal(
                    file_name,
                    &self.files_with_identical_names_referenced,
                    pretty_print,
                ),
                CheckingMethod::SizeName => self.save_results_to_file_as_json_internal(
                    file_name,
                    &self
                ...
                CheckingMethod::Size => self.save_results_to_file_as_json_internal(
                    file_name,
                    &self.files_with_identical_size_referenced,
                    pretty_print,
                ),
                CheckingMethod::Hash => self.save_results_to_file_as_json_internal(
                    file_name,
...
impl CommonData for DuplicateFinder {
    ...
    fn found_any_broken_files(&self) -> bool {
        self.get_information().number_of_duplicated_files_by_hash > 0
            ...
            || self
                .get_information()
                .number_of_duplicated_files_by_size_name
                > 0
    }
}
