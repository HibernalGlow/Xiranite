...
方向预读由独立 `useReaderImagePreloader` 消费 `ReaderSession.preload`：React 按后端 direction/tier/admission 候选顺序取得原始 URL 并调用浏览器 `Image.decode()`，每代最多保留 4 张；paused admission、session/generation 切换和卸载会取消或释放旧 URL，无 plan 的旧 transport 才回退相邻页。它不创建隐藏 React 页面树，不生成 Blob/Base64，不调用 sharp。Image 生命周期按 session+generation 以 50ms、每批最多 64 条上报 TTFB、decode duration、retained bytes 和 active leases，普通翻页不写 React state，也不新增 viewport-context PATCH。真实 `2000×3000` 高熵 JPEG CBZ 的 Chromium E2E 同时验证 asset URL 不含 transform 参数、热翻页 decode 门槛、桌面/卡片视口和零 Canvas；显式 transform route 只保留给浏览器不支持格式、用户主动转换和导出。`xneoview preload-plan <path>` 直接投影同一 Headless snapshot，OpenTUI 在页面列表标记 Near/Ahead/Background 并显示 generation/admission 计数；二者不另开页面流、不复制 scheduler，也不会让终端投机解码抢占当前翻页。
