...
`inspect/pages/frame/extract-page` 现可通过 `--connect <loopback-url>` 使用 `RemoteReaderHeadlessController` 复用常驻 XR 的唯一 Reader HTTP controller。adapter 仅持有自己创建的 opaque session，分页固定 `thumbnails=0`，原图直接消费受保护 asset stream，命令结束必发 DELETE；不会在 CLI 进程中再次打开 `thumbnails.db`、L3 cache、archive provider 或 scheduler。URL 当前硬限制为 loopback，token 只从 `XIRANITE_BACKEND_TOKEN` 或 `--token-env` 指定的环境变量进入 `x-xiranite-token` header，不提供明文 argv/query 参数。配置、数据库维护、导入、文件操作和 diagnostics 尚未声明远程契约，带 `--connect` 时必须明确拒绝，不能静默回退本地执行。
