    ...
    <div className="flex min-h-0 min-w-0 flex-col gap-1.5">
      {!props.compact && (
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="crashu-sources">源目录</Label>
      ...
      <div className="grid min-h-0 min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-1.5">
        ...
        <div className="grid content-start gap-1.5">
          <ActionIconButton disabled={props.disabled} icon={Clipboard} label="粘贴源目录" onClick={props.onPaste} />
          <ActionIconButton disabled={props.disabled || !props.value} icon={Eraser} label="清空源目录" onClick={props.onClear} />
        </div>
      </div>
    </div>
  )
...
export function TargetNamesInput(props: {
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-col gap-1.5">
      {!props.compact && (
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="crashu-targets">目标名称</Label>
          <Badge variant="outline" className="shrink-0">{props.targetCount} 条</Badge>
        </div>
      )}
      <Textarea
        id="crashu-targets"
  ...
  return (
    ...
    >
      <SwitchRow
        ...
        icon={ShieldAlert}
        label="预演"
        description="开启后只扫描和生成计划，不真实移动文件夹。关闭后会执行真实移动。"
        onCheckedChange={(dryRun) => props.onPatch({ dryRun })}
      />
      <SwitchRow
        checked={props.data.autoMove ?? false}
        disabled={props.disabled}
        icon={Zap}
        label="自动移动"
        description="扫描后直接执行移动，无需单独触发。需要填写目标目录。"
        onCheckedChange={(autoMove) => props.onPatch({ autoMove })}
      />
    </div>
  )
}
    ...
    <Popover>
      ...
      <PopoverContent align="end" className="w-[min(92vw,520px)]">
        <div className="mb-3">
          ...
          <p className="text-xs text-muted-foreground">相似度阈值、目标目录和冲突策略集中在这里。</p>
        </div>
        <div className="grid gap-3">
          ...
          <TextField
            label="目标目录"
            placeholder="填写目录可自动读取子文件夹作为目标名"
    ...
    <Popover>
      ...
      <PopoverContent align="end" className="w-72">
        <div className="mb-3">
          ...
          <p className="text-xs text-muted-foreground">保存 Crashu 的源目录、目标和移动策略到明文配置。</p>
        </div>
        <div className="grid gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-xl">
              <DialogHeader>
                <DialogTitle>Crashu 配置</DialogTitle>
                <DialogDescription>当前 nodes.crashu 默认值和配置文件位置。</DialogDescription>
              </DialogHeader>
