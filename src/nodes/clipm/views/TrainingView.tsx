  ...
  return <div className="grid min-h-0 flex-1 grid-cols-1 overflow-auto @5xl/clipm:grid-cols-[minmax(300px,0.7fr)_minmax(520px,1.4fr)] @5xl/clipm:overflow-hidden">
    <section className="flex min-h-[420px] flex-col border-r @5xl/clipm:min-h-0">
      ...
      <div className="grid gap-4 p-3">
        ...
        <div className="grid grid-cols-[1fr_96px] items-center gap-3 border px-3 py-2"><div><div className="text-xs font-medium">训练触发门槛</div><div className="text-[10px] text-muted-foreground">达到数量后领取全部待处理作品</div></div><Input aria-label="自动训练触发门槛" type="number" min={1} max={1000} step={1} value={batchSize} disabled={running || controller.environmentConfig.loading} onChange={(event) => setBatchSize(Number(event.currentTarget.value))} onBlur={() => { const normalized = Number.isInteger(batchSize) && batchSize >= 1 && batchSize <= 1000 ? batchSize : 20; setBatchSize(normalized); void controller.updateConfig({ auto_train_batch_size: normalized }) }} /></div>
