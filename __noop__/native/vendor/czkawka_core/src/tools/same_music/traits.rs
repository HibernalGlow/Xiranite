    ...
    fn search(
        ...
        match self.params.check_type {
            CheckingMethod::AudioTags => {
                ...
                if self.check_for_duplicate_tags(stop_flag, progress_sender)
            ...
            CheckingMethod::AudioContent => {
                ...
                if self.calculate_fingerprint(stop_flag, progress_sender)
                    == WorkContinueStatus::Stop
                {
                    self.common_data.stopped_search = true;
                    return;
                }
                if self.check_for_duplicate_fingerprints(stop_flag, progress_sender)
                    == WorkContinueStatus::Stop
                {
                    self.common_data.stopped_search = true;
                    return;
...
impl PrintResults for SameMusic {
    fn write_results<T: Write>(&self, writer: &mut T) -> std::io::Result<()> {
        if !self.duplicated_music_entries.is_empty() {
            writeln!(
                writer,
                "{} music files which have similar friends\n\n.",
                self.duplicated_music_entries.len()
            )?;
            ...
            for vec_file_entry in &self.duplicated_music_entries {
                writeln!(
                    writer,
                    "Found {} music files which have similar friends",
                    vec_file_entry.len()
                )?;
    ...
    fn save_results_to_file_as_json(
        &self,
        file_name: &str,
        pretty_print: bool,
    ) -> std::io::Result<()> {
        if self.get_use_reference() {
            self.save_results_to_file_as_json_internal(
                file_name,
