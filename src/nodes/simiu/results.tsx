...
export function SimiuStatsPanel({ result }: { result: SimiuData | null }) {
  const stats = [
    [tNode("simiu", "stats.images", "图片"), result?.imageCount ?? 0],
    [tNode("simiu", "stats.groups", "分组"), result?.groupCount ?? 0],
    [tNode("simiu", "stats.operations", "操作"), result?.operations.length ?? 0],
    [tNode("simiu", "stats.errors", "错误"), result?.errorCount ?? 0],
  ] as const
    ...
    <div data-testid="simiu-stats-panel" className="grid shrink-0 grid-cols-4 gap-1">
      {stats.map(([label, value]) => (
        <div key={label} className="min-w-0 rounded-md bg-muted/35 px-2 py-1.5 text-center">
          ...
          <div className={cn("text-sm font-semibold tabular-nums", label === tNode("simiu", "stats.groups", "分组") && Number(value) > 0 && "text-primary", label === tNode("simiu", "stats.errors", "错误") && Number(value) > 0 && "text-destructive")}>{value}</div>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <Tabs defaultValue={preferredTab} className="flex h-full min-h-0 flex-col">
      <TabsList variant="line" className="shrink-0">
        <TabsTrigger value="groups">{tNode("simiu", "tabs.groups", "分组")}</TabsTrigger>
        <TabsTrigger value="operations">{tNode("simiu", "tabs.operations", "操作")}</TabsTrigger>
        <TabsTrigger value="logs">{tNode("simiu", "tabs.logs", "日志")}</TabsTrigger>
      </TabsList>
      ...
      <TabsContent value="logs" className="min-h-0 flex-1">
        <TextPanel compact={props.compact} emptyText={tNode("simiu", "empty.logsText", "运行日志会显示在这里。")} icon={ScrollText} lines={props.logs} onCopy={props.onCopyLogs} />
      </TabsContent>
    </Tabs>
  )
}
...
function GroupPanel(props: {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col rounded-lg border bg-background/70">
      <PanelHeader count={props.groups.length} icon={FolderTree} label={tNode("simiu", "tabs.groups", "分组")} onCopy={props.onCopy} />
        ...
        {props.groups.length ? (
            ...
            {props.groups.map((group) => (
              <div key={`${group.parentDir}/${group.name}`} className="grid min-w-0 gap-1 rounded-md border bg-background/70 p-2">
                <div className="flex min-w-0 items-center justify-between gap-2">
                  ...
                  <Badge variant="outline">{tNode("simiu", "labels.fileCount", "{{count}} 个文件", { count: group.files.length })}</Badge>
                </div>
          ...
          <Empty className="h-full min-h-20 border-0 p-4">
            <EmptyHeader>
              ...
              <EmptyTitle className="text-sm">{tNode("simiu", "empty.groupsTitle", "等待分组")}</EmptyTitle>
              <EmptyDescription className="text-xs">{tNode("simiu", "empty.groupsDescription", "生成计划后会显示相似图片分组。")}</EmptyDescription>
            </EmptyHeader>
          </Empty>
        )}
      </ScrollArea>
...
function OperationPanel(props: {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col rounded-lg border bg-background/70">
      <PanelHeader count={props.operations.length} icon={ListChecks} label={tNode("simiu", "tabs.operations", "操作")} onCopy={props.onCopy} />
      <ScrollArea className="min-h-0 flex-1">
        {props.operations.length ? (
          ...
          <Empty className="h-full min-h-20 border-0 p-4">
            <EmptyHeader>
              ...
              <EmptyTitle className="text-sm">{tNode("simiu", "empty.operationsTitle", "等待操作")}</EmptyTitle>
              <EmptyDescription className="text-xs">{tNode("simiu", "empty.operationsDescription", "生成计划或应用分组后会显示文件操作。")}</EmptyDescription>
            </EmptyHeader>
          </Empty>
        )}
      </ScrollArea>
...
function TextPanel(props: {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col rounded-lg border bg-background/70">
      <PanelHeader count={props.lines.length} icon={props.icon} label={tNode("simiu", "tabs.logs", "日志")} onCopy={props.onCopy} />
      <ScrollArea className="min-h-0 flex-1">
        {props.lines.length ? (
          ...
          <Empty className="h-full min-h-20 border-0 p-4">
            <EmptyHeader>
              ...
              <EmptyTitle className="text-sm">{tNode("simiu", "empty.logsTitle", "等待日志")}</EmptyTitle>
    ...
    <>
      <div className="flex shrink-0 items-center justify-between gap-2 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2 text-xs font-medium text-muted-foreground">
          ...
          <span>{props.count ? tNode("simiu", "panel.count", "{{count}} 项{{label}}", { count: props.count, label: props.label }) : tNode("simiu", "panel.waiting", "等待{{label}}", { label: props.label })}</span>
        </div>
        <Button disabled={!props.count} size="xs" variant="ghost" onClick={props.onCopy}>
          <Copy data-icon="inline-start" />
          {tNode("simiu", "actions.copy", "复制")}
        </Button>
      </div>
      <Separator />
    </>
