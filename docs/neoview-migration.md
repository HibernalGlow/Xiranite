...
`folderMain` 前四条核心纵切已经接入：GUI/CLI/TUI 可复用的唯一 `ReaderFileTreeService` 维护稳定目录快照、前进/后退/上级/刷新、generation、递归扫描和订阅生命周期，旧 `CoreReaderDirectoryBrowser` 已删除；鉴权 HTTP 控制路由提供 128 项任意 cursor 分页、session 级排序/偏好 PATCH、`watch:true` 显式监听和异步释放。后端单层浏览继续使用可取消的原生 `opendir()`；递归索引/搜索通过同一 service 委托唯一 `readdirp` stream adapter；文件树事件通过按需动态导入的 `@parcel/watcher` adapter，只在显式监听的 session 建立订阅。当前目录直接子项变化会使快照变脏，下一次分页请求以单飞方式重读、重新 hydration/排序并递增 generation；导航换根、DELETE、session 淘汰和 service dispose 均关闭旧订阅。排序在分页前由唯一应用层排序器完成，八字段契约已冻结，name/type/path/random 即时重排，date/size 通过 Node/Bun 原生 `Readable.map({ concurrency })` 有界并发 metadata adapter 按需 `stat`，每个 mapper 在 I/O 前后检查 AbortSignal；不把跨 realm 的 Web signal 直接传入 Node stream。重排后返回焦点路径的新索引，前端直接请求对应稀疏页。排序偏好严格执行“临时规则 > 文件夹记忆 > 标签默认 > 全局默认”，全局/标签默认和最多 1000 项文件夹记忆写入现有 Reader SQLite 的独立 `xr_reader_*` 表，临时规则只存在 browser session；React 提供锁定、设为默认和清理记忆操作，不再写 localStorage。React `FolderMainCard` 是独立 lazy chunk，仅在可见时打开浏览会话；当前已使用 React Virtuoso 驱动最多 12 个批次的有界稀疏 catalog，提供 compact、cover/mosaic list、cover/mosaic grid、稳定选择、前进/后退快照、返回父目录建议索引与远端批次自动定位，并把可见项按最多 64 项注册到现有 opaque library thumbnail route。文件夹 4/9/16 图由后端 `sharp.composite()` 合成为单个 WebP，前端每个可见项始终只有一个 `<img>`；折叠/卸载后取消分页/缩略图并释放 context/session。当前仍标记 `partial`，因为前端尚未默认请求 watcher、递归搜索/树 UI 与流式 HTTP surface 未完成，原版四模式的完整视觉与设置验收、完整多选/键盘导航、真实多标签、上下文文件操作、目录递归大小、旧排序 localStorage 导入和 CLI/TUI 偏好命令仍待迁移。
