...
- `benchmark:neoview-thumbnail-system -- --assert` 必须将旧 demand 在 supersession 后的 `ready`、最终可见窗口就绪时仍存活的旧 flight、以及实际取消延迟 p95/max 写入不可覆盖报告并作为门禁；无须取消的已提前 settle demand 不计入延迟样本；
