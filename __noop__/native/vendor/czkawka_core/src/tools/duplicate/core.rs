...
use crate::common::cache::{
    CACHE_DUPLICATE_VERSION, load_cache_from_file_generalized_by_size,
...
use crate::tools::duplicate::{
    DuplicateEntry, DuplicateFinder, DuplicateFinderParameters, Info, PREHASHING_BUFFER_SIZE,
    THREAD_BUFFER, filter_hard_links, hash_calculation, hash_calculation_limit,
};
...
impl DuplicateFinder {
        ...
        let group_by_func = if self.get_params().case_sensitive_name_comparison {
            |fe: &FileEntry| {
                fe.path
                    .file_name()
                    .unwrap_or_else(|| {
                        panic!("Found invalid file_name \"{}\"", fe.path.to_string_lossy())
                    })
                    .to_string_lossy()
                    .to_string()
            }
                    ...
                    .filter_map(|(name, vector)| {
                        if vector.len() > 1 {
                            Some((
                                ...
                                vector
                                    .into_iter()
                                    .map(FileEntry::into_duplicate_entry)
                                    .collect(),
                            ))
                        } else {
                            None
                ...
                if self.common_data.use_reference_folders {
                    let vec = mem::take(&mut self.files_with_identical_names)
                        ...
                        .filter_map(|(_name, vec_file_entry)| {
                            let (mut files_from_referenced_folders, normal_files): (
                                Vec<_>,
                                Vec<_>,
                    ...
                    for (fe, vec_fe) in vec {
                        self.files_with_identical_names_referenced
                            .insert(fe.path.to_string_lossy().to_string(), (fe, vec_fe));
                    }
                }
    ...
    pub(crate) fn check_files_size_name(
        &mut self,
        stop_flag: &Arc<AtomicBool>,
        progress_sender: Option<&Sender<ProgressData>>,
    ) -> WorkContinueStatus {
            ...
            DirTraversalResult::SuccessFiles {
                ...
                if self.common_data.use_reference_folders {
                    let vec = mem::take(&mut self.files_with_identical_size_names)
                        ...
                        .collect::<Vec<(DuplicateEntry, Vec<DuplicateEntry>)>>();
                    for (fe, vec_fe) in vec {
                        self.files_with_identical_size_names_referenced.insert(
                            (fe.size, fe.path.to_string_lossy().to_string()),
                            (fe, vec_fe),
                        );
                    }
                }
    ...
    pub(crate) fn check_files_size(
        ...
        match result {
            DirTraversalResult::SuccessFiles {
                ...
                let grouped_file_entries: Vec<(u64, Vec<FileEntry>)> =
                    grouped_file_entries.into_iter().collect();
                let rayon_max_len = if self.get_params().ignore_hard_links {
                    3
                } else {
                    100
                };
    ...
    fn filter_reference_folders_by_size(&mut self) {
        if self.common_data.use_reference_folders
            && self.get_params().check_method == CheckingMethod::Size
        {
            ...
            for (fe, vec_fe) in vec {
                self.files_with_identical_size_referenced
                    .insert(fe.size, (fe, vec_fe));
            }
        }
    }

    #[fun_time(message = "prehash_load_cache_at_start", level = "debug")]
    fn prehash_load_cache_at_start(
        &mut self,
    ) -> (
        BTreeMap<u64, Vec<DuplicateEntry>>,
        BTreeMap<u64, Vec<DuplicateEntry>>,
        BTreeMap<u64, Vec<DuplicateEntry>>,
    ) {
        // Cache algorithm
        ...
        if self.get_params().use_prehash_cache {
            ...
            mem::swap(
                &mut self.files_with_identical_size,
                &mut non_cached_files_to_check,
            );
        }
        (
    ...
    fn prehashing(
        ...
        let (loaded_hash_map, records_already_cached, non_cached_files_to_check) =
            self.prehash_load_cache_at_start();
        ...
        let non_cached_files_to_check: Vec<(u64, Vec<DuplicateEntry>)> =
            non_cached_files_to_check.into_iter().collect();

        debug!("Starting calculating prehash");
        #[allow(clippy::type_complexity)]
        let pre_hash_results: Vec<(
            u64,
            BTreeMap<String, Vec<DuplicateEntry>>,
            Vec<String>,
        )> = non_cached_files_to_check
            .into_par_iter()
            ...
            .map(|(size, vec_file_entry)| {
                let mut hashmap_with_hash: BTreeMap<String, Vec<DuplicateEntry>> =
                    Default::default();
                ...
                THREAD_BUFFER.with_borrow_mut(|buffer| {
                    for mut file_entry in vec_file_entry {
                        ...
                        match hash_calculation_limit(
                            buffer,
                            &file_entry,
                            check_type,
                            PREHASHING_BUFFER_SIZE,
                            progress_handler.size_counter(),
                        ) {
                            Ok(hash_string) => {
            ...
            for vec_file_entry in hash_map.values() {
                if vec_file_entry.len() > 1 {
                    pre_checked_map
                        .entry(*size)
                        .or_default()
                        .append(&mut vec_file_entry.clone());
                }
            }
        }
    ...
    fn diff_loaded_and_prechecked_files(
        ...
        for (size, mut vec_file_entry) in used_map {
            if let Some(cached_vec_file_entry) = loaded_hash_map.get(&size) {
                ...
                for file_entry in vec_file_entry {
                    if let Some(cached_file_entry) =
                        cached_path_entries.remove(file_entry.path.as_path())
                    {
                        records_already_cached
                            .entry(size)
                            .or_default()
                            .push(cached_file_entry);
                    } else {
                        non_cached_files_to_check
                            .entry(size)
                            .or_default()
        ...
        debug!(
            ...
            format_size(
                non_cached_files_to_check
                    .values()
                    .map(|v| v.iter().map(|e| e.size).sum::<u64>())
                    .sum::<u64>(),
                BINARY
            ),
    ...
    fn full_hashing(
        ...
        let (loaded_hash_map, records_already_cached, non_cached_files_to_check) =
            self.full_hashing_load_cache_at_start(pre_checked_map);
        ...
        let progress_handler = prepare_thread_handler_common(
            ...
            non_cached_files_to_check
                .iter()
                .map(|(size, items)| (*size) * items.len() as u64)
                .sum::<u64>(),
        );
        ...
        debug!(
            ...
            non_cached_files_to_check
                .iter()
                .map(|(_size, v)| v.len() as u64)
                .sum::<u64>()
        );
        let mut full_hash_results: Vec<(u64, BTreeMap<String, Vec<DuplicateEntry>>, Vec<String>)> =
            non_cached_files_to_check
                .into_par_iter()
                .with_max_len(3)
                .map(|(size, vec_file_entry)| {
        ...
        self.full_hashing_save_cache_at_exit(
            records_already_cached,
            &mut full_hash_results,
            loaded_hash_map,
        );
        ...
        if self.common_data.use_reference_folders {
            ...
            for vec_of_vec in vec {
                self.files_with_identical_hashes_referenced
                    .insert(vec_of_vec[0].0.size, vec_of_vec);
            }
        }
    ...
    pub(crate) fn check_files_hash(
        ...
        if self.prehashing(stop_flag, progress_sender, &mut pre_checked_map)
            == WorkContinueStatus::Stop
        {
            return WorkContinueStatus::Stop;
        }

        if self.full_hashing(stop_flag, progress_sender, pre_checked_map)
            == WorkContinueStatus::Stop
        {
            return WorkContinueStatus::Stop;
        }
