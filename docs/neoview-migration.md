...
从 `folderMain` 开始，77 张 Card 统一执行“清单先于实现”的完成门禁，机器可读模板固定在 `migration/neoview/card-acceptance-contract.json`，完整可读索引固定在 `docs/neoview-card-functional-checklist.md`，基础审计执行 `bun run audit:neoview-card-checklists`，发布门禁追加 `--require-complete`。每张 Card 开工前必须从冻结源码列全：用户能力、UI 层级和控件、所有交互状态、设置/默认值/优先级、快捷键与可访问性、GUI/CLI/TUI 共用数据契约、加载/休眠/释放生命周期、性能预算、测试 ID 和有意偏离。仅有后端 API、一个 smoke control 或一条 happy path 均不能标记 `complete`；没有源码证据、自动化测试和视觉/几何证据的项保持 `pending/partial`。仓库内 JSON、生成文档和审计脚本共同构成跨任务持久迁移记忆，后续实现不得依赖聊天上下文记住遗漏；文档漂移时运行 `bun run generate:neoview-card-checklist`，源码 Card 映射漂移时运行 `bun run generate:neoview-card-sources` 后人工审查差异。
...
`folderMain` 前四条核心纵切已经接入：GUI/CLI/TUI 可复用的 `CoreReaderDirectoryBrowser` 维护稳定目录快照、前进/后退/上级/刷新和 generation；鉴权 HTTP 控制路由提供 128 项任意 cursor 分页、session 级排序/偏好 PATCH 和显式释放；后端单层浏览继续使用可取消的原生 `opendir()`，真正的递归索引/搜索收口到独立 `readdirp` stream adapter，文件树增量事件收口到按需动态导入的 `@parcel/watcher` adapter。排序在分页前由唯一应用层排序器完成，八字段契约已冻结，name/type/path/random 即时重排，date/size 通过 Node/Bun 原生 `Readable.map({ concurrency, signal })` 有界并发 metadata adapter 按需 `stat`；重排后返回焦点路径的新索引，前端直接请求对应稀疏页。排序偏好严格执行“临时规则 > 文件夹记忆 > 标签默认 > 全局默认”，全局/标签默认和最多 1000 项文件夹记忆写入现有 Reader SQLite 的独立 `xr_reader_*` 表，临时规则只存在 browser session；React 提供锁定、设为默认和清理记忆操作，不再写 localStorage。React `FolderMainCard` 是独立 lazy chunk，仅在可见时打开浏览会话；当前已使用 React Virtuoso 驱动最多 12 个批次的有界稀疏 catalog，提供 compact/cover-grid、稳定选择、前进/后退快照、返回父目录建议索引与远端批次自动定位，并把可见项按最多 64 项注册到现有 opaque library thumbnail route；折叠/卸载后取消分页/缩略图并释放 context/session。当前仍标记 `partial`，因为 cover/mosaic list、mosaic grid、details、完整多选/键盘导航、真实多标签、搜索、上下文文件操作、目录递归大小、EMM/评分批量 hydration、旧排序 localStorage 导入和 CLI/TUI 偏好命令仍待迁移。

#### `folderMain` 与统一文件树重构决策（2026-07-16）

文件浏览器是核心能力，不按普通轻 Card 处理，也不允许为了快速展示而维护 GUI 专用目录树。GUI、CLI、TUI 和后续 EMM 统一依赖一个 `ReaderFileTreeService` 应用边界；现有 `CoreReaderDirectoryBrowser` 在纵切完成后并入该边界，不保留旧 `opendir` 浏览器、递归扫描器和另一套 watcher 并存。前后端只共享 DTO、稳定 entry ID、generation、分页 cursor 和取消语义，不共享 React/Virtuoso 状态。
...
本轮聚焦证据：新增排序偏好/browser/route/SQLite 后端 4 文件 12 项通过，前端 client/catalog/Card 3 文件 12 项通过；累计覆盖排序 PATCH、四级优先级、SQLite 默认值/记忆、跨 route 恢复、临时锁定、清理记忆、焦点路径新索引、可见缩略图批次与卸载 context 释放。NeoView package build、根 `typecheck:app` 与清单审计通过；此前 `@parcel/watcher` Windows/Bun 实机 create/unsubscribe 探针保持通过。尚未运行生产 chunk、10K/100K corpus、Playwright 和完整性能审计，因此不得据此宣称 `folderMain` 完成。

#### `folderMain` 完整兼容清单（2026-07-16）

权威机器清单为 `migration/neoview/folder-main-compatibility.json`，共 74 项：`partial=23`、`pending=51`、`complete=0`。每项固定包含 `sourceEvidence`、`targetContract`、`surfaces`、`status`、`testIds` 和 `notes`；本文表格是便于阅读的索引，发生冲突时以源码证据与 JSON 契约为准。`complete` 必须同时满足功能、原版 UI、设置、快捷键、异常态、生命周期、自动化测试和适用的 GUI/CLI/TUI 共用契约。
...
- `folderMain` 只有 74 项全部 `complete` 后才能把 `migration/neoview/card-compatibility.json` 中对应 Card 从 `partial` 改为完成；其余 76 张 Card 已在 `docs/neoview-card-functional-checklist.md` 建立逐卡源组件/功能域索引，开工前仍须逐张扩展为与 `folderMain` 同结构的源码明细，不能把索引行当作完成清单。
...
`historyList/bookmarkList` 已接入统一 Reader library：两张 Card 各自保持独立 lazy loader，列表共同复用 100 项分页、`@tanstack/react-virtual` 可视窗口与 AbortController 生命周期；历史项可恢复原 source 并删除，书签支持 `all/default/favorites` 与自定义列表筛选、当前书籍添加、收藏切换、列表创建/删除。GUI 只通过鉴权 `/reader/library/*` 控制路由读写元数据，CLI/TUI 可直接复用 `ReaderLibraryService`，不维护第二套 store。旧 `neoview-history`、`neoview-unified-history`、`neoview-bookmarks`、`neoview-bookmark-lists-v2` 的 backup/full-export/localStorage 三种 JSON 包装已可用 `reader-data-inspect/import` 预览并迁入；两张 Card 仍为 `partial`，剩余批量选择/上下文文件操作、缩略图/评分呈现以及 TUI 命令界面。
...
| Phase 2：目录/归档/唯一数据链 | 进行中（主阅读链较完整） | 目录/单图、ZIP、non-solid/solid 7z/RAR、嵌套包、AES ZIP、loopback HTTP stream、ETag/Range/cancel、图片探测、惰性 sharp、L2/L3 cache、缩略图生成/批写/失败退避/在线维护 | RAR/7z 安全密码通道、缩略图离线维护与旧版本迁移、更多真实语料；PDF 明确 deferred，EPUB 独立评估 |
...
| Phase 4：复杂格式与 CLI/TUI | 进行中（图片阅读原型可用） | 7z/RAR/nested provider；共享 HeadlessController；`xneoview` inspect/pages/frame/extract；持久 TUI 工作台通过共享 `TerminalImagePreview` 流式显示单/双页 frame | 真实 SIXEL/Kitty 终端验收、缩略图 gallery、`--connect`、完整设置/feature 命令、平台能力诊断；PDF 不计入当前阶段验收，EPUB 独立评估 |
| Phase 5：全功能切换 | 未开始 | 完成标准与 30 项矩阵已冻结 | 30 项无 `pending`、全旧设置/数据导入、删除全部旧/临时链、发布验收 |

#### PDF 暂缓决策（2026-07-15）

PDF 后端 provider 当前由项目所有者明确后置，不进入近期核心迁移范围。路径检测仍将 `.pdf` 规范化为 `{ kind: "document", format: "pdf" }`，但 `PlatformReaderBookLoader` 必须明确返回 document capability unavailable；不得为“看起来支持”而把 PDF 原文件当图片直出，也不得保留未完成的 PDF.js/canvas 依赖、固定尺寸预栅格化链或 GUI 专用实现。
...
EPUB 仍与 PDF 分开评估，不因 PDF 后置而默认采用同一页面模型或依赖。

后端/共享核心的当前真实状态：

| 子系统 | 状态 | 当前实现 | 未完成边界 |
| --- | --- | --- | --- |
| Reader domain/application | 可用但未全兼容 | `packages/nodes/neoview/src/domain` 与 `application`；GUI/CLI/TUI 共用 session/controller；显式页码优先、默认恢复上次进度，翻页写入不阻塞主链且关闭强制刷盘；共享 `ReaderLibraryService` 提供最近阅读、书签和列表契约 | 完整阅读模式、历史/书签 UI 与 CLI/TUI surface、旧数据导入、统计/播放列表和全部错误恢复 |
| Archive provider | ZIP/7z/RAR 主纵切可用 | Store/Deflate ZIP、stdout non-solid、单进程 solid materializer、嵌套 lease、CRC/预算/取消 | RAR/7z 无 argv 密码、更多损坏/超大真实包；文档格式不属于 archive provider |
| HTTP 图片数据面 | 主链可用 | opaque token、GET/HEAD、ETag/304、文件 Range、ZIP 流、背压和断开取消；默认原图不进 sharp | 格式 fallback 策略、远程源、更多 Range/代理兼容 |
| Probe/transform/cache/scheduler | 部分完成 | 有界尺寸探测、惰性 sharp、singleflight、weighted LRU、`cacache` 内容寻址 L3、宿主 CPU lease | frame pin、方向权重、内存压力回收、所有重节点共享配额 |
| 旧缩略图数据库 | 主运行纵切可用、迁移未完成 | 沿用 `%APPDATA%/NeoView/thumbnails.db`；惰性单 writer、批量命中、生成/持久化、失败退避、统计与有界在线清理 | V1/V3/V4 迁移、外部 writer 协调、备份/checkpoint/vacuum、损坏恢复 |
...
| 缩略图 | 显示、生成与在线维护纵切可用 | 64 页 cursor 批次、横向 ThumbnailStrip 与纵向 PageList 虚拟窗口、PageList 服务端搜索、按视图跳过/启用预热、旧库批量预热、Page/file/folder/video 生成、WebP BLOB 单库持久化；共享 maintenance service 统一 stats、空 Blob、过期 file、无效路径和失败记录清理 | 区域筛选、备份/checkpoint/vacuum 离线编排与旧版本迁移 |
...
这仍不是第 12 节的全部完成状态：当前 frame pin、方向/距离权重、宿主内存压力 deep cleanup、跨进程 active lease 和所有缓存层的统一指标聚合尚待实现。缓存产物物化是受字节预算约束的明确例外，禁止把同一机制扩展为“所有原图默认进 JS 堆”。
...
当前已完成 application 计划层：`ReaderPreloadCoordinator` 复用唯一的 `buildFrameSnapshot`，按逻辑 frame 生成 near/view、ahead/ahead 和反向 background 三层候选；单双页、首尾单页、宽图单页和 RTL 不另写配对算法。ReaderSession 初始化、导航和 layout 变更各生成一次单调 `preloadGeneration`，计划包含 frame generation、方向、置信度、opaque page ID/索引和现有 `ResourcePriority`，通过 GUI HTTP DTO 与 HeadlessController 共用；它不打开 PageContent、不读取 archive、不创建队列/Worker/Buffer，也不取得 scheduler lease。当前仍是 partial：连续滚动/viewport 速度、稳定等待、内存/队列 admission、实际 active/cancel/reject/lease 指标，以及浏览器 `Image.decode()` 消费该 plan 尚未接入；在这些完成前，后端不得为“预热”主动吞读相邻原图。
...
历史/书签纵切已遵守该边界：最近阅读不复制第二张 history 表，而是按 `xr_reader_progress.updated_at` 倒序分页；书签写入 `xr_reader_bookmarks`，自定义列表写入 `xr_reader_bookmark_lists`，多对多关系写入 `xr_reader_bookmark_memberships`。进度恢复和 ReaderLibrary 不再各自打开一条可写连接或维护重复 schema，默认 GUI composition 只创建一个 `SqliteReaderDataStore`，由它同时实现 `ReaderProgressStore` 与 `ReaderLibraryStore`；CLI/TUI 的独立 composition 也复用同一 adapter。`all/default/favorites` 是应用层合成的系统视图，不写入 list 表；`default` membership 仍显式保存在关系表中，因此旧书签可以同时属于默认和自定义列表；`favorites` 同时匹配 bookmark 的 `starred` 和 `is_favorite` 自定义列表成员，保持旧版行为。书签和成员关系的复合更新使用短 `BEGIN IMMEDIATE` 事务及最多三次有界 busy retry，清理历史使用带 limit 的小批删除。旧 JSON payload 使用 `zod` schema 校验，而不是维护一套手写 JSON validator；数据导入以一个短事务写入这些表。嵌套 path stack 和视频位置分别保存在 `xr_reader_path_stacks`、`xr_reader_media_progress`，不会因当前 UI 尚未消费而丢弃；merge 只接受较新的记录，重复导入零变更，overwrite 只清理 `xr_` Reader 表且 CLI 必须显式 `--yes`。`reader-data-inspect` 只输出计数、迁移报告与 `[nodes.neoview.history]` TOML patch，不泄露路径；`reader-data-import` 支持原版 2.0 backup、1.0 full export 和直接 localStorage key object。HTTP 控制面仍仅传 JSON 元数据，不进入页面 asset/翻页热路径。TUI 数据管理入口、运行时视频进度写回和更多历史/书签旧交互尚未完成，因此相关 feature 继续保持 partial/pending。
...
- 已以原 `%APPDATA%\NeoView\thumbnails.db` 接入惰性单 writer、批量命中、受保护 HTTP URL、Page/file/folder/video 生成、失败退避与有界在线维护；继续补归档内视频、离线备份/checkpoint/vacuum 和 V1/V3/V4 迁移；
...
- 已验证 ZIP 流式读取、背压、取消、ETag、错误恢复和 session close；non-solid RAR/7z 已使用单 entry stdout 流，solid RAR/7z 已使用单进程顺序 materializer，并覆盖真实 CB7、CRC、预算、取消和临时目录清理；嵌套 ZIP/7z 已使用路径栈与物化 lease 打通，AES ZIP 根级/嵌套密码已使用 session scope；RAR/7z 加密、历史/UI 导航与 EPUB 仍需同等级实现和测试，PDF 不计入当前缺口。
...
- 基于真实兼容性测试选择 7-Zip、EPUB、特殊图片适配器；PDF 继续 deferred，恢复时单独重新选型；
...
- `sharp/libvips`、7-Zip、EPUB、FFmpeg 和所有 transitive native 依赖在选型时逐项登记许可；PDF 恢复评估时再登记候选实现，不提前引入 PDF.js；
