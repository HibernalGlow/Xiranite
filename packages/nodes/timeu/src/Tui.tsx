...
export function TimeuTui(
  props: TerminalUiScreenProps<TimeuInput, TimeuResult>,
) {
  ...
  return (
    ...
    >
      <TimeuWorkbench {...props} onThemePreview={setPreviewTheme} />
    </TerminalThemeProvider>
  );
}
function TimeuWorkbench({
  definition,
  language,
  preferences,
  onExit,
  onThemePreview,
}: TerminalUiScreenProps<TimeuInput, TimeuResult> & {
  ...
  const boolFields = fields.filter((field) =>
    ["recursive", "includeDirectories", "dryRun"].includes(field.id),
  );
  const controls = [
    "action",
    "listText",
    "recordPath",
    ...boolFields.map((field) => field.id),
    "execute",
    "settings",
    "tab-status",
    "tab-logs",
    ...
    return (
      ...
      >
        ...
        >
          ...
          <text>{session.dangerPrompt?.body}</text>
  ...
  return (
    ...
    >
      ...
      >
        <box flexDirection="column">
          <text fg={theme.colors.primary}>
            <b>{`${terminalIcon("status")} TIMEU // TIMESTAMP VAULT`}</b>
          </text>
          <text fg={theme.colors.mutedForeground}>
            {language === "zh"
              ? "路径队列、时间记录与安全恢复闸门"
        ...
        <box flexDirection="column" alignItems="flex-end">
          ...
          >{`${session.phase === "running" ? "◌ WORKING" : "● READY"} ${["⠁", "⠂", "⠄", "⡀", "⢀", "⠠"][frame % 6]}`}</text>
          {preferences ? (
            <ClickTarget
              id="settings"
      ...
      <box flexDirection="row" flexGrow={1} minHeight={0} gap={1} marginTop={1}>
        <WorkbenchPanel
          title={language === "zh" ? "路径队列" : "Path queue"}
          description={
            language === "zh"
              ? "每行一个文件或目录"
        ...
        >
          <box flexDirection="column" flexGrow={1} minHeight={0}>
            <WorkbenchField
              field={pathField}
              value={session.values.listText}
              error={session.fieldErrors.listText}
              focused={session.focusedControlId === "listText"}
              disabled={session.phase === "running"}
              t={t}
              onFocus={() => session.focus("listText")}
              onChange={(value) => session.setField("listText", value)}
            />
            <WorkbenchField
              field={recordField}
              value={session.values.recordPath}
              error={session.fieldErrors.recordPath}
              focused={session.focusedControlId === "recordPath"}
              disabled={session.phase === "running"}
              t={t}
              onFocus={() => session.focus("recordPath")}
              onChange={(value) => session.setField("recordPath", value)}
            />
            <box flexGrow={1} />
          </box>
        </WorkbenchPanel>
        <WorkbenchPanel
          title={language === "zh" ? "时间记录" : "Timestamp records"}
          description={
            language === "zh"
              ? "扫描结果与备份/恢复计划"
              : "Scan result and backup/restore plan"
          }
          flexGrow={1}
        >
          <box flexDirection="column" flexGrow={1} minHeight={0}>
              ...
              {session.resultSummary?.table ? (
                <>
                  {session.resultSummary.table.rows.map((row, index) => (
                    ...
                    >{`${row.status === "success" ? "✓" : row.status === "error" ? "!" : "·"} ${row.operation}  ${row.path}`}</text>
                  ))}
                </>
              ) : (
                <text fg={theme.colors.mutedForeground}>
                  {language === "zh"
                    ? "执行后显示当前时间、记录时间及恢复计划。"
                    : "Current, recorded and restore-plan timestamps appear after a run."}
                </text>
              )}
            </scrollbox>
            <ProgressBar
              ...
              label={
                session.status ||
                (language === "zh" ? "等待路径" : "Waiting for paths")
              }
            />
          </box>
        </WorkbenchPanel>
        <WorkbenchPanel
          title={language === "zh" ? "执行闸门" : "Execution gate"}
          description={
            language === "zh"
              ? "预演、递归和写入保护"
              : "Dry-run, recursion and write protection"
          }
          width="28%"
        >
