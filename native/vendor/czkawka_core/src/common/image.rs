...
use crate::common::consts::{
    HEIC_EXTENSIONS, IMAGE_RS_EXTENSIONS, IMAGE_RS_SIMILAR_IMAGES_EXTENSIONS, JXL_IMAGE_EXTENSIONS,
...
pub fn get_dynamic_image_from_path(path: &str) -> Result<DynamicImage, String> {
    ...
    if let Ok(res) = res {
        match res {
            Ok(t) => {
                ...
                match rotation {
                    ...
                    Some(ExifOrientation::MirrorHorizontalAndRotate270CW) => {
                        Ok(t.fliph().rotate270())
                    }
                    Some(ExifOrientation::Rotate90CW) => Ok(t.rotate90()),
                    Some(ExifOrientation::MirrorHorizontalAndRotate90CW) => {
                        Ok(t.fliph().rotate90())
                    }
        ...
        let message = create_crash_message(
            "Image-rs or libraw-rs or jxl-oxide",
            path,
            "https://github.com/image-rs/image/issues",
        );
        error!("{message}");
        Err(message)
    }
...
pub(crate) fn get_dynamic_image_from_heic(path: &str) -> anyhow::Result<DynamicImage> {
    ...
    let interleaved_plane = planes
        .interleaved
        .ok_or_else(|| anyhow::anyhow!("Failed to get interleaved plane"))?;
...
pub(crate) fn get_raw_image(
    path: impl AsRef<Path> + std::fmt::Debug,
) -> Result<DynamicImage, String> {
    let mut timer = Timer::new("Rawler");

    let raw_source = RawSource::new(path.as_ref())
        .map_err(|err| format!("Failed to create RawSource from path {path:?}: {err}"))?;
    ...
    let dynamic_image = developed_image
        .to_dynamic_image()
        .ok_or("Failed to convert image to DynamicImage".to_string())?;
...
pub fn check_if_can_display_image(path: &str) -> bool {
    ...
    allowed_extensions
        .iter()
        .any(|ext| extension_str.ends_with(ext))
}
...
pub(crate) fn get_rotation_from_exif(
    path: &str,
) -> Result<Option<ExifOrientation>, nom_exif::Error> {
    let res = panic::catch_unwind(|| {
    ...
    res.unwrap_or_else(|_| {
        ...
        error!("{message}");
        Err(nom_exif::Error::IOError(std::io::Error::other(
            "Panic in get_rotation_from_exif",
        )))
    })
}
