...
import { Clipboard, DatabaseZap, Eye, Info, Settings2, Video } from "lucide-react"
...
import { ACTIONS, type BitvAction } from "./constants"
...
export function ActionPicker(props: {
  ...
  onActionChange: (action: BitvAction) => void
}) {
  return (
    <ToggleGroup
      aria-label="bitv action"
      ...
      variant="outline"
      onValueChange={(value) => {
        if (value) props.onActionChange(value as BitvAction)
      }}
    >
      {ACTIONS.map((item) => (
    ...
    <div className="flex min-w-0 flex-col gap-1.5">
      {!props.compact && (
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="bitv-paths" className="flex items-center gap-1.5">
      ...
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-1.5">
        {props.compact ? (
          ...
          <Textarea
            ...
            placeholder={"每行一个视频路径，例如：\nD:/Videos/sample1.mp4\nD:/Videos/sample2.mkv"}
            value={props.data.pathsText ?? ""}
            onChange={(event) => props.onPatch({ pathsText: event.currentTarget.value })}
          />
        )}
...
export function PathFields(props: {
  ...
  return (
    <div className="grid gap-2 @3xl/bitv:grid-cols-2">
      <Input
        aria-label="bitv 配置文件"
        disabled={props.disabled}
        placeholder="配置文件，可选"
  ...
  return (
    <div className="grid gap-2 @3xl/bitv:grid-cols-2">
      <SwitchRow
        ...
        icon={Eye}
        label="预演"
        description="只生成命令计划，不真正调用 BitV 模块。"
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="bitv 运行选项" disabled={props.disabled} size="icon-sm" variant="outline">
              <Settings2 />
  ...
  return (
    <Popover>
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button aria-label="bitv 默认配置" disabled={props.disabled} size="icon-sm" variant={props.configDirty ? "secondary" : "outline"}>
              <DatabaseZap />
      ...
      <PopoverContent align="end" className="w-72">
        <div className="mb-3">
          ...
          <p className="text-xs text-muted-foreground">保存 BitV 的路径和运行选项。</p>
        </div>
        <div className="grid gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-xl">
              <DialogHeader>
                <DialogTitle>BitV 配置</DialogTitle>
                <DialogDescription>当前 nodes.bitv 默认值和配置文件位置。</DialogDescription>
              </DialogHeader>
