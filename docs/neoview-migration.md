...
当前边界也必须保持明确：encrypted RAR/7z 在 session secret store 和安全密码通道完成前明确拒绝；嵌套 archive 的物化 lease、方向改变后的选择性停止和跨 session 持久复用仍属于后续 Phase 4，不能把当前 provider 宣称为完整 RAR/7z 功能。
...
尚未完成的是内存/磁盘混合层、按方向停止未需要的后半段、完整结果按 archive fingerprint 跨 session 复用，以及嵌套 archive 的引用计数 lease。这些优化必须建立在当前唯一顺序 extractor 上，不能再引入第二条解压主链。
...
- 已验证 ZIP 流式读取、背压、取消、ETag、错误恢复和 session close；non-solid RAR/7z 已使用单 entry stdout 流，solid RAR/7z 已使用单进程顺序 materializer，并覆盖真实 CB7、CRC、预算、取消和临时目录清理；加密、嵌套与 PDF/EPUB 仍需同等级实现和测试。
...
- 已实现非 solid RAR/7z stdout stream 和 solid archive 单进程顺序预提取；继续补嵌套 lease、加密密码通道、方向取消与持久复用；
