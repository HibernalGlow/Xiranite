...
pub fn check_if_folder_contains_only_empty_folders(path: impl AsRef<Path>) -> Result<(), String> {
    ...
    if !path.is_dir() {
        return Err(format!(
            "Trying to remove folder \"{}\" which is not a directory",
    ...
    for entry in initial_entry {
        if let Ok(entry) = entry {
            ...
            return Err(format!(
                "Cannot read entry from directory \"{}\"",
                path.to_string_lossy()
            ));
        }
    }
...
pub fn remove_folder_if_contains_only_empty_folders(
    ...
    if remove_to_trash {
        trash::delete(path).map_err(|e| {
            format!(
                "Cannot move folder \"{}\" to trash, reason {e}",
                path.to_string_lossy()
            )
        })
    } else {
        fs::remove_dir_all(path).map_err(|e| {
            format!(
                "Cannot remove directory \"{}\", reason {e}",
                path.to_string_lossy()
            )
        })
    }
...
pub(crate) fn create_crash_message(
    library_name: &str,
    file_path: &str,
    home_library_url: &str,
) -> String {
    format!(
...
pub fn regex_check(expression_item: &SingleExcludedItem, directory_name: &str) -> bool {
    ...
    if !expression_item.expression.ends_with('*')
        && !directory_name.ends_with(
            expression_item
                .expression_splits
                .last()
                .expect("Cannot fail, because at least one item is available"),
        )
    {
        return false;
    }
    ...
    let mut last_split_point = directory_name
        .find(&expression_item.expression_splits[0])
        .expect("Cannot fail, because is checked earlier");
...
mod test {
    ...
    use crate::common::{
        make_hard_link, normalize_windows_path, regex_check,
    ...
    fn test_windows_path() {
        assert_eq!(
            PathBuf::from("C:\\path.txt"),
            normalize_windows_path("c:/PATH.tXt")
        );
        assert_eq!(
            PathBuf::from("H:\\reka\\weza\\roman.txt"),
            normalize_windows_path("h:/RekA/Weza\\roMan.Txt")
        );
        assert_eq!(PathBuf::from("T:\\a"), normalize_windows_path("T:\\A"));
        assert_eq!(
            PathBuf::from("\\\\aBBa"),
            normalize_windows_path("\\\\aBBa")
        );
