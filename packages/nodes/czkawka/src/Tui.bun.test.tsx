...
test("Czkawka TUI renders eleven scanners once and responds to mouse", async () => {
  ...
  const screen = await testRender(
    <CzkawkaTui
      ...
      onExit={() => undefined}
    />,
    { width: 150, height: 42, useMouse: true },
  )
  try {
    await act(async () => screen.renderOnce())
    const frame = screen.captureCharFrame()
    expect(frame).toContain("CZKAWKA // FILE FORENSICS")
    expect(frame).toContain("相似图片")
    expect(frame).toContain("扫描")
    expect(frame).toContain("扫描工具")
    expect(frame).toContain("扫描条件")
    // Tool palette is the only tool list — no second "Scanner" form field label
    // and no decorative SCANNER RAIL panel.
    expect(frame).not.toContain("SCANNER RAIL")
    expect(frame).not.toContain("全部 11 项工具")
    // Each scanner label appears once in the palette (not duplicated in a form field).
    const similarCount = frame.split("相似图片").length - 1
    ...
    // Clicking a tool chip switches the active scanner.
    await click(screen, "czkawka-tool-similar-images")
    ...
    ).toMatchObject({ action: "move", copyMode: true, selectedPaths: ["D:/a.bin"] })
  } finally {
...
test("Czkawka TUI renders the complete workbench in English", async () => {
  ...
  try {
    await act(async () => screen.renderOnce())
    const frame = screen.captureCharFrame()
    expect(frame).toContain("SCANNERS")
    expect(frame).toContain("Duplicates")
    expect(frame).toContain("Dirs")
    expect(frame).toContain("RESULTS")
    expect(frame).toContain("Pick a scanner and directories")
...
test("Czkawka TUI selects results, exposes media metadata, and opens the active path", async () => {
  ...
  const screen = await testRender(
    <CzkawkaTui
      ...
      onExit={() => undefined}
    />,
    { width: 170, height: 46, useMouse: true },
  )
  try {
    await act(async () => screen.renderOnce())
...
test("Czkawka TUI renders detailed operation outcomes and logs", async () => {
  const entry = {
    ...
    size: 10,
    modifiedDate: 1,
    secondaryPath: "E:/Review/a (1).jpg",
    ...
    <CzkawkaTui
      definition={{
        ...
        run: async (_input, onEvent) => {
          onEvent({ type: "progress", progress: 50, message: "planning move" })
          return {
            success: true,
            message: "move planned",
            data: {
              action: "move",
              tool: "similar-images",
              groups: [{ id: 0, entries: [entry], totalBytes: 10, reclaimableBytes: 0 }],
              entries: [entry],
              messages: "",
              stopped: false,
              groupCount: 1,
...
async function click(screen: Awaited<ReturnType<typeof testRender>>, id: string) {
  ...
  await act(async () =>
    screen.mockMouse.click(
      target!.x + Math.max(1, Math.floor(target!.width / 2)),
