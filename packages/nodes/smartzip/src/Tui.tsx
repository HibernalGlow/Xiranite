...
function SmartZipWorkbench({ definition, language, preferences, onExit, onThemePreview }: TerminalUiScreenProps<SmartZipInput, SmartZipResult> & { onThemePreview: (theme: string) => void }) {
  ...
  const executableFields = fields.filter((field) => ["smartZipExe", "smartZipAhk", "autohotkeyExe"].includes(field.id))
  ...
  return <box width="100%" height="100%" flexDirection="column" paddingLeft={1} paddingRight={1} overflow="hidden">
    ...
    <box height={12} minHeight={12} flexShrink={0} marginTop={1} borderStyle="rounded" borderColor={session.dangerous ? theme.colors.error : theme.colors.border} paddingLeft={1} paddingRight={1} flexDirection="row" gap={2}>
      <box width="56%" flexDirection="column"><box height={2}><text fg={theme.colors.mutedForeground}>{language === "zh" ? "可执行文件与运行记录" : "Executables and run record"}</text></box><box flexDirection="row" gap={1}><box width="50%"><WorkbenchField field={iniField} value={session.values.iniPath} error={session.fieldErrors.iniPath} focused={session.focusedControlId === "iniPath"} disabled={session.phase === "running"} t={t} onFocus={() => session.focus("iniPath")} onChange={(value) => session.setField("iniPath", value)} /></box><box width="50%"><WorkbenchField field={databaseField} value={session.values.databasePath} error={session.fieldErrors.databasePath} focused={session.focusedControlId === "databasePath"} disabled={session.phase === "running"} t={t} onFocus={() => session.focus("databasePath")} onChange={(value) => session.setField("databasePath", value)} /></box></box><box flexDirection="row" gap={1}>{executableFields.map((field) => <box key={field.id} width="33%"><WorkbenchField field={field} value={session.values[field.id]} error={session.fieldErrors[field.id]} focused={session.focusedControlId === field.id} disabled={session.phase === "running"} t={t} onFocus={() => session.focus(field.id)} onChange={(value) => session.setField(field.id, value)} /></box>)}</box></box>
