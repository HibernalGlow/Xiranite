...
function positionalPaths(args: string[], action: MelodeckInput["action"]): string[] { const valueFlags = new Set(["--volume", "--mpv-path", "--ipc"]); return args.filter((value, index) => !value.startsWith("-") && value !== action && !valueFlags.has(args[index - 1] ?? "")) }
