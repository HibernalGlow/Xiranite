...
pub fn get_config_cache_path() -> Option<ConfigCachePath> {
    CONFIG_CACHE_PATH
        .get()
        .expect("Cannot fail if set_config_cache_path was called before")
        .clone()
}
...
pub fn set_config_cache_path(
    ...
    let mut resolve_folder =
        |env_var: &str, default_folder: Option<PathBuf>, name: &'static str| {
            let default_folder_str = default_folder
                .as_ref()
                .map_or("<not available>".to_string(), |t| {
    ...
    let config_cache_path = if let (Some(config_folder), Some(cache_folder)) =
        (config_folder, cache_folder)
    {
        infos.push(format!(
        ...
        Some(ConfigCachePath {
            config_folder,
            cache_folder,
        })
    } else {
        warnings
            .push("Cannot set config/cache path - config and cache will not be used.".to_string());
        None
    };

    CONFIG_CACHE_PATH
        .set(config_cache_path)
        .expect("Cannot set config/cache path twice");
...
pub(crate) fn open_cache_folder(
    ...
    if save_to_cache {
        file_handler_default = Some(
                ...
                .open(&cache_file)
            {
                ...
                Err(e) => {
                    warnings.push(format!(
                        "Cannot create or open cache file \"{}\", reason {e}",
        ...
        if use_json {
            file_handler_json = Some(
                match OpenOptions::new()
                    .truncate(true)
                    .write(true)
                    .create(true)
                    .open(&cache_file_json)
                {
                    Ok(t) => t,
                    Err(e) => {
                        warnings.push(format!(
        ...
        if let Ok(t) = OpenOptions::new().read(true).open(&cache_file) {
            ...
            if use_json {
                file_handler_json =
                    Some(OpenOptions::new().read(true).open(&cache_file_json).ok()?);
            } else {
    ...
    Some((
        (file_handler_default, cache_file),
        (file_handler_json, cache_file_json),
    ))
}
