...
| P08 | `[-]` | 图片/视频信息卡 | 图片信息卡已接入；补视频信息卡后完成 |
| P09 | `[~]` | 缩略图缓存 | 优先共享 host/cache 能力；若无需缓存必须用性能证据证明 |
| P10 | `[ ]` | 预览列覆盖 | 重复文件、相似图片、相似视频、音频及其他媒体结果统一动态 preview cell |

验证证据（2026-07-15）：`LocalImagePreview` 和 `LocalImagePreviewDialog` 位于 `src/nodes/shared`，通过宿主 `getFileUrl` 直通本地图片，不引入 Czkawka 私有 HTTP server 或 Rust 缩略图管理器。共享组件识别 AVIF/JXL，支持 lazy/eager 加载、异步解码、失败/禁用/非图片回退；结果表将图片缩略图暴露为可访问按钮，并在对话框内按当前筛选和排序后的可见图片循环前后导航。信息卡已展示路径、大小、分辨率、修改时间、相似度和组号，预览状态按工具隔离。共享组件及结果表 18 项定向测试通过，虚拟行仍固定 52px 且 10,000 项时 DOM 行数低于 80；视频缩略图、播放器及信息卡尚未迁移，因此 P08 保持部分完成。
