...
- [x] CLI 真实 pseudo-tty 视觉验证已从 `repacku` 单测抽成 `scripts/cli-visual-testing.ts`，并新增 Node 父进程手动捕获脚本 `scripts/capture-cli-ui.ts`；`repacku`、`linedup`、`rawfilter`、`marku` 与 `cleanf` guided 首屏均已生成 ANSI/HTML/PNG artifacts，其中 Ink 首页已逐步改为带边框和分区的富文本终端 UI。
