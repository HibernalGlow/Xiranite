      ...
      <box flexDirection="row" flexGrow={1} minHeight={0} gap={1} marginTop={1}>
          ...
          <scrollbox flexGrow={1} scrollbarOptions={{ trackOptions: { foregroundColor: theme.colors.primary, backgroundColor: theme.colors.border } }}>
            <box flexDirection="column" gap={1}>
              ...
              <ActionTabs id="field-action" options={[
                ...
                { value: "clean_now", label: "立即清空" },
                { value: "start", label: "自动清理" },
        ...
        <WorkbenchPanel title="循环监控" description="倒计时、目标与执行安全门" width="34%">
          <box flexDirection="column" flexGrow={1} minHeight={0}>
            <box flexDirection="column" alignItems="center" borderStyle="rounded" borderColor={theme.colors.border} paddingLeft={1} paddingRight={1}>
              <ascii-font text={String(session.values.action === "start" ? session.values.interval ?? 10 : "READY")} font="tiny" color={[theme.colors.primary, theme.colors.focusRing]} />
              ...
              {display?.metrics?.map((metric) => <box key={metric.label} width="100%" flexDirection="row" justifyContent="space-between"><text fg={theme.colors.mutedForeground}>{metric.label}</text><text fg={theme.colors.foreground}><b>{metric.value}</b></text></box>)}
            </box>
            <scrollbox flexGrow={1} minHeight={3}>{session.preview.map((line, index) => <text key={`${line}-${index}`} fg={index === 0 ? theme.colors.foreground : theme.colors.mutedForeground}>{`${index ? "·" : "›"} ${line}`}</text>)}</scrollbox>
            <ProgressBar value={session.progress} label={session.status || "等待运行"} />
            {session.confirming ? (
              <box flexDirection="column"><text fg={theme.colors.error}>{session.dangerPrompt?.body}</text><box flexDirection="row"><WorkbenchButton id="confirm-execute" danger onClick={() => void session.confirmExecute()}>{session.dangerPrompt?.confirmLabel ?? "确认执行"}</WorkbenchButton><ClickTarget id="confirm-dismiss" onClick={session.dismissConfirmation}>取消</ClickTarget></box></box>
