...
- GUI HTTP 与 CLI/TUI headless controller 已共用视频进度恢复/合并写回 service；GUI archive video 已通过按需、singleflight、有字节预算且随 session 释放的临时物化获得标准 HTTP Range seek，普通文件仍直接 Range、archive image 仍零落盘流式；目录/单视频/压缩包字幕已作为 auxiliary asset 由惰性 `subsrt` 统一输出 WebVTT，不保留 renderer 手写解析与 Blob 链；播放器控件、音量/倍速、字幕选择/样式和切页播放资源治理仍待迁移；
