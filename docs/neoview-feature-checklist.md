...
- 已知差异：L2 已复用引用计数 lease 保留 response 与 current/near frame 实际生成的每页最新 presentation；diagnostics 从 L1/L2/L3/solid owner 单次快照派生统一 memory/disk/lease 视图且不复制计数器；current listing 预算降级、跨进程 lease 与真实 corpus 校准仍待完成
