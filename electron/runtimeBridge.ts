...
export interface WindowHost {
  getCapabilities(): Promise<WindowCapabilities> | WindowCapabilities
  controlMain(action: MainWindowAction): Promise<WindowCommandResult> | WindowCommandResult
  restoreMainForDrag(input: MainWindowDragInput): Promise<WindowCommandResult> | WindowCommandResult
  openComponent(input: OpenComponentWindowInput): Promise<WindowCommandResult> | WindowCommandResult
  focus(id: string): Promise<WindowCommandResult> | WindowCommandResult
  close(id: string): Promise<WindowCommandResult> | WindowCommandResult
  getFrame(id?: string): Promise<WindowFrame | null> | WindowFrame | null
  setFrame(id: string | undefined, frame: WindowFrame): Promise<WindowCommandResult> | WindowCommandResult
}
...
export interface RuntimeBridge {
  ...
  buildAppUrl(params?: Record<string, string>): string
}

export interface RuntimeBridgeOptions {
  windowHost?: WindowHost
  autoOpen?: boolean
  openInitialUrl?: (url: string) => void
}
...
let activeWindowHost: WindowHost | null = null
let activeBridge: RuntimeBridge | null = null
...
export function buildAppUrl(params?: Record<string, string>): string {
...
export function createBrowserFallbackWindowHost(): WindowHost {
  return {
    getCapabilities: () => ({
      supported: true,
    ...
    restoreMainForDrag: () => ({
      success: false,
      supported: false,
      message: "Browser-fallback mode cannot restore native windows for title-bar drag.",
    }),
    openComponent: (input) => {
      ...
      return {
        ...
        supported: true,
        id,
        message: `Opened ${input.moduleId} component window in browser-fallback mode.`,
      }
    },
    focus: (id) => {
    ...
    setFrame: (id) => ({
      ...
      supported: false,
      id,
      message: "Browser-fallback mode cannot resize native windows.",
    }),
  }
}

function getWindowHost(): WindowHost {
  activeWindowHost ??= createBrowserFallbackWindowHost()
  return activeWindowHost
}
...
const handlers: Record<string, IpcHandler> = {
  ...
  "window.capabilities": async () => getWindowHost().getCapabilities(),
  "window.main": async (payload) => {
    const action = stringValue(recordPayload(payload), "action")
    return getWindowHost().controlMain(action as MainWindowAction)
  },
  "window.restoreMainForDrag": async (payload) => {
    const body = recordPayload(payload)
    return getWindowHost().restoreMainForDrag({
      screenX: numberValue(body, "screenX", 0),
      screenY: numberValue(body, "screenY", 0),
      clientX: numberValue(body, "clientX", 0),
      clientY: numberValue(body, "clientY", 0),
      windowWidth: numberValue(body, "windowWidth", 1),
    })
  },
  "window.openComponent": async (payload) => {
    ...
    return getWindowHost().openComponent({
      componentId,
      moduleId,
      title: stringValue(body, "title") || moduleId,
      width: numberValue(body, "width", 460),
      height: numberValue(body, "height", 380),
    })
  },
  "window.focus": async (payload) => {
    const id = stringValue(recordPayload(payload), "id")
    return getWindowHost().focus(id)
  },
  "window.close": async (payload) => {
    const id = stringValue(recordPayload(payload), "id")
    return getWindowHost().close(id)
  },
  "window.getFrame": async (payload) => {
    const id = stringValue(recordPayload(payload), "id") || undefined
    return getWindowHost().getFrame(id)
  },
  "window.setFrame": async (payload) => {
    const body = recordPayload(payload)
    const id = stringValue(body, "id") || undefined
    return getWindowHost().setFrame(id, {
      x: numberValue(body, "x", 0),
      y: numberValue(body, "y", 0),
      width: numberValue(body, "width", 800),
      height: numberValue(body, "height", 600),
    })
  },
}

export async function startRuntimeBridge(options: RuntimeBridgeOptions = {}): Promise<RuntimeBridge> {
  if (activeBridge) return activeBridge

  activeWindowHost = options.windowHost ?? createBrowserFallbackWindowHost()
  const server = serve({
    port: PORT,
  ...
  if (options.autoOpen) {
    setTimeout(() => {
      const url = buildAppUrl()
      ;(options.openInitialUrl ?? openExternalUrl)(url)
...
export function stopRuntimeBridge(): void {
  activeBridge?.server.stop?.()
  activeBridge = null
  activeWindowHost = null
}
